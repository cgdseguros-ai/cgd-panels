...
(function () {
  ...
  function openAnaliseModal(){
    const ym = currentPanelCompetence();
    openModal(`ANÁLISE • ${ym}`, null);
    ...
    $modalTop.innerHTML = `
      <div class="rowFlex">
        <div class="inlineCtrl">
          <span class="pillSmall">Competência</span>
          <span class="pillSmall"><b>${ym}</b></span>
      ...
      <div class="kpiGrid">
        <div class="kpiCard"><b>Produzido na competência</b><span>${fmtMoney(totalProduzido,"BRL")}</span></div>
        ...
        <div class="kpiCard"><b>% meta sorteio</b><span>${metaGeral ? pctGeral.toFixed(1)+"%" : "—"}</span></div>
      </div>
    `;
    ...
    const tableRows = rows.map((r, idx)=>`
      <tr>
        ...
        <td class="tdRight"><b>${(r.metaPct||0).toFixed(1)}%</b></td>
    ...
    document.getElementById("pdfBtn").addEventListener("click", ()=>exportModalToPDF(`analise_${ym}`));
    document.getElementById("analiseCsv").addEventListener("click", ()=>{
      const header = ["RANK","USER","ID","COMPETENCIA","PRODUZIDO","META","PCT_META","EM_ANDAMENTO","VALOR_A_IMPLANTAR"];
      const lines = [header.join(";")];
      rows.forEach((r, idx)=>{
        lines.push([idx+1, `"${String(r.seller).replace(/"/g,'""')}"`, r.sid, ym, r.produzidoMes||0, r.metaMes||0, (r.metaPct||0).toFixed(2), r.andamentoQtd||0, r.andamentoValor||0].join(";"));
      });
      const blob = new Blob(["\ufeff"+lines.join("\n")], { type:"text/csv;charset=utf-8" });
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `analise_${ym}.csv`;
      document.body.appendChild(a);
      a.click();
  ...
  document.getElementById("admAnalise").addEventListener("click", openAnaliseModal);
  if ($compFilter){
