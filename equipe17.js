...
(function () {
  ...
  function leadPossuiPlano(lead) {
    const raw = String((lead && lead[LEAD_UF_POSSUI_PLANO]) || "").trim();
    if (!raw) return "";
    const up = raw.toUpperCase();
    if (up in {"Y":"SIM","N":"NÃO"}) return {"Y":"SIM","N":"NÃO"}[up];
    if (up in {"SIM":"SIM","NAO":"NÃO","NÃO":"NÃO"}) return {"SIM":"SIM","NAO":"NÃO","NÃO":"NÃO"}[up];
    return raw;
  }
...
restoreSyncQueue();
  ...
  function openFollowUpModal(user, prefillName, opts) {
    ...
    if (btnExisting && existingFutureFu) btnExisting.onclick = () => openEditPrazoModal(existingFutureFu.ID);
    ...
    function cardLead(l, column) {
      ...
      return `
        <div class="leadCard">
          ...
          <div class="leadBtns">${btns}${ageDays >= 45 ? `<button class="leadBtn" data-action="leadRevalidate" data-leadid="${l.ID}" data-userid="${user.userId}">REVALIDAR LEAD</button>` : ``}</div>
        </div>
      `;
    }
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadRevalidate") {
        ...
        const leadId = String(a.getAttribute("data-leadid")||"");
        if (!uid || !leadId) return;
        if (!confirm("Revalidar este lead? O atual será descartado e um novo será criado com os mesmos dados principais.")) return;
