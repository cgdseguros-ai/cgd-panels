    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        for (const line of lines) {
          ...
          const timeVal = String((line.querySelector('.rlTime') || {}).value || '13:00').trim();
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function parseMoneyPtBr(raw) {
    ...
    if (hasComma && hasDot) {
      ...
      normVal = clean.replace(',', '.');
    }
    const n = Number(normVal);
    return Number.isFinite(n) ? n : 0;
  }
  function validateProposalRequiredFields(dealLike) {
    ...
    proposalFieldDefs().forEach((fd) => {
      ...
      if (String(v == null ? '' : v).trim() === '') missing.push(fd.label);
    });
    return missing;
  }
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    let rows = [];
    if (stageIds.length) {
      const chunks = stageIds.slice();
      const parts = await Promise.all(chunks.map((sid) => bxAll('crm.deal.list', {
        filter: { ...(uid ? { ASSIGNED_BY_ID: Number(uid) } : {}), STAGE_ID: sid },
        order: { ID: 'DESC' },
        select,
      }).catch(() => [])));
      rows = parts.flat();
...
Operadora fechada: ${proposalFieldValueLabel(PROP_UF_OPER_FECHADA, deal[PROP_UF_OPER_FECHADA])}</textarea></div>
  ...
  async function openProposalEditor(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const missing = validateProposalRequiredFields({ ...deal, ...patch });
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    let sourceDeals = adminView
      ? ((Array.isArray(STATE.proposalDealsAll) && STATE.proposalDealsAll.length) ? STATE.proposalDealsAll.slice() : [])
      : (proposalDealsCacheGet(uid) || []).filter((d)=>proposalDealAllowed(d));
    if (!sourceDeals.length) {
      openModal(title, `<div style="padding:18px;font-weight:900">Carregando propostas...</div>`, { full: true, noStack: !!opts.noStack });
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "recurTransfer") {
        ...
        const rid = String(a.getAttribute("data-ruleid")||"");
        if (!uid || !rid) return;
        const dest = window.prompt('Transferir recorrência para qual USER ID?');
        if (dest == null) return;
        const toUid = String(dest).trim();
        if (!toUid) return;
        if (!USERS.some((u) => String(u.userId) === toUid)) return alert('USER não encontrado.');
