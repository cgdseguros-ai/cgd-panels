...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function renderProposalCard(deal, opts = {}) {
    ...
    return `
      <div class="eqd-card" style="border-left:4px solid ${stageColor};padding:${compact ? '3px 5px' : '4px 6px'}">
        <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:${compact ? '3px' : '4px'};align-items:start">
          <div style="min-width:0">
            ...
            <div style="font-weight:950;font-size:${compact ? '11px' : '12px'};line-height:1.1">${escHtml(bestTitleFromText(deal.TITLE || ''))}</div>
          </div>
          <div style="display:flex;gap:4px;flex-wrap:wrap;justify-content:flex-end;align-items:center">
            <select class="proposal-inline-stage" data-id="${escHtml(String(deal.ID))}" style="padding:2px 4px;border-radius:8px;border:1px solid rgba(30,40,70,.16);font-weight:900;max-width:${compact ? '145px' : '170px'};font-size:10px">${proposalCardStageOptions(deal.STAGE_ID)}</select>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(${compact ? '120px' : '150px'},1fr));gap:3px 6px;margin-top:4px">${rows}</div>
      </div>`;
  }
