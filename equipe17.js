...
(function () {
  ...
  function holdUiRefresh(ms) {
    const until = Date.now() + Math.max(0, Number(ms || 0));
    UI_REFRESH_HOLD_UNTIL = Math.max(UI_REFRESH_HOLD_UNTIL, until);
    return UI_REFRESH_HOLD_UNTIL;
  }

  function rememberPendingDealPatch(dealId, patchFields, ttlMs) {
    const id = String(dealId || '');
    if (!id) return;
    PENDING_DEAL_PATCHES.set(id, { patch: { ...(patchFields || {}) }, expiresAt: Date.now() + Math.max(1000, Number(ttlMs || 10000)) });
  }

  function rememberPendingLeadPatch(userId, leadId, patchFields, ttlMs) {
    const uid = String(userId || '');
    const lid = String(leadId || '');
    if (!uid || !lid) return;
    PENDING_LEAD_PATCHES.set(`${uid}::${lid}`, { patch: { ...(patchFields || {}) }, expiresAt: Date.now() + Math.max(1000, Number(ttlMs || 12000)) });
  }

  function prunePendingEntityPatches() {
    const now = Date.now();
    for (const [k, v] of Array.from(PENDING_DEAL_PATCHES.entries())) {
      if (!v || Number(v.expiresAt || 0) <= now) PENDING_DEAL_PATCHES.delete(k);
    }
    for (const [k, v] of Array.from(PENDING_LEAD_PATCHES.entries())) {
      if (!v || Number(v.expiresAt || 0) <= now) PENDING_LEAD_PATCHES.delete(k);
    }
  }

  function overlayPendingEntityPatches() {
    prunePendingEntityPatches();
    for (const [dealId, cfg] of PENDING_DEAL_PATCHES.entries()) {
      updateDealInState(dealId, cfg.patch || {});
    }
    for (const [key, cfg] of PENDING_LEAD_PATCHES.entries()) {
      const [uid, leadId] = String(key || '').split('::');
      const arr = (STATE.leadsByUser.get(String(uid)) || []);
      const item = arr.find((l) => String(l.ID) === String(leadId));
      if (item) Object.assign(item, cfg.patch || {});
    }
  }
...
function makeUserCard(u) {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      try {
        const shouldHoldLeads = Date.now() < UI_REFRESH_HOLD_UNTIL && !(opts && opts.forceLeadsDuringHold);
        if (!shouldHoldLeads) await loadLeadsSnapshotAllUsers({ force: !!forceNetwork });
      } catch (_) {}
      overlayPendingSyncState();
      overlayPendingEntityPatches();
      renderFooterPeople();
    ...
    btn.onclick = async () => {
      try {
        ...
        if (isFollowupDeal(d) && followMode === 'NAO_FEITO') {
          const localPatch = { TITLE: title, [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString(), _late:false, DATE_MODIFY:new Date().toISOString(), [UF_OBS]: obs };
          holdUiRefresh(8000);
          rememberPendingDealPatch(String(dealId), localPatch, 10000);
          updateDealInState(dealId, localPatch);
          rebuildDealsOpen();
          closeModal();
          renderCurrentView();
          await bx("crm.deal.update", { id: String(dealId), fields: { TITLE: title, [UF_PRAZO]: prazoIso, [UF_OBS]: obs } });
          setTimeout(() => { if (Date.now() >= UI_REFRESH_HOLD_UNTIL) refreshData(true).catch(()=>{}); }, 1600);
          return;
        }
  ...
  async function convertLinkedLeadFromFollowup(dealId) {
    ...
    const ok = await openSystemConfirmModal('Confirmar conversão', 'Tem certeza que deseja converter este FOLLOW-UP e o lead vinculado?', 'Converter');
    if (!ok) return;
  ...
  async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    if (target === String(sAtendido) || target === String(sConv)) {
      holdUiRefresh(9000);
      rememberPendingLeadPatch(uid, leadId, patch, 12000);
    }
