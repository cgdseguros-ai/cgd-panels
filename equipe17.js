...
(function () {
  ...
  async function safeDealUpdate(dealId, fields, opts = {}) {
    const id = String(dealId || "").trim();
    const clean = sanitizeDealUpdateFields(fields);
    if (!id || !Object.keys(clean).length) return false;
    try {
      await bx("crm.deal.update", { id, fields: clean });
      return true;
    } catch (e) {
      const msg = String((e && e.message) || e || "");
      if (opts && opts.allow400 && /HTTP 400/.test(msg)) return false;
      throw e;
    }
  }

  function recurrenceRuleSignature(rule) {
    const r = rule || {};
    return JSON.stringify({
      ...
      hh: Number(r.hh ?? 9),
      mm: Number(r.mm ?? 0),
      monthDay: Number(r.monthDay || 0),
      yearMD: String(r.yearMD || "").trim(),
      weekDays: Array.isArray(r.weekDays) ? r.weekDays.map((x) => Number(x)).sort((a,b)=>a-b) : [],
      obs: String(r.obs || "").trim(),
      etapaUf: String(r.etapaUf || "").trim(),
      tipo: String(r.tipo || "").trim(),
      urg: String(r.urg || "").trim(),
      colab: String(r.colab || "").trim(),
    });
  }

  function dedupeRecurringRules(rules) {
    const seen = new Set();
    const out = [];
  ...
  async function transferLinkedLeadIfAny(deal, targetUserId) {
    const d = deal || {};
    const targetUid = String(targetUserId || "").trim();
    const origem = String(getDealLeadOrigemId(d) || "").trim();
    if (!origem || !targetUid) return false;
    let lead = null;
    let ownerUid = "";
    for (const [uid, arr] of STATE.leadsByUser.entries()) {
      const found = (arr || []).find((l) => String(leadOrigemId(l) || l.ID || "").trim() === origem);
      if (found) { lead = found; ownerUid = String(uid || ""); break; }
    }
    if (!lead) {
      try {
        const rows = await bxAll("crm.lead.list", {
          filter: { [LEAD_UF_LEAD_ORIGEM]: origem },
          select: ["ID","STATUS_ID","ASSIGNED_BY_ID", LEAD_UF_LEAD_ORIGEM],
          order: { ID: "DESC" },
        });
        if (rows && rows.length) {
          lead = rows[0];
          ownerUid = String(lead.ASSIGNED_BY_ID || "");
        }
      } catch (_) {}
    }
    ...
    const sEm = String(leadStageId("EM ATENDIMENTO") || leadStageId("NOVO LEAD") || "").trim();
    const patch = { ASSIGNED_BY_ID: Number(targetUid) };
    if (sEm) patch.STATUS_ID = sEm;

    try { await bx("crm.lead.update", { id: leadId, fields: patch }); } catch (_) {}

    if (ownerUid) {
      const fromArr = (STATE.leadsByUser.get(ownerUid) || []).slice();
      const idx = fromArr.findIndex((l) => String(l.ID) === leadId);
      if (idx >= 0) fromArr.splice(idx, 1);
      STATE.leadsByUser.set(ownerUid, fromArr);
    }
    const toArr = (STATE.leadsByUser.get(targetUid) || []).slice();
    const merged = { ...(lead || {}), ASSIGNED_BY_ID: Number(targetUid), STATUS_ID: sEm || String(lead.STATUS_ID || "") };
    const existingIdx = toArr.findIndex((l) => String(l.ID) === leadId);
    if (existingIdx >= 0) toArr[existingIdx] = { ...toArr[existingIdx], ...merged };
    else toArr.unshift(merged);
    STATE.leadsByUser.set(targetUid, toArr);
    return true;
  }
  ...
  async function loadRecurrenceConfigDeals() {
    ...
    (cfgDeals || []).forEach((d) => {
      ...
      const curTs = cur ? Date.parse(cur.DATE_MODIFY || cur.DATE_CREATE || 0) || Number(cur.ID || 0) : -1;
      const nextTs = Date.parse(d.DATE_MODIFY || d.DATE_CREATE || 0) || Number(d.ID || 0);
      if (!cur || nextTs >= curTs) byUser.set(uid, d);
    });

    byUser.forEach((d, uid) => {
      STATE.recurConfigDealIdByUser.set(uid, String(d.ID));
      const raw = String(d[UF_OBS] || "").trim();
      const j = safeJsonParse(raw);
      const rules = dedupeRecurringRules((j && Array.isArray(j.rules)) ? j.rules : []);
  ...
  async function saveRulesForUser(userId, rules) {
    ...
    const cleanRules = dedupeRecurringRules(Array.isArray(rules) ? rules : []);
    const payload = JSON.stringify({ ver: 1, userId: uid, rules: cleanRules });

    // ✅ SALVAR AQUI (atualiza JSON de regras no UF_OBS do deal de configuração)
    await safeDealUpdate(String(dealId), { [UF_OBS]: payload }, { allow400: true });

    STATE.recurRulesByUser.set(uid, cleanRules);
  }
    ...
    for (const u of USERS) {
      ...
      for (const rule of rules) {
        ...
        for (const d of days) {
          ...
          const sameDayAlreadyExists = (STATE.dealsAll || []).some((existing) => {
            if (!existing) return false;
            if (String(existing.ASSIGNED_BY_ID || existing._assigned || "") !== uid) return false;
            if (String(existing.TITLE || "").trim() !== title) return false;
            const p = String(existing._prazo || existing[UF_PRAZO] || "").trim();
            if (!p) return false;
            const dt = new Date(p);
            if (Number.isNaN(dt.getTime())) return false;
            return ymdKey(dt) === key;
          });
          if (sameDayAlreadyExists) { markers.add(mk); continue; }
...
restoreSyncQueue();
  ...
  async function flushSyncQueue() {
    ...
    try {
      ...
      while (SYNC_QUEUE.length) {
        ...
        if (job.type === "dealUpdate") { if (isTempId(job.dealId)) { SYNC_QUEUE.push(SYNC_QUEUE.shift()); persistSyncQueue(); continue; } try { await safeDealUpdate(String(job.dealId), job.fields, { allow400: true }); } catch (e) { throw e; } SYNC_QUEUE.shift(); persistSyncQueue(); continue; }
  ...
  function openNoFutureFollowupLeadBatchModal(user, leads, title) {
    ...
    document.getElementById('noFuBatch').onclick = async () => {
      ...
      const defaultBase = new Date(base);
...
function makeUserCard(u) {
    ...
    document.getElementById("btSave").onclick = async () => {
      ...
      try {
        for (const id of ids) {
          ...
          const targetUser = USERS.find((u) => String(u.userId) === uid);
          const stageId = targetUser ? await stageIdForUserName(targetUser.name) : "";
          updateDealInState(id, { ASSIGNED_BY_ID: Number(uid), _assigned: String(uid), ...(stageId ? { STAGE_ID: String(stageId) } : {}), DATE_MODIFY: new Date().toISOString() });
          await safeDealUpdate(String(id), { ASSIGNED_BY_ID: Number(uid), ...(stageId ? { STAGE_ID: String(stageId) } : {}) }, { allow400: true });
          if (deal && getDealLeadOrigemId(deal)) await transferLinkedLeadIfAny(deal, uid);
        }
        rebuildDealsOpen();
        closeModal();
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        for (const id of ids) {
          ...
          await safeDealUpdate(String(id), { [UF_PRAZO]: prazoIso }, { allow400: true });
        }
  ...
  async function markDone(dealId) {
    ...
    rebuildDealsOpen();
    await safeDealUpdate(String(dealId), { STAGE_ID: String(STATE.doneStageId) }, { allow400: true });
  }
  ...
  async function openTransferDealUserModal(dealId){
    ...
    btn.onclick = async () => {
      try {
        ...
        rebuildDealsOpen();
        await safeDealUpdate(String(dealId), { ASSIGNED_BY_ID: Number(newUserId), STAGE_ID: String(newStageId) }, { allow400: true });
        if (getDealLeadOrigemId(d)) await transferLinkedLeadIfAny(d, newUserId);
        closeModal();
        renderCurrentView();
      } catch (e) {
        clearBusy();
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function openAdminLinkFupModal(adminUserId, selectedUid = '', preloadedOrphans = null) {
    ...
    document.querySelectorAll('.adminFupLinkOne').forEach((btn) => btn.onclick = async () => {
      ...
      btn.disabled = true;
      try {
        await safeDealUpdate(String(dealId), { [DEAL_UF_LEAD_ORIGEM]: origem }, { allow400: true });
