...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function renderProposalCard(deal, opts = {}) {
    ...
    return `
      <div class="eqd-card" style="border-left:5px solid ${stageColor};padding:8px 10px">
        <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:start">
          <div style="min-width:0">
            <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:3px">
              <span class="eqd-tag" style="background:${stageColor};color:#fff;border-color:${stageColor};padding:3px 8px">${escHtml(stageName || 'Sem stage')}</span>
              <span class="eqd-tag" style="padding:3px 8px">${escHtml(ownerName)}</span>
              <span class="eqd-tag" style="padding:3px 8px"><strong>${escHtml(formatBRL(amount))}</strong></span>
            </div>
            <div style="font-weight:950;font-size:13px;line-height:1.2">${escHtml(bestTitleFromText(deal.TITLE || ''))}</div>
          </div>
          <div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end;align-items:center">
            <select class="proposal-inline-stage" data-id="${escHtml(String(deal.ID))}" style="padding:6px 8px;border-radius:10px;border:1px solid rgba(30,40,70,.16);font-weight:900;max-width:220px">${proposalCardStageOptions(deal.STAGE_ID)}</select>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:4px 8px;margin-top:6px">${rows}</div>
      </div>`;
  }
