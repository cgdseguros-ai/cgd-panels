...
(function () {
  ...
  injectCSS(`
    ...
    .eqd-btn{border:1px solid rgba(255,255,255,.22);background:#1f2430;border-radius:999px;padding:8px 12px;font-size:12px;font-weight:950;cursor:pointer;white-space:nowrap;color:#fff;text-decoration:none;display:inline-flex;align-items:center;gap:8px;box-shadow:0 2px 8px rgba(0,0,0,.16);}
  ...
  async function loadLeadsForOneUser(userId) {
    ...
    const statusIds = [leadStageId("EM ATENDIMENTO"), leadStageId("ATENDIDO"), leadStageId("QUALIFICADO")].filter(Boolean);
    let leads = [];
    let okCalls = 0;
    if (statusIds.length) {
    ...
    <div id="eqd-app">
      <div class="eqd-topbar">
        ...
        <div class="eqd-actions">
          ...
          <button class="eqd-btn" id="eqd-prevDay">←</button>
          <button class="eqd-btn" id="eqd-nextDay">→</button>
  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal) && linkedLead) {
      tags.push(leadStageNameById(linkedLead.STATUS_ID) === "QUALIFICADO" ? `<span class="eqd-tag" style="background:#dcfce7;color:#166534">QUALIFICADO</span>` : ``);
      if (String(linkedLead.STATUS_ID||"") === String(leadStageId("QUALIFICADO"))) tags.push(`<span class="eqd-tag" style="background:#dcfce7;color:#166534">QUALIFICADO</span>`);
      tags.push(leadHelena(linkedLead) ? `<span class="eqd-tag" style="background:#ede9fe;color:#6d28d9">HELENA</span>` : `<span class="eqd-tag" style="background:#374151;color:#fff">HELENA NÃO</span>`);
      tags.push(leadPessoal(linkedLead) ? `<span class="eqd-tag" style="background:#dcfce7;color:#166534">PESSOAL</span>` : `<span class="eqd-tag" style="background:#6b7280;color:#fff">PESSOAL NÃO</span>`);
    ...
    const batchBox = context && context.allowBatch
      ? `<label class="eqd-batchLabel"><input type="checkbox" class="eqd-batch" data-id="${deal.ID}"> Lote</label>`
      : ``;
  ...
  function upsertDealLocal(deal) {
    ...
    const isDone = STATE.doneStageId && String(deal.STAGE_ID) === String(STATE.doneStageId);
    if (isDone) { if (idxOpen >= 0) STATE.dealsOpen.splice(idxOpen, 1); }
  ...
  function openFollowupListModalForUser(user) {
    ...
    const list = all.filter(isFollowupDeal);
  ...
  async function openManualLeadCreateModal(user, defaultStatusId) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (!STATUS_ID) throw new Error("Selecione a etapa do lead.");
        ...
        if (NAME) { fields.NAME = NAME; fields.TITLE = NAME; }
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    if (shouldLoadLeads) {
      ...
      leadsUser = await loadLeadsForOneUser(user.userId).catch(() => (STATE.leadsByUser.get(String(user.userId)) || []));
      clearBusy();
    }
    ...
    function cardLead(l, column) {
      ...
      const when = leadDataHora(l) ? fmtShortYear(leadDataHora(l)) : "—";
      ...
      return `
        <div class="leadCard">
          ...
          <div class="leadMeta">
            ...
            <button class="eqd-tag" style="background:${leadHelena(l) ? "#ede9fe" : "#374151"};color:${leadHelena(l) ? "#6d28d9" : "#fff"};border:none;cursor:pointer" data-action="leadToggleFlag" data-flag="HELENA" data-leadid="${l.ID}" data-userid="${user.userId}">${leadHelena(l) ? "HELENA" : "HELENA NÃO"}</button>
            <button class="eqd-tag" style="background:${leadPessoal(l) ? "#dcfce7" : "#6b7280"};color:${leadPessoal(l) ? "#166534" : "#fff"};border:none;cursor:pointer" data-action="leadToggleFlag" data-flag="PESSOAL" data-leadid="${l.ID}" data-userid="${user.userId}">${leadPessoal(l) ? "PESSOAL" : "PESSOAL NÃO"}</button>
          </div>
  ...
  function openLeadAnalysisModal(){
    const rows = USERS.map((u) => {
      const leads = STATE.leadsByUser.get(String(u.userId)) || [];
      const sAt = leadStageId("EM ATENDIMENTO");
      const sAtendido = leadStageId("ATENDIDO");
      const sQual = leadStageId("QUALIFICADO");
      const emAtendimento = leads.filter((l) => String(l.STATUS_ID) === String(sAt)).length;
      const atendido = leads.filter((l) => String(l.STATUS_ID) === String(sAtendido)).length;
      const qualificado = leads.filter((l) => String(l.STATUS_ID) === String(sQual)).length;
      const semFu = leads.filter((l) => [String(sAtendido), String(sQual)].includes(String(l.STATUS_ID)) && !hasFutureFollowupForLeadOrig(leadOrigemId(l) || l.ID)).length;
      const atrasadosFu = countUserStats(u.userId).overdueFollow;
      return {name:u.name, emAtendimento, atendido, qualificado, semFu, atrasadosFu};
    });
    openModal("ANÁLISE DE LEADS", `
      <div style="overflow:auto">
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">USER</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">EM ATENDIMENTO</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">ATENDIDO</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">QUALIFICADO</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">SEM FOLLOW-UP</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">FOLLOW-UP ATRASADO</th></tr></thead>
          <tbody>${rows.map((r)=>`<tr><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08);font-weight:900">${escHtml(r.name)}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${r.emAtendimento}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${r.atendido}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${r.qualificado}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${r.semFu}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${r.atrasadosFu}</td></tr>`).join("")}</tbody>
        </table>
      </div>
    `, { wide:true });
  }
  ...
  function openGlobalLeadsDayStats() {
    ...
    `, { wide: true });
    const inp = document.getElementById("rmSearch");
    const list = document.getElementById("rmList");
    if (inp && list) inp.oninput = () => {
      const kw = norm(String(inp.value||"").trim());
      const filtered = kw ? rules.filter((r) => norm([r.title||"", r.obs||"", r.type||""].join(" ")).includes(kw)) : rules;
      list.innerHTML = filtered.length ? filtered.map(ruleLine).join("") : `<div class="eqd-empty">Nenhuma regra encontrada.</div>`;
    };
  }
  ...
  function countUserStats(userId) {
    ...
    const overdueFollow = overdue.filter((d) => isFollowupDeal(d)).length;
    return { day: dayOpenPlusLate.length, doneDay: dayDone.length, overdue: overdue.length, followDay: followDay.length, overdueFollow };
  }
  ...
  function makeUserCard(u) {
    ...
    return `
      <div class="userCard" data-action="openUser" data-userid="${u.userId}">
        <div class="userEmoji" style="margin-right:10px;align-self:center">${emoji}</div>
        ...
        <div class="userRight">
          <div class="userLine">Tarefas do dia: <strong>${stats.day}</strong></div>
          <div class="userLine" style="white-space:nowrap">FOLLOW-UP do dia: <strong>${stats.followDay}</strong></div>
          <div class="userLine" style="white-space:nowrap">FOLLOW-UP atrasado: <strong>${stats.overdueFollow}</strong></div>
  ...
  function renderUserPanel(userId) {
    ...
    const followListBtn = `<button class="eqd-btn ${overdueFollow ? "blinkRedBorder" : ""}" data-action="followList" data-userid="${user.userId}" id="followListBtn">LISTA DE FOLLOW-UP</button>`;
    ...
    if (!isSpecial) {
      ...
      el.main.innerHTML = `
        <div class="panelHead">
          ...
          <div class="panelTools">
            ...
            <div class="eqd-batchBar"><button class="eqd-btn" id="selAllDay">SELECIONAR TODOS DO DIA</button><button class="eqd-btn" id="selAllLate">SELECIONAR ATRASADOS</button><button class="eqd-btn" id="batchDone">CONCLUIR EM LOTE</button><button class="eqd-btn" id="batchResched">REAGENDAR EM LOTE</button><button class="eqd-btn" id="batchTransfer">TRANSFERIR USER</button></div>
      ...
      document.getElementById("selAllDay").onclick = () => { document.querySelectorAll(".eqd-batch").forEach((x) => x.checked = true); };
      document.getElementById("selAllLate").onclick = () => { document.querySelectorAll(".dealCard").forEach((card) => { const cb = card.querySelector(".eqd-batch"); if (cb) cb.checked = /ATRASAD/i.test(card.textContent || ""); }); };
      document.getElementById("selAllDay").onclick = () => { document.querySelectorAll(".eqd-batch").forEach((x) => x.checked = true); };
      document.getElementById("selAllLate").onclick = () => { document.querySelectorAll(".dealCard").forEach((card) => { const cb = card.querySelector(".eqd-batch"); if (cb) cb.checked = /ATRASAD/i.test(card.textContent || ""); }); };
          ...
          return `
            <section class="panelCol">
              <div class="panelColHead" style="display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap">
                ...
                <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                  ${LEAD_USERS.has(String(uid)) ? `<button class="eqd-btn" style="padding:6px 10px;font-size:11px" data-action="leadsModal" data-userid="${uid}">LEADS</button>` : ``}<button class="eqd-btn" style="padding:6px 10px;font-size:11px" data-action="followUpModal" data-userid="${uid}">FOLLOW-UP</button>
                </div>
              </div>
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    btn.onclick = async () => {
      try {
        ...
        if (d[UF_TAREFA]) fields[UF_TAREFA] = d[UF_TAREFA]; if (d[UF_URGENCIA]) fields[UF_URGENCIA] = d[UF_URGENCIA]; if (d[UF_ETAPA]) fields[UF_ETAPA] = d[UF_ETAPA]; if (d[UF_COLAB]) fields[UF_COLAB] = d[UF_COLAB]; if (obs) fields[UF_OBS] = obs;
        const newId = await bx("crm.deal.add", { fields });
        upsertDealLocal(parseDeal({ ID:String(newId), ...fields, DATE_CREATE:new Date().toISOString(), DATE_MODIFY:new Date().toISOString() }, { urgMap: await enums(UF_URGENCIA), tarefaMap: await enums(UF_TAREFA), etapaMap: await enums(UF_ETAPA), colabMap: {}, colabIsEnum:false }));
        closeModal(); await refreshData(true, { forceRecur:false, forceFullDeals:false }); renderCurrentView();
  ...
  function openRecurrenceManager(userId){
    ...
    const rawRules = (STATE.recurRulesByUser.get(uid) || []).slice();
    const typeRank = (r) => ({MONTHLY:1,WEEKLY:2,DAILY_BUSINESS:3,YEARLY:4}[String(r && r.type || "")] || 9);
    const rules = rawRules.sort((a,b) => {
      const ta = typeRank(a), tb = typeRank(b);
      if (ta !== tb) return ta - tb;
      if (ta === 1) return Number(a.monthDay||99) - Number(b.monthDay||99);
    ...
    openModal(`Recorrência — ${user.name}`, `
      ...
      <div style="display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap"><div style="font-size:12px;font-weight:950;opacity:.85">Regras: <strong>${rules.length}</strong></div><input id="rmSearch" class="eqd-searchInput" style="color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Buscar regra..." /></div>
      ...
      <div style="margin-top:10px;display:flex;flex-direction:column;gap:8px">
        <div id="rmList">${rules.length ? rules.map(ruleLine).join("") : `<div class="eqd-empty">Sem regras cadastradas.</div>`}</div>
      </div>
    `, { wide: true });
    const inp = document.getElementById("rmSearch");
    const list = document.getElementById("rmList");
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadsModal") {
        const uid = String(userId||"");
        if (!uid) return;
        return openLeadsModalForUser(uid, "", { forceReload: true });
      }
      ...
      if (act === "leadToggleFlag") {
        const uid = String(a.getAttribute("data-userid")||"");
        const leadId = String(a.getAttribute("data-leadid")||"");
        const flag = String(a.getAttribute("data-flag")||"");
        const arr = (STATE.leadsByUser.get(uid) || []).slice();
        const i = arr.findIndex((l) => String(l.ID) === String(leadId));
        if (i < 0) return;
        const fld = flag === "HELENA" ? LEAD_UF_HELENA : LEAD_UF_PESSOAL;
        const cur = String(arr[i][fld] || "").trim();
        const next = cur ? "" : "SIM";
        arr[i] = { ...arr[i], [fld]: next };
        STATE.leadsByUser.set(uid, arr);
        enqueueSync({ type: "leadUpdate", leadId: String(leadId), fields: { [fld]: next } });
        return reopenLeadsModalSafe();
      }
      ...
      if (act === "followupPerdido") {
        const d = getDealById(id); const origId = d ? getDealLeadOrigemId(d) : ""; const lead = origId ? getLeadByOrigemId(origId) : null; if (!lead) return alert("Este FOLLOW-UP não tem lead de origem vinculado.");
  ...
  el.prevDay.onclick = () => { const d = new Date(selectedDate); d.setDate(d.getDate()-1); selectedDate = d; calendarCursor = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), 1); renderCurrentView(); };
  el.nextDay.onclick = () => { const d = new Date(selectedDate); d.setDate(d.getDate()+1); selectedDate = d; calendarCursor = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), 1); renderCurrentView(); };
  ...
  // =========================
  (async function init(){
    await refreshData(false, { forceRecur: true, forceFullDeals: true });
    renderFooterPeople();
    renderGeneral();
    setSoftStatus("JS: ok");
  })();
