...
// 1) CONFIG
// =========================
const PROXY_BASE = "https://cgd-bx-proxy.cgdseguros.workers.dev/bx/"; const LOGO_URL =
"https://bitrix24public.com/b24-
6iyx5y.bitrix24.com.br/docs/pub/c77325321d1ad38e8012b995a5f4e8dd/showFile/?&token=wzev6g2dxwkk";

const INTRANET_URL = "https://cgdcorretora.bitrix24.site/";
const VENDAS_URL = "https://cgdcorretorabase.bitrix24.site/vendas/";
const FINANCEIRO_URL = "https://cgdcorretorabase.bitrix24.site/controlefinanceiro/"; const SEGUROS_URL = "https://getcgdcorretora.bitrix24.site/seguros/";

const REFRESH_MS = 20000;

const ADMIN_PINS = new Set(["4455", "8123", "6677", "4627"]); const CATEGORY_MAIN = 17;
// ✅ RECORRÊNCIA (Bitrix)
const RECURRENCE_STAGE_ID = "C17:UC_IUQR52"; // GET • RECORRÊNCIA const RECURRENCE_TITLE_PREFIX = "RECORRÊNCIA • ";
...
const LEAD_USERS = new Set(["15", "19", "17", "23", "811", "3081", "3079", "3083", "3085",
"3389"]);
const SEGUROS_USERS = new Set(["815", "269", "29", "3101"]);

const SPECIAL_PANEL_USERS = new Set([ "3079", "3083", "3085", "3389",
...
// UF Deals
const UF_URGENCIA = "UF_CRM_1768174982"; const UF_TAREFA = "UF_CRM_1768185018696"; const UF_ETAPA = "UF_CRM_1768179977089"; const UF_COLAB = "UF_CRM_1770327799";
const UF_PRAZO = "UF_CRM_1768175087"; const UF_OBS = "UF_CRM_691385BE7D33D"; const DONE_STAGE_NAME = "CONCLUÍDO";

// LEADS — campos
const LEAD_UF_OPERADORA = "UF_CRM_1771282782"; const LEAD_UF_IDADE = "UF_CRM_1771339221";
const LEAD_UF_TELEFONE = "UF_CRM_1771282207";
const LEAD_UF_BAIRRO = "UF_CRM_LEAD_1731909705398"; const LEAD_UF_FONTE = "UF_CRM_1767285733843";
...
// =========================
function ensureRoot() {
let root = document.getElementById("eqd-root"); if (!root) {
root = document.createElement("div"); root.id = "eqd-root";
...
const s = document.getElementById("eqd-sentinel"); if (s) s.style.display = "none";
}
...
function showFatal(err) { try {
const root = ensureRoot(); root.innerHTML = `
...
<pre style="white-space:pre-wrap;background:#fff;border:1px solid
rgba(0,0,0,.12);border-radius:12px;padding:12px;max-width:1100px;overflow:auto">${String(  (err && (err.stack || err.message || err)) || err
)}</pre>
...
window.addEventListener("error", (e) => showFatal(e.error || e.message || e)); window.addEventListener("unhandledrejection", (e) => showFatal(e.reason || e));
...
radial-gradient(900px 650px at 55% 95%, rgba(255,150,200,.18), transparent 60%), linear-gradient(135deg, var(--bgA), var(--bgB) 50%, var(--bgC));
}

.eqd-topbar{
display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap; margin-bottom:12px;
background:#14161a;border:1px solid rgba(255,255,255,.08); padding:12px;border-radius:18px;
}
.eqd-titleWrap{display:flex;align-items:center;gap:12px;min-width:320px;}
.eqd-logo{width:60px;height:60px;border-radius:999px;border:1px solid
rgba(255,255,255,.14);background:rgba(255,255,255,.92);object-fit:contain;padding:8px;flex:0 0 auto;}
...
.eqd-btn{border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.12);border- radius:999px;padding:8px 12px;font-size:12px;font-weight:950;cursor:pointer;white-
space:nowrap;color:#fff;text-decoration:none;display:inline-flex;align-items:center;gap:8px;}
...
.eqd-searchInput{width:min(340px,54vw);border:1px solid
rgba(255,255,255,.16);background:rgba(255,255,255,.10);border-radius:999px;padding:9px 12px;font- size:12px;font-weight:900;outline:none;color:#fff;}
.eqd-searchInput::placeholder{color:rgba(255,255,255,.70)}
.eqd-searchSelect{border:1px solid
rgba(255,255,255,.16);background:rgba(255,255,255,.10);border-radius:999px;padding:8px 10px;font- size:12px;font-weight:950;outline:none;min-width:170px;color:#fff;}
...
radial-gradient(900px 620px at 82% 18%, rgba(80,170,255,.12), transparent 60%), linear-gradient(135deg, #14181d, #1b1f25);
}

.userGrid{display:grid;grid-template-columns:repeat(4,minmax(220px,1fr));gap:12px;}
@media (max-width:1200px){.userGrid{grid-template-columns:repeat(2,minmax(220px,1fr));}} @media (max-width:720px){.userGrid{grid-template-columns:1fr;}}

.userCard{
border:1px solid rgba(30,40,70,.12); border-radius:18px;
background:rgba(255,255,255,.52); backdrop-filter: blur(12px);
padding:12px;
cursor:pointer;
transition: transform .08s ease, box-shadow .08s ease; display:flex;gap:12px;align-items:center;
}
...
.userPhoto{
width:102px;height:102px;border-radius:999px; border:1px solid rgba(30,40,70,.14);
background:rgba(255,255,255,.92); object-fit:cover;
}
...
#eqd-app.eqd-dark .userName, #eqd-app.eqd-dark .userTeam, #eqd-app.eqd-dark
.userLine{color:#fff;}
...
padding:8px 10px;border-radius:18px;border:1px solid var(--border); background:rgba(255,255,255,.55);backdrop-filter:blur(10px);
}
...
#eqd-app:not(.eqd-dark) .panelTools .eqd-searchInput{ background:rgba(255,255,255,.92);
...
.panelCols{display:grid;grid-template-columns:repeat(3,minmax(260px,1fr));gap:12px;} @media (max-width:1100px){.panelCols{grid-template-columns:1fr;}}
.panelCol{
...
.eqd-modalOverlay{position:fixed;inset:0;background:rgba(10,14,22,.35);backdrop-
filter:blur(6px);display:none;align-items:center;justify-content:center;padding:16px;z-  index:99999;}
.eqd-modal{
width:min(920px,96vw); max-height:86vh;
border-radius:18px;border:1px solid rgba(255,255,255,.35); background:rgba(255,255,255,.78);
box-shadow:0 20px 60px rgba(10,14,22,.25);
overflow:hidden;display:flex;flex-direction:column;
}
.eqd-modal.wide{width:min(1280px,98vw);max-height:92vh;}
.eqd-modal.full{width:calc(100vw - 24px);max-width:none;max-height:calc(100vh - 24px);} #eqd-app.eqd-dark .eqd-modal{background:rgba(25,28,34,.92);border-
color:rgba(255,255,255,.10);color:var(--text);}
.eqd-modalHead{padding:12px 14px;display:flex;justify-content:space-between;align- items:center;gap:10px;border-bottom:1px solid
rgba(30,40,70,.12);background:rgba(255,255,255,.82);}
...
.eqd-warn{border:1px solid
rgba(255,80,120,.28);background:rgba(255,220,235,.55);color:rgba(120,0,40,.92);padding:10px  12px;border-radius:14px;font-size:11px;font-weight:900;white-space:pre-wrap;display:none;}

.eqd-card{--accent-rgb:140,160,240;position:relative;border-radius:16px;border:1px solid rgba(30,40,70,.14);background:rgba(255,255,255,.82);overflow:hidden;box-shadow:0 8px 18px
rgba(20,25,35,.08),0 10px 26px rgba(var(--accent-rgb),.10);flex:0 0 auto;color:rgba(18,26,40,.92);}
...
.eqd-tag{font-size:10.5px;padding:2px 7px;border-radius:999px;border:1px solid
rgba(30,40,70,.14);background:rgba(255,255,255,.78);color:rgba(18,26,40,.72);white-space:nowrap;}
.eqd-tagLate{border-color:rgba(255,80,120,.55);background:rgba(255,80,120,.22);font-
 
weight:950;color:rgba(130,0,50,.95);animation:eqdBlink 1.6s ease-in-out infinite;}
...
.eqd-tagObs{border-color:rgba(255,180,0,.55);background:rgba(255,200,0,.22);font- weight:950;color:rgba(120,70,0,.95);animation:eqdBlinkObs .95s ease-in-out
infinite;cursor:pointer;}
...
.eqd-smallBtn{cursor:pointer;border:1px solid
rgba(30,40,70,.14);background:rgba(255,255,255,.88);border-radius:999px;padding:6px 9px;font- size:11px;font-weight:950;color:rgba(18,26,40,.92);}
...
.eqd-empty{border:1px dashed rgba(30,40,70,.18);border-
radius:16px;padding:12px;background:rgba(255,255,255,.55);color:rgba(18,26,40,.62);font-  size:11px;font-weight:800;text-align:center;}

.leadCard{border:1px solid rgba(30,40,70,.12);border-
radius:16px;background:rgba(255,255,255,.80);padding:10px;display:flex;flex- direction:column;gap:6px}
...
.leadBtn{cursor:pointer;border:1px solid
rgba(30,40,70,.14);background:rgba(255,255,255,.88);border-radius:999px;padding:6px 9px;font- size:11px;font-weight:950}
...
.leadObsChip{
cursor:pointer;
border:1px solid rgba(30,40,70,.14); border-radius:999px;
...
.calCell{
border:1px solid rgba(30,40,70,.14); border-radius:12px;
padding:10px 0;
text-align:center; font-weight:950;
cursor:pointer;
background:rgba(255,255,255,.70); user-select:none;
}
...
background:rgba(20,22,26,.92); color:rgba(255,255,255,.84);
border-top:1px solid rgba(255,255,255,.10); padding:14px 18px;
display:flex;gap:16px;align-items:center;justify-content:space-between;flex-wrap:wrap; font-size:13px;
font-weight:850;
...
.eqd-footAvatar{
width:38px;height:38px;border-radius:999px;object-fit:cover; border:1px solid rgba(255,255,255,.14);
...
.eqd-footerCenter{ flex: 1 1 auto;
text-align:center; font-style: italic; opacity:.86;
font-weight:900;
...
function toPairs(prefix, obj, out) { out = out || [];
if (obj === null || obj === undefined) return out;
if (typeof obj === "object" && !Array.isArray(obj)) { for (const k of Object.keys(obj)) {
const key = prefix ? `${prefix}[${k}]` : k; toPairs(key, obj[k], out);
}
return out;
}
...
const key = prefix ? `${prefix}[${i}]` : String(i); toPairs(key, obj[i], out);
}
return out;
}
out.push([prefix, String(obj)]); return out;
}

async function fetchWithTimeout(url, options, timeoutMs) { const ctrl = new AbortController();
const t = setTimeout(() => ctrl.abort(), timeoutMs);
try { return await fetch(url, { ...options, signal: ctrl.signal }); } finally { clearTimeout(t); }
}

let BX_QUEUE = Promise.resolve(); let BX_INFLIGHT = 0;

async function bxRaw(method, params = {}) { BX_QUEUE = BX_QUEUE.then(async () => {
BX_INFLIGHT++;
try {
const pairs = toPairs("", params, []); const body = new URLSearchParams();
for (const [k, v] of pairs) { if (k) body.append(k, v); }

const maxTry = 3;
let lastErr = null;

for (let attempt = 1; attempt <= maxTry; attempt++) { try {
if (attempt > 1) {
const base = 250 * Math.pow(2, attempt - 2);
const jitter = Math.floor(Math.random() * 140); await sleep(base + jitter);
}
...
const data = await bxRaw(method, params); return data.result;
}

async function bxAll(method, params = {}) { let all = [];
let start = 0; while (true) {
const data = await bxRaw(method, { ...params, start }); const chunk = data.result || [];
all = all.concat(chunk);
if (data.next === undefined || data.next === null) break; start = data.next;
}
return all;
}
...
function escHtml(s) {
s = s === null || s === undefined ? "" : String(s); return s
.replace(/&/g, "&" + "amp;")
.replace(/</g, "&" + "lt;")
.replace(/>/g, "&" + "gt;")
.replace(/"/g, "&" + "quot;")
...
if (!dt) return "—";
const x = new Date(dt);
if (Number.isNaN(x.getTime())) return String(dt); const dd = String(x.getDate()).padStart(2, "0");
const mm = String(x.getMonth() + 1).padStart(2, "0"); const hh = String(x.getHours()).padStart(2, "0");
const mi = String(x.getMinutes()).padStart(2, "0"); return `${dd}/${mm} ${hh}:${mi}`;
}
...
function localInputToIsoWithOffset(v) { v = String(v || "").trim();
if (!v) return "";
v = v.replace(" ", "T");
 
const dt = new Date(v);
if (Number.isNaN(dt.getTime())) return "";
const pad = (n) => String(n).padStart(2, "0"); const y = dt.getFullYear();
const m = pad(dt.getMonth() + 1); const d = pad(dt.getDate());
const hh = pad(dt.getHours());
const mi = pad(dt.getMinutes());
const offMin = -dt.getTimezoneOffset(); const sign = offMin >= 0 ? "+" : "-";
const abs = Math.abs(offMin);
const oh = pad(Math.floor(abs / 60)); const om = pad(abs % 60);
return `${y}-${m}-${d}T${hh}:${mi}:00${sign}${oh}:${om}`;
}

function isoFromDateAndTimeParts(dateObj, hh, mm) { const pad = (n) => String(n).padStart(2, "0");
const y = dateObj.getFullYear();
const m = pad(dateObj.getMonth() + 1); const d = pad(dateObj.getDate());
const offMin = -dateObj.getTimezoneOffset(); const sign = offMin >= 0 ? "+" : "-";
const abs = Math.abs(offMin);
const oh = pad(Math.floor(abs / 60)); const om = pad(abs % 60);
return `${y}-${m}-${d}T${pad(hh)}:${pad(mm)}:00${sign}${oh}:${om}`;
}

function isUrgenteText(urgTxt) { const u = norm(urgTxt);
if (!u) return false;
if (u.includes("ATEN")) return false; return u.includes("URG");
}
function isAtencaoText(urgTxt) { const u = norm(urgTxt);
return u.includes("ATEN");
}
function bestTitleFromText(txt) {
const t = String(txt || "").trim(); if (!t) return "Negócio";
const first = t.split("\n")[0].trim(); return trunc(first || "Negócio", 72);
}

function dealAccent(deal) {
const seed = String(deal.ID || deal.TITLE || "Tarefa"); let h = 2166136261;
for (let i = 0; i < seed.length; i++) { h ^= seed.charCodeAt(i);
h = Math.imul(h, 16777619);
}
const hue = (h >>> 0) % 360; const s = 0.45, l = 0.55;
const c = (1 - Math.abs(2 * l - 1)) * s;
const x = c * (1 - Math.abs((hue / 60) % 2 - 1)); const m = l - c / 2;
let r = 0, g = 0, b = 0;
if (hue < 60) { r = c; g = x; }
...
// ✅ locks (evita clique duplo / “salvar 2x”) const ACTION_LOCKS = new Set();
...
function setLeadsCtx(userId, kw){ LAST_LEADS_CTX.userId = String(userId||""); LAST_LEADS_CTX.kw
= String(kw||""); }
...
function isAdmin(pin) { return ADMIN_PINS.has(String(pin || "").trim()); } function canOpenUserPanel(userId) {
const pin = askPin();
if (!pin) return false;
if (isAdmin(pin)) return true; return pin === String(userId);
}

// =========================
// 6) STATE / ENUMS / STAGES / USERS
// =========================
const STATE = { dealsAll: [],
dealsOpen: [],
doneStageId: null,
stageMapByName: new Map(), enumCache: new Map(),
dealFieldsMeta: null,

userPhotoById: new Map(), userNameById: new Map(),

lastOkAt: null, offline: false,

// leads
leadStageIdByName: new Map(), leadStageNameById: new Map(), leadsByUser: new Map(),
leadsAtendimentoIdsByUser: new Map(), leadsAlertUsers: new Set(),

footerPhotosLoaded: false,

// recorrência
recurConfigDealIdByUser: new Map(), recurRulesByUser: new Map(),
recurLastGenAt: 0,
};

async function enums(uf) {
if (STATE.enumCache.has(uf)) return STATE.enumCache.get(uf);
const list = await bx("crm.deal.userfield.list", { filter: { FIELD_NAME: uf } }); const f = Array.isArray(list) ? list[0] : null;
if (!f || !f.ID) { STATE.enumCache.set(uf, {}); return {}; } const d = await bx("crm.deal.userfield.get", { id: f.ID }); const m = {};
(d.LIST || []).forEach((e) => (m[String(e.ID)] = String(e.VALUE))); STATE.enumCache.set(uf, m);
 
return m;
}

async function enumHasOptions(uf) { const m = await enums(uf);
return m && Object.keys(m).length > 0;
}

function findEnumIdByLabel(map, wantedLabelNorm) { const entries = Object.entries(map || {});
for (const [id, label] of entries) if (norm(label) === wantedLabelNorm) return id;
for (const [id, label] of entries) if (norm(label).includes(wantedLabelNorm)) return id; return "";
}

async function loadStagesForCategory(categoryId) { const cid = Number(categoryId);
const stages = await bx("crm.dealcategory.stage.list", { id: cid }); const stageMapByName = new Map();
...
STATE.stageMapByName = stageMapByName; STATE.doneStageId = doneStageId;
}

async function stageIdForUserName(userName) {
const exact = STATE.stageMapByName.get(norm(userName)); if (exact) return exact;
...
async function ensureUserPhoto(userId, fallbackName) { const id = Number(userId);
if (!id) return "";
...
try {
const res = await bx("user.get", { ID: id }); const u = Array.isArray(res) ? res[0] : res; if (u) {
const fn = [u.NAME, u.LAST_NAME].filter(Boolean).join(" ").trim(); if (fn) name = fn;
STATE.userNameById.set(id, name); const p = u.PERSONAL_PHOTO;
if (p && typeof p === "string" && /^https?:\/\//i.test(p)) photoUrl = p;
}
} catch (_) {}

STATE.userPhotoById.set(id, photoUrl || "");
if (!STATE.userNameById.has(id)) STATE.userNameById.set(id, name); return photoUrl || "";
}
...
const prazoOk = prazoDate && !Number.isNaN(prazoDate.getTime()); const now = new Date();
...
const colabId = String(colabRaw || "").trim(); let colabTxt = "";
if (colabId) colabTxt = colabIsEnum ? String((colabMap || {})[colabId] || colabId).trim() : colabId;

const obsTxt = String(deal[UF_OBS] || "").trim(); const hasObs = !!obsTxt;
...
const list = await bxAll("crm.status.list", { filter: { ENTITY_ID: "STATUS" } }); const mapByName = new Map();
const mapNameById = new Map(); (list || []).forEach((s) => {
...
const n = [lead.NAME, lead.LAST_NAME].filter(Boolean).join(" ").trim(); return bestTitleFromText(t || n || `Lead`);
 
}
function leadOperadora(lead) { return String((lead && lead[LEAD_UF_OPERADORA]) || "").trim(); } function leadIdade(lead) { return String((lead && lead[LEAD_UF_IDADE]) || "").trim(); }
function leadTelefone(lead) { return String((lead && lead[LEAD_UF_TELEFONE]) || "").trim(); } function leadBairro(lead) { return String((lead && lead[LEAD_UF_BAIRRO]) || "").trim(); }
function leadFonte(lead) { return String((lead && lead[LEAD_UF_FONTE]) || "").trim(); }
function leadDataHora(lead) { return String((lead && lead[LEAD_UF_DATAHORA]) || lead.DATE_CREATE
|| "").trim(); }
...
const d = new Date(s);
if (!Number.isNaN(d.getTime())) return d; return null;
}

async function loadLeadsForOneUser(userId) { await loadLeadStages();

const select = [

"ID","TITLE","NAME","LAST_NAME","STATUS_ID","ASSIGNED_BY_ID","DATE_CREATE","DATE_MODIFY","SOURCE_I  D",
LEAD_UF_OPERADORA, LEAD_UF_IDADE, LEAD_UF_TELEFONE, LEAD_UF_BAIRRO, LEAD_UF_FONTE, LEAD_UF_DATAHORA, LEAD_UF_OBS,
].filter(Boolean);

const since = new Date(Date.now() - 1000 * 60 * 60 * 24 * 10).toISOString(); const leads = await bxAll("crm.lead.list", {
filter: { ">=DATE_CREATE": since, ASSIGNED_BY_ID: Number(userId) }, select,
order: { ID: "DESC" },
});
STATE.leadsByUser.set(String(userId), leads || []); const sAt = leadStageId("EM ATENDIMENTO");
const atList = sAt ? (leads || []).filter((l) => String(l.STATUS_ID) === String(sAt)) : []; const prev = STATE.leadsAtendimentoIdsByUser.get(String(userId)) || new Set();
const nowSet = new Set(atList.map((l) => String(l.ID)));

let hasNew = false;
for (const id of nowSet) { if (!prev.has(id)) { hasNew = true; break; } } if (hasNew && nowSet.size) STATE.leadsAlertUsers.add(String(userId));
if (!nowSet.size) STATE.leadsAlertUsers.delete(String(userId)); STATE.leadsAtendimentoIdsByUser.set(String(userId), nowSet);

return leads || [];
}

function play3Beeps() { try {
const ctx = new (window.AudioContext || window.webkitAudioContext)(); const now = ctx.currentTime;
const beep = (t) => {
const o = ctx.createOscillator(); const g = ctx.createGain();
o.type = "sine";
o.frequency.value = 880;
g.gain.value = 0.0001; o.connect(g);
g.connect(ctx.destination); o.start(t);
...
const j = JSON.parse(raw); if (!j) return false;
...
let colabIsEnum = false; let colabMap = {};
try {
...
const select = [
"ID","TITLE","STAGE_ID","DATE_CREATE","DATE_MODIFY","ASSIGNED_BY_ID", UF_TAREFA, UF_PRAZO, UF_URGENCIA, UF_ETAPA, UF_COLAB, UF_OBS,
];

const deals = await bxAll("crm.deal.list", { filter: { CATEGORY_ID: CATEGORY_MAIN },
select,
order: { ID: "DESC" },
});

const maps = { urgMap, tarefaMap, etapaMap, colabMap, colabIsEnum }; const parsed = (deals || []).map((d) => parseDeal(d, maps));
STATE.dealsAll = parsed;
STATE.dealsOpen = (parsed || []).filter((d) => !(STATE.doneStageId && String(d.STAGE_ID) === String(STATE.doneStageId)));

STATE.lastOkAt = new Date(); STATE.offline = false;
saveCache();

const missing = USERS.filter((u) => !STATE.userPhotoById.has(Number(u.userId))); await Promise.all(missing.map((u) => ensureUserPhoto(u.userId, u.name)));

if (!STATE.footerPhotosLoaded) {
await Promise.all(FOOTER_PARTNERS.map((p) => ensureUserPhoto(p.userId, ""))); STATE.footerPhotosLoaded = true;
}
...
function nextDays(fromDate, n){ const out = [];
const base = new Date(fromDate.getFullYear(), fromDate.getMonth(), fromDate.getDate(), 0,0,0,0);
for(let i=0;i<=n;i++){
const d = new Date(base);
d.setDate(base.getDate()+i); out.push(d);
}
return out;
}
function isWeekday(d){ const w = d.getDay();
return w !== 0 && w !== 6;
}
function occursOn(rule, d){
const t = (rule && rule.type) ? String(rule.type) : ""; if (t === "DAILY_BUSINESS") return isWeekday(d);
if (t === "WEEKLY") {
const arr = Array.isArray(rule.weekDays) ? rule.weekDays.map(Number) : []; return arr.includes(d.getDay());
}
if (t === "MONTHLY") {
const day = Number(rule.monthDay || 0); return day > 0 && d.getDate() === day;
}
if (t === "YEARLY") {
const md = String(rule.yearMD || "").trim(); // "MM-DD" const m = md.match(/^(\d{2})-(\d{2})$/);
if (!m) return false;
...
function extractMarkersFromDeals(deals){ const set = new Set();
(deals || []).forEach((d) => {
const txt = String(d && (d._obs || d[UF_OBS] || "")).trim(); if (!txt) return;
...
STATE.recurConfigDealIdByUser = new Map(); STATE.recurRulesByUser = new Map();

(cfgDeals || []).forEach((d) => {
const uid = String(d.ASSIGNED_BY_ID || "").trim(); if (!uid) return;

if (!STATE.recurConfigDealIdByUser.has(uid)) {
STATE.recurConfigDealIdByUser.set(uid, String(d.ID));
}

const raw = String(d[UF_OBS] || "").trim(); const j = safeJsonParse(raw);
const rules = (j && Array.isArray(j.rules)) ? j.rules : []; STATE.recurRulesByUser.set(uid, rules);
});
}

async function ensureConfigDealForUser(userId) { const uid = String(userId);
...
// ✅ SALVAR AQUI (cria o Deal de configuração de recorrência) const id = await bx("crm.deal.add", { fields });
const dealId = String(id);
STATE.recurConfigDealIdByUser.set(uid, dealId); STATE.recurRulesByUser.set(uid, []);
return dealId;
}

async function saveRulesForUser(userId, rules) { const uid = String(userId);
...
async function addRuleForUser(userId, rule) { const uid = String(userId);
const cur = STATE.recurRulesByUser.get(uid) || []; const next = cur.slice();
next.push(rule);
...
const next = cur.filter((r) => String(r.id) !== String(ruleId)); await saveRulesForUser(uid, next);
}

async function generateRecurringDealsWindow() { const now = Date.now();
if (now - STATE.recurLastGenAt < 60 * 1000) return; STATE.recurLastGenAt = now;

if (!STATE.recurRulesByUser || STATE.recurRulesByUser.size === 0) { await loadRecurrenceConfigDeals();
}

const markers = extractMarkersFromDeals(STATE.dealsAll || []); const today = new Date();
const days = nextDays(today, RECURRENCE_WINDOW_DAYS);

for (const u of USERS) {
const uid = String(u.userId);
const rules = STATE.recurRulesByUser.get(uid) || []; if (!rules.length) continue;

const stageId = await stageIdForUserName(u.name); if (!stageId) continue;

for (const rule of rules) {
const rid = String(rule.id || "").trim();
const title = String(rule.title || "").trim(); if (!rid || !title) continue;

const hh = Number(rule.hh ?? 9); const mm = Number(rule.mm ?? 0);
...
const fields = {
CATEGORY_ID: Number(CATEGORY_MAIN),
STAGE_ID: String(stageId), TITLE: title,
ASSIGNED_BY_ID: Number(uid), [UF_PRAZO]: iso,
};
// ✅ IMPORTANTE: agora aplica os UFs da regra na instância (isso faltava) if (rule.etapaUf) fields[UF_ETAPA] = String(rule.etapaUf);
if (rule.tipo) fields[UF_TAREFA] = String(rule.tipo); if (rule.urg) fields[UF_URGENCIA] = String(rule.urg); if (rule.colab) fields[UF_COLAB] = String(rule.colab);
fields[UF_OBS] = (baseObs ? (baseObs + "\n") : "") + marker; try {
setSoftStatus("Gerando recorrências…");
// ✅ SALVAR AQUI (cria instância do deal recorrente)
 
const newId = await bx("crm.deal.add", { fields }); markers.add(mk);
STATE.dealsAll.unshift({ ID: String(newId),
TITLE: title,
STAGE_ID: String(stageId),
ASSIGNED_BY_ID: Number(uid),
DATE_CREATE: new Date().toISOString(), DATE_MODIFY: new Date().toISOString(), [UF_PRAZO]: iso,
...
status: document.getElementById("eqd-status"), now: document.getElementById("eqd-now"),
main: document.getElementById("eqd-main"),
refresh: document.getElementById("eqd-refresh"), today: document.getElementById("eqd-today"),
calendar: document.getElementById("eqd-calendar"), multi: document.getElementById("eqd-multi"),
searchScope: document.getElementById("eqd-searchScope"), searchInput: document.getElementById("eqd-searchInput"), searchBtn: document.getElementById("eqd-searchBtn"),
darkToggle: document.getElementById("eqd-darkToggle"),
modalOverlay: document.getElementById("eqd-modalOverlay"), modalEl: document.getElementById("eqd-modal"),
modalTitle: document.getElementById("eqd-modalTitle"), modalBody: document.getElementById("eqd-modalBody"),
...
onerror="try{this.onerror=null;this.style.display='none'}catch(e){}" title="USER
${p.userId}" />`;
}).join("");
...
// 11) DARK TOGGLE
// =========================
const DARK_KEY = "eqd_dark_v7"; function applyDark(on) {
if (on) el.app.classList.add("eqd-dark"); else el.app.classList.remove("eqd-dark");
el.darkToggle.textContent = on ? "Modo claro" : "Modo escuro";
try { localStorage.setItem(DARK_KEY, on ? "1" : "0"); } catch (_) {}
}
(function initDark() { let on = false;
try { on = localStorage.getItem(DARK_KEY) === "1"; } catch (_) {} applyDark(on);
})();
...
// 12) MODAL
// =========================
function openModal(title, bodyHTML, opts) { el.modalTitle.textContent = title || "—"; el.modalBody.innerHTML = bodyHTML || ""; el.modalOverlay.style.display = "flex";
el.modalOverlay.setAttribute("aria-hidden", "false");

el.modalEl.classList.remove("wide"); el.modalEl.classList.remove("full");
if (opts && opts.wide) el.modalEl.classList.add("wide"); if (opts && opts.full) el.modalEl.classList.add("full");
}
function closeModal() {
el.modalOverlay.style.display = "none";
el.modalOverlay.setAttribute("aria-hidden", "true"); el.modalBody.onclick = null;
el.modalEl.classList.remove("wide"); el.modalEl.classList.remove("full");
}
...
function setSoftStatus(msg) { el.status.textContent = msg || "JS: ok"; } function setBusy(msg) { el.status.textContent = msg || "Executando…"; } function clearBusy() { el.status.textContent = "JS: ok"; }

// =========================
// 13) CLOCK
// =========================
function tickClock() { const d = new Date();
el.now.textContent = `${fmtDateOnly(d)} • ${String(d.getHours()).padStart(2,
...
// 14) BUSCA (admin)
// =========================
function fillSearchScope() { el.searchScope.innerHTML =
...
function ensureAdminForSearch() { const now = Date.now();
if (now - LAST_ADMIN_PIN_OK_AT < 10 * 60 * 1000) return true; const pin = askPin();
if (!isAdmin(pin)) return false; LAST_ADMIN_PIN_OK_AT = now;
return true;
}

function dealUserNameByAssigned(assignedId) {
const u = USERS.find((x) => String(x.userId) === String(assignedId)); return u ? u.name : `USER ${assignedId || "—"}`;
}

function sortDeals(deals) { function createdMs(d) {
const x = d && d.DATE_CREATE ? new Date(d.DATE_CREATE) : null; const t = x ? x.getTime() : NaN;
return Number.isFinite(t) ? t : 0;
}
function prazoMs(d) {
const v = d && d._prazo;
if (!v) return Number.POSITIVE_INFINITY; const x = new Date(v);
const t = x.getTime();
...
.slice()
.sort((a, b) => {
const ua = isUrgenteText(a._urgTxt) ? 0 : 1; const ub = isUrgenteText(b._urgTxt) ? 0 : 1; if (ua !== ub) return ua - ub;

const la = a._late ? 0 : 1; const lb = b._late ? 0 : 1;
if (la !== lb) return la - lb;

const pa = prazoMs(a), pb = prazoMs(b); if (pa !== pb) return pa - pb;
...
const kwRaw = String(el.searchInput.value || "").trim(); const kw = norm(kwRaw);
const scope = String(el.searchScope.value || "  ALL  ");

if (!kw) { openModal("Busca", `<div class="eqd-empty">Digite uma palavra-chave.</div>`); return; }

 
let base = (STATE.dealsOpen || []).slice();
if (scope !== "  ALL  ") base = base.filter((d) => String(d.ASSIGNED_BY_ID "") === String(scope));
...
const whoLine = scope === "  ALL  " ? `<div style="font-size:11px;font- weight:950;opacity:.80">USER: ${escHtml(who)}</div>` : ``;
...
const MONTHS_PT =
["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novem  bro","Dezembro"];
...
const first = new Date(y, m, 1); const jsDow = first.getDay();
const monIndex = (jsDow + 6) % 7;
const start = new Date(y, m, 1 - monIndex); const cells = [];
for (let i = 0; i < 42; i++) { const d = new Date(start);
d.setDate(start.getDate() + i); cells.push(d);
}
return { first, cells };
}

function renderCalendarBody(selOverride) { const sel = selOverride || selectedDate;
const { cells } = monthGrid(calendarCursor); const today = new Date();
const title = `${MONTHS_PT[calendarCursor.getMonth()]} ${calendarCursor.getFullYear()}`;

const dowRow = DOW_PT.map((d) => `<div class="calDow">${d}</div>`).join(""); const cellHtml = cells.map((d) => {
const off = d.getMonth() !== calendarCursor.getMonth();
const cls = ["calCell", off ? "off" : "", sameDay(d, today) ? "today" : "", sameDay(d, sel)
? "sel" : ""].filter(Boolean).join(" "); const y = d.getFullYear();
const m = String(d.getMonth()+1).padStart(2,"0"); const dd = String(d.getDate()).padStart(2,"0");
...
<div style="font-size:11px;font-weight:900;opacity:.72">Duplo clique no dia para
abrir</div>
</div>
</div>
...
const y = Number(m[1]), mo = Number(m[2])-1, d = Number(m[3]); const dt = new Date(y, mo, d, 0,0,0,0);
return Number.isNaN(dt.getTime()) ? null : dt;
}

function attachCalendarHandlers(host, onApply) { let lastKey = "";
let lastAt = 0;
...
if (act === "calPrev" || act === "calNext" || act === "calToday") return doNav(act); if (act === "calPick") {
const key = a.getAttribute("data-day"); const d = parseDayKey(key);
if (!d) return;

selectedDate = d;
calendarCursor = new Date(d.getFullYear(), d.getMonth(), 1); host.innerHTML = renderCalendarBody();

const now = Date.now();
const isDouble = (key === lastKey) && (now - lastAt <= 360); lastKey = key; lastAt = now;
...
const a = e.target.closest('[data-action="calPick"]'); if (!a) return;
const key = a.getAttribute("data-day"); const d = parseDayKey(key);
if (!d) return; onApply(d);
});
}

function openCalendarModal() {
calendarCursor = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), 1); openModal("Calendário", `<div id="calHost">${renderCalendarBody()}</div>`);
const host = document.getElementById("calHost");

attachCalendarHandlers(host, (d) => {
selectedDate = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 0,0,0,0); closeModal();
renderCurrentView();
});
}
...
const title = (showWarn ? "⚠ " : "") + bestTitleFromText(deal.TITLE || ""); const prazoTxt = deal._prazo ? fmt(deal._prazo) : "Sem prazo";
const tags = [];
...
if (deal._tarefaTxt) tags.push(`<span class="eqd-tag">Tipo: ${trunc(deal._tarefaTxt, 26)}
</span>`);
if (deal._colabTxt) tags.push(`<span class="eqd-tag">COLAB: ${trunc(deal._colabTxt, 28)}
</span>`);
if (deal._etapaTxt) tags.push(`<span class="eqd-tag">ETAPA: ${trunc(deal._etapaTxt, 18)}
</span>`);
if (deal._urgTxt) {
...
function updateDealInState(dealId, patchFields) { const id = String(dealId);
const all = (STATE.dealsAll || []);
const open = (STATE.dealsOpen || []);
const a = all.find(d => String(d.ID) === id); const b = open.find(d => String(d.ID) === id);
const apply = (d) => { if (d) Object.assign(d, patchFields || {}); }; apply(a); apply(b);
}
...
if (normalUrgId) fields[UF_URGENCIA] = normalUrgId; if (followTipoId) fields[UF_TAREFA] = followTipoId; if (aguardEtapaId) fields[UF_ETAPA] = aguardEtapaId;
// ✅ SALVAR AQUI (cria deal de follow-up) await bx("crm.deal.add", { fields });
}

function openFollowUpModal(user, prefillName, opts) { const dt = new Date();
dt.setMinutes(dt.getMinutes() + 60);
const localDefault = new Date(dt.getTime() - dt.getTimezoneOffset() *
...
const warn = document.getElementById("fuWarn"); const btn = document.getElementById("fuSave");

btn.onclick = async () => {
const lk = `fuSave:${user.userId}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none"; warn.textContent = "";
const nm = String(document.getElementById("fuNome").value || "").trim(); if (!nm) throw new Error("Preencha o NOME DO NEGÓCIO.");
const prazoLocal = String(document.getElementById("fuPrazo").value || "").trim(); const prazoIso = localInputToIsoWithOffset(prazoLocal);
if (!prazoIso) throw new Error("Prazo inválido."); setBusy("Salvando follow-up…");
// ✅ SALVAR AQUI
...
setLeadsCtx(opts.returnToLeads.userId, opts.returnToLeads.kw || ""); return reopenLeadsModalSafe();
}
renderCurrentView();
} catch (e) {
...
if (t.includes(norm("FOLLOW-UP"))) return true; const title = norm(d.TITLE || "");
return title.startsWith(norm("FOLLOW-UP"));
 

function openFollowupListModalForUser(user) {
const all = (STATE.dealsAll || []).filter((d) => String(d.ASSIGNED_BY_ID || d._assigned || "")
=== String(user.userId));
const listAll = all.filter(isFollowupDeal);

const from = new Date(Date.now() - 1000*60*60*24*10).getTime(); const list = listAll.filter((d) => {
if (!d._prazo) return false;
const t = new Date(d._prazo).getTime(); return Number.isFinite(t) && t >= from;
});

const body = `
<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:space- between">
<div style="font-size:12px;font-weight:950;opacity:.85">Total: <strong>${list.length}
</strong></div>
<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
<input id="fuListSearch" class="eqd-searchInput"
style="width:min(520px,70vw);color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Pesquisar follow-up..." />
<button class="eqd-btn eqd-btnPrimary" id="fuListBtn">Buscar</button>
<button class="eqd-btn" id="fuListClear">Limpar</button>
</div>
</div>
<div id="fuListBox" style="margin-top:10px;display:flex;flex-direction:column;gap:8px">
</div>
`;
openModal(`Lista de FOLLOW-UP — ${user.name}`, body, { wide: true });

const box = document.getElementById("fuListBox"); const render = (kwRaw) => {
const kw = norm(String(kwRaw || "").trim()); const filtered = kw
? list.filter((d) => norm([d.TITLE || "", d._obs || "", d._colabTxt || "", d._etapaTxt || "", d._urgTxt || ""].join(" ")).includes(kw))
: list.slice();

const ordered = sortDeals(filtered);
box.innerHTML = ordered.length ? ordered.map((d) => makeDealCard(d, { allowBatch: false
})).join("") : `<div class="eqd-empty">Nenhum follow-up.</div>`;
};
...
document.getElementById("fuListSearch").value = ""; render(""); };
}
...
if (STATE.dealFieldsMeta) return STATE.dealFieldsMeta; const meta = await bx("crm.deal.fields", {});
STATE.dealFieldsMeta = meta || {}; return STATE.dealFieldsMeta;
}

function getFieldItemsFromMeta(meta, fieldCode) { const f = meta && meta[fieldCode];
const items = f && Array.isArray(f.items) ? f.items : [];
return items.slice().sort((a, b) => Number(a.SORT || 0) - Number(b.SORT || 0));
 

function renderOptions(items, placeholder) { const ph = placeholder || "Selecione…";
const first = `<option value="">${escHtml(ph)}</option>`; const rest = items
.map((it) => `<option value="${escHtml(String(it.ID))}">${escHtml(String(it.VALUE ?? it.NAME
?? it.ID))}</option>`)
.join("");
return first + rest;
}

function openNewTaskModalForUser(user, opts) { const dt0 = new Date();
...
const daysRow = [0, 1, 2, 3, 4, 5, 6].map((i) => { return `
...
rgba(0,0,0,.10);padding:10px;border-radius:12px;background:rgba(255,255,255,.55)">
${daysRow}
</div>
</div>
...
<textarea id="ntObs" rows="4" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px;font-weight:900;outline:none" placeholder="Observações...">
</textarea>
</div>
...
// preencher selects (async () => {
try {
const meta = await ensureDealFieldsMeta();
 
const etapaItems = getFieldItemsFromMeta(meta, UF_ETAPA); const tipoItems = getFieldItemsFromMeta(meta, UF_TAREFA); const urgItems = getFieldItemsFromMeta(meta, UF_URGENCIA);

const selEtapa = document.getElementById("ntEtapa"); const selTipo = document.getElementById("ntTipo");
const selUrg = document.getElementById("ntUrg");

if (selEtapa) selEtapa.innerHTML = renderOptions(etapaItems, "Selecione a etapa…"); if (selTipo) selTipo.innerHTML = renderOptions(tipoItems, "Selecione o tipo…");
if (selUrg) selUrg.innerHTML = renderOptions(urgItems, "Selecione a urgência…");
} catch (_) {
const selEtapa = document.getElementById("ntEtapa"); const selTipo = document.getElementById("ntTipo");
const selUrg = document.getElementById("ntUrg");
if (selEtapa) selEtapa.innerHTML = `<option value="">(falha ao carregar)</option>`; if (selTipo) selTipo.innerHTML = `<option value="">(falha ao carregar)</option>`;
...
const monthlyBox = document.getElementById("ntMonthlyBox"); const yearlyBox = document.getElementById("ntYearlyBox");
const warn = document.getElementById("ntWarn"); const btn = document.getElementById("ntSave");

function refreshRecUI() {
const v = String(selRec.value || "NONE");
weeklyBox.style.display = v === "WEEKLY" ? "block" : "none";
monthlyBox.style.display = v === "MONTHLY" ? "block" : "none"; yearlyBox.style.display = v === "YEARLY" ? "block" : "none";
}
selRec.onchange = refreshRecUI; refreshRecUI();

btn.onclick = async () => {
const lk = `ntSave:${user.userId}`; if (!lockTry(lk)) return;

try {
btn.disabled = true;
warn.style.display = "none"; warn.textContent = "";

const nomeNegocio = String(document.getElementById("ntTitle").value || "").trim(); if (!nomeNegocio) throw new Error("Preencha o NOME DO NEGÓCIO.");

const prazoLocal = String(document.getElementById("ntPrazo").value || "").trim(); const prazoIso = localInputToIsoWithOffset(prazoLocal);
...
const etapaUfVal = String((document.getElementById("ntEtapa") || {}).value || "").trim(); const tipoVal = String((document.getElementById("ntTipo") || {}).value || "").trim();
const urgVal = String((document.getElementById("ntUrg") || {}).value || "").trim();

const recType = String(selRec.value || "NONE"); const dt = new Date(prazoIso);
const hh = dt.getHours();
const mm = dt.getMinutes(); setBusy("Salvando…");
if (recType === "NONE") {
const stageId = await stageIdForUserName(user.name);
...
const fields = {
CATEGORY_ID: Number(CATEGORY_MAIN),
STAGE_ID: String(stageId), TITLE: nomeNegocio,
ASSIGNED_BY_ID: Number(user.userId), [UF_PRAZO]: prazoIso,
};

if (obs) fields[UF_OBS] = obs;
if (etapaUfVal) fields[UF_ETAPA] = etapaUfVal; if (tipoVal) fields[UF_TAREFA] = tipoVal;
if (urgVal) fields[UF_URGENCIA] = urgVal; if (colabTxt) fields[UF_COLAB] = colabTxt;
// ✅ SALVAR AQUI (cria uma tarefa normal) await bx("crm.deal.add", { fields });

closeModal();
await refreshData(true); renderCurrentView();
return;
}

const rule = {
id: makeRuleId(),
title: nomeNegocio, type: recType,
hh,
mm,
obs: obs || "",
createdAt: new Date().toISOString(), etapaUf: etapaUfVal || "",
tipo: tipoVal || "", urg: urgVal || "",
colab: colabTxt || "",
};

if (recType === "WEEKLY") {
const dows = [...document.querySelectorAll(".ntDow:checked")].map((x) => Number(x.value));
if (!dows.length) throw new Error("Selecione ao menos 1 dia da semana."); rule.weekDays = dows;
}

if (recType === "MONTHLY") {
const md = Number(document.getElementById("ntMonthDay").value || 0); if (!(md >= 1 && md <= 31)) throw new Error("Dia do mês inválido."); rule.monthDay = md;
}

if (recType === "YEARLY") {
const v = String(document.getElementById("ntYearMD").value || "").trim(); const m = v.match(/^(\d{4})-(\d{2})-(\d{2})$/);
if (!m) throw new Error("Escolha uma data válida no campo ANUAL."); rule.yearMD = `${m[2]}-${m[3]}`;
}

if (!STATE.recurRulesByUser || STATE.recurRulesByUser.size === 0) { await loadRecurrenceConfigDeals();
}
// ✅ SALVAR AQUI (salva a regra no deal de RECORRÊNCIA do user) await addRuleForUser(user.userId, rule);
...
// =========================
function leadMatchesKw(l, kwNorm) { if (!kwNorm) return true;
...
const cur = leadObs(lead); openModal(`OBS — Lead`, `
...
const warn = document.getElementById("lobWarn"); const btn = document.getElementById("lobSave");

btn.onclick = async () => {
const lk = `leadObsSave:${userId}:${leadId}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none";
const val = String(document.getElementById("lobText").value || "").trim(); setBusy("Salvando OBS do lead…");
// ✅ SALVAR AQUI (salva obs no lead)
await bx("crm.lead.update", { id: String(leadId), fields: { [LEAD_UF_OBS]: val } });

await loadLeadsForOneUser(userId); clearBusy();
closeModal();
...
btn.disabled = false; lockRelease(lk);
 
}
};
}

async function openManualLeadCreateModal(user, defaultStatusId) { const now = new Date();
...
${sAt ? `<option value="${escHtml(sAt)}"
${String(defaultStatusId||"")===String(sAt)?"selected":""}>EM ATENDIMENTO</option>` : ``}
${sAtendido ? `<option value="${escHtml(sAtendido)}"
${String(defaultStatusId||"")===String(sAtendido)?"selected":""}>ATENDIDO</option>` : ``}
${sQual ? `<option value="${escHtml(sQual)}"
${String(defaultStatusId||"")===String(sQual)?"selected":""}>QUALIFICADO</option>` : ``}
</select>
</div>
...
<textarea id="nlObs" rows="5" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px;font-weight:900;outline:none" placeholder="Observações...">
</textarea>
</div>
...
const warn = document.getElementById("nlWarn"); const btn = document.getElementById("nlSave");

btn.onclick = async () => {
const lk = `leadCreate:${user.userId}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none"; warn.textContent = "";

const STATUS_ID = String(document.getElementById("nlStatus").value || "").trim(); if (!STATUS_ID) throw new Error("Selecione a ETAPA.");

const NAME = String(document.getElementById("nlName").value || "").trim();
const LAST_NAME = String(document.getElementById("nlLast").value || "").trim(); const TITLE = String(document.getElementById("nlTitle").value || "").trim();
const op = String(document.getElementById("nlOp").value || "").trim();
const idade = String(document.getElementById("nlIdade").value || "").trim(); const tel = String(document.getElementById("nlTel").value || "").trim();
const bairro = String(document.getElementById("nlBairro").value || "").trim(); const fonte = String(document.getElementById("nlFonte").value || "").trim();
const dhLocal = String(document.getElementById("nlDh").value || "").trim(); const dhIso = dhLocal ? localInputToIsoWithOffset(dhLocal) : "";
const obs = String(document.getElementById("nlObs").value || "").trim();
if (!NAME && !TITLE) throw new Error("Preencha pelo menos NOME ou TÍTULO."); const fields = {
ASSIGNED_BY_ID: Number(user.userId), STATUS_ID,
};
...
if (idade) fields[LEAD_UF_IDADE] = idade; if (tel) fields[LEAD_UF_TELEFONE] = tel;
if (bairro) fields[LEAD_UF_BAIRRO] = bairro; if (fonte) fields[LEAD_UF_FONTE] = fonte;
if (dhIso) fields[LEAD_UF_DATAHORA] = dhIso; if (obs) fields[LEAD_UF_OBS] = obs;
...
await loadLeadsForOneUser(user.userId); clearBusy();
closeModal();
reopenLeadsModalSafe();
} catch (e) { clearBusy();
warn.style.display = "block";
...
const user = USERS.find((u) => String(u.userId) === String(userId)); if (!user) return;
setLeadsCtx(user.userId, String(kwRaw || "")); setBusy("Carregando LEADS…");
const leadsUser = await loadLeadsForOneUser(user.userId).catch(() => []);
clearBusy();
STATE.leadsAlertUsers.delete(String(user.userId)); const sAt = leadStageId("EM ATENDIMENTO");
const sAtendido = leadStageId("ATENDIDO"); const sQual = leadStageId("QUALIFICADO"); const sPerdido = leadStageId("PERDIDO");
...
const ok = sAtendido ? filtered.filter((l) => String(l.STATUS_ID) === String(sAtendido)) : []; const q = sQual ? filtered.filter((l) => String(l.STATUS_ID) === String(sQual)) : [];

openModal(`Leads — ${user.name}`, `
<div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:space-between;align- items:center">
<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
<input id="leadSearch" class="eqd-searchInput"
style="width:min(720px,85vw);color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Buscar lead por palavra (nome, operadora, tel, bairro, fonte, data/hora,
...
<button class="eqd-btn" style="padding:6px 10px;font-size:11px" data-
action="leadNewManual" data-userid="${user.userId}" data-defaultstatus="${escHtml(sAt||"")}">+ Criar</button>
</div>
...
<button class="eqd-btn" style="padding:6px 10px;font-size:11px" data- action="leadNewManual" data-userid="${user.userId}" data-
defaultstatus="${escHtml(sAtendido||"")}">+ Criar</button>
</div>
...
<button class="eqd-btn" style="padding:6px 10px;font-size:11px" data-
action="leadNewManual" data-userid="${user.userId}" data-defaultstatus="${escHtml(sQual||"")}">+ Criar</button>
</div>
...
function cardLead(l, column) { const op = leadOperadora(l); const idade = leadIdade(l);
const bairro = leadBairro(l); const fonte = leadFonte(l);
const tel = leadTelefone(l);
const when = leadDataHora(l) ? fmt(leadDataHora(l)) : "—"; const stageName = leadStageNameById(l.STATUS_ID) || "";
...
`<button class="leadBtn ${toStatus === sPerdido ? "leadBtnD" : toStatus ? "leadBtnP" : ""}" data-action="leadMove" data-leadid="${l.ID}" data-tostatus="${toStatus || ""}" data-
userid="${user.userId}">${label}</button>`;
...
document.getElementById("ld_q").innerHTML = q.length ? q.map((l) => cardLead(l, "Q")).join("")
: `<div class="eqd-empty">Nenhum</div>`;
...
// =========================
function overdueEmoji(overdueCount) { if (overdueCount <= 0) return "🟢"; if (overdueCount === 2) return "🟠"; if (overdueCount === 3) return "🟣"; if (overdueCount >= 4) return "🔴"; return "🟠";
}

function countUserStats(userId) { const id = String(userId);
const all = (STATE.dealsAll || []).filter((d) => String(d.ASSIGNED_BY_ID || d._assigned || "")
=== id);
...
const ds = dayStart(selectedDate).getTime(); const de = dayEnd(selectedDate).getTime();

const dayOpenPlusLate = open.filter((d) => { if (!d._prazo) return false;
const t = new Date(d._prazo).getTime(); if (!Number.isFinite(t)) return false; return (t >= ds && t <= de) || t < ds;
});

const dayDone = done.filter((d) => { if (!d._prazo) return false;
...
const photo = STATE.userPhotoById.get(Number(u.userId)) || ""; const stats = countUserStats(u.userId);
...
<img class="userPhoto" src="${photo || ""}" alt="${escHtml(u.name)}" referrerpolicy="no-
referrer"
...
function renderGeneral() { el.main.innerHTML = `
<div style="margin-bottom:10px;font-weight:950;opacity:.75">Dia selecionado:
${fmtDateOnly(selectedDate)}</div>
<div class="userGrid">
...
// =========================
let currentView = { kind: "general", userId: null, multi: null }; let lastViewStack = [];
...
function dealsOfSelectedDayPlusOverdueForUser(userId) { const ds = dayStart(selectedDate).getTime();
const de = dayEnd(selectedDate).getTime();
return (STATE.dealsOpen || []).filter((d) => {
if (String(d.ASSIGNED_BY_ID || d._assigned || "") !== String(userId)) return false; if (!d._prazo) return false;
const t = new Date(d._prazo).getTime(); if (!Number.isFinite(t)) return false; return (t >= ds && t <= de) || t < ds;
});
}
...
const cols = [[], [], []];
for (let i = 0; i < sortedDeals.length; i++) cols[i % 3].push(sortedDeals[i]); return cols;
}

function dealsForSpecialPanel(userId) {
const all = dealsOfSelectedDayPlusOverdueForUser(userId); const ds = dayStart(selectedDate).getTime();
const de = dayEnd(selectedDate).getTime();

const follow = all.filter((d) => isFollowupDeal(d) && d._prazo && (new Date(d._prazo).getTime() >= ds && new Date(d._prazo).getTime() <= de));
const tasks = all.filter((d) => !isFollowupDeal(d)); return { tasks, follow };
}

function leadsAttendedOnSelectedDay(userId) {
const leads = STATE.leadsByUser.get(String(userId)) || []; const sAtendido = leadStageId("ATENDIDO");
const ds = dayStart(selectedDate).getTime(); const de = dayEnd(selectedDate).getTime();

return (leads || []).filter((l) => { if (!sAtendido) return false;
if (String(l.STATUS_ID) !== String(sAtendido)) return false; const dt = tryParseDateAny(leadDataHora(l));
if (!dt) return false; const t = dt.getTime();
return t >= ds && t <= de;
});
}

function renderLeadMiniCardForPanel(l) { const op = leadOperadora(l);
const idade = leadIdade(l); const tel = leadTelefone(l);
const bairro = leadBairro(l); const fonte = leadFonte(l);
const when = leadDataHora(l) ? fmt(leadDataHora(l)) : "—"; const stageName = leadStageNameById(l.STATUS_ID) || "";
...
async function openBatchRescheduleAdvanced(ids) { const now = new Date();
now.setMinutes(now.getMinutes() + 60);
const localDefault = new Date(now.getTime() -
now.getTimezoneOffset()*60000).toISOString().slice(0,16);

openModal("Reagendar em lote", `
<div class="eqd-warn" id="brWarn"></div>
<div style="display:grid;grid-template-columns:1fr;gap:10px">
<div style="font-size:12px;font-weight:950;opacity:.85">Selecionadas:
<strong>${ids.length}</strong></div>
<div>
 

</div>
 
<div style="font-size:11px;font-weight:900;margin-bottom:6px">NOVO PRAZO (data e hora)

<input id="brPrazo" type="datetime-local" value="${localDefault}"
style="width:100%;padding:10px;border-radius:12px;border:1px solid
 
rgba(30,40,70,.16);font-weight:900" />
</div>
<div  style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap">
<button class="eqd-btn" data-action="modalClose">Cancelar</button>
...
const warn = document.getElementById("brWarn"); const btn = document.getElementById("brSave");

btn.onclick = async () => {
const lk = `batchResched:${ids.join(",")}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none";
const prazoLocal = String(document.getElementById("brPrazo").value || "").trim(); const prazoIso = localInputToIsoWithOffset(prazoLocal);
if (!prazoIso) throw new Error("Prazo inválido."); setBusy("Salvando lote…");
// ✅ SALVAR AQUI (update em lote — serial pra evitar 429) for (const id of ids) {
await bx("crm.deal.update", { id: String(id), fields: { [UF_PRAZO]: prazoIso } });
updateDealInState(id, { [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString(),
_late: false });
}
...
const user = USERS.find((u) => Number(u.userId) === Number(userId)); if (!user) return renderGeneral();
const photo = STATE.userPhotoById.get(Number(user.userId)) || ""; const hasLeadsBtn = LEAD_USERS.has(String(user.userId));
const leadsBtn = hasLeadsBtn ? `<button class="eqd-btn" data-action="leadsModal" data-
userid="${user.userId}" id="btnLeads">LEADS</button>` : ``;
...
const recurBtn = `<button class="eqd-btn" data-action="recurManager" data-
 
userid="${user.userId}">RECORRÊNCIA</button>`;
const isSpecial = SPECIAL_PANEL_USERS.has(String(user.userId)); if (!isSpecial) {
const dealsDay = dealsOfSelectedDayPlusOverdueForUser(user.userId); const ordered = sortDeals(dealsDay);
...
${cols[i].length ? cols[i].map((d) => makeDealCard(d, { allowBatch: true
})).join("") : `<div class="eqd-empty">Sem itens do dia</div>`}
</div>
</div>
`).join("")}
</div>
...
if (hasLeadsBtn) {
const btn = document.getElementById("btnLeads"); if (btn) {
...
const doUserSearch = () => {
const kw = norm(String(document.getElementById("userSearch").value || "").trim()); if (!kw) return alert("Digite uma palavra.");
const hits = ordered.filter((d) => norm([d.TITLE || "", d._obs || "", d._tarefaTxt || "", d._colabTxt || "", d._etapaTxt || "", d._urgTxt || ""].join(" ")).includes(kw));
openModal(`Busca — ${user.name} • ${hits.length}`, hits.length ? hits.map((d) =>
makeDealCard(d, { allowBatch: false })).join("") : `<div class="eqd-empty">Nada encontrado.
</div>`);
};
...
if (!ids.length) return alert("Selecione tarefas marcando 'Lote' nos cards."); await openBatchRescheduleAdvanced(ids);
};

return;
}

const { tasks, follow } = dealsForSpecialPanel(user.userId); const orderedTasks = sortDeals(tasks);
...
const leadsDay = leadsAttendedOnSelectedDay(user.userId); el.main.innerHTML = `
<div class="panelHead">
<div class="panelUserInfo">
<img class="panelUserPhoto" src="${photo || ""}" referrerpolicy="no-referrer"
onerror="try{this.onerror=null;this.src='';this.style.display='none'}catch(e){}" />
...
${orderedTasks.length ? orderedTasks.map((d) => makeDealCard(d, { allowBatch: true
})).join("") : `<div class="eqd-empty">Sem itens</div>`}
</div>
</div>
...
${orderedFollow.length ? orderedFollow.map((d) => makeDealCard(d, { allowBatch: true
})).join("") : `<div class="eqd-empty">Sem follow-up no dia</div>`}
</div>
</div>
...
// 22) MULTI SELEÇÃO
// =========================
let lastMultiSelection = []; function openMultiSelect() {
const pin = askPin();
...
${lastMultiSelection.includes(Number(u.userId)) ? "checked" : ""}>
...
const sel = [...document.querySelectorAll(".ms-u:checked")].map((x) => Number(x.value)); if (sel.length < 1) return alert("Selecione ao menos 1.");
if (sel.length > 6) return alert("Máximo 6."); lastMultiSelection = sel.slice();
closeModal();
pushView(currentView);
currentView = { kind: "multi", userId: null, multi: sel.slice() }; renderMultiColumns(sel);
};
}
...
<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin- top:10px">
${userIds.map((uid) => {
const u = USERS.find((x) => String(x.userId) === String(uid)) || { userId: uid, name:
`User ${uid}` };
return `
...
function renderMultiColumns(userIds) { const cols = userIds.length;
const ds = dayStart(selectedDate).getTime(); el.main.innerHTML = `
<div class="panelHead">
...
const photo = STATE.userPhotoById.get(Number(uid)) || ""; const deals = (STATE.dealsOpen || []).filter((d) => {
if (String(d.ASSIGNED_BY_ID || d._assigned || "") !== String(uid)) return false; if (!d._prazo) return false;
const t = new Date(d._prazo).getTime(); if (!Number.isFinite(t)) return false;
return (t >= ds && t <= dayEnd(selectedDate).getTime()) || t < ds;
});
const ordered = sortDeals(deals); return `
...
if (!forceNetwork) loadCache(); try {
await loadDeals();
await loadRecurrenceConfigDeals().catch(()=>{});
await generateRecurringDealsWindow().catch(()=>{}); renderFooterPeople();
...
if (currentView.kind === "multi") return renderMultiColumns(currentView.multi || []); renderGeneral();
}

async function markDone(dealId) {
if (!STATE.doneStageId) throw new Error("Etapa CONCLUÍDO não encontrada."); setBusy("Concluindo…");
// ✅ SALVAR AQUI (move para concluído)
await bx("crm.deal.update", { id: String(dealId), fields: { STAGE_ID: String(STATE.doneStageId) } });
updateDealInState(dealId, { STAGE_ID: String(STATE.doneStageId) }); clearBusy();
}

async function deleteDeal(dealId) { setBusy("Excluindo…");
// ✅ SALVAR AQUI (exclui)
await bx("crm.deal.delete", { id: String(dealId) });
STATE.dealsAll = (STATE.dealsAll || []).filter((d)=>String(d.ID)!==String(dealId));
STATE.dealsOpen = (STATE.dealsOpen || []).filter((d)=>String(d.ID)!==String(dealId)); clearBusy();
}

function openDoneMenu(dealId) { openModal("Concluir", `
...
const warn = document.getElementById("dnWarn"); const btn = document.getElementById("dnSave"); btn.onclick = async () => {
 
const lk = `done:${dealId}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none"; await markDone(dealId);
closeModal();
await refreshData(true); renderCurrentView();
} catch (e) {
warn.style.display = "block";
...
const d = (STATE.dealsAll || []).find(x=>String(x.ID)===String(dealId)); const dt = d && d._prazo ? new Date(d._prazo) : new Date();
dt.setMinutes(dt.getMinutes() + 60);
const localDefault = new Date(dt.getTime() -
...
const warn = document.getElementById("epWarn"); const btn = document.getElementById("epSave"); btn.onclick = async () => {
const lk = `prazo:${dealId}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none";
const prazoLocal = String(document.getElementById("epPrazo").value||"").trim(); const prazoIso = localInputToIsoWithOffset(prazoLocal);
if (!prazoIso) throw new Error("Prazo inválido.");
...
updateDealInState(dealId, { [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString() clearBusy();
closeModal();
await refreshData(true); renderCurrentView();
} catch (e) { clearBusy();
warn.style.display = "block";
...
const d = (STATE.dealsAll || []).find(x=>String(x.ID)===String(dealId)); const cur = d ? String(d.TITLE||"") : "";
...
const warn = document.getElementById("etWarn"); const btn = document.getElementById("etSave"); btn.onclick = async () => {
const lk = `title:${dealId}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none";
const title = String(document.getElementById("etTitle").value||"").trim(); if (!title) throw new Error("Título vazio.");

setBusy("Salvando título…");
// ✅ SALVAR AQUI
await bx("crm.deal.update", { id: String(dealId), fields: { TITLE: title } }); updateDealInState(dealId, { TITLE: title });
clearBusy();
...
const d = (STATE.dealsAll || []).find(x=>String(x.ID)===String(dealId)); const cur = d ? String(d[UF_OBS] || d._obs || "") : "";
...
const warn = document.getElementById("eoWarn"); const btn = document.getElementById("eoSave"); btn.onclick = async () => {
const lk = `obs:${dealId}`;
 
if (!lockTry(lk)) return; try {
btn.disabled = true;
warn.style.display = "none";
...
// ✅ SALVAR AQUI
await bx("crm.deal.update", { id: String(dealId), fields: { [UF_OBS]: val } }); updateDealInState(dealId, { [UF_OBS]: val, _obs: val, _hasObs: !!val });
clearBusy();
...
const d = (STATE.dealsAll || []).find(x=>String(x.ID)===String(dealId)); const cur = d ? String(d[UF_URGENCIA] || d._urgId || "") : "";
...
const sel = document.getElementById("euSel"); if (sel) sel.value = cur;

const warn = document.getElementById("euWarn"); const btn = document.getElementById("euSave"); btn.onclick = async () => {
const lk = `urg:${dealId}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none";
...
// ✅ SALVAR AQUI
await bx("crm.deal.update", { id: String(dealId), fields: { [UF_URGENCIA]: v } }); updateDealInState(dealId, { [UF_URGENCIA]: v, _urgId: v });
clearBusy();
...
const d = (STATE.dealsAll || []).find(x=>String(x.ID)===String(dealId)); const cur = d ? String(d[UF_COLAB] || d._colabTxt || "") : "";
...
const warn = document.getElementById("ccWarn"); const btn = document.getElementById("ccSave"); btn.onclick = async () => {
const lk = `colab:${dealId}`; if (!lockTry(lk)) return;
try {
btn.disabled = true;
warn.style.display = "none";
...
// ✅ SALVAR AQUI
await bx("crm.deal.update", { id: String(dealId), fields: { [UF_COLAB]: v } }); updateDealInState(dealId, { [UF_COLAB]: v, _colabTxt: v, _colabId: v });
clearBusy();
...
async function leadMove(userId, leadId, toStatus) { if (!toStatus) return;
setBusy("Movendo lead…");
// ✅ SALVAR AQUI (move lead)
await bx("crm.lead.update", { id: String(leadId), fields: { STATUS_ID: String(toStatus) } }); await loadLeadsForOneUser(userId);
clearBusy();
}
...
if (t === "DAILY_BUSINESS") return `Diária (dias úteis) •
${String(r.hh).padStart(2,"0")}:${String(r.mm).padStart(2,"0")}`;
if (t === "WEEKLY") return `Semanal (${(r.weekDays||[]).map(dowNamePt).join(", ")}) •
${String(r.hh).padStart(2,"0")}:${String(r.mm).padStart(2,"0")}`; if (t === "MONTHLY") return `Mensal (dia ${r.monthDay}) •
${String(r.hh).padStart(2,"0")}:${String(r.mm).padStart(2,"0")}`; if (t === "YEARLY") return `Anual (${r.yearMD}) •
${String(r.hh).padStart(2,"0")}:${String(r.mm).padStart(2,"0")}`; return t || "—";
};

openModal(`Recorrência — ${user.name}`, `
<div class="eqd-warn" id="rmWarn"></div>
<div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align- items:center">
<div style="font-size:12px;font-weight:950;opacity:.85">Regras: <strong>${rules.length}
</strong></div>
...
<button class="leadBtn leadBtnD" data-action="recurDelete" data-
userid="${user.userId}"  data-ruleid="${escHtml(String(r.id||""))}">EXCLUIR</button>
</div>
<div class="leadMeta">
...
const warn = document.getElementById("rmWarn"); const host = el.modalBody;

host.onclick = async (e) => {
const b = e.target.closest("[data-action='recurDelete']"); if (!b) return;
const rid = String(b.getAttribute("data-ruleid")||"").trim(); const uid = String(b.getAttribute("data-userid")||"").trim(); if (!rid || !uid) return;
if (!confirm("Excluir esta regra de recorrência?")) return; const lk = `recurDel:${uid}:${rid}`;
if (!lockTry(lk)) return;
try {
setBusy("Excluindo regra…");
 
// ✅ SALVAR AQUI (remove regra no JSON de recorrência) await deleteRuleForUser(uid, rid);
clearBusy(); closeModal();
await refreshData(true); renderCurrentView();
} catch (err) { clearBusy();
warn.style.display = "block";
warn.textContent = "Falha:\n" + (err.message || err);
...
// =========================
document.addEventListener("click", async (e) => { const a = e.target.closest("[data-action]");
if (!a) return;
const act = a.getAttribute("data-action");
if (act === "modalClose") return closeModal(); if (act === "openUser") {
const uid = a.getAttribute("data-userid"); if (!canOpenUserPanel(uid)) return;
pushView(currentView);
currentView = { kind:"user", userId: Number(uid), multi:null }; return renderCurrentView();
}

if (act === "backToPrevious") { currentView = popView();
...
if (act === "leadsModal") {
const uid = a.getAttribute("data-userid"); return openLeadsModalForUser(uid, "");
}

if (act === "followUpModal") {
const uid = a.getAttribute("data-userid");
const user = USERS.find(u=>String(u.userId)===String(uid)); if (!user) return;
...
if (act === "recurManager") {
const uid = a.getAttribute("data-userid"); return openRecurrenceManager(uid);
}
...
if (act === "leadObsModal") {
const uid = a.getAttribute("data-userid"); const lid = a.getAttribute("data-leadid"); return openLeadObsModal(uid, lid);
}
...
if (act === "leadMove") {
const uid = a.getAttribute("data-userid"); const lid = a.getAttribute("data-leadid"); const to = a.getAttribute("data-tostatus"); await leadMove(uid, lid, to);
return reopenLeadsModalSafe();
}

if (act === "leadFollowupModal") {
const uid = a.getAttribute("data-userid"); const lid = a.getAttribute("data-leadid");
const user = USERS.find(u=>String(u.userId)===String(uid)); if (!user) return;

// tenta prefill do nome
const leads = STATE.leadsByUser.get(String(uid)) || []; const lead = leads.find(l=>String(l.ID)===String(lid)); const prefill = lead ? leadTitle(lead) : "";
return openFollowUpModal(user, prefill, { returnToLeads: { userId: uid, kw: LAST_LEADS_CTX.kw || "" } });
}

// deal actions
if (act === "doneMenu") {
const id = a.getAttribute("data-id"); return openDoneMenu(id);
}
if (act === "editPrazo") {
const id = a.getAttribute("data-id"); return openEditPrazo(id);
}
if (act === "editTitle") {
const id = a.getAttribute("data-id"); return openEditTitle(id);
}
if (act === "editUrg") {
const id = a.getAttribute("data-id"); return openEditUrg(id);
}
if (act === "editObs") {
...
if (act === "changeColab") {
const id = a.getAttribute("data-id"); return openChangeColab(id);
}
if (act === "delete") {
const id = a.getAttribute("data-id");
if (!confirm("Excluir este negócio?")) return; const lk = `del:${id}`;
if (!lockTry(lk)) return; try {
await deleteDeal(id);
await refreshData(true); renderCurrentView();
} finally {
...
el.today.addEventListener("click", () => { selectedDate = new Date();
renderCurrentView();
});
el.multi.addEventListener("click", openMultiSelect); el.refresh.addEventListener("click", async () => {
await refreshData(true); renderCurrentView();
});
...
// =========================
(async function boot() {
await loadStagesForCategory(CATEGORY_MAIN); loadCache();
await refreshData(true); renderGeneral();
setInterval(async () => { await refreshData(true); renderCurrentView();
}, REFRESH_MS);
})();
