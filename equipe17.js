...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function moveProposalStageSafe(dealId, newStageId, extraPatch = {}, ctx = {}) {
    ...
    try {
      ...
      try { logSyncError({ entityType: "deal", entityId: dId, source: "proposalStageChange", action: "moveProposalStageSafe", message: e && e.message ? e.message : String(e), payload: { dealId: dId, newStageId: stageId, patch } }); } catch (_) {}
      throw e;
    } finally {
      lockRelease(lk);
    }
...
${marker}` : marker;
  ...
  function logSyncError(err = {}) {
    ...
    rows.push({
      ...
      entityType: String(err.entityType || ""),
      entityId: String(err.entityId || ""),
      operationId: String(err.operationId || ""),
      source: String(err.source || "sync"),
      action: String(err.action || ""),
      message: String(err.message || "Erro de sincronização"),
    ...
    const html = `
      ...
      ${rows.length ? `<div style="display:flex;flex-direction:column;gap:8px;max-height:72vh;overflow:auto;padding-right:4px">${rows.map((e) => `
        <div style="border:1px solid rgba(0,0,0,.12);border-radius:14px;padding:10px;background:rgba(255,255,255,.88)">
          ...
          <div style="font-size:12px;font-weight:900;opacity:.8;margin-top:4px">${escHtml(e.entityType || "")} ${escHtml(e.entityId || "")} • ${escHtml(fmtDateTime(e.createdAt || ""))}</div>
          <div style="white-space:pre-wrap;margin-top:6px;font-size:12px">${escHtml(e.message || "")}</div>
