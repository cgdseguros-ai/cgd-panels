  ...
  async function openRecurrenceBatchModalForUser(user) {
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        setTimeout(() => { try { openRecurrenceManager(String(user.userId)); } catch(_) {} }, 20);
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (recType === "NONE") {
          ...
          if (proposalMeta) { proposalMeta.tipoTarefa = taskTypeLabelFromValue(tipoVal || '') || String(tipoVal || '') || proposalMeta.tipoTarefa || ''; proposalMeta.tipoTarefaId = String(tipoVal || ''); }
          ...
          if (tipoVal) {
            ...
            parsedDeal._tarefaTxt = taskTypeLabelFromValue(tipoVal) || parsedDeal._tarefaTxt || '';
          }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalMissingFieldDefs(nextStageId, patch, baseDeal) {
    ...
    addReq('VALOR PRODUZIDO', 'OPPORTUNITY', 'AGUARDANDO DOCUMENTOS');
  ...
  async function openProposalRequiredFieldsModal(deal, nextStageId, returnCtx = {}, patch = {}) {
    ...
    if (btn) btn.onclick = async () => {
      ...
      try {
        ...
        PROPOSALS_REFRESH_HOLD_UNTIL = Date.now() + 2500;
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "proposalDelete") { if (!confirm("Excluir este contrato/proposta?")) return; await bx("crm.deal.delete", { id: String(id||"") }); removeProposalDealCaches(String(id||"")); return openProposalsModal(String((LAST_PROPOSALS_CTX&&LAST_PROPOSALS_CTX.userId)||userId||""), { ...(LAST_PROPOSALS_CTX||{}), noStack: true }); }
