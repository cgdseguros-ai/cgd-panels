...
(function () {
  ...
  function saveCache() {
    try {
      ...
      try { console.warn("[EQD][CACHE] Falha ao salvar cache local:", e); } catch (_) {}
    }
  }
  function loadCache() {
    try {
...
function persistSyncQueue(){ try { localStorage.setItem(SYNC_QUEUE_STORAGE_KEY, JSON.stringify(SYNC_QUEUE)); } catch(e) { try { console.warn("[EQD][SYNC] Falha ao salvar fila local:", e); } catch(_) {} } }
function restoreSyncQueue(){ try { const raw = localStorage.getItem(SYNC_QUEUE_STORAGE_KEY); const arr = raw ? JSON.parse(raw) : []; if (Array.isArray(arr)) arr.forEach((j) => SYNC_QUEUE.push(j)); } catch(e) { try { console.warn("[EQD][SYNC] Falha ao restaurar fila local:", e); } catch(_) {} } }
restoreSyncQueue();
...
function makeUserCard(u) {
  ...
  async function deleteDeal(dealId){
    const ok = await openSystemConfirmModal("Confirmar exclusao", "Excluir este item?", "Excluir");
    if (!ok) return;
  ...
  async function deleteLead(userId, leadId, opts){
    ...
    const futureFollowups = countOpenFutureFollowupsForLeadOrig(origemDelete);
    const msg = [
      forcePermanent ? "Excluir este lead definitivamente?" : "Excluir este lead?",
      futureFollowups ? `Atencao: tambem serao excluidos ${futureFollowups} FOLLOW-UP(s) futuro(s) vinculados a este lead.` : ""
    ].filter(Boolean).join("\n\n");
    const ok = await openSystemConfirmModal("Confirmar exclusao", msg, "Excluir");
    if (!ok) return;
...
document.addEventListener("click", async (e) => {
    const a = e.target.closest("[data-action]");
    if (!a) return;
    const root = document.getElementById("eqd-root");
