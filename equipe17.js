...
${marker}` : marker;
  ...
  function renderRecurrenceAuditResults(payload = {}) {
    ...
    return `<div style="display:flex;gap:8px;align-items:end;justify-content:space-between;flex-wrap:wrap;margin-bottom:10px">
      <div>
        ...
        <div style="font-size:12px;font-weight:850;opacity:.72">Modo diagnóstico: nada é corrigido sem confirmação dupla. Correção disponível apenas para grupos de confiança média/alta.</div>
      </div>
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        ...
        <button class="eqd-btn eqd-btnPrimary" id="recAuditRunBtn">Buscar auditoria</button>
      </div>
    </div>
  ...
  async function openRecurrenceAuditModal(opts = {}) {
    ...
    const ctx = { includePast: !!opts.includePast, includeDone: !!opts.includeDone, daysAhead: Math.max(7, Number(opts.daysAhead || 90) || 90), userScopeId: requestedScopeId };
    openModal("AUDITORIA DE RECORRÊNCIAS", `<div class="eqd-empty" style="padding:24px">Buscando duplicidades e recorrências antigas...</div>`, { full: true, noStack: true });
    let deals = [];
    let duplicateGroups = [];
    let legacyRows = [];
    try {
      ...
      try { logSyncError({ entityType: "recurrence", source: "recurrenceAudit", action: "scan", message: e && e.message ? e.message : String(e), payload: ctx }); } catch (_) {}
      return openModal("AUDITORIA DE RECORRÊNCIAS", `<div class="eqd-warn">Falha ao executar auditoria: ${escHtml(e && e.message ? e.message : String(e))}</div>`, { full: true, noStack: true });
    }
    openModal("AUDITORIA DE RECORRÊNCIAS", renderRecurrenceAuditResults({ opts: ctx, duplicateGroups, legacyRows }), { full: true, noStack: true });
    const run = document.getElementById("recAuditRunBtn");
    if (run) run.onclick = () => openRecurrenceAuditModal({
      includePast: !!((document.getElementById("recAuditPast") || {}).checked),
      includeDone: !!((document.getElementById("recAuditDone") || {}).checked),
      daysAhead: Number((document.getElementById("recAuditDays") || {}).value || 90) || 90,
  ...
  async function correctDuplicateRecurringDeals(ids = [], action = "markDone") {
    ...
    return openRecurrenceAuditModal({ includePast: !!((document.getElementById("recAuditPast") || {}).checked), includeDone: !!((document.getElementById("recAuditDone") || {}).checked), daysAhead: Number((document.getElementById("recAuditDays") || {}).value || 90) || 90, userScopeId: String((document.getElementById("recAuditScopeUserId") || {}).value || "") });
  }
