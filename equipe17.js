...
(function () {
  ...
  async function loadLeadsForOneUser(userId) {
    ...
    const select = [
      ...
      LEAD_UF_OPERADORA, LEAD_UF_IDADE, LEAD_UF_TELEFONE, LEAD_UF_BAIRRO, LEAD_UF_FONTE, LEAD_UF_DATAHORA, LEAD_UF_OBS, LEAD_UF_ATENDIDO_DIA, LEAD_UF_LEAD_ORIGEM, LEAD_UF_HELENA, LEAD_UF_PESSOAL,
    ].filter(Boolean);

    const statusIds = [leadStageId("NOVO LEAD"), leadStageId("EM ATENDIMENTO"), leadStageId("ATENDIDO"), leadStageId("QUALIFICADO")].filter(Boolean);
    let leads = [];
    let okCalls = 0;
    if (statusIds.length) {
  ...
  async function loadDeals(opts = {}) {
    ...
    if (!canIncremental) {
      ...
      STATE.dealsAll = (deals || []).map((d) => parseDeal(d, maps));
  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal) && linkedLeadOrig) {
      if (linkedLead && leadStageNameById(linkedLead.STATUS_ID) === "QUALIFICADO") tags.push(`<span class="eqd-tag" style="background:#dbeafe;color:#1d4ed8;border-color:#93c5fd">QUALIFICADO</span>`);
      ...
      tags.push(`<span class="eqd-tag">FUPs concluídos: ${doneCount}</span>`);
    }
    ...
    if (isFollowupDeal(deal) && linkedLead) {
      ...
      const atendidoEm = getLeadAtendidoStamp(linkedLead);
      if (atendidoEm) parts.push(`Atendido em: <strong>${escHtml(atendidoEm)}</strong>`);
    ...
    const actionButtons = isFollowupDeal(deal)
      ? `
            ${linkedLead && String(linkedLead.STATUS_ID) === String(leadStageId("PERDIDO")) ? `<button class="eqd-smallBtn eqd-smallBtnPrimary" data-action="resgatarLead" data-id="${deal.ID}">RESGATAR LEAD</button>` : ``}
  ...
  function enqueueSync(job) {
    SYNC_QUEUE.push(job);
    const delay = job && (job.type === "dealDelete" || job.type === "leadDelete") ? 80 : 450;
  ...
  async function flushSyncQueue() {
    ...
    try {
      ...
      while (SYNC_QUEUE.length) {
        ...
        if (!job) { SYNC_QUEUE.shift(); continue; }
        if (job.type === "dealAdd") { const realId = await bx("crm.deal.add", { fields: job.fields }); replaceDealIdLocal(job.tempId, String(realId)); SYNC_QUEUE.shift(); continue; }
        if (job.type === "dealUpdate") { if (isTempId(job.dealId)) { SYNC_QUEUE.push(SYNC_QUEUE.shift()); continue; } await bx("crm.deal.update", { id: String(job.dealId), fields: job.fields }); SYNC_QUEUE.shift(); continue; }
        if (job.type === "dealDelete") { if (!isTempId(job.dealId)) await bx("crm.deal.delete", { id: String(job.dealId) }); SYNC_QUEUE.shift(); continue; }
        if (job.type === "leadUpdate") { await bx("crm.lead.update", { id: String(job.leadId), fields: job.fields }); SYNC_QUEUE.shift(); continue; }
        if (job.type === "leadDelete") { await bx("crm.lead.delete", { id: String(job.leadId) }); SYNC_QUEUE.shift(); continue; }
        SYNC_QUEUE.shift();
      }
    } catch (_) {
  ...
  function openExistingFollowupModal(fupDeal, opts) {
    if (!fupDeal) return;
    const whenLocal = fupDeal._prazo ? new Date(new Date(fupDeal._prazo).getTime() - new Date().getTimezoneOffset()*60000).toISOString().slice(0,16) : "";
    openModal("FOLLOW-UP já agendado", `
      <div class="eqd-warn" id="exFuWarn"></div>
      <div style="display:flex;flex-direction:column;gap:10px">
        <div style="font-size:12px;font-weight:950;opacity:.85">Já existe um FOLLOW-UP futuro para este lead.</div>
        <div style="font-size:12px;font-weight:900">${escHtml(bestTitleFromText(fupDeal.TITLE||""))}</div>
        <input id="exFuPrazo" type="datetime-local" value="${whenLocal}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" />
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap">
          <button class="eqd-btn" data-action="modalClose">Cancelar</button>
          <button class="eqd-btn eqd-btnPrimary" id="exFuSave">REAGENDAR ESTE FOLLOW-UP</button>
        </div>
      </div>
    `);
    document.getElementById('exFuSave').onclick = async () => {
      const prazoIso = localInputToIsoWithOffset(String(document.getElementById('exFuPrazo').value||''));
      if (!prazoIso) return alert('Prazo inválido.');
      updateDealInState(String(fupDeal.ID), { [UF_PRAZO]: prazoIso, _prazo: prazoIso, DATE_MODIFY: new Date().toISOString() });
      enqueueSync({ type:'dealUpdate', dealId:String(fupDeal.ID), fields:{ [UF_PRAZO]: prazoIso } });
      closeModal();
      if (opts && opts.returnToLeads) { setLeadsCtx(opts.returnToLeads.userId, opts.returnToLeads.kw || ''); reopenLeadsModalSafe(); }
  ...
  function openFollowUpModal(user, prefillName, opts) {
    ...
    const alreadyFuture = origId ? countOpenFutureFollowupsForLeadOrig(origId) : 0;
    const existingFutureDeal = origId ? getFutureFollowupDealForLeadOrig(origId) : null;

    openModal(`FOLLOW-UP — ${user.name}`, `
      <div class="eqd-warn" id="fuWarn"></div>
      ${alreadyFuture ? `<div style="font-size:12px;font-weight:950;color:#92400e;background:#fef3c7;border:1px solid rgba(146,64,14,.18);padding:10px;border-radius:12px;margin-bottom:10px">Este lead já possui <strong>${alreadyFuture}</strong> follow-up agendado para hoje ou data futura. Use o botão abaixo para reagendar o que já existe.</div>
      <div style="margin-bottom:10px"><button class="eqd-btn" id="fuEditExisting">EDITAR FOLLOW-UP JÁ AGENDADO</button></div>` : ``}
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (origId && alreadyFuture && existingFutureDeal) {
          openExistingFollowupModal(existingFutureDeal, opts);
          return;
        }
  ...
  function firstNameFromText(v) {
    return String((v || "").trim().split(/\s+/)[0] || "").trim();
  }
  ...
  async function promptLinkLeadForOldFollowup(deal, actionLabel) {
    ...
    if (ownerUid && (!STATE.leadsByUser.get(ownerUid) || !STATE.leadsByUser.get(ownerUid).length)) {
      try { await loadLeadsSnapshotAllUsers({ force: true }); } catch (_) {}
    }
    const arr = ownerUid ? (STATE.leadsByUser.get(ownerUid) || []) : Array.from(STATE.leadsByUser.values()).flat();
    const titleBase = String(bestTitleFromText(String(deal.TITLE || "")).replace(/^FOLLOW-UP\s*/i, "")).trim();
    const first = norm(firstNameFromText(titleBase));
    let candidates = (arr || []).filter((l) => {
      const nm = norm(firstNameFromText(leadTitle(l)));
      return first && nm === first;
    });
    if (!candidates.length && titleBase) {
      const needle = norm(titleBase);
      candidates = (arr || []).filter((l) => norm(leadTitle(l)).includes(first || needle) || needle.includes(norm(firstNameFromText(leadTitle(l)))));
    }
    if (!candidates.length) return null;
    return await new Promise((resolve) => {
      openModal(`Vincular FOLLOW-UP ao lead`, `
        <div style="display:flex;flex-direction:column;gap:10px">
          <div style="font-size:12px;font-weight:900;line-height:1.4">Este FOLLOW-UP antigo não tem lead vinculado. Deseja vincular para <strong>${escHtml(actionLabel || "continuar")}</strong>?</div>
          <select id="oldFuLeadPick" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">
            <option value="">Selecione um lead…</option>
            ${candidates.map((l) => `<option value="${escHtml(String(l.ID))}">${escHtml(leadTitle(l))} • ${escHtml(leadStageNameById(l.STATUS_ID)||"Sem etapa")}</option>`).join("")}
          </select>
      ...
      if (okbtn) okbtn.onclick = async () => {
        const sel = String((document.getElementById('oldFuLeadPick') || {}).value || '').trim();
        if (!sel) return alert('Selecione um lead.');
        let lead = (arr || []).find((l) => String(l.ID) === sel || String(leadOrigemId(l) || '') === sel) || null;
        if (!lead) {
          try { await loadLeadsSnapshotAllUsers({ force: true }); } catch (_) {}
          lead = getLeadByOrigemId(sel) || Array.from(STATE.leadsByUser.values()).flat().find((l) => String(l.ID) === sel) || null;
        }
        if (lead) {
          ...
          if (origem) {
            ...
            enqueueSync({ type: 'dealUpdate', dealId: String(deal.ID), fields: { [DEAL_UF_LEAD_ORIGEM]: origem } });
          }
        }
        done(lead);
      };
  ...
  function getFutureFollowupDealForLeadOrig(origId, ignoreDealId) {
    const needle = String(origId || "").trim();
    if (!needle) return null;
    const base = dayStart(new Date()).getTime();
    return sortDeals((STATE.dealsOpen || []).filter((d) => isFollowupDeal(d) && String(d.ID) !== String(ignoreDealId || "") && getDealLeadOrigemId(d) === needle && d._prazo && new Date(d._prazo).getTime() >= base))[0] || null;
  }
  function countDoneFollowupsForLeadOrig(origId) {
    const needle = String(origId || "").trim();
    if (!needle) return 0;
    return (STATE.dealsAll || []).filter((d) => isFollowupDeal(d) && getDealLeadOrigemId(d) === needle && STATE.doneStageId && String(d.STAGE_ID) === String(STATE.doneStageId) && String(d[UF_OBS]||"").indexOf("[FUP_NAO_FEITO]") < 0).length;
  }
  ...
  function openFollowupListModalForUser(user) {
    ...
    openModal(`Lista de FOLLOW-UP — ${user.name}`, `
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:space-between">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <button class="eqd-btn" id="fuListSelect10New">10 RECENTES</button>
          <button class="eqd-btn" id="fuListSelect10Old">10 ANTIGOS</button>
          <button class="eqd-btn" id="fuListSelect20New">20 RECENTES</button>
          <button class="eqd-btn" id="fuListSelect20Old">20 ANTIGOS</button>
          <button class="eqd-btn" id="fuListOntem">ONTEM</button>
          <button class="eqd-btn" id="fuListHoje">HOJE</button>
          <button class="eqd-btn" id="fuListAmanha">AMANHÃ</button>
          <input id="fuListDate" type="date" style="padding:6px 10px;border-radius:999px;border:1px solid rgba(0,0,0,.18)">
          <input id="fuListOper" placeholder="Operadora" style="padding:6px 10px;border-radius:999px;border:1px solid rgba(0,0,0,.18)">
    ...
    const render = (kwRaw) => {
      ...
      const dateVal = String((document.getElementById('fuListDate')||{}).value || '');
      const operVal = norm(String((document.getElementById('fuListOper')||{}).value || '').trim());
      const filtered = (kw ? list.filter((d) => norm([d.TITLE || "", d._obs || "", d._colabTxt || "", d._etapaTxt || "", d._urgTxt || ""].join(" ")).includes(kw)) : list.slice()).filter((d)=>{
        const lead = getLeadByOrigemId(getDealLeadOrigemId(d));
        if (operVal && norm(leadOperadora(lead)) !== operVal) return false;
        const due = d._prazo ? ymdKey(new Date(d._prazo)) : '';
        if (dateVal && due !== dateVal.replace(/-/g,'-')) return false;
        if (fuFilterMode==='ONTEM') { const x=new Date(); x.setDate(x.getDate()-1); if (due!==ymdKey(x)) return false; }
        if (fuFilterMode==='HOJE') { if (due!==ymdKey(new Date())) return false; }
        if (fuFilterMode==='AMANHA') { const x=new Date(); x.setDate(x.getDate()+1); if (due!==ymdKey(x)) return false; }
        return true;
      });
    ...
    const markByAge = (n, newest) => { const boxes=[...document.querySelectorAll('#fuListBox .eqd-batch')]; boxes.forEach(x=>x.checked=false); const deals=boxes.map(x=>getDealById(x.getAttribute('data-id'))).filter(Boolean).sort((a,b)=>{ const la=getLeadByOrigemId(getDealLeadOrigemId(a)); const lb=getLeadByOrigemId(getDealLeadOrigemId(b)); const ta=(getLeadMetricDate(la)?.getTime()||0); const tb=(getLeadMetricDate(lb)?.getTime()||0); return newest ? tb-ta : ta-tb; }).slice(0,n).map(d=>String(d.ID)); boxes.forEach(x=>{ if(deals.includes(String(x.getAttribute('data-id')))) x.checked=true; }); };
    document.getElementById('fuListSelect10New').onclick = () => markByAge(10,true);
    document.getElementById('fuListSelect10Old').onclick = () => markByAge(10,false);
    document.getElementById('fuListSelect20New').onclick = () => markByAge(20,true);
    document.getElementById('fuListSelect20Old').onclick = () => markByAge(20,false);
    document.getElementById('fuListOntem').onclick = () => { fuFilterMode='ONTEM'; render(document.getElementById('fuListSearch').value); };
    document.getElementById('fuListHoje').onclick = () => { fuFilterMode='HOJE'; render(document.getElementById('fuListSearch').value); };
    document.getElementById('fuListAmanha').onclick = () => { fuFilterMode='AMANHA'; render(document.getElementById('fuListSearch').value); };
    document.getElementById('fuListDate').onchange = () => render(document.getElementById('fuListSearch').value);
    document.getElementById('fuListOper').oninput = () => render(document.getElementById('fuListSearch').value);
    ...
    openModal(`Novo Lead — ${user.name}`, `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div style="grid-column:1 / -1">
          ...
          <input id="nlName" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" placeholder="Ex.: JOÃO SILVA" />
        </div>
        <div>
        ...
        <div>
          ...
          <select id="nlPlano" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Selecione…</option><option value="SIM">SIM</option><option value="NAO">NÃO</option></select>
        </div>
        <div style="grid-column:1 / -1">
  ...
  function openManualLeadBatchModal(user) {
    const row = () => `<div class="lbRow" style="display:grid;grid-template-columns:2fr .8fr 1.3fr 1.2fr 1.2fr .9fr auto;gap:8px;margin-bottom:8px"><input data-f="nome" placeholder="NOME" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><input data-f="idade" placeholder="IDADE" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><input data-f="tel" placeholder="TELEFONE" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><input data-f="bairro" placeholder="BAIRRO" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><input data-f="fonte" placeholder="FONTE" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><select data-f="plano" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">POSSUI PLANO</option><option value="SIM">SIM</option><option value="NAO">NÃO</option></select><button type="button" class="eqd-btn" data-action="leadBatchRemove">X</button></div>`;
    openModal(`Lead em lote — ${user.name}`, `<div class="eqd-warn" id="lbWarn"></div><div id="lbRows">${row()}${row()}${row()}</div><div style="display:flex;gap:8px;justify-content:space-between;flex-wrap:wrap;margin-top:10px"><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="eqd-btn" id="lbAdd">+ linha</button></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="lbSave">SALVAR LOTE</button></div></div>`, { full:true });
    const rows=document.getElementById('lbRows'); document.getElementById('lbAdd').onclick=()=>rows.insertAdjacentHTML('beforeend', row());
    rows.onclick=(e)=>{ const b=e.target.closest('[data-action="leadBatchRemove"]'); if(b) b.closest('.lbRow')?.remove(); };
    document.getElementById('lbSave').onclick=async()=>{ const warn=document.getElementById('lbWarn'); try{ const items=[...document.querySelectorAll('.lbRow')].map(r=>({ nome:r.querySelector('[data-f="nome"]').value.trim(), idade:r.querySelector('[data-f="idade"]').value.trim(), tel:r.querySelector('[data-f="tel"]').value.trim(), bairro:r.querySelector('[data-f="bairro"]').value.trim(), fonte:r.querySelector('[data-f="fonte"]').value.trim(), plano:r.querySelector('[data-f="plano"]').value.trim()})).filter(x=>x.nome); if(!items.length) throw new Error('Preencha ao menos um lead.'); setBusy('Salvando lote…'); for(const it of items){ const fields={ ASSIGNED_BY_ID:Number(user.userId), NAME:it.nome, TITLE:it.nome, STATUS_ID:String(leadStageId("NOVO LEAD")||""), [LEAD_UF_IDADE]:it.idade||'', [LEAD_UF_TELEFONE]:it.tel||'', [LEAD_UF_BAIRRO]:it.bairro||'', [LEAD_UF_FONTE]:it.fonte||'', [LEAD_UF_DATAHORA]: nowLocalStamp() }; if(it.plano) fields[LEAD_UF_POSSUI_PLANO]=it.plano; await bx('crm.lead.add',{fields}); } clearBusy(); closeModal(); return openLeadsModalForUser(String(user.userId), '', { forceReload:true }); } catch(e){ clearBusy(); warn.style.display='block'; warn.textContent='Falha\n'+(e.message||e); } };
  }
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    if (shouldLoadLeads) {
      ...
      setTimeout(async () => {
        try {
          ...
          if (modal.style.display === 'block' && LAST_LEADS_CTX.userId === String(user.userId)) openLeadsModalForUser(user.userId, LAST_LEADS_CTX.kw || '', { useCache: true });
        } catch (_) {}
      }, 0);
    }
      ...
      return `
        <div class="leadCard">
          ...
          <div class="leadMeta">
            ...
            ${fonte ? `<span>Fonte: <strong>${escHtml(fonte)}</strong></span>` : ``}${String(l[LEAD_UF_POSSUI_PLANO]||"").trim() ? `<span>Possui plano: <strong>${escHtml(String(l[LEAD_UF_POSSUI_PLANO]||""))}</strong></span>` : ``}
          ...
          <div class="leadBtns">${btns}${has45 ? `<button class="leadBtn" data-action="leadRevalidate" data-leadid="${l.ID}" data-userid="${user.userId}">REVALIDAR</button>` : ``}</div>
        </div>
      `;
    }
    ...
    document.getElementById("leadSearchBtn").onclick = () => openLeadsModalForUser(user.userId, String(document.getElementById("leadSearch").value || "").trim(), { useCache: true });
    document.getElementById("leadSearch").onkeydown = (e) => { if (e.key === "Enter") openLeadsModalForUser(user.userId, String(e.target.value || "").trim(), { useCache: true }); };
    document.getElementById("leadSearchClear").onclick = () => openLeadsModalForUser(user.userId, "", { useCache: true });
    const okNoFuBtn = document.getElementById("okNoFuBtn"); if (okNoFuBtn) okNoFuBtn.onclick = () => openModal(`ATENDIDOS sem FOLLOW-UP — ${user.name}`, ok.filter((l) => !hasFutureFollowupForLeadOrig(leadOrigemId(l) || l.ID)).map((l) => cardLead(l, "OK")).join("") || `<div class="eqd-empty">Nenhum</div>` , { wide:true });
    const qNoFuBtn = document.getElementById("qNoFuBtn"); if (qNoFuBtn) qNoFuBtn.onclick = () => openModal(`QUALIFICADOS sem FOLLOW-UP — ${user.name}`, q.filter((l) => !hasFutureFollowupForLeadOrig(leadOrigemId(l) || l.ID)).map((l) => cardLead(l, "Q")).join("") || `<div class="eqd-empty">Nenhum</div>` , { wide:true });
  }
    ...
    return `
      <div class="leadCard">
        ...
        <div class="leadMeta">
          ...
          ${stageName ? `<span>Etapa: <strong>${escHtml(stageName)}</strong></span>` : ``}
          ${atendidoStamp ? `<span>Atendido em: <strong>${escHtml(atendidoStamp)}</strong></span>` : ``}
          ${leadAgeDays(l) >= 45 ? `<span style="color:#b91c1c">Alerta: <strong>${leadAgeDays(l)} dias</strong></span>` : ``}
        </div>
        ${leadAgeDays(l) >= 45 ? `<div class="leadBtns" style="margin-top:8px"><button class="leadBtn" data-action="leadRevalidate" data-leadid="${l.ID}" data-userid="${l._ownerUserId || l.ASSIGNED_BY_ID || ''}">REVALIDAR</button></div>` : ``}
      </div>
    `;
  }
  ...
  async function openBatchRescheduleAdvanced(ids, opts = {}) {
    ...
    openModal("Reagendar em lote", `
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:10px">
        ...
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">STATUS DO FOLLOW-UP</div>
          <select id="brFeito" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="S">FEITO</option><option value="N">NÃO FEITO</option></select>
        </div>
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap">
          <button class="eqd-btn" data-action="modalClose">Cancelar</button>
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const feitoFlag = String(document.getElementById("brFeito").value || "S");
        const prazoIso = localInputToIsoWithOffset(prazoLocal);
        if (!prazoIso) throw new Error("Prazo inválido.");

        setBusy("Reagendando em lote…");
        const nowIso = nowLocalStamp();
        for (const id of ids) {
  ...
  async function markDone(dealId) {
    ...
    rebuildDealsOpen();
    enqueueSync({ type: "dealUpdate", dealId: String(dealId), fields: { STAGE_ID: String(STATE.doneStageId) } });
  }
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    openModal("Concluir e reagendar", `
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:10px">
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="drSave">CONCLUIR E CRIAR NOVO</button></div>
      </div>
    `);
    ...
    btn.onclick = async () => {
      try {
        ...
        if (!title) throw new Error("Título vazio."); if (!prazoIso) throw new Error("Prazo inválido.");
        ...
        if (isFollowupDeal(d)) {
          const obsBase = String(d[UF_OBS] || '').replace(/\[FUP_NAO_FEITO\]/g,'').trim();
          const nextObs = feitoFlag === 'N' ? `${obsBase} [FUP_NAO_FEITO]`.trim() : obsBase;
          updateDealInState(dealId, { [UF_OBS]: nextObs, _obs: nextObs, _hasObs: !!nextObs, DATE_MODIFY: new Date().toISOString() });
          enqueueSync({ type:'dealUpdate', dealId:String(dealId), fields:{ [UF_OBS]: nextObs } });
        }
        ...
        if (d[UF_TAREFA]) fields[UF_TAREFA] = d[UF_TAREFA]; if (d[UF_URGENCIA]) fields[UF_URGENCIA] = d[UF_URGENCIA]; if (d[UF_ETAPA]) fields[UF_ETAPA] = d[UF_ETAPA]; if (d[UF_COLAB]) fields[UF_COLAB] = d[UF_COLAB]; if (d[DEAL_UF_LEAD_ORIGEM]) fields[DEAL_UF_LEAD_ORIGEM] = d[DEAL_UF_LEAD_ORIGEM]; else if (resolvedLead) fields[DEAL_UF_LEAD_ORIGEM] = String(leadOrigemId(resolvedLead) || resolvedLead.ID || ""); if (obs) fields[UF_OBS] = obs;
        const tempId = makeTempId("TMP_DEAL"); upsertDealLocal(parseLocalDealFromFields(tempId, fields)); enqueueSync({ type: "dealAdd", tempId, fields });
  ...
  async function openChangeColabModal(dealId){
    ...
    if (isEnum) {
      ...
      const fuEditExisting = document.getElementById("fuEditExisting");
    if (fuEditExisting && existingFutureDeal) fuEditExisting.onclick = () => openExistingFollowupModal(existingFutureDeal, opts);
  ...
  function openLeadLostFollowupPrompt(uid, lead) {
    openModal('Lead perdido', `<div class="eqd-warn" id="lpWarn"></div><div style="display:flex;flex-direction:column;gap:10px"><div style="font-size:12px;font-weight:950">Deseja agendar FOLLOW-UP para este lead perdido?</div><div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap"><button class="eqd-btn" id="lpNo">Não</button><button class="eqd-btn eqd-btnPrimary" id="lpYes">Sim</button></div></div>`);
    document.getElementById('lpNo').onclick = async()=>{ closeModal(); await moveLeadStage(uid, String(lead.ID), leadStageId('PERDIDO')); reopenLeadsModalSafe(); };
    document.getElementById('lpYes').onclick = async()=>{ closeModal(); await moveLeadStage(uid, String(lead.ID), leadStageId('PERDIDO')); const u=USERS.find(x=>String(x.userId)===String(uid)); if(u) openFollowUpModal(u, leadTitle(lead), { leadId: leadOrigemId(lead)||String(lead.ID), returnToLeads:{ userId:uid, kw:LAST_LEADS_CTX.kw||'' } }); };
  }
  ...
  async function moveLeadStage(userId, leadId, toStatus){
    ...
    if (i >= 0) {
      ...
      if (target === String(sAtendido) && !curMark) {
        const nowIso = nowLocalStamp();
        patch[LEAD_UF_ATENDIDO_DIA] = nowIso;
        cur[LEAD_UF_ATENDIDO_DIA] = nowIso;
      }
      cur.STATUS_ID = target;
      arr[i] = cur;
      STATE.leadsByUser.set(uid, arr);
    }

    enqueueSync({ type: "leadUpdate", leadId: String(leadId), fields: patch });
    reopenLeadsModalSafe();
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadNewBatch") {
        ...
        const u = USERS.find(x => String(x.userId) === uid);
        if (!u) return;
        return openManualLeadBatchModal(u);
      }
      if (act === "leadRevalidate") {
        const uid = String(a.getAttribute("data-userid")||"");
        const leadId = String(a.getAttribute("data-leadid")||"");
        if (!uid || !leadId) return;
        if (!confirm('Revalidar este lead? O lead atual será marcado como perdido/descartado e um novo lead será criado com os dados principais.')) return;
        const arr = (STATE.leadsByUser.get(uid) || []).slice();
        const l = arr.find(x => String(x.ID) === leadId);
        if (!l) return;
        const fields = { ASSIGNED_BY_ID:Number(uid), NAME: leadTitle(l), TITLE: leadTitle(l), STATUS_ID:String(leadStageId('NOVO LEAD')||''), [LEAD_UF_OPERADORA]: leadOperadora(l), [LEAD_UF_IDADE]: leadIdade(l), [LEAD_UF_TELEFONE]: leadTelefone(l), [LEAD_UF_BAIRRO]: leadBairro(l), [LEAD_UF_FONTE]: leadFonte(l), [LEAD_UF_POSSUI_PLANO]: String(l[LEAD_UF_POSSUI_PLANO]||''), [LEAD_UF_DATAHORA]: nowLocalStamp() };
        await bx('crm.lead.add', { fields });
        await moveLeadStage(uid, leadId, leadStageId('PERDIDO'));
        return openLeadsModalForUser(uid, LAST_LEADS_CTX.kw || '', { forceReload:true });
      }
      ...
      if (act === "resgatarLead") {
        ...
        let lead = await resolveLeadForDeal(d);
        if (!lead) lead = await promptLinkLeadForOldFollowup(d, 'resgatar');
        if (!lead) return alert('Este FOLLOW-UP não tem lead de origem vinculado.');
        await moveLeadStage(String(lead.ASSIGNED_BY_ID || lead._ownerUserId || ''), String(lead.ID), leadStageId('EM ATENDIMENTO'));
        return renderCurrentView();
      }
      if (act === "followupPerdido") {
        ...
        if (!lead) return alert("Este FOLLOW-UP não tem lead de origem vinculado.");
        openLeadLostFollowupPrompt(String(lead.ASSIGNED_BY_ID || lead._ownerUserId || ""), lead);
        return;
        return renderCurrentView();
      }
  ...
  (async function init(){
    ...
    renderGeneral();
    setSoftStatus("JS: ok");
    await refreshData(false, { forceRecur: false, forceFullDeals: false });
    renderCurrentView();
  })();
