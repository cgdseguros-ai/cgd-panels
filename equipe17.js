...
${curObs}` : transferAlert;
  ...
  function makeDealCard(deal, context) {
    ...
    if (!isFollowupDeal(deal)) {
      ...
      if (proposalMeta) {
        const tipoTask = deal._tarefaTxt || taskTypeLabelFromValue(deal[UF_TAREFA] || deal._tarefaId || '') || String(deal[UF_TAREFA] || deal._tarefaId || '') || proposalMeta.tipoTarefa || '';
        const tagsMeta = [
  ...
  async function openRecurrenceBatchModalForUser(user) {
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        refreshData(true, { forceRecur: true }).catch(()=>{});
        renderCurrentView();
      } catch (e) {
        warn.style.display = 'block';
        warn.textContent = 'Falha:\n' + (e.message || e);
...
function makeUserCard(u) {
  ...
  function openEditMonthlyRecurrenceModal(userId, ruleId) {
    return openEditRecurrenceRuleModal(userId, ruleId);
  }
  ...
  function openEditRecurrenceRuleModal(userId, ruleId) {
    ...
    const timeVal = `${String(rule.hh ?? 9).padStart(2,'0')}:${String(rule.mm ?? 0).padStart(2,'0')}`;
    openModal('Editar recorrência', `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div><div style="font-size:11px;font-weight:900;margin-bottom:4px">HORÁRIO</div><input id="erTime" type="time" value="${escHtml(timeVal)}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div><div style="font-size:11px;font-weight:900;margin-bottom:4px">TIPO</div><select id="erType" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="DAILY_BUSINESS">Diária (dias úteis)</option><option value="WEEKLY">Semanal</option><option value="MONTHLY">Mensal</option><option value="YEARLY">Anual</option></select></div>
        <div id="erWeeklyBox" style="grid-column:1 / -1;display:none"><div style="font-size:11px;font-weight:900;margin-bottom:4px">DIAS DA SEMANA</div><div style="display:flex;gap:12px;flex-wrap:wrap;border:1px solid rgba(0,0,0,.10);padding:10px;border-radius:12px;background:rgba(255,255,255,.55)">${[0,1,2,3,4,5,6].map((i)=>`<label style="display:flex;gap:6px;align-items:center;font-size:12px;font-weight:900"><input type="checkbox" class="erDow" value="${i}" ${(Array.isArray(rule.weekDays)&&rule.weekDays.map(Number).includes(i))?'checked':''}>${dowNamePt(i)}</label>`).join('')}</div></div>
        <div id="erMonthlyBox" style="display:none"><div style="font-size:11px;font-weight:900;margin-bottom:4px">DIA DO MÊS</div><input id="erMonthDay" type="number" min="1" max="31" value="${escHtml(String(rule.monthDay || 1))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div id="erYearlyBox" style="display:none"><div style="font-size:11px;font-weight:900;margin-bottom:4px">DATA DO ANO</div><input id="erYearMD" type="date" value="${/^(\d{2})-(\d{2})$/.test(String(rule.yearMD||'')) ? `2026-${String(rule.yearMD)}` : ''}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div><div style="font-size:11px;font-weight:900;margin-bottom:4px">TIPO DA TAREFA</div><select id="erTipo" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Carregando…</option></select></div>
        <div><div style="font-size:11px;font-weight:900;margin-bottom:4px">CRIAR FUTURAS</div><select id="erBatchMode" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="COUNT">Por quantidade</option><option value="UNTIL">Até uma data</option></select></div>
        <div id="erCountBox"><div style="font-size:11px;font-weight:900;margin-bottom:4px">QUANTIDADE</div><input id="erCount" type="number" min="1" max="999" value="${escHtml(String(rule.materializeCount || 12))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div id="erUntilBox" style="display:none"><div style="font-size:11px;font-weight:900;margin-bottom:4px">ATÉ A DATA</div><input id="erUntil" type="date" value="${escHtml(String(rule.untilDate || ''))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:4px">OBS</div><textarea id="erObs" rows="3" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px;font-weight:900">${escHtml(String(rule.obs || ''))}</textarea></div>
        <div style="grid-column:1 / -1;display:flex;gap:10px;justify-content:flex-end"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="erSave">Salvar</button></div>
      </div>
    `, { wide: true });
    const typeSel = document.getElementById('erType');
    const weeklyBox = document.getElementById('erWeeklyBox');
    const monthlyBox = document.getElementById('erMonthlyBox');
    const yearlyBox = document.getElementById('erYearlyBox');
    const batchSel = document.getElementById('erBatchMode');
    const countBox = document.getElementById('erCountBox');
    const untilBox = document.getElementById('erUntilBox');
    typeSel.value = String(rule.type || 'DAILY_BUSINESS');
    batchSel.value = String(rule.batchMode || (rule.untilDate ? 'UNTIL' : 'COUNT'));
    const refresh = () => {
      const t = String(typeSel.value || 'DAILY_BUSINESS');
      weeklyBox.style.display = t === 'WEEKLY' ? 'block' : 'none';
      monthlyBox.style.display = t === 'MONTHLY' ? 'block' : 'none';
      yearlyBox.style.display = t === 'YEARLY' ? 'block' : 'none';
      const bm = String(batchSel.value || 'COUNT');
      countBox.style.display = bm === 'COUNT' ? 'block' : 'none';
      untilBox.style.display = bm === 'UNTIL' ? 'block' : 'none';
    };
    typeSel.onchange = refresh; batchSel.onchange = refresh; refresh();
    (async()=>{ try { const meta = await ensureDealFieldsMeta(); const tipoItems = getFieldItemsFromMeta(meta, UF_TAREFA); const selTipo = document.getElementById('erTipo'); if (selTipo) { selTipo.innerHTML = renderOptions(tipoItems, 'Selecione o tipo…'); selTipo.value = String(rule.tipo || ''); } } catch(_){} })();
    const warn = document.getElementById('erWarn');
    document.getElementById('erSave').onclick = async () => {
      try {
        const title = String(document.getElementById('erTitle').value || '').trim();
        if (!title) throw new Error('Preencha o nome.');
        const tm = String(document.getElementById('erTime').value || '').trim().match(/^(\d{2}):(\d{2})$/);
        if (!tm) throw new Error('Horário inválido.');
        const updated = { ...rule, title, type: String(typeSel.value || 'DAILY_BUSINESS'), hh: Number(tm[1]), mm: Number(tm[2]), tipo: String((document.getElementById('erTipo')||{}).value || '').trim(), obs: String((document.getElementById('erObs')||{}).value || '').trim(), batchMode: String(batchSel.value || 'COUNT'), materializeCount: String(batchSel.value||'COUNT')==='COUNT' ? Math.max(1, Math.min(999, Number((document.getElementById('erCount')||{}).value || 12) || 12)) : '', untilDate: String(batchSel.value||'COUNT')==='UNTIL' ? String((document.getElementById('erUntil')||{}).value || '').trim() : '' };
        delete updated.weekDays; delete updated.monthDay; delete updated.yearMD;
        if (updated.type === 'WEEKLY') {
          ...
          updated.weekDays = dows;
        }
        if (updated.type === 'MONTHLY') {
          const md = Number(document.getElementById('erMonthDay').value || 0); if (!(md>=1 && md<=31)) throw new Error('Dia do mês inválido.'); updated.monthDay = md;
        }
        if (updated.type === 'YEARLY') {
          const v = String(document.getElementById('erYearMD').value || '').trim(); const m = v.match(/^(\d{4})-(\d{2})-(\d{2})$/); if (!m) throw new Error('Escolha uma data válida.'); updated.yearMD = `${m[2]}-${m[3]}`;
        }
        rules[idx] = updated;
        await saveRulesForUser(uid, rules);
        MODAL_STACK = []; closeModal();
      } catch(e) { if (warn) { warn.style.display='block'; warn.textContent = 'Falha\n' + (e.message || e); } }
    };
  }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalFieldDefs() {
    return [
      ...
      { code: PROP_UF_LEAD_FONTE, label: 'LEAD FONTE', kind: 'text' },
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const select = [
      ...
      PROP_UF_TELEFONE, PROP_UF_OPER_LEAD, PROP_UF_OPER_FECHADA, PROP_UF_LEAD_FONTE, PROP_UF_LEAD_ORIGEM, PROP_UF_DATAHORA_LEAD, DEAL_UF_POSSUI_PLANO
    ];
  ...
  function proposalMissingFieldDefs(nextStageId, patch, baseDeal) {
    const stageName = proposalStageNameFromId(nextStageId) || proposalStageLabelForDeal(baseDeal);
  ...
  async function openProposalRequiredFieldsModal(deal, nextStageId, returnCtx = {}, patch = {}) {
    ...
    if (btn) btn.onclick = async () => {
      ...
      try {
        ...
        UI_REFRESH_HOLD_UNTIL = Date.now() + 7000;
        patchProposalDealCaches(String(deal.ID), { ...deal, ...finalPatch, _proposalStageName: proposalStageLabelById(finalPatch.STAGE_ID || deal.STAGE_ID) || miss.stageName });
...
${marker}` : marker;
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    let sourceDeals = adminView ? collectAllProposalDeals().filter((d)=>proposalDealAllowed(d)) : (proposalDealsCacheGet(uid) || []).filter((d)=>proposalDealAllowed(d));
    ...
    if (!sourceDeals.length) {
      ...
      sourceDeals = adminView ? await loadProposalDeals(false, {}) : await loadProposalDeals(false, { userId: uid });
    ...
    const applyBindings = () => {
      ...
      document.querySelectorAll('.proposal-inline-stage').forEach((sel) => {
        sel.onchange = async () => {
          ...
          const nextStageLabel = sel.selectedOptions && sel.selectedOptions[0] ? String(sel.selectedOptions[0].textContent || '').trim() : proposalStageLabelById(nextStage);
          ...
          PROPOSALS_REFRESH_HOLD_UNTIL = Date.now() + 15000;
          UI_REFRESH_HOLD_UNTIL = Date.now() + 7000;
          patchProposalDealCaches(dealId, { STAGE_ID: nextStage, _proposalStageName: nextStageLabel || proposalStageLabelById(nextStage) });
