...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalDealsCacheGet(uid) {
    ...
    if (!rec) return null;
    if ((Date.now() - Number(rec.at || 0)) > 180000) return null;
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const stale = (Date.now() - Number(STATE.proposalLastLoadedAt || 0)) > 180000;
    if (!force && uid) {
  ...
  async function openProposalStageMoveModal(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const reqErr = validateProposalRequiredFields(nextStage, { STAGE_ID: nextStage }, deal);
        ...
        closeModal();
        return openProposalsModal(String(returnCtx.userId || deal.ASSIGNED_BY_ID || ''), returnCtx);
      } catch (e) {
        warn.style.display = 'block';
        warn.textContent = 'Falha:\n' + (e.message || e);
      } finally { btn.disabled = false; }
  ...
  async function openProposalEditor(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const reqErr = validateProposalRequiredFields(String(patch.STAGE_ID || deal.STAGE_ID || ''), patch, deal);
        if (reqErr) throw new Error(reqErr);
        await safeDealUpdate(String(deal.ID), patch);
        patchProposalDealCaches(String(deal.ID), { ...patch, _proposalStageName: proposalStageNameFromId(patch.STAGE_ID || deal.STAGE_ID) });
        closeModal();
        return openProposalsModal(String(returnCtx.userId || deal.ASSIGNED_BY_ID || ''), returnCtx);
      } catch (e) {
        warn.style.display = 'block';
