...
function makeUserCard(u) {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      await loadDeals({ forceFull: !!(opts && opts.forceFullDeals), openOnly: !!(opts && opts.initialOpenOnly) && !(Array.isArray(STATE.dealsAll) && STATE.dealsAll.length) });
      ...
      if (opts.deferLeads) {
        ...
        if (shouldRefreshLeads) {
          try { await loadLeadsSnapshotAllUsers({ force: !!forceNetwork }); LAST_LEADS_REFRESH_AT = Date.now(); } catch (_) {}
        }
        overlayPendingSyncState();
      }
      renderFooterPeople();
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  (async function init(){
    ...
    Promise.resolve().then(async () => {
      try {
        await refreshData(false, { forceRecur: false, forceFullDeals: false, deferLeads: true, deferRecur: true, initialOpenOnly: !hasWarmCache });
        renderCurrentView();
        if (!hasWarmCache) ensureFullDealsLoadedInBackground(2200);
      } catch (_) {}
    });
  })();
