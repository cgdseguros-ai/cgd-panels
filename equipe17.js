...
(function () {
  ...
  const LEAD_USERS = new Set(["15", "19", "17", "23", "811", "3081", "3079", "3083", "3085", "3389", "815"]);
  ...
  async function loadDeals(opts = {}) {
    ...
    const canIncremental = !forceFull && !!STATE.lastDealsSyncAt && (nowMs - Number(STATE.lastFullDealsSyncAt || 0) < 10 * 60 * 1000) && Array.isArray(STATE.dealsAll) && STATE.dealsAll.length;
  ...
  async function openGlobalLeadsModal(filterUserId = "__ALL__", forceReload = false) {
    ...
    const nowMs = Date.now();
    const needsLoad = forceReload || !STATE.globalLeadsLoadedAt || (nowMs - Number(STATE.globalLeadsLoadedAt || 0) > 2 * 60 * 1000) || targetUsers.some((u) => !STATE.leadsByUser.has(String(u.userId)));
    if (needsLoad) { setBusy("Carregando LEADS gerais…"); for (const u of targetUsers) await loadLeadsForOneUser(u.userId).catch(() => []); STATE.globalLeadsLoadedAt = Date.now(); clearBusy(); }

    let allLeads = [];
    for (const u of targetUsers) {
      const arr = STATE.leadsByUser.get(String(u.userId)) || [];
      allLeads = allLeads.concat(arr.map((l) => ({ ...l, _ownerUserId: String(u.userId) })));
    }
    ...
    openModal("LEADS GERAL", `
      <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:space-between;align-items:center">
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><select id="glUser" class="eqd-searchSelect" style="color:#111;background:#fff;border-color:rgba(0,0,0,.18)">${userOptions}</select><button class="eqd-btn eqd-btnPrimary" id="glFilterBtn">Filtrar</button><button class="eqd-btn" id="glRefreshBtn">Atualizar LEADS</button></div>
    ...
    document.getElementById("glFilterBtn").onclick = () => openGlobalLeadsModal(String(document.getElementById("glUser").value || "__ALL__"), false);
    document.getElementById("glRefreshBtn").onclick = () => openGlobalLeadsModal(String(document.getElementById("glUser").value || "__ALL__"), true);
  }
  ...
  setInterval(() => { flushSyncQueue().catch(() => {}); }, 4000);
