...
(function () {
  ...
  function activeLeadStageIds() { return [leadStageId("NOVO LEAD"), leadStageId("EM ATENDIMENTO"), leadStageId("ATENDIDO"), leadStageId("QUALIFICADO")].filter(Boolean); }
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    function cardLead(l, column) {
      ...
      if (column === "AT") {
        ...
        btns = `${mkBtn("QUALIFICADO",sQual)}${mkBtn("PERDIDO",sPerdido)}${mkBtn("CONVERTER",sConv)}
                <button class="leadBtn" data-action="leadEdit" data-leadid="${l.ID}" data-userid="${user.userId}">EDITAR</button>
                <button class="leadBtn" data-action="leadTransferModal" data-leadid="${l.ID}" data-userid="${user.userId}">TRANSFERIR LEAD</button>
                <button class="leadBtn leadBtnD" data-action="leadDelete" data-leadid="${l.ID}" data-userid="${user.userId}">EXCLUIR</button>
...
function makeUserCard(u) {
  ...
  async function loadOperationalDataAfterMetrics(opts = {}) {
    ...
    if (shouldRefreshLeads) {
      ...
      await bootTimerMeasure("Leads ativos", () => loadLeadsSnapshotAllUsers({ force: !!opts.forceLeads, activeOnly: true }).catch(()=>{}), "NOVO LEAD, EM ATENDIMENTO, ATENDIDO e QUALIFICADO antes do READY");
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          pendingMutRegister("deal", dealId, opId, { [UF_PRAZO]: prazoIso }, 45000, { source: "reschedulePrazoOnly" });
          ...
          try {
            ...
            const confirmed = await confirmRemoteEntityFields("deal", dealId, { [UF_PRAZO]: prazoIso }, { attempts: 3, baseDelay: 450 });
            if (confirmed) pendingMutResolve("deal", dealId, opId);
          } catch (err) {
