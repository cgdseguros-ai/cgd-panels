...
restoreSyncQueue();
  ...
  function renderRecurrenceLineEditor(index) {
    const daysRow = [1,2,3,4,5,6,0].map((i) => `
      <label style="display:flex;gap:6px;align-items:center;font-size:11px;font-weight:900">
        <input type="checkbox" class="rlDow" data-index="${index}" value="${i}" ${[1,2,3,4,5].includes(i) ? 'checked' : ''}>
        ${dowNamePt(i)}
      </label>`).join('');
    return `
      <div class="recurLine" data-index="${index}" style="border:1px solid rgba(0,0,0,.12);border-radius:14px;padding:10px;background:rgba(255,255,255,.62);display:grid;grid-template-columns:2fr 1fr 1fr;gap:10px;margin-bottom:10px">
        <div>
        ...
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">HORÁRIO</div>
          <input class="rlTime" type="time" value="09:00" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" />
        </div>
        <div>
        ...
        <div class="rlWeeklyWrap" style="grid-column:1 / -1;display:none">
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">DIAS DA SEMANA</div>
          <div style="display:flex;gap:10px;flex-wrap:wrap;border:1px solid rgba(0,0,0,.10);padding:10px;border-radius:12px;background:rgba(255,255,255,.55)">${daysRow}</div>
        </div>
        <div class="rlMonthlyWrap" style="display:none">
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">DIA DO MÊS</div>
          <input class="rlMonthDay" type="number" min="1" max="31" value="1" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" />
        </div>
        <div class="rlYearlyWrap" style="display:none">
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">DATA DO ANO</div>
          <input class="rlYearMD" type="date" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" />
        </div>
        <div>
        ...
        <div>
          ...
          <select class="rlUrg" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Carregando…</option></select>
        </div>
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">COLAB</div>
          <input class="rlColab" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" placeholder="Opcional" />
        </div>
        <div style="grid-column:1 / -1">
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">OBS</div>
          <textarea class="rlObs" rows="2" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px;font-weight:900;outline:none" placeholder="Observações..."></textarea>
        </div>
        <div style="grid-column:1 / -1;display:flex;justify-content:flex-end">
          <button class="eqd-btn eqd-btnDanger rlRemove" type="button">Excluir linha</button>
        </div>
      </div>`;
  }
  ...
  async function openRecurrenceBatchModalForUser(user) {
    openModal(`Nova recorrência em lote — ${user.name}`, `
      ...
      <div style="font-size:12px;font-weight:900;opacity:.82;margin-bottom:10px">Cadastre várias regras de recorrência, uma por linha.</div>
    ...
    const addRow = async () => {
      ...
      try {
        ...
        const selUrg = line.querySelector('.rlUrg');
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        for (const line of lines) {
          ...
          const timeVal = String((line.querySelector('.rlTime') || {}).value || '09:00').trim();
  ...
  function openNewTaskModalForUser(user, opts) {
    ...
    function refreshRecUI() {
      ...
      batchMode.parentElement.style.display = isRec ? 'block' : 'none';
      const bm = String(batchMode.value || 'COUNT');
      batchCountBox.style.display = isRec && bm === 'COUNT' ? 'block' : 'none';
      untilBox.style.display = isRec && bm === 'UNTIL' ? 'block' : 'none';
    }
