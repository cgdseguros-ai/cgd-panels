    ...
    for (const d of createdDates) {
      ...
      try {
        ...
        STATE.dealsAll.unshift({
          ID: String(newId), TITLE: title, STAGE_ID: String(stageId), ASSIGNED_BY_ID: Number(uid),
          DATE_CREATE: new Date().toISOString(), DATE_MODIFY: new Date().toISOString(),
          [UF_PRAZO]: iso, [UF_OBS]: fields[UF_OBS], [UF_ETAPA]: fields[UF_ETAPA] || '', [UF_TAREFA]: fields[UF_TAREFA] || '',
          [UF_URGENCIA]: fields[UF_URGENCIA] || '', [UF_COLAB]: fields[UF_COLAB] || '', _prazo: new Date(iso).toISOString(),
          _late: false, _urgId: String(fields[UF_URGENCIA] || ''), _urgTxt: '', _tarefaId: String(fields[UF_TAREFA] || ''), _tarefaTxt: '',
          _etapaId: String(fields[UF_ETAPA] || ''), _etapaTxt: '', _colabId: String(fields[UF_COLAB] || ''), _colabTxt: String(fields[UF_COLAB] || ''),
          _obs: fields[UF_OBS], _hasObs: true, _assigned: String(uid), _accent: dealAccent({ ID: String(newId), TITLE: title }),
        });
      } catch (_) {}
    }
    return created;
  ...
  function openNewTaskModalForUser(user, opts) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        rebuildDealsOpen();
        renderCurrentView();
        setTimeout(() => { refreshData(true, { deferLeads:false }).catch(()=>{}); }, 80);
        return;
      } catch (e) {
        warn.style.display = "block";
        warn.textContent = "Falha:\n" + (e.message || e);
