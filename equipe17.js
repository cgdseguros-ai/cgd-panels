...
function makeUserCard(u) {
  ...
  function renderUserPanel(userId) {
    ...
    const stoppedProposalsTopBtn = isAdminPanelUser(user.userId) ? `<button class="eqd-btn eqd-btnPrimary" data-action="stoppedProposalsPanel" data-userid="${user.userId}">PROPOSTAS PARADAS</button>` : ``;
...
${marker}` : marker;
  ...
  function renderStoppedProposalsContent(deals, opts = {}) {
    const minDays = Number(opts.minDays || 7) || 0;
    ...
    const minOptions = [3,7,15,30,60].map((n) => `<option value="${n}" ${Number(minDays)===n?'selected':''}>Acima de ${n} dias</option>`).join('');
    const grouped = new Map();
    rows.forEach((d) => {
    ...
    const groupedHtml = rows.length ? [...grouped.entries()].map(([stage, arr]) => `
      ...
      </div>`).join('') : `<div class="eqd-empty">Nenhuma proposta parada encontrada com os filtros atuais.</div>`;
    return `
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">
        ...
        <span class="eqd-tag">Base: última modificação do deal</span>
      </div>
      ${groupedHtml}`;
  }
  async function openStoppedProposalsPanel(opts = {}) {
    const ctx = {
      ...
      minDays: Number(opts.minDays || 7) || 7,
      search: String(opts.search || '')
    };
    if (!collectAllProposalDeals().length) {
      openModal('PROPOSTAS PARADAS', `<div class="eqd-empty" style="padding:24px">Carregando propostas...</div>`, { full: true, noStack: true });
    ...
    const apply = () => openStoppedProposalsPanel({
      ...
      minDays: Number((document.getElementById('stoppedProposalDays') || {}).value || 7) || 7,
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "stoppedProposalsPanel") { return openStoppedProposalsPanel({ minDays: 7 }); }
