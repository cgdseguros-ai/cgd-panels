  ...
  async function deleteFutureRecurringInstancesForRule(userId, ruleId) {
    ...
    for (const d of targets) {
      try {
        ...
        await bx('crm.deal.delete', { id: String(d.ID) }).catch(() => null);
      } catch (_) {}
    }
    rebuildDealsOpen();
    renderCurrentView();
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalFieldDefs() {
    return [
      ...
      { code: PROP_UF_PLANO, label: 'PLANO ESCOLHIDO', kind: 'field' },
  ...
  async function openProposalCreateModal(userId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const newId = await bx('crm.deal.add', { fields });
        const deal = { ID: newId, ...fields, _proposalStageName: proposalStageNameFromId(fields.STAGE_ID) };
