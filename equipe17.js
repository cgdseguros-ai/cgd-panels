...
restoreSyncQueue();
  ...
  function clearQueuedDealUpdateFields(dealId, fieldNames) {
    const id = String(dealId || '').trim();
    const names = Array.isArray(fieldNames) ? fieldNames.map((x) => String(x || '').trim()).filter(Boolean) : [];
    if (!id || !names.length) return;
    for (let i = SYNC_QUEUE.length - 1; i >= 0; i--) {
      const job = SYNC_QUEUE[i];
      if (!job || job.type !== 'dealUpdate' || String(job.dealId || '').trim() !== id) continue;
      const fields = { ...(job.fields || {}) };
      names.forEach((k) => { delete fields[k]; });
      if (!Object.keys(fields).length) SYNC_QUEUE.splice(i, 1);
      else job.fields = fields;
    }
    persistSyncQueue();
  }
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (existingFutureDeal) {
          ...
          clearQueuedDealUpdateFields(String(existingFutureDeal.ID), ['TITLE', UF_PRAZO]);
  ...
  async function openBatchRescheduleAdvanced(ids, opts = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        for (const id of ids) {
          clearQueuedDealUpdateFields(String(id), [UF_PRAZO]);
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          UI_REFRESH_HOLD_UNTIL = Date.now() + 5000;
          clearQueuedDealUpdateFields(String(dealId), [UF_PRAZO]);
