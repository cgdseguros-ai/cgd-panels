...
${RECURRENCE_PAYLOAD_PREFIX}${enc}` : JSON.stringify(payload);
  ...
  function setBootProgress(pct, label) {
    try {
      ...
      if (el && el.meta) el.meta.textContent = label || "Carregando...";
    } catch (_) {}
  }
    ...
    try {
      ...
      for (const job of (SYNC_QUEUE || [])) {
        ...
        if (job.type === 'dealUpdate' && job.dealId && job.fields) {
          ...
          updateDealInState(String(job.dealId), { ...mergedFields, DATE_MODIFY: new Date().toISOString() });
          continue;
        }
