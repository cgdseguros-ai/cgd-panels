...
document.addEventListener("click", async (e) => {
  ...
  try {
    ...
    // IMPORTANTE: metricsReady só pode virar true depois de loadDeals/refreshData
    // concluir a carga usada para calcular os cards do painel geral.
    setBootPhase(BOOT_PHASES.METRICS_LOADING);
    setBootProgress(12, "Carregando dados das métricas...");

    try { clearExpiredLocalDealPatches(); } catch(_) {}

    await refreshData(true, {
      forceEvenDuringBoot: true,
      forceRecur: true,
      forceFullDeals: true,
      deferLeads: false
    });
    ...
    // ─── FASE 3 — FINALIZING: SINCRONIZAÇÃO CRÍTICA ANTES DAS FOTOS ───────
    setBootPhase(BOOT_PHASES.FINALIZING);
    setBootProgress(82, "Sincronizando alterações pendentes...");
