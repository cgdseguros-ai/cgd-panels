...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function ensureProposalMeta(force) {
    ...
    const [fieldsMeta, stages] = await Promise.all([
    ...
    (stages || []).forEach((s) => {
      ...
      const sid = String((s && s.STATUS_ID) || '').trim();
      if (!nm || !sid) return;
      byId.set(sid, nm);
  ...
  async function loadProposalDeals(force) {
    await ensureProposalMeta(false);
    const stale = (Date.now() - Number(STATE.proposalLastLoadedAt || 0)) > 60000;
    if (!force && Array.isArray(STATE.proposalDealsAll) && STATE.proposalDealsAll.length && !stale) return STATE.proposalDealsAll;
    const select = [
    ...
    const rows = await bxAll('crm.deal.list', {
      filter: { CATEGORY_ID: Number(CATEGORY_PROPOSALS) },
      order: { ID: 'DESC' },
      select,
    }).catch(() => []);
    STATE.proposalDealsAll = (rows || []).map((d) => ({ ...d, _proposalStageName: proposalStageNameFromId(d.STAGE_ID) }));
  ...
  function getProposalDealsForUser(userId) {
    const uid = String(userId || '');
    return (STATE.proposalDealsAll || []).filter((d) => String(d.ASSIGNED_BY_ID || '') === uid && proposalAllowedStage(proposalStageNameFromId(d.STAGE_ID)));
  }
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    const sourceDeals = adminView ? (STATE.proposalDealsAll || []).filter((d) => proposalAllowedStage(proposalStageNameFromId(d.STAGE_ID))) : getProposalDealsForUser(uid);
    const filterUid = adminView ? String(opts.filterUserId || '') : uid;
    const filteredDeals = filterUid ? sourceDeals.filter((d) => String(d.ASSIGNED_BY_ID || '') === filterUid) : sourceDeals.slice();
    const grouped = proposalStageOrder().map((nm) => ({ name: nm, deals: filteredDeals.filter((d) => proposalStageNameFromId(d.STAGE_ID) === nm) })).filter((g) => g.deals.length || adminView);
    ...
    openModal(title, `
      ...
      <div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:10px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ${adminView ? `<select id="proposalAdminUserFilter" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">${filterOptions}</select><select id="proposalRankField" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">${rankOptions}</select><button class="eqd-btn" id="proposalAdminApply">Aplicar</button>` : ''}
          <button class="eqd-btn" data-action="modalClose">Fechar</button>
        </div>
      </div>
    ...
    const apply = document.getElementById('proposalAdminApply');
    if (apply) apply.onclick = () => openProposalsModal(uid, { adminView: true, filterUserId: String((document.getElementById('proposalAdminUserFilter') || {}).value || ''), rankField: String((document.getElementById('proposalRankField') || {}).value || 'OPPORTUNITY') });
  }
