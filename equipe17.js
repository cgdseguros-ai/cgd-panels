...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalAmountNumber(deal) {
    const raw = String((deal && deal.OPPORTUNITY) == null ? '' : deal.OPPORTUNITY).replace(/\./g,'').replace(',', '.').trim();
    const n = Number(raw);
  ...
  function proposalSummaryRows(deal) {
    return proposalFieldDefs().map((fd) => {
      const txt = proposalFieldValueLabel(fd.code, deal ? deal[fd.code] : '');
      if (!txt || txt === '—') return '';
  ...
  function renderProposalCard(deal, opts = {}) {
    ...
    return `
      <div class="eqd-card" style="border-left:5px solid ${stageColor};padding:10px 12px">
        <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:start">
          <div style="min-width:0">
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:4px">
              <span class="eqd-tag" style="background:${stageColor};color:#fff;border-color:${stageColor}">${escHtml(stageName || 'Sem stage')}</span>
              <span class="eqd-tag">${escHtml(ownerName)}</span>
              <span class="eqd-tag"><strong>${escHtml(formatBRL(amount))}</strong></span>
            </div>
            <div style="font-weight:950;font-size:14px;line-height:1.25">${escHtml(bestTitleFromText(deal.TITLE || ''))}</div>
          </div>
          <div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end">
            ...
            <button class="eqd-btn" data-action="proposalMove" data-id="${escHtml(String(deal.ID))}" data-userid="${escHtml(String(opts.userId || deal.ASSIGNED_BY_ID || ''))}">Mover stage</button>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:6px 10px;margin-top:8px">${rows}</div>
      </div>`;
  }
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    let sourceDeals = adminView ? await loadProposalDeals(false, {}) : await loadProposalDeals(false, { userId: uid });
    ...
    openModal(title, `
      ...
      <div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:12px">
        <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
          <span class="eqd-tag">Total de cards: <strong>${filteredDeals.length}</strong></span>
          <span class="eqd-tag">Valor produzido em aberto: <strong>${escHtml(formatBRL(topOpenTotal))}</strong></span>
          <span class="eqd-tag">Fechados${closedMonth ? ' (' + proposalMonthLabel(closedMonth) + ')' : ''}: <strong>${escHtml(formatBRL(closedTotal))}</strong></span>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ${adminView ? `<select id="proposalAdminUserFilter" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">${filterOptions}</select>` : ``}
          ${adminView ? `<select id="proposalRankField" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">${rankOptions}</select>` : ``}
          <button class="eqd-btn" id="proposalApplyFilter">Aplicar</button>
          <button class="eqd-btn" data-action="modalClose">Fechar</button>
        </div>
      </div>
      ${adminView ? `<div style="display:grid;grid-template-columns:minmax(260px,360px) 1fr;gap:12px;margin-bottom:12px"><div style="border:1px solid rgba(0,0,0,.1);border-radius:16px;padding:10px;background:rgba(255,255,255,.8)"><div style="font-weight:950;margin-bottom:8px">Ranqueamento</div>${rankingRows.length ? rankingRows.map((r, i) => `<div style="display:flex;justify-content:space-between;gap:8px;margin:6px 0"><span style="font-size:12px;font-weight:900">${i+1}. ${escHtml(trunc(r.label, 28))}</span><span style="font-size:12px;font-weight:900">${escHtml(String(r.value))}</span></div>`).join('') : `<div class="eqd-empty">Sem dados.</div>`}</div><div style="border:1px solid rgba(0,0,0,.1);border-radius:16px;padding:10px;background:rgba(255,255,255,.8)"><div style="font-weight:950;margin-bottom:8px">Gráfico</div>${rankingRows.length ? renderProposalChart(rankingRows) : `<div class="eqd-empty">Sem dados.</div>`}</div></div>` : ''}
      <div style="display:flex;flex-direction:column;gap:10px">${grouped.map((g) => {
        const stageTotal = g.deals.reduce((s,d) => s + proposalAmountNumber(d), 0);
        const closedToolbar = g.name === 'NEGÓCIOS FECHADOS' ? `<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><label style="font-size:12px;font-weight:900">Competência</label><select id="proposalClosedMonth" style="padding:8px 10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Todas</option>${closedMonthOptions.map((mk) => `<option value="${escHtml(mk)}" ${mk===closedMonth?'selected':''}>${escHtml(proposalMonthLabel(mk))}</option>`).join('')}</select></div>` : '';
        return `<div style="border:1px solid rgba(0,0,0,.1);border-radius:16px;padding:10px;background:rgba(255,255,255,.78)"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:8px"><div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><span style="font-weight:950;color:${proposalStageBadgeColor(g.name)}">${escHtml(g.name)}</span><span class="eqd-tag">${g.deals.length} card(s)</span><span class="eqd-tag">${escHtml(formatBRL(stageTotal))}</span></div>${closedToolbar}</div><div style="display:flex;flex-direction:column;gap:8px">${g.deals.length ? g.deals.map((d) => renderProposalCard(d, { userId: d.ASSIGNED_BY_ID })).join('') : `<div class="eqd-empty">Sem contratos neste stage.</div>`}</div></div>`;
      }).join('')}</div>
    `, { full: true });
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "proposalDelete") { if (!confirm("Excluir este contrato/proposta?")) return; await bx("crm.deal.delete", { id: String(id||"") }); removeProposalDealCaches(String(id||"")); return openProposalsModal(String(userId||""), { adminView: String(currentView && currentView.userId || '') === '1' && String(userId||'') === '1' }); }
