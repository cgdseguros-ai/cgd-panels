...
${marker}` : marker;
  ...
  function openSalesFunnelPanel(userId, opts = {}) {
    ...
    const render = () => {
      ...
      const max=Math.max(1,data.totalPassedAtendido,data.totalConverted,data.totalLost);
      const bar=(label,val)=>`<div style="margin:8px 0"><div style="display:flex;justify-content:space-between;font-weight:950;font-size:12px"><span>${escHtml(label)}</span><span>${val}</span></div><div style="height:12px;border-radius:99px;background:rgba(0,0,0,.08);overflow:hidden"><div style="height:12px;width:${Math.max(4,Math.round(val/max*100))}%;background:rgba(0,0,0,.35)"></div></div></div>`;
