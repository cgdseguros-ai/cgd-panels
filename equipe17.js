...
(function () {
  ...
  injectCSS(`
    ...
    .leadStageChips{display:flex;gap:7px;justify-content:flex-end;align-items:center;flex-wrap:wrap;margin-top:6px;margin-left:auto;}
    .leadStageChip{cursor:pointer;user-select:none;display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:999px;border:2px solid rgba(17,24,39,.18);background:#9ca3af;color:#fff;font-size:22px;font-weight:950;line-height:1;box-shadow:0 3px 8px rgba(0,0,0,.10);}
    ...
    btn.onclick = async () => {
      try {
        ...
        const doneMode = await new Promise((resolve) => {
          ...
          `, { wide:true, allowStack:true, noStack:true });
          document.getElementById('drAskCancel').onclick = () => { closeModal(); resolve(''); };
          document.getElementById('drAskNo').onclick = () => { closeModal(); resolve('NAO_FEITO'); };
          document.getElementById('drAskYes').onclick = () => { closeModal(); resolve('FEITO'); };
        });
        if (!doneMode) { btn.disabled = false; return; }
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          const opId = `reschedPrazo_${Date.now()}`;
          pendingMutRegister("deal", dealId, opId, { [UF_PRAZO]: prazoIso }, 90000, { source: "reschedulePrazoOnly" });
          setBusy("Reagendando no Bitrix...");
          try {
            await safeDealUpdate(String(dealId), { [UF_PRAZO]: prazoIso });
            await confirmRemoteEntityFields("deal", dealId, { [UF_PRAZO]: prazoIso }, { attempts: 2, baseDelay: 500 }).catch(()=>null);
            ...
            pendingMutResolve("deal", dealId, opId);
            rebuildDealsOpen();
            saveCache();
            forceCloseAllModals();
            ...
            pendingMutFail("deal", dealId, opId);
            showRuntimeWarn(err);
            throw err;
          } finally { clearBusy(); }
          return;
...
${marker}` : marker;
  ...
  function proposalLostStageId() {
    return String(STATE.proposalStageIdByName.get(norm('PERDIDOS/RECUSADOS')) || STATE.proposalStageIdByName.get(norm('RECUSADO EM NEGOCIAÇÃO')) || '');
  }
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "linkedLeadMoveStage") {
        ...
        return moveLeadStage(uidLead, String(lead.ID), to);
      }
