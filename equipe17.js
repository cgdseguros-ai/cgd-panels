...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function renderProposalCard(deal, opts = {}) {
    ...
    return `
      <div class="eqd-card" style="border-left:4px solid ${stageColor};padding:6px 8px">
        <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:6px;align-items:start">
          ...
          <div style="display:flex;gap:4px;flex-wrap:wrap;justify-content:flex-end;align-items:center">
            <select class="proposal-inline-stage" data-id="${escHtml(String(deal.ID))}" style="padding:4px 6px;border-radius:8px;border:1px solid rgba(30,40,70,.16);font-weight:900;max-width:190px;font-size:11px">${proposalCardStageOptions(deal.STAGE_ID)}</select>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:2px 6px;margin-top:4px">${rows}</div>
      </div>`;
  }
  ...
  function renderProposalsModalContent(sourceDeals, opts = {}) {
    ...
    const closedMonth = String(opts.closedMonth || '');
    ...
    return `
      ...
      <div style="display:flex;flex-direction:column;gap:8px">${grouped.map((g) => {
        const stageTotal = g.deals.reduce((s,d) => s + proposalAmountNumber(d), 0);
        const closedToolbar = g.name === 'NEGÓCIOS FECHADOS' ? `<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><label style="font-size:12px;font-weight:900">Competência</label><select id="proposalClosedMonth" style="padding:7px 10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Todas</option>${closedMonthOptions.map((mk) => `<option value="${escHtml(mk)}" ${mk===closedMonth?'selected':''}>${escHtml(proposalMonthLabel(mk))}</option>`).join('')}</select></div>` : '';
        return `<div style="border:1px solid rgba(0,0,0,.1);border-radius:14px;padding:6px;background:rgba(255,255,255,.84)"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:4px"><div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><span style="font-weight:950;color:${proposalStageBadgeColor(g.name)}">${escHtml(g.name)}</span><span class="eqd-tag">${g.deals.length} card(s)</span><span class="eqd-tag">${escHtml(formatBRL(stageTotal))}</span></div>${closedToolbar}</div><div style="display:flex;flex-direction:column;gap:6px">${g.deals.length ? g.deals.map((d) => renderProposalCard(d, { userId: d.ASSIGNED_BY_ID })).join('') : `<div class="eqd-empty">Sem contratos neste stage.</div>`}</div></div>`;
      }).join('')}</div>`;
  }
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    LAST_PROPOSALS_CTX = { userId: uid, adminView, filterUserId: String(opts.filterUserId || ''), rankField: String(opts.rankField || 'OPPORTUNITY'), closedMonth: String(opts.closedMonth || '') };
    ...
    const applyBindings = () => {
      ...
      if (closedSel) closedSel.onchange = () => openProposalsModal(uid, {
        adminView,
        filterUserId: adminView ? String((document.getElementById('proposalAdminUserFilter') || {}).value || '') : uid,
        rankField: adminView ? String((document.getElementById('proposalRankField') || {}).value || 'OPPORTUNITY') : 'OPPORTUNITY',
        closedMonth: String(closedSel.value || ''),
        noStack: true
      });
    ...
    setTimeout(async () => {
      try {
        ...
        if (getProposalsCtxKey(LAST_PROPOSALS_CTX||{}) !== getProposalsCtxKey({ userId: uid, adminView, filterUserId: String(opts.filterUserId || ''), rankField: String(opts.rankField || 'OPPORTUNITY'), closedMonth: String(opts.closedMonth || '') })) return;
