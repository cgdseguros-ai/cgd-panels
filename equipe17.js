  ...
  function setBootProgress(pct, label) {
    try {
      ...
      BOOT_PROGRESS_TIMER = setInterval(() => {
        BOOT_PROGRESS = Math.min(BOOT_PROGRESS_TARGET, BOOT_PROGRESS + Math.max(1, Math.ceil((BOOT_PROGRESS_TARGET - BOOT_PROGRESS) / 4)));
        if (el && el.meta) el.meta.textContent = `${Math.round(BOOT_PROGRESS)}% • ${label || "Carregando painel..."}`;
        if (BOOT_PROGRESS >= BOOT_PROGRESS_TARGET) {
          clearInterval(BOOT_PROGRESS_TIMER);
          BOOT_PROGRESS_TIMER = null;
        }
      }, 35);
      if (el && el.meta && !BOOT_PROGRESS_TIMER) el.meta.textContent = `${Math.round(BOOT_PROGRESS_TARGET)}% • ${label || "Carregando painel..."}`;
    } catch (_) {}
  }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const stale = (Date.now() - Number(STATE.proposalLastLoadedAt || 0)) > 300000;
    if (!force && uid) {
    ...
    let rows = [];
    if (uid) {
      const stageIds = proposalStageOrder().map((nm) => String(STATE.proposalStageIdByName.get(norm(nm)) || '')).filter(Boolean);
      if (stageIds.length) {
        const lists = await Promise.all(stageIds.map((sid) => bxAll('crm.deal.list', {
          filter: { ASSIGNED_BY_ID: Number(uid), CATEGORY_ID: 0, STAGE_ID: sid },
          order: { ID: 'DESC' },
          select,
        }).catch(() => [])));
        rows = lists.flat();
      ...
      if (!rows.length) {
        rows = await bxAll('crm.deal.list', {
          filter: { ASSIGNED_BY_ID: Number(uid), CATEGORY_ID: 0 },
          order: { ID: 'DESC' },
          select,
        }).catch(() => []);
      }
      ...
      const stageIds = proposalStageOrder().map((nm) => String(STATE.proposalStageIdByName.get(norm(nm)) || '')).filter(Boolean);
      if (stageIds.length) {
        const parts = await Promise.all(stageIds.map((sid) => bxAll('crm.deal.list', {
          filter: { STAGE_ID: sid },
          order: { ID: 'DESC' },
          select,
        }).catch(() => [])));
