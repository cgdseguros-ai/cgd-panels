...
(function () {
  ...
  const STATE = {
    ...
    dealMutationHoldUntil: DEAL_MUTATION_HOLD_UNTIL,
  ...
  const LOCAL_LEAD_PATCH_TTL_MS = 15000;
  function localLeadPatchKey(leadId){ return `lead:${String(leadId || "")}`; }
  function registerLocalLeadPatch(leadId, fields, ttlMs = LOCAL_LEAD_PATCH_TTL_MS) {
    const id = String(leadId || '').trim();
    if (!id || !fields || typeof fields !== 'object') return;
    try {
      const key = localLeadPatchKey(id);
      const cur = STATE.localLeadPatches instanceof Map ? (STATE.localLeadPatches.get(key) || {}) : {};
      const next = { ...(cur.fields || {}), ...(fields || {}) };
      const expiresAt = Date.now() + Math.max(1000, Number(ttlMs || LOCAL_LEAD_PATCH_TTL_MS) || LOCAL_LEAD_PATCH_TTL_MS);
      if (!(STATE.localLeadPatches instanceof Map)) STATE.localLeadPatches = new Map();
      STATE.localLeadPatches.set(key, { leadId: id, fields: next, expiresAt });
    } catch (_) {}
  }
  function clearExpiredLocalLeadPatches() {
    try {
      if (!(STATE.localLeadPatches instanceof Map)) STATE.localLeadPatches = new Map();
      const now = Date.now();
      for (const [k, v] of STATE.localLeadPatches.entries()) {
        if (!v || Number(v.expiresAt || 0) <= now) STATE.localLeadPatches.delete(k);
      }
    } catch (_) {}
  }
  function applyLocalLeadPatchesToList(list) {
    try {
      clearExpiredLocalLeadPatches();
      const arr = Array.isArray(list) ? list : [];
      if (!(STATE.localLeadPatches instanceof Map) || !STATE.localLeadPatches.size || !arr.length) return arr;
      return arr.map((lead) => {
        const id = String((lead && lead.ID) || '').trim();
        if (!id) return lead;
        const patch = STATE.localLeadPatches.get(localLeadPatchKey(id));
        if (!patch || !patch.fields) return lead;
        return { ...(lead || {}), ...(patch.fields || {}), DATE_MODIFY: new Date().toISOString() };
      });
    } catch (_) {
      return Array.isArray(list) ? list : [];
    }
  }
  function applyLocalLeadPatches() {
    try {
      clearExpiredLocalLeadPatches();
      if (!(STATE.localLeadPatches instanceof Map) || !STATE.localLeadPatches.size) return;
      for (const [uid, arr] of (STATE.leadsByUser || new Map()).entries()) {
        STATE.leadsByUser.set(String(uid), applyLocalLeadPatchesToList(arr));
      }
    } catch (_) {}
  }
  function holdDealMutation(dealId, ms = 12000) {
    const did = String(dealId || "").trim();
    if (!did) return 0;
    const until = Date.now() + Math.max(1000, Number(ms) || 0);
    DEAL_MUTATION_HOLD_UNTIL.set(did, until);
    return until;
  }
  function isDealMutationHeld(dealId) {
    const did = String(dealId || "").trim();
    if (!did) return false;
    const until = Number(DEAL_MUTATION_HOLD_UNTIL.get(did) || 0);
    if (!until) return false;
    if (Date.now() >= until) { DEAL_MUTATION_HOLD_UNTIL.delete(did); return false; }
    return true;
  }
  ...
  async function loadLeadsSnapshotAllUsers(opts = {}) {
    ...
    for (const [uid, currentArr] of (STATE.leadsByUser || new Map()).entries()) {
      ...
      grouped.set(String(uid), applyLocalLeadPatchesToList(nextArr));
    }
    for (const [uid, rawArr] of grouped.entries()) {
      const arr = applyLocalLeadPatchesToList(rawArr).slice().sort((a,b)=> Number(b.ID||0)-Number(a.ID||0));
      STATE.leadsByUser.set(String(uid), arr);
      const sNovo = leadStageId("NOVO LEAD");
      const sAt = leadStageId("EM ATENDIMENTO");
...
restoreSyncQueue();
  ...
  function enqueueSync(job, opts = {}) {
    if (!job) return;
    if (job.type === 'leadUpdate' && job.leadId && job.fields) registerLocalLeadPatch(String(job.leadId), job.fields, 15000);
    if (job.type === 'dealUpdate' && job.dealId && job.fields) registerLocalDealPatch(String(job.dealId), job.fields, 15000);
    SYNC_QUEUE.push(job);
    persistSyncQueue();
  ...
  function dropPendingSyncJobs(matchFn) {
    try {
      if (typeof matchFn !== 'function') return 0;
      const before = SYNC_QUEUE.length;
      for (let i = SYNC_QUEUE.length - 1; i >= 0; i--) {
        const job = SYNC_QUEUE[i];
        if (matchFn(job)) SYNC_QUEUE.splice(i, 1);
      }
      if (SYNC_QUEUE.length !== before) persistSyncQueue();
      return before - SYNC_QUEUE.length;
    } catch (_) {
      return 0;
    }
  }
        ...
        if (job.type === 'leadUpdate' && job.leadId && job.fields) {
          for (const [uid, arr] of STATE.leadsByUser.entries()) {
            ...
            if (idx >= 0) {
              ...
              STATE.leadsByUser.set(uid, applyLocalLeadPatchesToList(arr));
              break;
            }
          }
          continue;
  ...
  async function flushSyncQueue() {
    ...
    try {
      ...
      while (SYNC_QUEUE.length) {
        ...
        if (job.type === "leadUpdate") { registerLocalLeadPatch(String(job.leadId), job.fields || {}, 15000); holdLeadMutation(String(job.leadId), 15000); await bx("crm.lead.update", { id: String(job.leadId), fields: job.fields }); registerLocalLeadPatch(String(job.leadId), job.fields || {}, 12000); SYNC_QUEUE.shift(); persistSyncQueue(); continue; }
...
function makeUserCard(u) {
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          holdDealMutation(String(dealId), 15000);
          registerLocalDealPatch(String(dealId), { [UF_PRAZO]: prazoIso }, 20000);
          dropPendingSyncJobs((job) => job && job.type === 'dealUpdate' && String(job.dealId || '') === String(dealId));
          ...
          setTimeout(() => { refreshData(true, { deferLeads:false }).catch(()=>{}); }, 900);
          return;
        }
        ...
        try {
          UI_REFRESH_HOLD_UNTIL = Date.now() + 12000;
          holdDealMutation(String(dealId), 15000);
          registerLocalDealPatch(String(dealId), { STAGE_ID: String(STATE.doneStageId) }, 20000);
          dropPendingSyncJobs((job) => job && job.type === 'dealUpdate' && String(job.dealId || '') === String(dealId));
          await safeDealUpdate(String(dealId), { STAGE_ID: String(STATE.doneStageId) });
          const created = await bx("crm.deal.add", { fields });
        ...
        setTimeout(() => { refreshData(true, { deferLeads:false }).catch(()=>{}); }, 1200);
      } catch (e) {
        warn.style.display = "block";
        warn.textContent = "Falha:\n" + (e.message || e);
  ...
  async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    UI_REFRESH_HOLD_UNTIL = Date.now() + 12000;
    registerLocalLeadPatch(String(leadId), patch, 20000);
    holdLeadMutation(leadId, 15000);
    await bx("crm.lead.update", { id: String(leadId), fields: patch });
    registerLocalLeadPatch(String(leadId), patch, 12000);
    if (i >= 0) {
      arr[i] = { ...(arr[i] || {}), ...patch };
      STATE.leadsByUser.set(uid, arr);
    }
