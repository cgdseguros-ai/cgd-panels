...
function makeMarketingCard() {
  return `<div class="userCard" data-action="openMktPanel"><div class="userEmoji" style="order:-1;align-self:center;min-width:22px;text-align:center">🟠</div><div class="userInfoLeft"><div class="userName">MARKETING</div><div class="userTeam">Agenda coletiva</div></div><div class="userPhotoWrap" style="background:transparent;border:none;box-shadow:none;padding:0"><img class="userPhoto" src="${MARKETING_LOGO_URL}" alt="Marketing" referrerpolicy="no-referrer" style="object-fit:cover;transform:scale(1.22);border:none;box-shadow:none;background:transparent" onerror="try{this.onerror=null;this.src='';this.style.display='none'}catch(e){}" /></div><div class="userRight"><div class="userLine">Users: <strong>${MARKETING_MEMBERS.length}</strong></div><div class="userLine">Em implantação</div></div></div>`;
}

function openMarketingPanelStub(selectedDateStr = "") {
  const typeTags = ["GRAVAÇÃO","REUNIÃO","ROTEIRO","EDIÇÃO DE VIDEO","FOTOGRAFIA"];
  ...
  const today = new Date();
  const selected = selectedDateStr ? tryParseDateAny(`${selectedDateStr} 00:00`) : today;
  const monthBase = new Date(selected.getFullYear(), selected.getMonth(), 1);
  ...
  for (let i=0;i<35;i++) {
    ...
    const dayEmojis = [...new Set(dayDeals.map((x) => mktEmojiForType(x._tarefaTxt || x.TITLE || '')).filter(Boolean))].slice(0,3);
    ...
    days.push(`<button class="eqd-btn" data-action="mktPickDay" data-day="${key}" style="min-height:80px;border-radius:16px;justify-content:flex-start;padding:10px 9px;background:${key===selectedKey?'#111827':(participantIds.length?'#f8fafc':'#fff')};color:${key===selectedKey?'#fff':(inMonth?'#111':'#94a3b8')};border:2px solid ${key===selectedKey?'#111827':borderColor};flex-direction:column;align-items:flex-start;gap:5px;box-shadow:none"><div style="display:flex;width:100%;justify-content:space-between;align-items:center"><span style="font-weight:950;font-size:14px">${d.getDate()}</span><span style="font-size:16px">${dayEmojis.join(' ') || ''}</span></div><div style="display:flex;gap:4px;flex-wrap:wrap">${chips || '<span style="font-size:10px;opacity:.55">—</span>'}</div></button>`);
  }

  openModal("Painel MKT — implantação", `<div style="display:grid;grid-template-columns:minmax(408px,52%) 1fr;gap:12px;min-height:72vh">
  ...
  if (createBtn) createBtn.onclick = async () => {
    ...
    const tipoItems = (meta ? getFieldItemsFromMeta(meta, 'UF_CRM_1768185018696') : []).filter((x) => typeTags.some((tt) => norm(tt) === norm(x.text || x.VALUE || x.NAME || '')));
    ...
    openModal("Nova tarefa MKT", `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">TIPO</div><select id="mktType" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)">${tipoItems.map((x)=>`<option value="${x.value}">${escHtml(x.text)}</option>`).join('')}</select></div>
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    openModal("Concluir e reagendar", `
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:10px">
        ...
        ${isFollowupDeal(d) ? `<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">ESTE FOLLOW-UP FOI FEITO?</div><select id="drFollowMode" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="FEITO">SIM, FOI FEITO • conclui e cria novo</option><option value="NAO_FEITO">NÃO FOI FEITO • apenas reagenda</option></select></div>` : `<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">URGÊNCIA</div><select id="drUrg" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Carregando…</option></select></div>`}<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">OBS</div><textarea id="drObs" rows="5" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px;font-weight:850;outline:none">${escHtml(String(d[UF_OBS] || ""))}</textarea></div>
    ...
    btn.onclick = async () => {
      try {
        ...
        if (isFollowupDeal(d)) {
          ...
          followMode = String((document.getElementById('drFollowMode') || {}).value || 'FEITO');
        }
        ...
        const realId = String((created && (created.result || created)) || '').trim() || makeTempId('TMP_DEAL');
        upsertDealLocal(parseLocalDealFromFields(realId, fields));
        closeModal(); rebuildDealsOpen(); renderCurrentView(); setTimeout(() => { refreshData(true).catch(()=>{}); }, 50);
