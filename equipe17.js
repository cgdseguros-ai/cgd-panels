...
function openMarketingPanelStub(selectedDateStr = "") {
  ...
  for (let i=0;i<42;i++) {
    ...
    days.push(`<button class="eqd-btn" data-action="mktPickDay" data-day="${key}" style="min-height:96px;border-radius:16px;justify-content:flex-start;padding:10px 9px;background:${key===selectedKey?'#111827':(participantIds.length?'#f8fafc':'#fff')};color:${key===selectedKey?'#fff':(inMonth?'#111':'#94a3b8')};border:2px solid ${key===selectedKey?'#111827':borderColor};flex-direction:column;align-items:flex-start;gap:5px;box-shadow:none"><div style="display:flex;width:100%;justify-content:space-between;align-items:center"><span style="font-weight:950;font-size:14px">${d.getDate()}</span><span style="font-size:16px">${dayEmojis.join(' ') || ''}</span></div><div style="display:flex;gap:4px;flex-wrap:wrap">${chips || '<span style="font-size:10px;opacity:.55">—</span>'}</div></button>`);
  }
  ...
  document.querySelectorAll('[data-action="mktNavMonth"]').forEach((btn) => btn.onclick = () => openMarketingPanelStub(String(btn.getAttribute('data-day') || '')));
