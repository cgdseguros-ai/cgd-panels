...
function makeUserCard(u) {
  ...
  async function loadMetricsCore(opts = {}) {
    ...
    // PATCH V5.2 — evita contagem parcial: carregar TODAS as tarefas abertas da pipeline 17 já no boot.
    // Antes carregava só uma janela recente (hoje - 5 dias) e depois completava vencidos antigos em background,
    // o que fazia a quantidade de tarefas aumentar ao longo do dia e podia esconder tarefas antigas do dia.
    await bootTimerMeasure("Métricas: deals/tarefas abertas", () => loadDeals({ forceFull: true, openOnly: true, deferPhotos: true }), "Cards operacionais: todas as tarefas abertas, vencidas, do dia e futuras; concluídas ficam fora da contagem");
