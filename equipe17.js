...
(function () {
  ...
  injectCSS(`
    ...
    @keyframes eqdBluePulse{0%,100%{background:#1d4ed8;border-color:#60a5fa;box-shadow:0 0 0 0 rgba(37,99,235,.45);transform:translateY(0)}50%{background:#2563eb;border-color:#bfdbfe;box-shadow:0 0 0 8px rgba(37,99,235,.10);transform:translateY(-1px)}}
    .eqd-btnWaitingLead{font-size:15px!important;min-height:39px!important;padding:8px 15px!important;border-radius:999px!important;background:#1f2937!important;border-color:#1f2937!important;color:#fff!important}
    .eqd-btnWaitingLead.hasWaiting{animation:eqdBluePulse 1.15s ease-in-out infinite!important}
  ...
  async function autofillNewLeadEntryDateIfMissing(leads, opts = {}) {
    ...
    for (const l of targets) {
      try {
        l[LEAD_UF_DATAHORA] = nowIso;
        l.DATE_MODIFY = nowIso;
        // Sincroniza automaticamente no Bitrix, sem travar a renderização por muito tempo.
        await bx("crm.lead.update", { id: String(l.ID), fields: { [LEAD_UF_DATAHORA]: nowIso } });
      } catch (err) {
        try { logSyncError({ entityType: "lead", entityId: String(l.ID), source: "novoLeadEntryDate", action: "crm.lead.update", message: (err && err.message) ? err.message : String(err), payload: { id: String(l.ID), fields: { [LEAD_UF_DATAHORA]: nowIso } }, retryAvailable: true, retryMethod: "crm.lead.update", retryParams: { id: String(l.ID), fields: { [LEAD_UF_DATAHORA]: nowIso } }, retryLabel: "Retry data NOVO LEAD" }); } catch (_) {}
      }
    }
    return leads;
  }
  ...
  async function takeWaitingLeadForUser(userId, leadId) {
    ...
    if (!current || !String(current[LEAD_UF_DATAHORA] || "").trim()) fields[LEAD_UF_DATAHORA] = nowIso;
...
function makeUserCard(u) {
  ...
  async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    if (target === String(sAtendimento) && cur && !String(cur[LEAD_UF_DATAHORA] || "").trim()) patch[LEAD_UF_DATAHORA] = new Date().toISOString();
    if (cur) {
