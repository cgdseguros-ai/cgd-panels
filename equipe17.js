/* eqd.js — GET • CGD CORRETORA (v5.0)
   PATCH REAL V5.6 — 2026-05-13: boot essencial incremental real + background sem duplicidade
...
(function () {
  ...
  function canReleasePanel() {
    // PATCH V5.4: libera somente quando o essencial + recorrências + fotos estiverem prontos.
    // Histórico completo, futuros além de 5 dias e leads inativos continuam em background.
    return (
  ...
  function setBackgroundLoadStatus(label, step, total, done = false) {
    try {
      ...
      el.bgLoadStatus.title = done
        ...
        : "Ainda carregando dados complementares em segundo plano: histórico, futuros além de 5 dias e leads inativos.";
      if (done) {
        setTimeout(() => {
          try {
  ...
  function schedulePostReadyBackgroundDataLoads() {
    try {
      ...
      setTimeout(async () => {
        ...
        try {
          setBackgroundLoadStatus("recorrências 6-120 dias", 2, total, false);
          await bootTimerMeasure(
        ...
        try {
          setBackgroundLoadStatus("futuros > 5 dias", 3, total, false);
          await bootTimerMeasure(
        ...
        try {
          setBackgroundLoadStatus("histórico antigo", 4, total, false);
          await bootTimerMeasure(
        ...
        try {
          setBackgroundLoadStatus("leads inativos", 5, total, false);
...
document.addEventListener("click", async (e) => {
  ...
  try {
    ...
    try {
      ...
      setTimeout(() => refreshData(true, { forceFullDeals:true }).catch(()=>{}), 12000);
    }
    ...
    // ─── FASE 4 — RECORRÊNCIAS + FOTOS ANTES DO READY ─────────────────────
    await loadRecurrencesBeforeReady({ silent: false, forceRecur: true });
    BOOT_STATE.criticalSyncReady = true;
    bootTimerMark("RECURRENCES_READY_BEFORE_RELEASE");
    ...
    // ─── FASE 5 — READY → RESTANTE EM BACKGROUND COM INDICADOR ────────────
    setBootProgress(98, "Liberando painel: essencial, recorrências e fotos prontos...");
    ...
    if (canReleasePanel()) {
      ...
      renderCurrentView(true);
      setSoftStatus("JS: ok");
      setBackgroundLoadStatus("preparando", 0, 3, false);
