...
(function () {
  ...
  const LEAD_USERS = new Set(["1", "15", "19", "17", "23", "811", "3081", "3079", "3083", "3085", "3389", "815"]);
  ...
  function makeDealCard(deal, context) {
    ...
    if (deal._etapaTxt) tags.push(`<span class="eqd-tag eqd-tagClickable" data-action="quickEtapa" data-id="${deal.ID}">ETAPA: ${trunc(deal._etapaTxt, 18)}</span>`);
    ...
    const extraMetaInside = context && context.extraMetaInside
  ...
  function makeTempId(prefix) {
    TEMP_SEQ += 1;
    return `${prefix || "TMP"}_${Date.now()}_${TEMP_SEQ}`;
  }

  function isTempId(v) { return /^TMP_/i.test(String(v || "")); }
  ...
  function upsertDealLocal(deal) {
    ...
    if (!id) return null;
    const i = (STATE.dealsAll || []).findIndex((d) => String(d.ID) === id);
    if (i >= 0) STATE.dealsAll[i] = deal; else STATE.dealsAll.unshift(deal);
    rebuildDealsOpen();
    saveCache();
    return deal;
  }
  ...
  function parseLocalDealFromFields(id, fields) {
    return parseDeal({
      ID: String(id), TITLE: fields.TITLE || "Negócio", STAGE_ID: fields.STAGE_ID || "", DATE_CREATE: new Date().toISOString(), DATE_MODIFY: new Date().toISOString(), ASSIGNED_BY_ID: Number(fields.ASSIGNED_BY_ID || 0),
      [UF_TAREFA]: fields[UF_TAREFA] || "", [UF_PRAZO]: fields[UF_PRAZO] || "", [UF_URGENCIA]: fields[UF_URGENCIA] || "", [UF_ETAPA]: fields[UF_ETAPA] || "", [UF_COLAB]: fields[UF_COLAB] || "", [UF_OBS]: fields[UF_OBS] || ""
    }, { urgMap: STATE.enumCache.get(UF_URGENCIA) || {}, tarefaMap: STATE.enumCache.get(UF_TAREFA) || {}, etapaMap: STATE.enumCache.get(UF_ETAPA) || {}, colabMap: STATE.enumCache.get(UF_COLAB) || {}, colabIsEnum: !!(STATE.enumCache.get(UF_COLAB) && Object.keys(STATE.enumCache.get(UF_COLAB)).length) });
  }
  ...
  function replaceDealIdLocal(oldId, newId) {
    ...
    rebuildDealsOpen(); saveCache();
  }

  function enqueueSync(job) { SYNC_QUEUE.push(job); setTimeout(() => { flushSyncQueue().catch(() => {}); }, 80); }
  ...
  async function flushSyncQueue() {
    ...
    try {
      while (SYNC_QUEUE.length) {
        ...
        if (job.type === "dealUpdate") { if (isTempId(job.dealId)) break; await bx("crm.deal.update", { id: String(job.dealId), fields: job.fields }); SYNC_QUEUE.shift(); continue; }
    ...
    } catch (_) { setTimeout(() => { flushSyncQueue().catch(() => {}); }, 2500); } finally { SYNC_QUEUE_RUNNING = false; }
  }
  ...
  function openFollowupListModalForUser(user) {
    ...
    const list = listAll.filter((d) => d._prazo && Number.isFinite(new Date(d._prazo).getTime()) && new Date(d._prazo).getTime() >= from);
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    const shouldLoadLeads = !!(opts && opts.forceReload) || !STATE.leadsByUser.has(String(user.userId));
    let leadsUser = STATE.leadsByUser.get(String(user.userId)) || [];
    if (shouldLoadLeads) {
      setBusy("Carregando LEADS…");
      leadsUser = await loadLeadsForOneUser(user.userId).catch(() => []);
      clearBusy();
    }
  ...
  async function loadGlobalLeadsBulk(forceReload = false) {
    ...
    const rows = (leads || []).filter((l) => targetUsers.has(String(l.ASSIGNED_BY_ID || ""))).map((l) => ({ ...l, _ownerUserId: String(l.ASSIGNED_BY_ID || "") }));
  ...
  function getLeadDayKeyFromDataHora(l) {
  ...
  function openGlobalLeadsDayStats() {
    ...
    const rows = USERS.filter((u) => LEAD_USERS.has(String(u.userId))).map((u) => {
      ...
      const emAtendimento = mine.filter((l) => String(l.STATUS_ID) === String(sAt)).length;
      const atendidoNoDia = mine.filter((l) => getLeadDayKeyFromDataHora(l) === todayKey).length;
      return { name: u.name, emAtendimento, atendidoNoDia };
    });
    openModal(`LEADS • DIA (${fmtDateOnly(new Date())})`, renderGlobalLeadStatsTable(rows), { wide: true });
  }
  ...
  function openGlobalLeadsHistoryModal(days) {
    ...
    const body = users.map((u) => {
      ...
      const cols = dates.map((d) => {
        const key = ymdKey(d);
        const qty = mine.filter((l) => getLeadDayKeyFromDataHora(l) === key).length;
        return `<td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${qty}</td>`;
      }).join("");
      return `<tr><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08);font-weight:900">${escHtml(u.name)}</td>${cols}</tr>`;
    }).join("");
    openModal(`LEADS • HISTÓRICO ${days} DIAS`, `
      <div style="font-size:12px;font-weight:950;opacity:.82;margin-bottom:8px">Quantidade de leads com DATA/HORA de atendimento por dia.</div>
      <div style="overflow:auto">
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">USER</th>${headerDays}</tr></thead>
          <tbody>${body}</tbody>
        </table>
      </div>
    `, { full: true });
  ...
  function renderGlobalLeadsModal(filterUserId = "__ALL__") {
    ...
    const card = (l) => { const owner = USERS.find((u) => String(u.userId) === String(l._ownerUserId)); return `<div class="leadCard"><div class="leadTitle">${escHtml(leadTitle(l))}</div><div class="leadMeta"><span>USER: <strong>${escHtml(owner ? owner.name : l._ownerUserId)}</strong></span><span>Criado em: <strong>${l.DATE_CREATE ? fmt(l.DATE_CREATE) : "—"}</strong></span>${leadOperadora(l) ? `<span>Operadora: <strong>${escHtml(leadOperadora(l))}</strong></span>` : ``}${leadTelefone(l) ? `<span>Telefone: <strong>${escHtml(leadTelefone(l))}</strong></span>` : ``}</div></div>`; };
    ...
    document.getElementById("glRefreshBtn").onclick = async () => { await loadGlobalLeadsBulk(true); renderGlobalLeadsModal(String(document.getElementById("glUser").value || "__ALL__")); };
  ...
  async function openBatchRescheduleAdvanced(ids, opts = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        for (const id of ids) {
          updateDealInState(id, { [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString(), _late: false, DATE_MODIFY: new Date().toISOString() });
  ...
  async function openEditUrgModal(dealId){
    ...
    btn.onclick = async () => {
      ...
      try{
        ...
        updateDealInState(dealId, { [UF_URGENCIA]: v, _urgId: v, _urgTxt: "", DATE_MODIFY: new Date().toISOString() });
  ...
  async function quickToggleEtapa(dealId){
    ...
    if (cur.includes(norm("AGUARDANDO"))) {
      const nextId = findEnumIdByLabel(etapaMap, norm("EM ANDAMENTO"));
      if (nextId) {
    ...
    return openEditEtapaModal(dealId);
  }
  ...
  async function moveLeadStage(userId, leadId, toStatus){
    ...
    if (i >= 0) { arr[i] = { ...arr[i], STATUS_ID: String(toStatus) }; STATE.leadsByUser.set(String(userId), arr); }
    enqueueSync({ type: "leadUpdate", leadId: String(leadId), fields: { STATUS_ID: String(toStatus) } });
    reopenLeadsModalSafe();
  }
      ...
      return `
        <div style="border:1px solid rgba(0,0,0,.12);border-radius:14px;padding:10px;background:rgba(255,255,255,.62);display:flex;gap:10px;align-items:flex-start;justify-content:space-between;flex-wrap:wrap">
          ...
          <div style="display:flex;gap:8px;align-items:center">
            ${t === "MONTHLY" ? `<button class="eqd-btn" data-action="recurEditMonth" data-userid="${uid}" data-ruleid="${escHtml(String(r.id||""))}">Editar dia</button>` : ``}
  ...
  el.globalLeads.onclick = () => openGlobalLeadsModal("__ALL__");
  ...
  setInterval(() => { if (!document.hidden) flushSyncQueue().catch(() => {}); }, 5000);
