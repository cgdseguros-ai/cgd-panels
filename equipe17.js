...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalAmountNumber(deal) {
    ...
    else if (/^-?\d+(,\d+)?$/.test(raw)) n = Number(raw.replace(',', '.'));
    else if (/^-?[\d\.]+,\d+$/.test(raw)) n = Number(raw.replace(/\./g,'').replace(',', '.'));
    else if (/^-?[\d,]+\.\d+$/.test(raw)) n = Number(raw.replace(/,/g,''));
  ...
  function proposalSummaryRows(deal) {
    return proposalFieldDefs().filter((fd) => fd.code !== 'CURRENCY_ID').map((fd) => {
      const txt = fd.code === 'OPPORTUNITY' ? formatBRL(proposalAmountNumber(deal)) : proposalFieldValueLabel(fd.code, deal ? deal[fd.code] : '');
      if (!txt || txt === '—') return '';
      return `<div style="font-size:10px;line-height:1.2"><strong>${escHtml(fd.label)}:</strong> ${escHtml(trunc(String(txt), 64))}</div>`;
    }).filter(Boolean).join('');
  }
  ...
  function renderProposalCard(deal, opts = {}) {
    ...
    return `
      <div class="eqd-card" style="border-left:4px solid ${stageColor};padding:5px 7px">
        <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:5px;align-items:start">
          <div style="min-width:0">
            <div style="display:flex;gap:4px;align-items:center;flex-wrap:wrap;margin-bottom:2px">
              <span class="eqd-tag" style="background:${stageColor};color:#fff;border-color:${stageColor};padding:2px 6px">${escHtml(stageName || 'Sem stage')}</span>
              <span class="eqd-tag" style="padding:2px 6px">${escHtml(ownerName)}</span>
              <span class="eqd-tag" style="padding:2px 6px"><strong>${escHtml(formatBRL(amount))}</strong></span>
            </div>
            <div style="font-weight:950;font-size:12px;line-height:1.1">${escHtml(bestTitleFromText(deal.TITLE || ''))}</div>
          </div>
          <div style="display:flex;gap:4px;flex-wrap:wrap;justify-content:flex-end;align-items:center">
            <select class="proposal-inline-stage" data-id="${escHtml(String(deal.ID))}" style="padding:4px 6px;border-radius:8px;border:1px solid rgba(30,40,70,.16);font-weight:900;max-width:180px;font-size:11px">${proposalCardStageOptions(deal.STAGE_ID)}</select>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(116px,1fr));gap:1px 5px;margin-top:3px">${rows}</div>
      </div>`;
  }
  ...
  function renderProposalsModalContent(sourceDeals, opts = {}) {
    ...
    const nonClosedNames = ['NEGOCIAÇÃO QUENTE','AGUARDANDO DOCUMENTOS','EM DIGITAÇÃO DE PROPOSTA','ENVIADO PARA ASSINATURA','AGUARDANDO ENTREVISTA MÉDICA','EM ANÁLISE','PENDÊNCIA','AGUARDANDO PAGAMENTO'];
    ...
    const stagesHtml = grouped.map((g) => {
      ...
      const cardsHtml = g.deals.length ? g.deals.map((d) => renderProposalCard(d, { userId: d.ASSIGNED_BY_ID })).join('') : `<div class="eqd-empty">Sem contratos neste stage.</div>`;
      if (viewMode === 'kanban') {
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "proposalCreateTask") { await createTaskFromProposalDeal(String(id||''), LAST_PROPOSALS_CTX || { userId: String(userId||''), adminView: false }); const warn = document.getElementById('proposalWarn'); if (warn) { warn.style.display='block'; warn.textContent='Tarefa criada no painel individual.'; } return; }
