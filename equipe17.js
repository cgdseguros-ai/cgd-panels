...
${marker}` : marker;
  ...
  function calcLeadFunnelMetrics(userId, opts = {}) {
    ...
    for (const l of leads) {
      ...
      passed++;
      const convAt = firstLeadDateForStageNames(l, ["LEAD CONVERTIDO", "CONVERTIDO", "CONVERTIDOS"], false) || (isConvertedLead(l) ? parseFlexDate(l.DATE_MODIFY || l.DATE_CREATE || "") : null);
      const lostAt = firstLeadDateForStageNames(l, ["LEAD DESCARTADO", "PERDIDO", "PERDIDOS"], false) || (isLostLead(l) ? parseFlexDate(l.DATE_MODIFY || l.DATE_CREATE || "") : null);
      const isConv = !!convAt || isConvertedLead(l); const isLost = !!lostAt || isLostLead(l);
    ...
    const row = (label, val, detail, w, c1, c2, textColor = '#fff') => `
      <div style="display:flex;align-items:center;gap:10px;margin:10px 0;">
        ...
        <div style="flex:1;min-width:160px;">
          <div style="width:${w}%;min-width:170px;margin:0 auto;border-radius:14px;padding:12px 14px;box-sizing:border-box;color:${textColor};font-weight:950;background:linear-gradient(135deg,${c1},${c2});box-shadow:0 8px 22px rgba(15,23,42,.14);clip-path:polygon(7% 0,93% 0,100% 100%,0 100%);">
      ...
      <div style="border:1px solid rgba(15,23,42,.10);border-radius:18px;padding:14px;background:linear-gradient(180deg,rgba(255,255,255,.96),rgba(248,250,252,.92));margin-bottom:12px;">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap;margin-bottom:10px;">
          <div>
            ...
            <div style="font-size:12px;font-weight:800;color:#64748b">Base: leads que passaram por ATENDIDO no período selecionado.</div>
          </div>
