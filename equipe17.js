/* eqd.js — GET • CGD CORRETORA (v5.0)
   PATCH REAL V4 — 2026-04-30: reagendamento, chips follow-up sem modal, propostas perdidas
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          pendingMutRegister("deal", dealId, `reschedPrazo_${Date.now()}`, { [UF_PRAZO]: prazoIso }, 120000, { source: "reschedulePrazoOnlyV4" });
          ...
          enqueueSync({ type: "dealUpdate", dealId: String(dealId), fields: { [UF_PRAZO]: prazoIso } }, { delayMs: 80 });
          rebuildDealsOpen();
          saveCache();
          renderCurrentView(true);
          setTimeout(() => flushSyncQueue().catch((err) => showRuntimeWarn(err)), 120);
          return;
        }
        ...
        rescheduleDealAtomic(dealId, fields, d)
          .catch((err) => showRuntimeWarn(err))
          .finally(() => clearBusy());
      } catch (e) {
