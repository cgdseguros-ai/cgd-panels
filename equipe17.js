...
function makeUserCard(u) {
  ...
  function renderUserPanel(userId) {
    ...
    const user1AdminToggleBtn = String(user.userId) === "1" ? `<div class="eqd-adminToggle" data-action="toggleAdminResources" style="display:flex;align-items:center;gap:6px;cursor:pointer;font-weight:950;padding:6px 2px"><span id="user1AdminArrow">▸</span><span>RECURSOS ADM</span></div>` : ``;
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const assigneeFilter = uid ? { ASSIGNED_BY_ID: Number(uid) } : {};
    ...
    const mapped = (rows || [])
      .filter((d) => {
        const catRaw = d && d.CATEGORY_ID != null ? String(d.CATEGORY_ID).trim() : '';
        const categoryLooksDefault = !catRaw || catRaw === '0';
        if (!categoryLooksDefault) return false;
        const inferred = inferProposalStageNameFromAny(proposalStageNameFromId(d && d.STAGE_ID)) || inferProposalStageNameFromAny(d && d.STAGE_ID);
        return !!inferred;
      })
      .map((d) => ({ ...d, _proposalStageName: inferProposalStageNameFromAny(proposalStageNameFromId(d.STAGE_ID)) || inferProposalStageNameFromAny(d.STAGE_ID) || String(d.STAGE_ID || '') }));
  ...
  function proposalDealAllowed(deal) {
    const nm = proposalStageNameFromId(deal && deal.STAGE_ID) || String((deal && deal._proposalStageName) || '');
    if (!STATE.proposalStageNameById || !STATE.proposalStageNameById.size) return !PROPOSAL_HIDDEN_STAGE_NAMES.has(String(nm || '').trim());
