...
${marker}` : marker;
  ...
  function renderRecurrenceAuditResults(payload = {}) {
    ...
    const legacyHtml = legacyRows.length ? `<div style="overflow:auto"><table class="eqd-table"><thead><tr><th>ID</th><th>Título</th><th>USER</th><th>Prazo</th><th>Stage</th><th>Motivo</th><th>Ação</th></tr></thead><tbody>${legacyRows.map((r)=>{ const d = r.deal || {}; return `<tr><td><strong>${escHtml(String(d.ID || ""))}</strong></td><td>${escHtml(bestTitleFromText(String(d.TITLE || "")))}</td><td>${escHtml(recurrenceAuditUserName(d.ASSIGNED_BY_ID))}</td><td>${escHtml(fmtDateTime(d[UF_PRAZO] || ""))}</td><td>${escHtml(recurrenceAuditStageName(d.STAGE_ID))}</td><td>${escHtml(r.reason || "")}</td><td><button class="eqd-btn" data-action="recurrenceAuditIgnoreLegacy" data-id="${escHtml(String(d.ID || ""))}">Ignorar</button></td></tr>`; }).join("")}</tbody></table></div>` : `<div class="eqd-empty">Nenhuma recorrência antiga/legada encontrada com os filtros atuais.</div>`;
