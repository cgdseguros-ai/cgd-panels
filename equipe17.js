  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal)) {
      if (linkedLeadOrig) {
        ...
        tags.push(leadStageMoveChipsHtml(linkedLead, deal.ID, { dealId: deal.ID }));
      }
    ...
    function cardLead(l, column) {
      ...
      return `
        <div class="leadCard" data-lead-col="${escHtml(column)}" data-fonte="${escHtml(norm(fonte))}" data-operadora="${escHtml(norm(op))}" data-proximo-vencer="${isLeadProximoVencer(l) ? '1' : '0'}">
          <div style="display:flex;justify-content:flex-start;gap:8px;align-items:center;flex-wrap:wrap">
...
function makeUserCard(u) {
  ...
  function renderUserPanel(userId) {
    ...
    if (!isSpecial) {
      ...
      const followStageFilter = document.getElementById('followStageFilter');
      if (followStageFilter) followStageFilter.onchange = () => {
        const val = norm(followStageFilter.value || '');
        document.querySelectorAll('#sp_follow .eqd-card').forEach((card)=>{
          const dealId = String(card.getAttribute('data-deal') || '');
          const d = getDealById(dealId);
          const lead = d && getDealLeadOrigemId(d) ? getLeadByOrigemId(getDealLeadOrigemId(d)) : null;
          const nm = norm(lead ? leadStageNameById(lead.STATUS_ID) : '');
          card.style.display = (!val || nm === val) ? '' : 'none';
        });
      };
    ...
    el.main.innerHTML = `
      ...
      <div class="panelCols">
        ...
        <div class="panelCol">
          <div class="panelColHead">FOLLOW-UP DO DIA (${orderedFollow.length}) <select id="followStageFilter" style="margin-left:8px;border-radius:999px;padding:4px 8px;font-size:10px;font-weight:900"><option value="">Todos</option><option value="ATENDIDO">ATENDIDOS</option><option value="INTERAGIDO">INTERAGIDO</option><option value="QUALIFICADO">QUALIFICADO</option></select></div>
    ...
    const followStageFilter2 = document.getElementById('followStageFilter');
    if (followStageFilter2) followStageFilter2.onchange = () => {
      const val = norm(followStageFilter2.value || '');
      document.querySelectorAll('#sp_follow .eqd-card').forEach((card)=>{
        const dealId = String(card.getAttribute('data-deal') || '');
        const d = getDealById(dealId);
        const lead = d && getDealLeadOrigemId(d) ? getLeadByOrigemId(getDealLeadOrigemId(d)) : null;
...
${marker}` : marker;
  ...
  function calcLeadFunnelMetrics(userId, opts = {}) {
    ...
    const byUser = new Map();
    let passed=0, interagido=0, qual=0, conv=0, lost=0, leadDescartado=0, leadConvertido=0;
    const stagePassCounts = Object.fromEntries(["ATENDIDO","INTERAGIDO","QUALIFICADO","CONVERTIDO","CONVERTIDOS","PERDIDO","LEAD DESCARTADO","LEAD CONVERTIDO","AGUARDANDO DOCUMENTOS","EM ANÁLISE","NEGÓCIOS FECHADOS"].map((x)=>[x,0]));
    const convDur=[], lostDur=[];

    const convertedStageId = String(leadStageId("LEAD CONVERTIDO") || "");
    const lostStageId = String(leadStageId("LEAD DESCARTADO") || "");
    const qualifiedStageId = String(leadStageId("QUALIFICADO") || "");

    for (const l of leads) {
      const owner = String(l._ownerUserId || l.ASSIGNED_BY_ID || "");
      const at = firstLeadDateForStageNames(l, ["ATENDIDO"], true);
      if (!at || at < start || at > end) continue;

      passed++;

      const statusNameNow = leadStatusName(l);
      const histNames = leadHistoryFor(l.ID || "").map((r)=>String(r.toStageName || leadStageNameById(r.toStatus) || r.toStatus || ""));
      const passedStage = (stageName) => {
        const ns = norm(stageName);
        return norm(statusNameNow) === ns || histNames.some((h)=>norm(h) === ns || (norm(h) && norm(h).includes(ns)));
      };
      for (const k of Object.keys(stagePassCounts)) if (passedStage(k)) stagePassCounts[k]++;

      // QUALIFICADO: contar se passou por QUALIFICADO no histórico ou se está atualmente em QUALIFICADO.
      const qualAt = firstLeadDateForStageNamesFunnel(l, ["QUALIFICADO"], false);
      const isInteragido = passedStage("INTERAGIDO");
      const isQual = !!qualAt || String(l.STATUS_ID || "") === qualifiedStageId || isQualifiedLeadFunnel(l);

      // Convertido/perdido: usar prioritariamente os STAGES EXATOS solicitados.
      // Convertidos = LEAD CONVERTIDO | Perdidos = LEAD DESCARTADO.
      const convAt = firstLeadDateForStageNamesFunnel(l, ["LEAD CONVERTIDO"], false) ||
        (convertedStageId && String(l.STATUS_ID || "") === convertedStageId ? parseFlexDate(l.DATE_MODIFY || l.DATE_CREATE || "") : null) ||
        (isConvertedLeadFunnel(l) ? parseFlexDate(l.DATE_MODIFY || l.DATE_CREATE || "") : null);

      const lostAt = firstLeadDateForStageNamesFunnel(l, ["LEAD DESCARTADO"], false) ||
        (lostStageId && String(l.STATUS_ID || "") === lostStageId ? parseFlexDate(l.DATE_MODIFY || l.DATE_CREATE || "") : null) ||
        (isLostLeadFunnel(l) ? parseFlexDate(l.DATE_MODIFY || l.DATE_CREATE || "") : null);

      const isConv = !!convAt || (convertedStageId && String(l.STATUS_ID || "") === convertedStageId) || isConvertedLeadFunnel(l);
      const isLost = !!lostAt || (lostStageId && String(l.STATUS_ID || "") === lostStageId) || isLostLeadFunnel(l);

      if (isInteragido) interagido++;
      if (isQual) qual++;
      if (isConv) conv++;
      if (isLost) lost++;
      if (passedStage("LEAD DESCARTADO")) leadDescartado++;
      if (passedStage("LEAD CONVERTIDO")) leadConvertido++;
      ...
      r.passed++;
      if (isInteragido) r.interagido++;
      if (isQual) r.qualified++;
      if (isConv) r.converted++;
      if (isLost) r.lost++;
      if (passedStage("LEAD DESCARTADO")) r.leadDescartado++;
      if (passedStage("LEAD CONVERTIDO")) r.leadConvertido++;

      if (convAt && convAt >= at) convDur.push(convAt.getTime()-at.getTime());
      if (lostAt && lostAt >= at) lostDur.push(lostAt.getTime()-at.getTime());
    }
    // Pipeline 0: soma os deals/propostas atualmente nos stages solicitados, respeitando o escopo de USER do funil.
    (STATE.proposalDealsAll || []).forEach((d)=>{
      if (!allMode && !allowedUserIds.has(String(d.ASSIGNED_BY_ID || ''))) return;
      const nm = norm((d && d._proposalStageName) || proposalStageNameFromId(d && d.STAGE_ID) || '');
      if (nm === 'AGUARDANDO DOCUMENTOS') stagePassCounts['AGUARDANDO DOCUMENTOS']++;
      if (nm === 'EM ANALISE') stagePassCounts['EM ANÁLISE']++;
      if (nm === 'NEGOCIOS FECHADOS') stagePassCounts['NEGÓCIOS FECHADOS']++;
    });

    const avg = (a)=> a.length ? a.reduce((x,y)=>x+y,0)/a.length : null;
    const userRows=[...byUser.values()].map((r)=>({ ...r, name: (USERS.find((u)=>String(u.userId)===String(r.userId))||{}).name || r.userId, rate: r.passed ? r.converted/r.passed : 0 })).sort((a,b)=>b.converted-a.converted || b.qualified-a.qualified || b.passed-a.passed);
    return { totalPassedAtendido: passed, totalInteragido: interagido, totalQualified: qual, totalConverted: conv, totalLost: lost, totalLeadDescartado: leadDescartado, totalLeadConvertido: leadConvertido, stagePassCounts, conversionRate: passed ? conv/passed : 0, qualificationRate: passed ? qual/passed : 0, lossRate: passed ? lost/passed : 0, avgTimeAtendidoToConverted: avg(convDur), avgTimeAtendidoToLost: avg(lostDur), userRows };
  }

  function renderSalesFunnelChart(data) {
    const passed = Number(data && data.totalPassedAtendido || 0);
    const qualified = Number(data && data.totalQualified || 0);
    const converted = Number(data && data.totalConverted || 0);
    const lost = Number(data && data.totalLost || 0);
    const closed = converted + lost;
    const maxVal = Math.max(1, passed, qualified, converted, lost);
    const safePct = (num, den) => den > 0 ? `${String(Math.round((num / den) * 1000) / 10).replace('.', ',')}%` : '0%';
    const bar = (label, val, detail, color, bg, border) => {
      const pct = Math.max(2, Math.round((Number(val || 0) / maxVal) * 100));
      return `
        <div style="display:grid;grid-template-columns:130px 1fr 72px;gap:10px;align-items:center;margin:10px 0;">
          <div style="font-size:12px;font-weight:950;color:#334155;text-align:right">${escHtml(label)}</div>
          <div style="height:34px;border-radius:999px;background:${bg};border:1px solid ${border};overflow:hidden;position:relative;">
            <div style="height:100%;width:${pct}%;min-width:26px;border-radius:999px;background:${color};box-shadow:0 8px 20px rgba(15,23,42,.12);"></div>
            <div style="position:absolute;inset:0;display:flex;align-items:center;padding:0 12px;font-size:11px;font-weight:900;color:#0f172a;">${escHtml(detail || '')}</div>
          </div>
    ...
    return `
      <div style="border:1px solid rgba(15,23,42,.10);border-radius:18px;padding:14px;background:linear-gradient(180deg,rgba(255,255,255,.96),rgba(248,250,252,.92));margin-bottom:12px;">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap;margin-bottom:10px;">
          <div>
            <div style="font-size:15px;font-weight:1000;color:#0f172a">Funil de vendas — barras</div>
            <div style="font-size:12px;font-weight:800;color:#64748b">Base: leads que passaram por ATENDIDO no período. Convertidos = LEAD CONVERTIDO; Perdidos = LEAD DESCARTADO.</div>
          </div>
          <div style="display:grid;grid-template-columns:repeat(4,minmax(86px,1fr));gap:8px;min-width:360px;">
            <div style="border-radius:14px;padding:9px;background:#eff6ff;border:1px solid #bfdbfe"><div style="font-size:10px;font-weight:900;color:#1d4ed8">ATENDIDOS</div><strong style="font-size:18px;color:#1d4ed8">${passed}</strong></div>
            <div style="border-radius:14px;padding:9px;background:#eef2ff;border:1px solid #c7d2fe"><div style="font-size:10px;font-weight:900;color:#4338ca">QUALIFICADOS</div><strong style="font-size:18px;color:#4338ca">${qualified}</strong></div>
            <div style="border-radius:14px;padding:9px;background:#ecfdf5;border:1px solid #bbf7d0"><div style="font-size:10px;font-weight:900;color:#166534">CONVERTIDOS</div><strong style="font-size:18px;color:#15803d">${converted}</strong></div>
            <div style="border-radius:14px;padding:9px;background:#fef2f2;border:1px solid #fecaca"><div style="font-size:10px;font-weight:900;color:#991b1b">PERDIDOS</div><strong style="font-size:18px;color:#b91c1c">${lost}</strong></div>
          </div>
        </div>
        ${bar('ATENDIDOS', passed, '100% da base do funil', 'linear-gradient(90deg,#2563eb,#38bdf8)', '#eff6ff', '#bfdbfe')}
        ${bar('QUALIFICADOS', qualified, `${safePct(qualified, passed)} dos atendidos`, 'linear-gradient(90deg,#4f46e5,#818cf8)', '#eef2ff', '#c7d2fe')}
        ${bar('CONVERTIDOS', converted, `${safePct(converted, passed)} dos atendidos`, 'linear-gradient(90deg,#16a34a,#22c55e)', '#ecfdf5', '#bbf7d0')}
        ${bar('PERDIDOS', lost, `${safePct(lost, passed)} dos atendidos`, 'linear-gradient(90deg,#dc2626,#fb7185)', '#fef2f2', '#fecaca')}
        ${(data.stagePassCounts ? Object.entries(data.stagePassCounts) : []).map(([k,v])=>bar(k, Number(v||0), 'Total no estágio / passaram pelo estágio', 'linear-gradient(90deg,#64748b,#94a3b8)', '#f8fafc', '#e2e8f0')).join('')}
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:8px;margin-top:10px;font-size:12px;font-weight:900;color:#334155;">
          ${(data.stagePassCounts ? Object.entries(data.stagePassCounts) : []).map(([k,v])=>`<div style="border:1px solid #e2e8f0;background:#fff;border-radius:12px;padding:8px"><div style="font-size:10px;color:#64748b">${escHtml(k)}</div><strong style="font-size:16px;color:#0f172a">${Number(v||0)}</strong></div>`).join('')}
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center;margin-top:12px;font-size:12px;font-weight:900;color:#334155;">
          <span style="border-radius:999px;background:#eef2ff;border:1px solid #c7d2fe;padding:6px 10px">Qualificação: ${safePct(qualified, passed)}</span>
          <span style="border-radius:999px;background:#eff6ff;border:1px solid #bfdbfe;padding:6px 10px">Conversão: ${safePct(converted, passed)}</span>
          <span style="border-radius:999px;background:#fef2f2;border:1px solid #fecaca;padding:6px 10px">Perda: ${safePct(lost, passed)}</span>
          <span style="border-radius:999px;background:#f8fafc;border:1px solid #e2e8f0;padding:6px 10px">Convertidos + Perdidos: ${closed}</span>
        </div>
      </div>`;
  }
  ...
  function openSalesFunnelPanel(userId, opts = {}) {
    ...
    const render = async () => {
      ...
      const html = `<div style="display:flex;gap:8px;justify-content:space-between;align-items:end;flex-wrap:wrap;margin-bottom:12px"><div style="font-weight:950;font-size:16px">${state.allUsers && isAdmin ? 'FUNIL DE VENDAS — GERAL' : 'FUNIL DE VENDAS'}</div><div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">${isAdmin?`<label style="font-size:12px;font-weight:900"><input id="funnelAllUsers" type="checkbox" ${state.allUsers?'checked':''}> Geral</label><select id="funnelUserSelect" class="eqd-input" ${state.allUsers?'disabled':''}>${userOptions}</select>`:''}${periodBtns}<input type="date" id="funnelStart" class="eqd-input" value="${escHtml(state.startDate)}"><input type="date" id="funnelEnd" class="eqd-input" value="${escHtml(state.endDate)}"><button class="eqd-btn" id="funnelApplyCustom">Aplicar</button></div></div><div class="eqd-grid" style="grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:10px;margin-bottom:12px"><div class="kpi"><div>Passaram por ATENDIDO</div><strong>${data.totalPassedAtendido}</strong></div><div class="kpi"><div>Interagidos</div><strong>${data.totalInteragido || 0}</strong></div><div class="kpi"><div>Qualificados</div><strong>${data.totalQualified}</strong></div><div class="kpi"><div>Convertidos</div><strong>${data.totalConverted}</strong></div><div class="kpi"><div>Perdidos</div><strong>${data.totalLost}</strong></div><div class="kpi"><div>Lead descartado</div><strong>${data.totalLeadDescartado || 0}</strong></div><div class="kpi"><div>Lead convertido</div><strong>${data.totalLeadConvertido || 0}</strong></div><div class="kpi"><div>Qualificação</div><strong>${qRate}</strong></div><div class="kpi"><div>Conversão</div><strong>${rate}</strong></div><div class="kpi"><div>Média ATENDIDO → CONVERTIDO</div><strong>${msToHuman(data.avgTimeAtendidoToConverted)}</strong></div><div class="kpi"><div>Média ATENDIDO → PERDIDO</div><strong>${msToHuman(data.avgTimeAtendidoToLost)}</strong></div></div>${funnelChart}${ranking}<div style="margin-top:12px;font-size:12px;font-weight:800;opacity:.75">Histórico completo disponível a partir da ativação do rastreamento. A seção “passaram por estágio” soma cada lead uma vez em cada etapa por onde passou, não apenas o estágio atual. Dados antigos podem ficar incompletos.</div><div style="margin-top:6px;font-size:11px;font-weight:800;opacity:.62">${escHtml(leadFunnelStatusDebugText())}</div>`;
