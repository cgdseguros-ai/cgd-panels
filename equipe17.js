...
restoreSyncQueue();
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    openModal(`Leads — ${user.name}`, `
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;font-size:12px;font-weight:950">
        <button class="eqd-pill" id="leadVencerFilterBtn" title="Filtrar somente leads próximos de vencer" style="background:#fff;color:#111;border-color:rgba(0,0,0,.12);cursor:pointer">Próximos de vencer: <strong style="margin-left:4px">${[...ok,...itg,...q].filter((l)=>isLeadProximoVencer(l)).length}</strong></button>
        <span class="eqd-pill" style="background:#fff;color:#111;border-color:rgba(0,0,0,.12)">Sem FUP futuro: <strong style="margin-left:4px">${[...ok,...itg,...q].filter((l)=>!hasFutureFollowupForLeadOrig(leadOrigemId(l)||l.ID)).length}</strong></span>
      </div>
    ...
    function cardLead(l, column) {
      ...
      return `
        <div class="leadCard" data-lead-col="${escHtml(column)}" data-fonte="${escHtml(norm(fonte))}" data-operadora="${escHtml(norm(op))}" data-proximo-vencer="${isLeadProximoVencer(l) ? '1' : '0'}">
    ...
    function applyLeadColumnFilters(){
      document.querySelectorAll('.leadColFilters').forEach((box)=>{
        ...
        document.querySelectorAll(`.leadCard[data-lead-col="${col}"]`).forEach((card)=>{
          ...
          card.style.display = ((!fonte || cf.includes(fonte)) && (!oper || co.includes(oper))) ? '' : 'none';
        });
      });
    }
    ...
    if (vencerBtn) vencerBtn.onclick = () => {
      const active = vencerBtn.getAttribute('data-active') === '1';
      vencerBtn.setAttribute('data-active', active ? '0' : '1');
      vencerBtn.style.boxShadow = active ? '' : '0 0 0 3px rgba(245,158,11,.28)';
      document.querySelectorAll('.leadCard').forEach((card)=>{
        if (active) card.style.display = '';
        else card.style.display = card.getAttribute('data-proximo-vencer') === '1' ? '' : 'none';
      });
    };
