/* eqd.js — GET • CGD CORRETORA (v5.0)
   PATCH REAL V5.2 — 2026-05-13: proteção HELENA/WPP + reagendamento anti-rollback pipeline 17
...
(function () {
  ...
  async function loadDeals(opts = {}) {
    ...
    const nowMs = Date.now();
    const specialWindowLoad = recentOnly || oldOnly;
    ...
    if (openOnly) {
      ...
      STATE.dealsLoadedMode = recentOnly ? "recent" : (oldOnly ? "full" : "full");
...
function makeUserCard(u) {
  ...
  function renderGeneral() {
    ...
    el.main.innerHTML = `
      ...
      <div class="userGrid">
        ${USERS.map(makeUserCard).join("")}${makeMarketingCard()}${makeTriadeCard()}
      </div>
    `;
  ...
  async function loadMetricsCore(opts = {}) {
    ...
    await bootTimerMeasure("Métricas rápidas: deals/tarefas recentes", () => loadDeals({ forceFull: true, openOnly: false, deferPhotos: true, recentOnly: true, daysBack: 5 }), "Cards operacionais: prazo de hoje - 5 dias em diante, futuros e não concluídos");
  ...
  function schedulePostReadyBackgroundDataLoads() {
    try {
      setTimeout(async () => {
        try {
          await bootTimerMeasure("Background: deals/tarefas antigos", () => loadDeals({ forceFull: true, openOnly: false, deferPhotos: true, oldOnly: true, olderThanDays: 5, append: true }).catch(()=>{}), "Vencidos há mais de 5 dias, concluídos/perdidos e histórico com prazo antigo; não bloqueia READY");
          try { applyLocalDealPatches(); } catch(_) {}
          try { overlayPendingSyncState(); } catch(_) {}
          try { rebuildDealsOpen(); } catch(_) {}
        ...
        try {
          await bootTimerMeasure("Leads perdidos/convertidos em background", () => loadLeadsSnapshotAllUsers({ force: true, inactiveOnly: true, merge: true }).catch(()=>{}), "PERDIDOS e CONVERTIDOS carregados após READY para não travar o painel LEADS operacional");
          if (currentView && currentView.kind === "general" && el && el.modalOverlay && el.modalOverlay.style.display !== "flex") renderCurrentView(true);
        } catch(_) {}
      }, 500);
    } catch(_) {}
...
document.addEventListener("click", async (e) => {
  ...
  try {
    ...
    // ─── FASE 3 — FINALIZING: DADOS COMPLEMENTARES/SYNC ANTES DAS FOTOS ───
    setBootPhase(BOOT_PHASES.FINALIZING);
    await loadOperationalDataAfterMetrics({ silent: false, forceRecur: true, forceLeads: true });
    bootTimerMark("OPERATIONAL_DATA_DONE");
    BOOT_STATE.criticalSyncReady = true;
    renderCurrentView(true);

    // ─── FASE 4 — FOTOS POR ÚLTIMO → READY ────────────────────────────────
    setBootProgress(92, "Carregando fotos e finalizando...");
    restoreDeferredPhotosForFinalizing();
    await bootTimerMeasure("Fotos/fallback", () => loadUserPhotosWithFallback(), "Fotos carregadas por último; fallback evita travar");
    bootTimerMark("PHOTOS_DONE");
    BOOT_STATE.photosResolved = true;
