...
${marker}` : marker;
  ...
  function calcLeadFunnelMetrics(userId, opts = {}) {
    ...
    const byUser = new Map(); let passed=0, conv=0, lost=0; const convDur=[], lostDur=[];
    for (const l of leads) {
      ...
      passed++;
      const convAt = firstLeadDateForStageNamesFunnel(l, ["LEAD CONVERTIDO", "CONVERTIDO", "CONVERTIDOS"], false) || (isConvertedLeadFunnel(l) ? parseFlexDate(l.DATE_MODIFY || l.DATE_CREATE || "") : null);
      const lostAt = firstLeadDateForStageNamesFunnel(l, ["LEAD DESCARTADO", "DESCARTADO", "LEAD PERDIDO", "PERDIDO", "PERDIDOS"], false) || (isLostLeadFunnel(l) ? parseFlexDate(l.DATE_MODIFY || l.DATE_CREATE || "") : null);
      const isConv = !!convAt || isConvertedLeadFunnel(l); const isLost = !!lostAt || isLostLeadFunnel(l);
      if (isConv) conv++; if (isLost) lost++;
      if (!byUser.has(owner)) byUser.set(owner, { userId: owner, passed:0, converted:0, lost:0 });
      const r=byUser.get(owner); r.passed++; if (isConv) r.converted++; if (isLost) r.lost++;
    ...
    const userRows=[...byUser.values()].map((r)=>({ ...r, name: (USERS.find((u)=>String(u.userId)===String(r.userId))||{}).name || r.userId, rate: r.passed ? r.converted/r.passed : 0 })).sort((a,b)=>b.converted-a.converted || b.passed-a.passed);
    return { totalPassedAtendido: passed, totalConverted: conv, totalLost: lost, conversionRate: passed ? conv/passed : 0, avgTimeAtendidoToConverted: avg(convDur), avgTimeAtendidoToLost: avg(lostDur), userRows };
  }
  function renderSalesFunnelChart(data) {
    ...
    const segWidth = (val) => {
      if (passed <= 0) return 28;
      return Math.max(28, Math.min(100, Math.round((Number(val || 0) / passed) * 100)));
    };
    const row = (label, val, detail, w, c1, c2, textColor = '#fff') => `
      <div style="display:flex;align-items:center;gap:10px;margin:10px 0;">
        <div style="width:132px;font-size:12px;font-weight:950;color:#334155;text-align:right">${escHtml(label)}</div>
        <div style="flex:1;min-width:160px;">
          <div style="width:${w}%;min-width:170px;margin:0 auto;border-radius:14px;padding:12px 14px;box-sizing:border-box;color:${textColor};font-weight:950;background:linear-gradient(135deg,${c1},${c2});box-shadow:0 8px 22px rgba(15,23,42,.14);clip-path:polygon(0 0,100% 0,93% 100%,7% 100%);">
            <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;">
              <span>${escHtml(label)}</span>
              <strong style="font-size:20px;line-height:1">${Number(val || 0)}</strong>
            </div>
            <div style="font-size:11px;font-weight:800;opacity:.92;margin-top:3px">${escHtml(detail || '')}</div>
          </div>
        </div>
      </div>`;
    return `
      <div style="border:1px solid rgba(15,23,42,.10);border-radius:18px;padding:14px;background:linear-gradient(180deg,rgba(255,255,255,.96),rgba(248,250,252,.92));margin-bottom:12px;">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap;margin-bottom:10px;">
          <div>
            <div style="font-size:15px;font-weight:1000;color:#0f172a">Funil de vendas</div>
            <div style="font-size:12px;font-weight:800;color:#64748b">Base: leads que passaram por ATENDIDO no período selecionado. Convertidos = LEAD CONVERTIDO; Perdidos = LEAD DESCARTADO.</div>
          </div>
          <div style="display:grid;grid-template-columns:repeat(3,minmax(92px,1fr));gap:8px;min-width:290px;">
            ...
            <div style="border-radius:14px;padding:9px;background:#eff6ff;border:1px solid #bfdbfe"><div style="font-size:10px;font-weight:900;color:#1d4ed8">FECHADOS</div><strong style="font-size:18px;color:#1d4ed8">${closed}</strong></div>
          </div>
        </div>
        ${row('ATENDIDOS', passed, '100% da base do funil', 100, '#2563eb', '#38bdf8')}
        ${row('CONVERTIDOS', converted, `${safePct(converted, passed)} dos atendidos`, segWidth(converted), '#16a34a', '#22c55e')}
        ${row('PERDIDOS', lost, `${safePct(lost, passed)} dos atendidos`, segWidth(lost), '#dc2626', '#fb7185')}
  ...
  function openSalesFunnelPanel(userId, opts = {}) {
    const uid = String(userId || ""); const isAdmin = opts.adminView === true || isAdminPanelUser(uid);
    const state = { allUsers: opts.allUsers !== undefined ? !!opts.allUsers : !!opts.adminView, selectedUserId: opts.selectedUserId || uid, period: opts.period || "30", startDate:"", endDate:"" };
    const render = () => {
