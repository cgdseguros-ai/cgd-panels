...
(function () {
  ...
  function openGlobalLeadsDayStats() {
    ...
    openModal(`LEADS • DIA (${fmtDateOnly(new Date())})`, `
      <div style="font-size:12px;font-weight:950;opacity:.82;margin-bottom:8px">
        * ATENDIDO NO DIA usa o campo DATA/HORA do lead como referência, mesmo que hoje ele já esteja em outro stage.
      </div>
  ...
  function openGlobalLeadsHistoryModal(days) {
    ...
    const body = users.map((u) => {
      ...
      const total = dates.reduce((acc, d) => acc + mine.filter((l) => getLeadDayKeyFromDataHora(l) === ymdKey(d)).length, 0);
    ...
    openModal(`LEADS • ${days === 7 ? "SEMANA" : "MÊS"}`, `
      <div style="font-size:12px;font-weight:950;opacity:.82;margin-bottom:8px">
        Histórico de leads marcados como ATENDIDOS no dia pelo campo DATA/HORA, mesmo que depois mudem de stage.
      </div>
  ...
  function openEditRecurrenceRuleModal(userId, ruleId) {
    ...
    if (t === "MONTHLY") {
      const cur = Number(rule.monthDay || 1);
      const val = Number(prompt("Novo dia do mês (1 a 31):", String(cur)) || "");
      if (!(val >= 1 && val <= 31)) return;
      rules[idx] = { ...rule, monthDay: val };
      setBusy("Salvando regra…");
      saveRulesForUser(uid, rules).then(() => { clearBusy(); closeModal(); openRecurrenceManager(uid); }).catch((e) => { clearBusy(); alert("Falha: " + (e.message || e)); });
      return;
    }
