...
(function () {
  ...
  const DEAL_UF_HELENA = "UF_CRM_1772544366";
  const LEAD_UF_WPP_DIRETO = "UF_CRM_1772560431";
  const DEAL_UF_WPP_DIRETO = "UF_CRM_1772572060";
  const DEAL_UF_IDADE = "UF_CRM_DEAL_1731909675411";
  const DEAL_UF_BAIRRO = "UF_CRM_67D2E113EA615";
  const DEAL_UF_TELEFONE = "UF_CRM_1772572168";
  const DEAL_UF_OPERADORA = "UF_CRM_1771388342";
  const DEAL_UF_FONTE = "UF_CRM_1767283877974";
  ...
  const LAST_LEAD_FILTERS = { operadora: "", atendidoDate: "" };
  ...
  function parseDeal(deal, maps) {
    ...
    const helenaOn = dealHelenaOn(deal);
    const wppDiretoOn = dealWppDiretoOn(deal);
    ...
    return Object.assign(deal, {
      ...
      _helenaOn: helenaOn,
      _wppDiretoOn: wppDiretoOn,
      _leadOrigemId: String(deal[DEAL_UF_LEAD_ORIGEM] || "").trim(),
      _leadPhone: String(deal[DEAL_UF_TELEFONE] || "").trim(),
  ...
  function leadAtendidoDia(lead) { return String((lead && lead[LEAD_UF_ATENDIDO_DIA]) || "").trim(); }
  function leadHelenaOn(lead) { const v = norm((lead && lead[LEAD_UF_HELENA]) || ""); return v.includes("SIM"); }
  function leadWppDiretoOn(lead) { const v = norm((lead && lead[LEAD_UF_WPP_DIRETO]) || ""); return v.includes("SIM"); }
  function dealHelenaOn(deal) { const v = norm((deal && deal[DEAL_UF_HELENA]) || ""); return v.includes("SIM"); }
  function dealWppDiretoOn(deal) { const v = norm((deal && deal[DEAL_UF_WPP_DIRETO]) || ""); return v.includes("SIM"); }
  function sameYmdFromDateAndIso(dateObj, isoLike) {
    const d = tryParseDateAny(isoLike);
    return !!(d && dateObj && d.getFullYear()===dateObj.getFullYear() && d.getMonth()===dateObj.getMonth() && d.getDate()===dateObj.getDate());
  }
  function isQualifiedLeadByDeal(deal) {
    const leadId = String((deal && deal[DEAL_UF_LEAD_ORIGEM]) || "").trim();
    if (!leadId) return false;
    for (const arr of STATE.leadsByUser.values()) {
      const lead = (arr || []).find((l) => String(l.ID) === leadId);
      if (lead) return norm(String(lead.STATUS_ID || "")) === norm(leadStageId("QUALIFICADO"));
    }
    return false;
  }
  function hasOverdueFollowupForUser(userId) {
    return (STATE.dealsOpen || []).some((d) => String(d.ASSIGNED_BY_ID || d._assigned || "") === String(userId) && isFollowupDeal(d) && d._late);
  }
  ...
  async function loadLeadsForOneUser(userId) {
    ...
    const select = [
      ...
      LEAD_UF_OPERADORA, LEAD_UF_IDADE, LEAD_UF_TELEFONE, LEAD_UF_BAIRRO, LEAD_UF_FONTE, LEAD_UF_DATAHORA, LEAD_UF_OBS, LEAD_UF_ATENDIDO_DIA, LEAD_UF_HELENA, LEAD_UF_WPP_DIRETO,
    ].filter(Boolean);
  ...
  async function loadDeals() {
    ...
    const select = [
      ...
      UF_TAREFA, UF_PRAZO, UF_URGENCIA, UF_ETAPA, UF_COLAB, UF_OBS, DEAL_UF_HELENA, DEAL_UF_WPP_DIRETO, DEAL_UF_IDADE, DEAL_UF_BAIRRO, DEAL_UF_TELEFONE, DEAL_UF_OPERADORA, DEAL_UF_FONTE, DEAL_UF_LEAD_ORIGEM,
    ];
  ...
  function makeDealCard(deal, context) {
    ...
    tags.push(`<span class="eqd-tag eqd-tagClickable" data-action="toggleDealHelena" data-id="${deal.ID}" style="background:${deal._helenaOn ? 'rgba(155,80,255,.20)' : 'rgba(60,60,70,.18)'};border-color:${deal._helenaOn ? 'rgba(155,80,255,.42)' : 'rgba(60,60,70,.30)'};color:${deal._helenaOn ? 'rgba(90,20,150,.95)' : 'rgba(70,70,70,.92)'}">${deal._helenaOn ? 'HELENA' : 'HELENA NÃO'}</span>`);
    tags.push(`<span class="eqd-tag eqd-tagClickable" data-action="toggleDealWpp" data-id="${deal.ID}" style="background:${deal._wppDiretoOn ? 'rgba(22,163,74,.16)' : 'rgba(90,90,90,.14)'};border-color:${deal._wppDiretoOn ? 'rgba(22,163,74,.34)' : 'rgba(90,90,90,.26)'};color:${deal._wppDiretoOn ? 'rgba(10,120,40,.96)' : 'rgba(80,80,80,.92)'}">${deal._wppDiretoOn ? 'WPP DIRETO' : 'WPP DIRETO NÃO'}</span>`);
    if (deal._qualLeadAlert) tags.push(`<span class="eqd-tag" style="background:rgba(110,90,255,.16);border-color:rgba(110,90,255,.34);color:rgba(70,40,160,.96)">QUALIFICADO</span>`);
    if (deal._urgTxt) {
  ...
  // 17) FOLLOW-UP (deal)
  // =========================
  async function createFollowUpDealForUser(user, negocioNome, prazoIso, sourceLead) {
    ...
    if (sourceLead) {
      if (leadIdade(sourceLead)) fields[DEAL_UF_IDADE] = leadIdade(sourceLead);
      if (leadBairro(sourceLead)) fields[DEAL_UF_BAIRRO] = leadBairro(sourceLead);
      if (leadTelefone(sourceLead)) fields[DEAL_UF_TELEFONE] = leadTelefone(sourceLead);
      if (leadOperadora(sourceLead)) fields[DEAL_UF_OPERADORA] = leadOperadora(sourceLead);
      if (leadFonte(sourceLead)) fields[DEAL_UF_FONTE] = leadFonte(sourceLead);
      fields[DEAL_UF_LEAD_ORIGEM] = String(sourceLead.ID || "");
      fields[DEAL_UF_HELENA] = leadHelenaOn(sourceLead) ? "SIM" : "NÃO";
      fields[DEAL_UF_WPP_DIRETO] = leadWppDiretoOn(sourceLead) ? "SIM" : "NÃO";
    }
  ...
  function openFollowUpModal(user, prefillName, opts) {
    ...
    const sourceLead = opts && opts.sourceLead ? opts.sourceLead : null;
    const tel = sourceLead ? leadTelefone(sourceLead) : "";
    const userDeals = (STATE.dealsAll || []).filter((d) => String(d.ASSIGNED_BY_ID || d._assigned || "") === String(user.userId));
    const byPhone = tel ? userDeals.filter((d) => isFollowupDeal(d) && String(d[DEAL_UF_TELEFONE] || "").trim() === tel) : [];
    const futureCount = byPhone.filter((d) => { if (!d._prazo) return false; const t = new Date(d._prazo).getTime(); return Number.isFinite(t) && t >= Date.now(); }).length;
    const historyHtml = sourceLead ? `<div style="grid-column:1 / -1;font-size:11px;font-weight:900;opacity:.85;border:1px solid rgba(0,0,0,.10);padding:10px;border-radius:12px;background:rgba(255,255,255,.55)">Telefone: <strong>${escHtml(tel || '—')}</strong> • Follow-ups antigos: <strong>${byPhone.length}</strong> • Agendados hoje/futuros: <strong>${futureCount}</strong></div>` : ``;
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        // ✅ SALVAR AQUI
        await createFollowUpDealForUser(user, nm, prazoIso, sourceLead);
  ...
  async function openFollowupListModalForUser(user) {
    if (LEAD_USERS.has(String(user.userId)) && !STATE.leadsByUser.has(String(user.userId))) {
      try { await loadLeadsForOneUser(user.userId); } catch (_) {}
    }
    ...
    const render = (kwRaw) => {
      ...
      const filtered = kw
        ? list.filter((d) => norm([d.TITLE || "", d._obs || "", d._colabTxt || "", d._etapaTxt || "", d._urgTxt || "", d[DEAL_UF_TELEFONE] || ""].join(" ")).includes(kw))
        : list.slice();

      const ordered = sortDeals(filtered).map((d) => Object.assign({}, d, { _qualLeadAlert: isQualifiedLeadByDeal(d) }));
  ...
  async function openLeadsModalForUser(userId, kwRaw) {
    ...
    const kw = norm(String(kwRaw || "").trim());
    let filtered = kw ? leadsUser.filter((l) => leadMatchesKw(l, kw)) : leadsUser.slice();
    if (LAST_LEAD_FILTERS.operadora) {
      filtered = filtered.filter((l) => norm(leadOperadora(l)) === norm(LAST_LEAD_FILTERS.operadora));
    }
    if (LAST_LEAD_FILTERS.atendidoDate) {
      const d = parseDayKey(LAST_LEAD_FILTERS.atendidoDate);
      if (d) filtered = filtered.filter((l) => sameYmdFromDateAndIso(d, leadAtendidoDia(l)));
    }

    const at = sAt ? filtered.filter((l) => String(l.STATUS_ID) === String(sAt)) : [];
    const ok = sAtendido ? filtered.filter((l) => String(l.STATUS_ID) === String(sAtendido)).sort((a,b) => {
      const ta = tryParseDateAny(leadAtendidoDia(a)); const tb = tryParseDateAny(leadAtendidoDia(b));
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:space-between;align-items:center">
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <input id="leadSearch" class="eqd-searchInput"
            style="width:min(520px,85vw);color:#111;background:#fff;border-color:rgba(0,0,0,.18)"
            placeholder="Buscar lead por palavra (nome, operadora, tel, bairro, fonte, data/hora, etapa…)"
            value="${escHtml(String(kwRaw || ""))}" />
          <input id="leadAtendidoDate" type="date" value="${escHtml(LAST_LEAD_FILTERS.atendidoDate || "")}" style="padding:9px 10px;border-radius:999px;border:1px solid rgba(0,0,0,.18);font-weight:900" />
          <input id="leadOperadoraFilter" placeholder="Filtrar operadora" value="${escHtml(LAST_LEAD_FILTERS.operadora || "")}" style="padding:9px 12px;border-radius:999px;border:1px solid rgba(0,0,0,.18);font-weight:900;min-width:180px" />
    ...
    function cardLead(l, column) {
      ...
      const atendidoTxt = leadAtendidoDia(l) ? fmt(leadAtendidoDia(l)) : "";
      ...
      const helenaChip = `<span class="leadObsChip ${leadHelenaOn(l) ? 'on' : 'off'}" style="background:${leadHelenaOn(l) ? 'rgba(155,80,255,.22)' : 'rgba(80,80,80,.18)'};border-color:${leadHelenaOn(l) ? 'rgba(155,80,255,.50)' : 'rgba(80,80,80,.35)'};color:${leadHelenaOn(l) ? 'rgba(90,20,150,.96)' : 'rgba(70,70,70,.92)'}" data-action="toggleLeadHelena" data-leadid="${l.ID}" data-userid="${user.userId}">${leadHelenaOn(l) ? 'HELENA' : 'HELENA NÃO'}</span>`;
      const wppChip = `<span class="leadObsChip ${leadWppDiretoOn(l) ? 'on' : 'off'}" style="background:${leadWppDiretoOn(l) ? 'rgba(22,163,74,.18)' : 'rgba(80,80,80,.18)'};border-color:${leadWppDiretoOn(l) ? 'rgba(22,163,74,.42)' : 'rgba(80,80,80,.35)'};color:${leadWppDiretoOn(l) ? 'rgba(10,120,40,.96)' : 'rgba(70,70,70,.92)'}" data-action="toggleLeadWpp" data-leadid="${l.ID}" data-userid="${user.userId}">${leadWppDiretoOn(l) ? 'WPP DIRETO' : 'WPP DIRETO NÃO'}</span>`;
      ...
      return `
        <div class="leadCard">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
            ...
            <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">${helenaChip}${wppChip}${obsChip}</div>
          </div>
          ...
          <div class="leadMeta">
            ...
            ${atendidoTxt ? `<span>Atendido: <strong>${escHtml(atendidoTxt)}</strong></span>` : ``}
    ...
    document.getElementById("leadSearchBtn").onclick = () => { LAST_LEAD_FILTERS.operadora = String(document.getElementById("leadOperadoraFilter").value || "").trim(); LAST_LEAD_FILTERS.atendidoDate = String(document.getElementById("leadAtendidoDate").value || "").trim(); openLeadsModalForUser(user.userId, String(document.getElementById("leadSearch").value || "").trim()); };
    document.getElementById("leadSearch").onkeydown = (e) => { if (e.key === "Enter") { LAST_LEAD_FILTERS.operadora = String(document.getElementById("leadOperadoraFilter").value || "").trim(); LAST_LEAD_FILTERS.atendidoDate = String(document.getElementById("leadAtendidoDate").value || "").trim(); openLeadsModalForUser(user.userId, String(e.target.value || "").trim()); } };
    document.getElementById("leadSearchClear").onclick = () => { LAST_LEAD_FILTERS.operadora = ""; LAST_LEAD_FILTERS.atendidoDate = ""; openLeadsModalForUser(user.userId, ""); };
  }
  ...
  function leadsAttendedOnSelectedDay(userId) {
    ...
    return (leads || []).filter((l) => {
      const dt = tryParseDateAny(leadAtendidoDia(l));
      if (!dt) return false;
      const t = dt.getTime();
      return t >= ds && t <= de;
    }).sort((a,b) => {
  ...
  function renderUserPanel(userId) {
    ...
    const followListBtn = `<button class="eqd-btn ${hasOverdueFollowupForUser(user.userId) ? 'blinkRedBorder' : ''}" data-action="followList" data-userid="${user.userId}">LISTA DE FOLLOW-UP</button>`;
  ...
  async function moveLeadStage(userId, leadId, toStatus){
    ...
    const fields = { STATUS_ID: String(toStatus) };
    const sAtendido = leadStageId("ATENDIDO");
    const leads = STATE.leadsByUser.get(String(userId)) || [];
    const lead = leads.find((l) => String(l.ID) === String(leadId));
    if (String(toStatus) === String(sAtendido) && lead && !leadAtendidoDia(lead)) {
      fields[LEAD_UF_ATENDIDO_DIA] = new Date().toISOString();
    }

    // ✅ SALVAR AQUI
    await bx("crm.lead.update", { id: String(leadId), fields });
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "toggleLeadHelena" || act === "toggleLeadWpp") {
        const uid = String(a.getAttribute("data-userid")||"");
        const leadId = String(a.getAttribute("data-leadid")||"");
        if (!uid || !leadId) return;
        ...
        const lead = leads.find((l) => String(l.ID) === String(leadId));
        if (!lead) return;
        const field = act === "toggleLeadHelena" ? LEAD_UF_HELENA : LEAD_UF_WPP_DIRETO;
        const curOn = act === "toggleLeadHelena" ? leadHelenaOn(lead) : leadWppDiretoOn(lead);
        const nextVal = curOn ? "NÃO" : "SIM";
        await bx("crm.lead.update", { id: String(leadId), fields: { [field]: nextVal } });
        lead[field] = nextVal;
        return reopenLeadsModalSafe();
      }

      if (act === "toggleDealHelena" || act === "toggleDealWpp") {
        const field = act === "toggleDealHelena" ? DEAL_UF_HELENA : DEAL_UF_WPP_DIRETO;
        const d = getDealById(id);
        if (!d) return;
        const curOn = act === "toggleDealHelena" ? dealHelenaOn(d) : dealWppDiretoOn(d);
        const nextVal = curOn ? "NÃO" : "SIM";
        await bx("crm.deal.update", { id: String(id), fields: { [field]: nextVal } });
        updateDealInState(id, { [field]: nextVal, _helenaOn: field === DEAL_UF_HELENA ? !curOn : d._helenaOn, _wppDiretoOn: field === DEAL_UF_WPP_DIRETO ? !curOn : d._wppDiretoOn });
        return renderCurrentView();
      }
      ...
      if (act === "leadFollowupModal") {
        ...
        return openFollowUpModal(u, nm, { sourceLead: lead || null, returnToLeads: { userId: uid, kw: LAST_LEADS_CTX.kw || "" } });
      }
