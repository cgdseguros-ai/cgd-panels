...
(function () {
  ...
  function sanitizeDealUpdateFields(fields) {
    const src = fields || {};
    const out = {};
    Object.keys(src).forEach((k) => {
      if (k === '__stageName') return;
      let v = src[k];
      if (v === undefined) return;
      if (v === null) v = '';
      if (typeof v === 'string') v = v.trim();
      if ((k === 'CURRENCY_ID' || k === 'COMMENTS') && !v) return;
      out[k] = v;
    });
    return out;
  }
  async function safeDealUpdate(dealId, fields) {
    const clean = sanitizeDealUpdateFields(fields);
    if (!Object.keys(clean).length) return true;
    try {
      return await bx("crm.deal.update", { id: String(dealId), fields: clean });
    } catch (e) {
      const msg = String((e && e.message) || e || "");
      if (!/HTTP 400\s+em\s+crm\.deal\.update/i.test(msg)) throw e;
      try {
        const fresh = await bx("crm.deal.get", { id: String(dealId) });
        if (dealFieldsMatchRequested(fresh, clean)) return true;
      } catch (_) {}
      throw e;
    }
  }
    ...
    try {
      ...
      for (const u of USERS) {
        ...
        for (const rule of rules) {
          ...
          try {
            const holdUntil = Number(STATE.recurRecentRuleCreates.get(rid) || 0);
            if (holdUntil && holdUntil > Date.now()) continue;
          } catch (_) {}
...
${curObs}` : transferAlert;
  ...
  async function confirmActionModal(message, title='Confirmar') {
    return await new Promise((resolve) => {
      openModal(title, `
        <div style="display:flex;flex-direction:column;gap:12px">
          <div style="font-size:13px;font-weight:900;line-height:1.4">${escHtml(String(message || ''))}</div>
          <div style="display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap">
            <button class="eqd-btn" id="confirmActionNo">Cancelar</button>
            <button class="eqd-btn eqd-btnPrimary" id="confirmActionYes">Confirmar</button>
          </div>
        </div>
      `, { wide:true });
      const noBtn = document.getElementById('confirmActionNo');
      const yesBtn = document.getElementById('confirmActionYes');
      if (noBtn) noBtn.onclick = () => { closeModal(); resolve(false); };
      if (yesBtn) yesBtn.onclick = () => { closeModal(); resolve(true); };
    });
  }
  ...
  function openGlobalNoFutureFollowupModal() {
    ...
    const render = () => {
      ...
      document.querySelectorAll('.eqd-adminNoFuSingleConv').forEach((btn) => btn.onclick = async () => {
        ...
        const ok = await confirmActionModal('Tem certeza que deseja converter?');
        if (!ok) return;
        try {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (recType === "NONE") {
          ...
          rebuildDealsOpen();
          renderCurrentView();
          UI_REFRESH_HOLD_UNTIL = Date.now() + 2500;
        setTimeout(() => { refreshData(true).catch(()=>{}); }, 2200);
          return;
        }
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    btn.onclick = async () => {
      try {
        ...
        setTimeout(() => { refreshData(true).catch(()=>{}); }, 2400);
      } catch (e) {
        warn.style.display = "block";
        warn.textContent = "Falha:\n" + (e.message || e);
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function renderProposalPie(parts) {
    const vals = (parts || []).map((p) => Math.max(0, Number(p.value || 0)));
    const total = vals.reduce((a,b)=>a+b,0);
    if (!total) return '';
    let acc = 0;
    const cx=18, cy=18, r=16;
    const segs = (parts || []).filter((p)=>Number(p.value||0)>0).map((p) => {
      ...
      return `<path d="M ${cx} ${cy} L ${x1.toFixed(2)} ${y1.toFixed(2)} A ${r} ${r} 0 ${large} 1 ${x2.toFixed(2)} ${y2.toFixed(2)} Z" fill="${p.color}" />`;
    }).join('');
    return `<svg width="36" height="36" viewBox="0 0 36 36" aria-hidden="true" style="display:block"><circle cx="18" cy="18" r="16" fill="rgba(0,0,0,.06)" />${segs}<circle cx="18" cy="18" r="8" fill="#fff" /></svg>`;
  }
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const stale = (Date.now() - Number(STATE.proposalLastLoadedAt || 0)) > 1800000;
    if (!force && uid) {
...
${marker}` : marker;
  ...
  function renderProposalsModalContent(sourceDeals, opts = {}) {
    ...
    const pieParts = grouped.filter((g) => nonClosedNames.includes(g.name)).map((g) => ({ name: g.name, value: g.deals.reduce((s,d) => s + proposalAmountNumber(d), 0), color: proposalStageBadgeColor(g.name) }));
    const topOpenTotal = pieParts.reduce((sum, p) => sum + p.value, 0);
    ...
    return `
      ...
      <div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:10px">
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ...
          ${topOpenTotal ? `<span class="eqd-tag" style="padding:2px 6px">${renderProposalPie(pieParts)}</span>` : ``}
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    if (!opts.noPreloadMessage) openModal(title, `<div class="eqd-empty" style="padding:24px">Carregando propostas...</div>`, { full: true, noStack: !!opts.noStack });
    await ensureProposalMeta(false);
