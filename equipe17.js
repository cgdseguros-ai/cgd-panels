...
function makeMarketingCard() {
  return `<div class="userCard" data-action="openMktPanel"><div class="userEmoji" style="order:-1;align-self:center;min-width:22px;text-align:center">🟠</div><div class="userInfoLeft"><div class="userName">MARKETING</div><div class="userTeam">Agenda coletiva</div></div><div class="userPhotoWrap"><img class="userPhoto" src="${MARKETING_LOGO_URL}" alt="Marketing" referrerpolicy="no-referrer" style="object-fit:cover;transform:scale(1.22)" onerror="try{this.onerror=null;this.src='';this.style.display='none'}catch(e){}" /></div><div class="userRight"><div class="userLine">Users: <strong>${MARKETING_MEMBERS.length}</strong></div><div class="userLine">Em implantação</div></div></div>`;
}
...
function openMarketingPanelStub(selectedDateStr = "") {
  ...
  for (let i=0;i<35;i++) {
    ...
    days.push(`<button class="eqd-btn" data-action="mktPickDay" data-day="${key}" style="min-height:66px;border-radius:14px;justify-content:flex-start;padding:8px 8px;background:${key===selectedKey?'#111827':'#fff'};color:${key===selectedKey?'#fff':(inMonth?'#111':'#94a3b8')};border:2px solid ${key===selectedKey?'#111827':borderColor};flex-direction:column;align-items:flex-start;gap:4px;box-shadow:none"><div style="display:flex;width:100%;justify-content:space-between;align-items:center"><span style="font-weight:950">${d.getDate()}</span>${hasGrav?'<span style="font-size:14px">📹</span>':''}</div><div style="display:flex;gap:4px;flex-wrap:wrap">${chips || '<span style="font-size:10px;opacity:.55">—</span>'}</div></button>`);
  }

  openModal("Painel MKT — implantação", `<div style="display:grid;grid-template-columns:minmax(340px,43%) 1fr;gap:12px;min-height:72vh">
    <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:12px">
      ...
      <img src="${MARKETING_LOGO_URL}" style="width:132px;height:132px;object-fit:cover;transform:scale(1.22);border-radius:16px;background:#fff;border:1px solid rgba(0,0,0,.08);padding:0;align-self:flex-start" />
