...
function makeMarketingCard() {
  return `<div class="userCard" data-action="openMktPanel"><div class="userEmoji" style="order:-1;align-self:center;min-width:22px;text-align:center">🟠</div><div class="userInfoLeft"><div class="userName">MARKETING</div><div class="userTeam">Agenda coletiva</div></div><div class="userPhotoWrap"><img class="userPhoto" src="${MARKETING_LOGO_URL}" alt="Marketing" referrerpolicy="no-referrer" style="object-fit:cover;transform:scale(1.12)" onerror="try{this.onerror=null;this.src='';this.style.display='none'}catch(e){}" /></div><div class="userRight"><div class="userLine">Users: <strong>${MARKETING_MEMBERS.length}</strong></div><div class="userLine">Em implantação</div></div></div>`;
}
...
function openMarketingPanelStub(selectedDateStr = "") {
  ...
  const baseDate = selectedDateStr ? (tryParseDateAny(`${selectedDateStr} 00:00`) || new Date()) : new Date();
  ...
  const mktDeals = (STATE.dealsOpen || []).filter((d) => String(d.TITLE || "").trim().startsWith("[MKT]")).sort((a,b) => {
    const ta = a && a._prazo ? new Date(a._prazo).getTime() : 0;
    const tb = b && b._prazo ? new Date(b._prazo).getTime() : 0;
    return ta - tb;
  });
  const selectedKey = selectedDateStr || `${year}-${String(month+1).padStart(2,'0')}-${String(baseDate.getDate()).padStart(2,'0')}`;
  ...
  const days = [];
  for (let i=0;i<startWeekday;i++) days.push('<div></div>');
  for (let day=1; day<=totalDays; day++) {
    const key = `${year}-${String(month+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
    const dayDeals = mktDeals.filter((d) => {
      if (!d._prazo) return false;
      const dt = new Date(d._prazo);
      const dkey = `${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}-${String(dt.getDate()).padStart(2,'0')}`;
      return dkey === key;
    });
    const hasGrav = dayDeals.some((d) => /GRAVAÇÃO/i.test(String(d._tarefaTxt || d.TITLE || "")));
    days.push(`<button class="eqd-btn" data-action="mktPickDay" data-day="${key}" style="min-height:48px;border-radius:12px;justify-content:flex-start;padding:6px 8px;background:${key===selectedKey?'#111827':'#fff'};color:${key===selectedKey?'#fff':'#111'};border:1px solid rgba(0,0,0,.08);flex-direction:column;gap:2px"><span>${day}</span>${hasGrav?'<span style="font-size:14px">📹</span>':''}</button>`);
  }
  ...
  openModal("Painel MKT — implantação", `<div style="display:grid;grid-template-columns:minmax(320px,42%) 1fr;gap:12px;min-height:72vh">
    <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:12px">
      ...
      <img src="${MARKETING_LOGO_URL}" style="width:126px;height:126px;object-fit:cover;transform:scale(1.12);border-radius:16px;background:#fff;border:1px solid rgba(0,0,0,.08);padding:0;align-self:flex-start" />
      <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:6px;font-size:11px;font-weight:900;opacity:.66"><div>SEG</div><div>TER</div><div>QUA</div><div>QUI</div><div>SEX</div><div>SÁB</div><div>DOM</div></div>
      <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:6px">${days.join('')}</div>
    ...
    <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:10px">
      ...
      <div style="font-size:12px;opacity:.78">Dia selecionado: <strong>${escHtml(selectedKey ? fmtDateOnly(tryParseDateAny(selectedKey + ' 00:00') || new Date()) : 'Todos')}</strong></div>
      <div style="display:flex;flex-direction:column;gap:8px;min-height:260px;max-height:58vh;overflow:auto">${filteredDeals.length ? filteredDeals.map((d) => {
      ...
      }).join('') : `<div class="eqd-empty">Nenhuma tarefa MKT nesse dia.</div>`}</div>
    </div>
  ...
  if (createBtn) createBtn.onclick = async () => {
    ...
    const tipoItems = (meta ? getFieldItemsFromMeta(meta, UF_TAREFA) : []).filter((x) => typeTags.some((tt) => norm(tt) === norm(x.text)));
    const now = new Date();
    now.setHours(11,0,0,0);
    const localDefault = new Date(now.getTime() - now.getTimezoneOffset()*60000).toISOString().slice(0,16);
    openModal("Criar tarefa MKT", `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">PARTICIPANTES</div><div style="display:flex;flex-wrap:wrap;gap:8px">${members.map((u)=>`<label style="display:inline-flex;gap:6px;align-items:center;padding:6px 10px;border:1px solid rgba(0,0,0,.08);border-radius:999px"><input class="mktChk" type="checkbox" value="${u.userId}" checked /> ${escHtml(u.name)}</label>`).join('')}</div></div>
    ...
    document.getElementById('mktSave').onclick = async () => {
      ...
      try {
        ...
        if (!prazoIso) throw new Error('Preencha a data e hora.');
        if (!checked.length) throw new Error('Selecione ao menos um participante.');
        if (!await confirmWeekendModal(prazoIso, 'a tarefa MKT')) return;
        const tipoText = (tipoItems.find((x)=>String(x.value)===typeId)||{}).text || '';
