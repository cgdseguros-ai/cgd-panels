...
(function () {
  ...
  function setLeadsCtx(userId, kw, extra){
    LAST_LEADS_CTX.userId = String(userId||"");
    LAST_LEADS_CTX.kw = String(kw||"");
    LAST_LEADS_CTX.dateFilter = String((extra && extra.dateFilter) || LAST_LEADS_CTX.dateFilter || "");
    LAST_LEADS_CTX.operFilter = String((extra && extra.operFilter) || LAST_LEADS_CTX.operFilter || "");
  }
  function reopenLeadsModalSafe() {
    if (!LAST_LEADS_CTX.userId) return;
    openLeadsModalForUser(LAST_LEADS_CTX.userId, LAST_LEADS_CTX.kw || "", { useCache: true, dateFilter: LAST_LEADS_CTX.dateFilter || '', operFilter: LAST_LEADS_CTX.operFilter || '' });
  }
  ...
  function leadDateOnly(lead) {
    const raw = String(leadDataHora(lead) || '').trim();
    if (!raw) return "";
    let m = raw.match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (m) return `${m[1]}-${m[2]}-${m[3]}`;
    m = raw.match(/^(\d{2})\/(\d{2})\/(\d{2,4})/);
    if (m) {
      let yy = Number(m[3]);
      if (yy < 100) yy += 2000;
      return `${yy}-${m[2]}-${m[1]}`;
...
restoreSyncQueue();
  ...
  function firstNameFromText(v) {
    const parts = String((v || "").trim()).split(/\s+/).map((s) => String(s || '').trim()).filter(Boolean);
  ...
  function openNoFollowupBatchModal(user, statusId, statusLabel) {
    const base = new Date();
    base.setHours(11, 0, 0, 0);
    const localDefault = new Date(base.getTime() - base.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
    const all = (STATE.leadsByUser.get(String(user.userId)) || []).filter((l) => String(l.STATUS_ID || '') === String(statusId || '') && !hasFutureFollowupForLeadOrig(leadOrigemId(l) || l.ID));
    let operFilter = '';
    let searchFilter = '';

    openModal(`${statusLabel} sem FOLLOW-UP — ${user.name}`, `
      <div style="display:grid;grid-template-columns:minmax(260px,340px) 1fr auto;gap:10px;align-items:end;margin-bottom:12px">
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">FILTRAR OPERADORA</div>
          <input id="nfOper" class="eqd-searchInput" style="width:100%;color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Ex.: Amil" />
        </div>
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">BUSCAR LEAD</div>
          <input id="nfSearch" class="eqd-searchInput" style="width:100%;color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Nome, telefone, bairro..." />
        </div>
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">PRAZO DO FOLLOW-UP</div>
          <input id="nfPrazo" type="datetime-local" value="${localDefault}" style="width:220px;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" />
        </div>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;justify-content:space-between;margin-bottom:10px">
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          <button class="eqd-btn" id="nfSelAll">Selecionar todos visíveis</button>
          <button class="eqd-btn" id="nfSel10">Selecionar 10 visíveis</button>
          <button class="eqd-btn" id="nfSelNone">Limpar seleção</button>
          <span id="nfInfo" style="font-size:12px;font-weight:900;opacity:.82"></span>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          <button class="eqd-btn eqd-btnPrimary" id="nfCreateBatch">Agendar FOLLOW-UP em lote</button>
          <button class="eqd-btn" data-action="modalClose">Fechar</button>
        </div>
      </div>
      <div id="nfList" style="display:flex;flex-direction:column;gap:10px;max-height:65vh;overflow:auto"></div>
    `, { full: true });

    function filteredRows(){
      const opN = norm(operFilter);
      const qN = norm(searchFilter);
      return all.filter((l) => {
        if (opN && !norm(leadOperadora(l)).includes(opN)) return false;
        if (qN) {
          const blob = norm([leadTitle(l), leadTelefone(l), leadBairro(l), leadFonte(l), leadOperadora(l)].join(' '));
    ...
    function render(){
      ...
      const info = document.getElementById('nfInfo');
      const rows = filteredRows();
      if (info) info.textContent = `Visíveis: ${rows.length} • Total base: ${all.length}`;
      box.innerHTML = rows.length ? rows.map((l) => `
        <label style="display:grid;grid-template-columns:auto 1fr;gap:10px;align-items:flex-start;padding:12px;border-radius:14px;border:1px solid rgba(0,0,0,.08);background:#fff">
          <input type="checkbox" class="nfChk" data-leadid="${escHtml(String(l.ID))}" style="margin-top:4px;width:18px;height:18px" />
          <div>
            <div style="font-size:16px;font-weight:950;color:#111">${escHtml(leadTitle(l))}</div>
            <div class="leadMeta" style="margin-top:6px">
              ${leadOperadora(l) ? `<span>Operadora: <strong>${escHtml(leadOperadora(l))}</strong></span>` : ``}
              ${leadTelefone(l) ? `<span>Telefone: <strong>${escHtml(leadTelefone(l))}</strong></span>` : ``}
              ${leadBairro(l) ? `<span>Bairro: <strong>${escHtml(leadBairro(l))}</strong></span>` : ``}
              ${leadDataHora(l) ? `<span>Data/Hora: <strong>${escHtml(fmt(leadDataHora(l)))}</strong></span>` : ``}
              <span>Origem ID: <strong>${escHtml(leadOrigemId(l) || String(l.ID))}</strong></span>
            </div>
          </div>
        </label>
      `).join('') : `<div class="eqd-empty">Nenhum lead sem FOLLOW-UP com os filtros atuais.</div>`;
    }

    const rerender = () => { operFilter = String((document.getElementById('nfOper')||{}).value || '').trim(); searchFilter = String((document.getElementById('nfSearch')||{}).value || '').trim(); render(); };
    render();
    const nfOper = document.getElementById('nfOper'); if (nfOper) nfOper.oninput = rerender;
    const nfSearch = document.getElementById('nfSearch'); if (nfSearch) nfSearch.oninput = rerender;
    document.getElementById('nfSelAll').onclick = () => document.querySelectorAll('#nfList .nfChk').forEach((el) => { el.checked = true; });
    document.getElementById('nfSel10').onclick = () => { let n = 0; document.querySelectorAll('#nfList .nfChk').forEach((el) => { if (n < 10) { el.checked = true; n++; } }); };
    document.getElementById('nfSelNone').onclick = () => document.querySelectorAll('#nfList .nfChk').forEach((el) => { el.checked = false; });
    document.getElementById('nfCreateBatch').onclick = async () => {
      const ids = Array.from(document.querySelectorAll('#nfList .nfChk:checked')).map((el) => String(el.getAttribute('data-leadid') || ''));
      if (!ids.length) return alert('Selecione pelo menos um lead.');
      const prazoLocal = String((document.getElementById('nfPrazo')||{}).value || '').trim();
      const prazoIso = localInputToIsoWithOffset(prazoLocal);
      if (!prazoIso) return alert('Prazo inválido.');
      const rows = all.filter((l) => ids.includes(String(l.ID)));
      setBusy(`Agendando ${rows.length} FOLLOW-UP(s)…`);
      try {
        for (let i = 0; i < rows.length; i++) {
          const l = rows[i];
          const extraFields = {};
          const origId = leadOrigemId(l) || String(l.ID);
          if (origId) extraFields[DEAL_UF_LEAD_ORIGEM] = origId;
          await createFollowUpDealForUser(user, leadTitle(l), prazoIso, extraFields);
          setBusy(`Agendando ${i + 1}/${rows.length} FOLLOW-UP(s)…`);
        }
        closeModal();
        renderCurrentView();
        reopenLeadsModalSafe();
      } catch (e) {
        alert('Falha ao agendar em lote: ' + (e && (e.message || e) || e));
      } finally { clearBusy(); }
    };
  }
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    await loadLeadStages();
    setLeadsCtx(user.userId, String(kwRaw || ""), { dateFilter: String((opts && opts.dateFilter) || ''), operFilter: String((opts && opts.operFilter) || '') });
    ...
    const okNoFuBtn = document.getElementById("okNoFuBtn"); if (okNoFuBtn) okNoFuBtn.onclick = () => openNoFollowupBatchModal(user, sAtendido, 'ATENDIDOS');
    const qNoFuBtn = document.getElementById("qNoFuBtn"); if (qNoFuBtn) qNoFuBtn.onclick = () => openNoFollowupBatchModal(user, sQual, 'QUALIFICADOS');
  }
  ...
  async function openLeadAtendidoConfirmModal(userId, leadId, toStatus){
    ...
    return await new Promise((resolve) => {
      openModal(`Confirmar ATENDIDO`, `
        ...
        <div style="display:flex;flex-direction:column;gap:12px;min-height:240px">
  ...
  async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    if (target === String(sAtendido) && !(opts && opts.skipAtendidoModal)) {
      ...
      if (!ans) return;
      return moveLeadStage(uid, leadId, target, { skipAtendidoModal: true, forcedHelena: ans.hel, forcedPessoal: ans.pes });
    }
