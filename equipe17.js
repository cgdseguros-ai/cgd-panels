...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function moveProposalStageSafe(dealId, newStageId, extraPatch = {}) {
  ...
  async function openProposalRequiredFieldsModal(deal, nextStageId, returnCtx = {}, patch = {}) {
    ...
    if (btn) btn.onclick = async () => {
      ...
      try {
        ...
        patchProposalDealCaches(String(deal.ID), { ...deal, ...finalPatch, _proposalStageName: proposalStageLabelById(finalPatch.STAGE_ID || deal.STAGE_ID) });
        await safeDealUpdate(String(deal.ID), finalPatch);
        MODAL_STACK = [];
        closeModal();
  ...
  async function openProposalStageMoveModal(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        closeModal();
        openProposalsModal(String(returnCtx.userId || deal.ASSIGNED_BY_ID || ''), { ...returnCtx, noStack: true });
        await safeDealUpdate(String(deal.ID), { STAGE_ID: nextStage });
        return;
      } catch (e) {
        warn.style.display = 'block';
        warn.textContent = 'Falha:\n' + (e.message || e);
  ...
  async function openProposalEditor(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        patchProposalDealCaches(String(deal.ID), { ...patch, _proposalStageName: proposalStageLabelById(nextStageId) });
        closeModal();
        openProposalsModal(String(returnCtx.userId || deal.ASSIGNED_BY_ID || ''), { ...returnCtx, noStack: true });
    ...
    const applyBindings = () => {
      ...
      document.querySelectorAll('.proposal-inline-stage').forEach((sel) => {
        ...
        sel.onchange = async () => {
          ...
          try {
            await moveProposalStageSafe(dId, newStageId);
          } catch (remoteErr) {
            sel.value = String(prev && prev.STAGE_ID || '');
