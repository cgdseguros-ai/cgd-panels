/* eqd.js — GET • CGD CORRETORA (v5.0)
   PATCH REAL V5.2 — 2026-05-08: carregamento estável de tarefas abertas + UX otimista + reagendamento sem rollback após novo card criado
...
(function () {
  ...
  function loadCache() {
    try {
      ...
      if (["open","full","recent","operational","operational_d7"].includes(j.dealsLoadedMode)) STATE.dealsLoadedMode = j.dealsLoadedMode;
  ...
  let OPERATIONAL_WINDOW_KEY = "";
  let OPERATIONAL_WINDOW_PROMISE = null;

  // PATCH V5.2.3 — janela operacional prioritária
  // O GET deve ficar rápido para o uso real: hoje + próximos 7 dias.
  // Dias fora dessa janela são carregados sob demanda quando selecionados, sem puxar todo o histórico/futuro no boot.
  const OPERATIONAL_FORWARD_DAYS = 7;
  ...
  function getOperationalWindowBounds() {
    const today = dayStart(new Date());
    const end7 = dayEnd(addDaysLocal(today, OPERATIONAL_FORWARD_DAYS));
    const selStart = dayStart(selectedDate);
    const selEnd = dayEnd(selectedDate);
    const selectedInsidePriority = selStart.getTime() >= today.getTime() && selStart.getTime() <= end7.getTime();
    return { today, end7, selStart, selEnd, selectedInsidePriority };
  }

  function selectedOperationalWindowKey() {
    try {
      const b = getOperationalWindowBounds();
      const selectedPart = b.selectedInsidePriority ? "D+7" : ymdKey(selectedDate);
      return `${ymdKey(b.today)}:${ymdKey(addDaysLocal(b.today, OPERATIONAL_FORWARD_DAYS))}:${selectedPart}`;
    } catch (_) { return fmtDateOnly(selectedDate); }
  }

  async function ensureOperationalWindowForSelectedDate(opts = {}) {
    const force = !!(opts && opts.force);
    const silent = !!(opts && opts.silent);
    const key = selectedOperationalWindowKey();
    if (!force && OPERATIONAL_WINDOW_KEY === key && Array.isArray(STATE.dealsAll)) return true;
    if (OPERATIONAL_WINDOW_PROMISE) return OPERATIONAL_WINDOW_PROMISE;
    OPERATIONAL_WINDOW_PROMISE = (async () => {
      try {
        if (!silent) setSoftStatus(`Carregando tarefas de hoje até D+7...`);
        await loadDealsOperationalWindow({ deferPhotos: true });
        try { applyLocalDealPatches(); } catch(_) {}
        try { overlayPendingSyncState(); } catch(_) {}
        try { rebuildDealsOpen(); } catch(_) {}
        OPERATIONAL_WINDOW_KEY = key;
        if (el && el.modalOverlay && el.modalOverlay.style.display !== "flex") renderCurrentView(true);
        if (!silent) setSoftStatus("Tarefas carregadas");
        return true;
      } catch (e) {
        showRuntimeWarn(e);
        return false;
      } finally {
        OPERATIONAL_WINDOW_PROMISE = null;
      }
    })();
    return OPERATIONAL_WINDOW_PROMISE;
  }

  function setSelectedDateAndLoad(d, opts = {}) {
    if (!(d instanceof Date) || Number.isNaN(d.getTime())) return;
    selectedDate = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 0, 0, 0, 0);
    calendarCursor = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), 1);
    renderCurrentView(true);
    ensureOperationalWindowForSelectedDate({ force: true, silent: !!(opts && opts.silent) });
  }
  ...
  function ensureFullDealsLoadedInBackground(delayMs = 0) {
    ...
    FULL_DEALS_BG_PROMISE = new Promise((resolve) => {
      setTimeout(async () => {
        try {
          // Não carregar todos os deals no background imediato: isso competia com os cards operacionais.
          // Mantém apenas uma carga complementar leve de histórico antigo.
          await bootTimerMeasure("Background: deals antigos", () => loadDeals({ forceFull: true, openOnly: false, deferPhotos: true, oldOnly: true, olderThanDays: 30, append: true }).catch(()=>{}), "Carga complementar leve, sem travar a agenda D+7");
          STATE.dealsLoadedMode = "operational_d7";
          saveCache();
  ...
  // PATCH V5.2.3 — carga operacional prioritária D+7
  // Carrega rápido o que a empresa usa de verdade: atrasadas + hoje até 7 dias à frente.
  // Se a user selecionar um dia fora desse intervalo, carrega também aquele dia específico sob demanda.
  // O restante histórico/futuro distante fica fora do caminho crítico para não travar o GET.
  async function loadDealsOperationalWindow(opts = {}) {
    const deferPhotos = !!(opts && opts.deferPhotos);
    const [urgMap, tarefaMap, etapaMap] = await Promise.all([
      enums(UF_URGENCIA), enums(UF_TAREFA), enums(UF_ETAPA),
    ]);
    ...
    const { today, end7, selStart, selEnd, selectedInsidePriority } = getOperationalWindowBounds();
    ...
    // 1) Prioritário: tudo aberto vencido + hoje até D+7.
    // Sem limite inferior para não sumirem atrasadas antigas.
    const openPriorityFilter = { CATEGORY_ID: CATEGORY_MAIN };
    if (STATE.doneStageId) openPriorityFilter["!STAGE_ID"] = String(STATE.doneStageId);
    openPriorityFilter["<=" + UF_PRAZO] = end7Iso;
    requests.push(bxAll("crm.deal.list", { filter: openPriorityFilter, select, order: { ID: "DESC" } }));

    // 2) Concluídas da janela D+7 para manter a métrica ao navegar nos próximos dias.
    if (STATE.doneStageId) {
      requests.push(bxAll("crm.deal.list", {
        filter: { CATEGORY_ID: CATEGORY_MAIN, STAGE_ID: String(STATE.doneStageId), [">=" + UF_PRAZO]: todayIso, ["<=" + UF_PRAZO]: end7Iso },
        select,
        order: { ID: "DESC" },
      }).catch(() => []));
    }

    // 3) Se o dia selecionado estiver fora de hoje..D+7, carrega apenas aquele dia sob demanda.
    // Assim agenda antiga ou muito futura abre sem depender de carga completa.
    if (!selectedInsidePriority) {
      const openSelectedFilter = { CATEGORY_ID: CATEGORY_MAIN };
      if (STATE.doneStageId) openSelectedFilter["!STAGE_ID"] = String(STATE.doneStageId);
      openSelectedFilter[">=" + UF_PRAZO] = selStartIso;
      openSelectedFilter["<=" + UF_PRAZO] = selEndIso;
      requests.push(bxAll("crm.deal.list", { filter: openSelectedFilter, select, order: { ID: "DESC" } }).catch(() => []));

      if (STATE.doneStageId) {
        requests.push(bxAll("crm.deal.list", {
          filter: { CATEGORY_ID: CATEGORY_MAIN, STAGE_ID: String(STATE.doneStageId), [">=" + UF_PRAZO]: selStartIso, ["<=" + UF_PRAZO]: selEndIso },
          select,
          order: { ID: "DESC" },
        }).catch(() => []));
      }
    ...
    const parsed = chunks.flat().map((d) => parseDeal(d, maps)).filter(Boolean);
    STATE.dealsAll = dedupeDealsById(parsed);
    STATE.dealsOpen = STATE.dealsAll.filter((d) => !STATE.doneStageId || String(d.STAGE_ID) !== String(STATE.doneStageId));
    STATE.dealsLoadedMode = "operational_d7";
    try { OPERATIONAL_WINDOW_KEY = selectedOperationalWindowKey(); } catch(_) {}
    STATE.lastDealsSyncAt = new Date().toISOString();
    STATE.lastOkAt = new Date();
    STATE.offline = false;
    saveCache();
...
${curObs}` : transferAlert;
  ...
  function openModal(title, bodyHTML, opts) {
    ...
    const allowStack = !!(opts && opts.allowStack);
...
restoreSyncQueue();
    ...
    try {
      ...
      for (const tx of arr) {
        ...
        if (tx.status === 'newCreated_pendingOldDone' && tx.oldDealId && tx.newRealId) {
          const recoverDoneStageId = String(STATE.doneStageId || tx.doneStageId || '');
          if (recoverDoneStageId) {
            updateDealInState(String(tx.oldDealId), { STAGE_ID: recoverDoneStageId, _hiddenByResched: true, DATE_MODIFY: new Date().toISOString(), __skipPending: true });
            registerLocalDealPatch(String(tx.oldDealId), { STAGE_ID: recoverDoneStageId, _hiddenByResched: true }, 180000);
          }
          enqueueSync({
            type: 'dealUpdate',
            dealId: String(tx.oldDealId),
            fields: { STAGE_ID: recoverDoneStageId },
  ...
  function leadStageLabelChipHtml(lead) {
    const nm = String(leadStageNameById(lead && lead.STATUS_ID) || '').toUpperCase();
    if (nm === 'ATENDIDO') return ` <span class="eqd-tag" style="background:#ffedd5;color:#9a3412">ATENDIDO</span>`;
    if (nm === 'INTERAGIDO') return ` <span class="eqd-tag" style="background:#dcfce7;color:#166534">INTERAGIDO</span>`;
    if (nm === 'QUALIFICADO') return ` <span class="eqd-tag" style="background:#dbeafe;color:#1d4ed8">QUALIFICADO</span>`;
    return '';
  }
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    function cardLead(l, column) {
      ...
      if (column === "AT") {
        ...
        btns = `${mkBtn("PERDIDO",sPerdido)}${mkBtn("CONVERTER",sConv)}
                <button class="leadBtn" data-action="leadEdit" data-leadid="${l.ID}" data-userid="${user.userId}">EDITAR</button>
                <button class="leadBtn" data-action="leadTransferModal" data-leadid="${l.ID}" data-userid="${user.userId}">TRANSFERIR LEAD</button>
                <button class="leadBtn leadBtnD" data-action="leadDelete" data-leadid="${l.ID}" data-userid="${user.userId}">EXCLUIR</button>
...
function makeUserCard(u) {
  ...
  function renderUserPanel(userId) {
    ...
    if (!isSpecial) {
      if (STATE.dealsLoadedMode !== "full") ensureOperationalWindowForSelectedDate({ silent: true });
  ...
  async function loadMetricsCore(opts = {}) {
    ...
    // PATCH V5.2.1 — carga híbrida: não trava o boot carregando TODAS as tarefas abertas.
    // Carrega rapidamente o necessário para o painel geral: abertas vencidas/até o dia selecionado
    // + concluídas do dia. Futuras/histórico completo entram depois, em background.
    await bootTimerMeasure("Métricas: janela D+7", () => loadDealsOperationalWindow({ deferPhotos: true }), "Cards operacionais: vencidas + hoje até 7 dias à frente; dias fora da janela carregam sob demanda");
  ...
  function schedulePostReadyBackgroundDataLoads() {
    try {
      setTimeout(async () => {
        try {
          await bootTimerMeasure("Background: deals/tarefas antigos", () => loadDeals({ forceFull: true, openOnly: false, deferPhotos: true, oldOnly: true, olderThanDays: 30, append: true }).catch(()=>{}), "Histórico antigo depois de 3 min; não compete com hoje+D7");
          try { applyLocalDealPatches(); } catch(_) {}
          try { overlayPendingSyncState(); } catch(_) {}
          try { rebuildDealsOpen(); } catch(_) {}
          // Não re-renderiza o painel geral após histórico pesado para evitar salto visual de contagem.
        } catch(_) {}
        try {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      // Mesmo em atualização manual, prioriza a janela operacional D+7.
      // Carga completa fica fora do caminho crítico para não travar os deals/cards.
      await loadDealsOperationalWindow({ deferPhotos: true });
  ...
  async function rescheduleDealAtomic(oldDealId, newFields, originalDeal) {
    ...
    try {
      ...
      // PATCH V5.2 — regra anti-desfazer:
      // 1) Se o NOVO deal ainda não foi criado, o rollback local é seguro.
      // 2) Se o NOVO deal JÁ foi criado, não ressuscitar o antigo visualmente.
      //    Mantemos o antigo oculto/concluído localmente e deixamos uma transação pendente
      //    para concluir o antigo em segundo plano. Isso evita o efeito “reagendamento desfeito”.
      removeDealLocal(tempId);
      if (realId) {
        updateDealInState(oldId, { STAGE_ID: doneStageId, _hiddenByResched: true, DATE_MODIFY: new Date().toISOString(), __skipPending: true });
        registerLocalDealPatch(String(oldId), { STAGE_ID: doneStageId, _hiddenByResched: true }, 180000);
        pendingMutRegister("deal", oldId, `reschedDoneRetry_${Date.now()}`, { STAGE_ID: doneStageId, _hiddenByResched: true }, 180000, { source: "rescheduleRetry" });
        registerLocalDealPatch(String(realId), { ...newFields, _pendingNewDeal: false, _rescheduledFrom: oldId }, 180000);
        ...
        enqueueSync({
          ...
          reason: "rescheduleOldDoneRetry",
          txId,
          maxAttempts: 6
        }, { delayMs: 900, maxAttempts: 6 });
        showDiscreetToast("Reagendamento criado. O card antigo será concluído pelo SYNC em segundo plano.");
      } else {
        updateDealInState(oldId, { STAGE_ID: oldStageId, _hiddenByResched: false, DATE_MODIFY: new Date().toISOString(), __skipPending: true });
        removeRescheduleTx(txId);
      }
      UI_REFRESH_HOLD_UNTIL = Math.max(UI_REFRESH_HOLD_UNTIL, Date.now() + (realId ? 120000 : 30000));
      rebuildDealsOpen();
      saveCache();
      renderCurrentView(true);
...
${marker}` : marker;
  ...
  function calcLeadFunnelMetrics(userId, opts = {}) {
    ...
    const byUser = new Map();
    let passed=0, qual=0, conv=0, lost=0;
    ...
    for (const l of leads) {
      ...
      if (!byUser.has(owner)) byUser.set(owner, { userId: owner, passed:0, qualified:0, converted:0, lost:0 });
    ...
    return { totalPassedAtendido: passed, totalQualified: qual, totalConverted: conv, totalLost: lost, conversionRate: passed ? conv/passed : 0, qualificationRate: passed ? qual/passed : 0, lossRate: passed ? lost/passed : 0, avgTimeAtendidoToConverted: avg(convDur), avgTimeAtendidoToLost: avg(lostDur), userRows };
  }
...
document.addEventListener("click", async (e) => {
  ...
  // 25) TOPBAR ACTIONS
  // =========================
  el.today.onclick = () => setSelectedDateAndLoad(new Date());
  el.tomorrow.onclick = () => setSelectedDateAndLoad(new Date(Date.now() + 86400000));
  if (el.prevday) el.prevday.onclick = () => setSelectedDateAndLoad(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate()-1));
  if (el.nextday) el.nextday.onclick = () => setSelectedDateAndLoad(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate()+1));
  ...
  el.refresh.onclick = async () => {
    ...
    await refreshData(true, { forceRecur: true, forceFullDeals: false });
    renderCurrentView(true);
  };
