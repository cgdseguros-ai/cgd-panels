...
(function () {
  ...
  function openFollowupListModalForUser(user) {
    ...
    const render = (kwRaw) => {
      ...
      box.innerHTML = ordered.length
        ? ordered.map((d) => `
            <div>
              <div style="font-size:11px;font-weight:900;opacity:.72;margin:0 0 6px 2px">Criado em: <strong>${d.DATE_CREATE ? fmt(d.DATE_CREATE) : "—"}</strong></div>
              ${makeDealCard(d, { allowBatch: false })}
            </div>
          `).join("")
        : `<div class="eqd-empty">Nenhum follow-up.</div>`;
    };
  ...
  async function openGlobalLeadsModal(filterUserId = "__ALL__") {
    ...
    if (filterUserId !== "__ALL__") {
      allLeads = allLeads.filter((l) => String(l._ownerUserId) === String(filterUserId));
    }

    allLeads.sort((a, b) => {
      const ta = new Date(a.DATE_CREATE || 0).getTime();
      const tb = new Date(b.DATE_CREATE || 0).getTime();
      return tb - ta;
    });
    ...
    const userOptions =
      `<option value="__ALL__">Todas as vendedoras</option>` +
      targetUsers.map((u) => `<option value="${u.userId}" ${String(filterUserId) === String(u.userId) ? "selected" : ""}>${escHtml(u.name)}</option>`).join("");

    const card = (l) => {
      const owner = USERS.find((u) => String(u.userId) === String(l._ownerUserId));
      return `
        <div class="leadCard">
          <div class="leadTitle">${escHtml(leadTitle(l))}</div>
          <div class="leadMeta">
            <span>USER: <strong>${escHtml(owner ? owner.name : l._ownerUserId)}</strong></span>
            <span>Criado em: <strong>${l.DATE_CREATE ? fmt(l.DATE_CREATE) : "—"}</strong></span>
    ...
    openModal("LEADS GERAL", `
      <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:space-between;align-items:center">
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <select id="glUser" class="eqd-searchSelect" style="color:#111;background:#fff;border-color:rgba(0,0,0,.18)">
            ${userOptions}
          </select>
          <button class="eqd-btn eqd-btnPrimary" id="glFilterBtn">Filtrar</button>
        </div>

        <div style="font-size:12px;font-weight:950;opacity:.82">
          EM ATENDIMENTO: <strong>${at.length}</strong> •
          ATENDIDO: <strong>${ok.length}</strong> •
          QUALIFICADO: <strong>${q.length}</strong>
        </div>
      </div>

      <div style="display:grid;grid-template-columns:repeat(3,minmax(380px,1fr));gap:12px;margin-top:10px">
        <div class="panelCol">
          <div class="panelColHead">EM ATENDIMENTO (${at.length})</div>
          <div class="panelColBody">${at.length ? at.map(card).join("") : `<div class="eqd-empty">Nenhum</div>`}</div>
        </div>

        <div class="panelCol">
          <div class="panelColHead">ATENDIDO (${ok.length})</div>
          <div class="panelColBody">${ok.length ? ok.map(card).join("") : `<div class="eqd-empty">Nenhum</div>`}</div>
        </div>

        <div class="panelCol">
          <div class="panelColHead">QUALIFICADO (${q.length})</div>
          <div class="panelColBody">${q.length ? q.map(card).join("") : `<div class="eqd-empty">Nenhum</div>`}</div>
        </div>
      </div>
    `, { full: true });

    document.getElementById("glFilterBtn").onclick = () => {
      const uid = String(document.getElementById("glUser").value || "__ALL__");
      openGlobalLeadsModal(uid);
    };
  }
  ...
  async function deleteLead(userId, leadId){
    ...
    setBusy("Excluindo lead…");

    // ✅ SALVAR AQUI
    await bx("crm.lead.delete", { id: String(leadId) });
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        await bx("crm.lead.update", {
          id: String(leadId),
          fields: { ASSIGNED_BY_ID: Number(targetUserId) }
        });

        await loadLeadsForOneUser(fromUserId);
        await loadLeadsForOneUser(targetUserId);
  ...
  async function openTransferDealUserModal(dealId){
    ...
    btn.onclick = async () => {
      try {
        ...
        setBusy("Transferindo negócio…");

        // ✅ SALVAR AQUI
        await bx("crm.deal.update", {
          id: String(dealId),
          fields: {
            ASSIGNED_BY_ID: Number(newUserId),
            STAGE_ID: String(newStageId)
          }
        });
  ...
  el.refresh.onclick = async () => {
    await refreshData(true, { forceRecur: true });
    renderCurrentView();
  };
  ...
  // =========================
  (async function init(){
    await refreshData(false, { forceRecur: true });
    renderFooterPeople();
    renderGeneral();
    setSoftStatus("JS: ok");
  })();
  ...
  setInterval(async () => {
    try{
      await refreshData(true, { forceRecur: false });
