/* eqd.js — GET • CGD CORRETORA (v5.0)
   PATCH REAL V5.1 — 2026-05-03: SYNC no canto superior direito + correção atômica do reagendamento pipeline 17
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          for (let idx = 0; idx < ids.length; idx++) {
            ...
            try {
              ...
              pendingMutRegister("deal", id, `batchReschedPrazo_${Date.now()}_${idx}`, { [UF_PRAZO]: prazoIso }, 180000, { source: "batchReschedulePrazoOnly" });
              ...
              enqueueSync({ type: "dealUpdate", dealId: String(id), fields: { [UF_PRAZO]: prazoIso } }, { maxAttempts: 5 });
              successes.push(id);
            } catch (e) {
  ...
  async function confirmRemoteEntityFields(entityType, id, expectedFields, opts = {}) {
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      ...
      try {
        ...
        if (ok) {
          // PATCH 2026-05-03: limpeza por confirmação remota, não apenas por tempo.
          if (type === "deal") clearLocalDealPatch(String(id));
          try {
      ...
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          try {
            ...
            saveCache();
            renderCurrentView(true);
            setTimeout(() => { refreshData(true, { deferLeads:false }).catch(()=>{}); }, 250);
          } catch (err) {
