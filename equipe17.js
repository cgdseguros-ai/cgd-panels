...
${curObs}` : transferAlert;
  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal)) {
      ...
      tags.push(`<span class="eqd-tag" title="Conta FUPs concluídos pelo lead vinculado ou, em FUPs antigos, por título/USER">FUPs concluídos: ${doneCount}</span>`);
...
restoreSyncQueue();
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    openModal(`Leads — ${user.name}`, `
      <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:space-between;align-items:center">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;flex:1;justify-content:flex-end">
          ...
          <button class="eqd-btn" data-action="modalClose">Fechar</button>
          <button class="eqd-btn eqd-btnWaitingLead ${waitingNovoCount ? "hasWaiting" : ""}" style="margin-left:auto" data-action="leadOpenWaitingNew" data-userid="${user.userId}">NOVOS LEADS (Aguardando)${waitingNovoCount ? ` • ${waitingNovoCount}` : ``}</button>
        </div>
      </div>
    ...
    function cardLead(l, column) {
      ...
      return `
        <div class="leadCard" data-lead-col="${escHtml(column)}" data-fonte="${escHtml(norm(fonte))}" data-operadora="${escHtml(norm(op))}" data-proximo-vencer="${isLeadProximoVencer(l) ? '1' : '0'}" data-lead-dia="${isLeadAtendidoNoDia(l) ? '1' : '0'}">
