...
${curObs}` : transferAlert;
  ...
  function makeDealCard(deal, context) {
    ...
    if (!isFollowupDeal(deal)) {
      ...
      if (proposalMeta) {
        const wanted = [
          ['TIPO DA TAREFA', deal._tarefaTxt || ''],
...
${marker}` : marker;
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    openModal(title, renderProposalsModalContent(sourceDeals, LAST_PROPOSALS_CTX), { full: true, noStack: !!opts.noStack });
    const applyBindings = () => {
      ...
      document.querySelectorAll('.proposal-inline-stage').forEach((sel) => {
        sel.onchange = async () => {
          ...
          const reqErr = validateProposalRequiredFields(nextStage, { STAGE_ID: nextStage }, prev);
          if (reqErr) {
            const warn = document.getElementById('proposalWarn');
            if (warn) { warn.style.display = 'block'; warn.textContent = reqErr; }
