...
${curObs}` : transferAlert;
  ...
  function attachCalendarHandlers(host, onApply) {
    ...
    host.addEventListener("click", (e) => {
      if (e.target && e.target.id === "user1AdminToggle") { toggleUser1AdminPanel(); return; }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const allowedPairs = proposalStageOrder().map((nm) => ({
      name: nm,
      norm: norm(nm),
      id: String(STATE.proposalStageIdByName.get(norm(nm)) || '').trim()
    }));
    const allowedStageIds = new Set(allowedPairs.map((x) => x.id).filter(Boolean));
    ...
    // 1) Busca precisa por stage permitido na pipeline 0
    const preciseStageIds = [...allowedStageIds];
    for (const sid of preciseStageIds) {
      const rows = await bxAll('crm.deal.list', {
        filter: { ...assigneeFilter, CATEGORY_ID: Number(CATEGORY_PROPOSALS), STAGE_ID: sid },
        order: { ID: 'DESC' },
        select,
      }).catch(() => []);
      pushRows(rows);
    ...
    // 2) Fallback: pipeline 0 inteira do user
    if (!rowsById.size) {
      const rows = await bxAll('crm.deal.list', {
        filter: { ...assigneeFilter, CATEGORY_ID: Number(CATEGORY_PROPOSALS) },
        order: { ID: 'DESC' },
        select,
      }).catch(() => []);
      pushRows(rows);
    ...
    // 3) Fallback mais amplo: todos os deals do user e filtragem local por stage/categoria
    if (!rowsById.size) {
      const rows = await bxAll('crm.deal.list', {
    ...
    const mapped = [...rowsById.values()]
      .filter((d) => {
        ...
        const stageNorm = norm(stageName);
        const allowedByName = allowedPairs.some((x) => stageNorm === x.norm || stageNorm.includes(x.norm) || x.norm.includes(stageNorm));
        return (categoryLooksDefault || allowedStageIds.has(stageId)) && (allowedStageIds.has(stageId) || allowedByName || proposalAllowedStage(stageName));
      })
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    let grouped = proposalStageOrder().map((nm) => ({ name: nm, deals: filteredDeals.filter((d) => proposalStageNameFromId(d.STAGE_ID) === nm) }));
