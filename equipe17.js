...
${marker}` : marker;
  ...
  function renderRecurrenceAuditResults(payload = {}) {
    ...
    const daysAhead = Number(opts.daysAhead || 90) || 90;
  ...
  async function openRecurrenceAuditModal(opts = {}) {
    ...
    const ctx = { includePast: !!opts.includePast, includeDone: !!opts.includeDone, daysAhead: Math.max(7, Number(opts.daysAhead || 30) || 30), userScopeId: requestedScopeId };
    const report = createRecurrenceAuditReport(mode, ctx);
    openModal("AUDITORIA DE RECORRÊNCIAS", `<div id="recAuditMonitorBox">${renderRecurrenceAuditMonitor(report)}</div><div class="eqd-empty" style="padding:18px">Preparando auditoria...</div>`, { full: true, noStack: true });
    ...
    try {
      ...
      return openModal("AUDITORIA DE RECORRÊNCIAS", `${renderRecurrenceAuditMonitor(report)}<div class="eqd-warn">Falha ao executar auditoria: ${escHtml(e && e.message ? e.message : String(e))}</div>`, { full: true, noStack: true });
    } finally {
      clearInterval(tick);
    }
    openModal("AUDITORIA DE RECORRÊNCIAS", renderRecurrenceAuditResults({ opts: ctx, duplicateGroups, legacyRows, auditReport: report }), { full: true, noStack: true });
    const readForm = () => ({
      ...
      userScopeId: String((document.getElementById("recAuditScopeUserId") || {}).value || ""),
    });
