/* eqd.js — GET • CGD CORRETORA (v5.0)
   PATCH REAL V5.3 — 2026-05-13: boot essencial rápido + agenda VISITAS coletiva
...
(function () {
  ...
  async function loadDeals(opts = {}) {
    ...
    const nowMs = Date.now();
    const specialWindowLoad = recentOnly || oldOnly || essentialOnly;
    ...
    const baseFilter = () => {
      ...
      if (essentialOnly) {
        // PATCH V5.3: boot essencial = tarefas/FUPs em aberto, atrasados e até 5 dias à frente.
    ...
    if (openOnly) {
      ...
      STATE.dealsLoadedMode = essentialOnly ? "essential" : (recentOnly ? "recent" : (oldOnly ? "full" : "full"));
      if (!recentOnly || oldOnly) STATE.lastFullDealsSyncAt = nowMs;
    } else {
      const deals = await bxAll("crm.deal.list", {
...
${RECURRENCE_PAYLOAD_PREFIX}${enc}` : JSON.stringify(payload);
  ...
  async function loadRecurrencesBeforeReady(opts = {}) {
    ...
    if (opts.forceRecur || !STATE.recurRulesByUser.size || (now - LAST_RECUR_LOAD_AT > 10 * 60 * 1000)) {
      ...
      await bootTimerMeasure("Boot essencial: recorrências", () => loadRecurrenceConfigDeals().catch(()=>{}), "Regras de recorrência antes de liberar o painel");
      LAST_RECUR_LOAD_AT = Date.now();
    }
    if (opts.forceRecur || (Date.now() - LAST_RECUR_GEN_AT > 5 * 60 * 1000)) {
      progress(87, "Gerando janela de recorrências...");
      await bootTimerMeasure("Boot essencial: gerar recorrências", () => generateRecurringDealsWindow().catch(()=>{}), "Geração das tarefas recorrentes antes de liberar o painel");
  ...
  async function loadPhotosBeforeReady(opts = {}) {
    ...
    await bootTimerMeasure("Boot essencial: fotos/fallback", () => loadUserPhotosWithFallback().catch(()=>{}), "Fotos são a última etapa antes de liberar o painel");
    try {
      if (!STATE.footerPhotosLoaded) {
        await Promise.allSettled(FOOTER_PARTNERS.map((p) => ensureUserPhoto(p.userId, "")));
        STATE.footerPhotosLoaded = true;
      }
    } catch (_) {}
...
function makeUserCard(u) {
  ...
  async function loadMetricsCore(opts = {}) {
    ...
    await bootTimerMeasure("Boot essencial: tarefas/FUPs até 5 dias", () => loadDeals({ forceFull: true, openOnly: false, deferPhotos: true, essentialOnly: true, daysAhead: 5 }), "Libera o painel com atrasadas, hoje e até 5 dias à frente; o restante carrega depois");
  ...
  function schedulePostReadyBackgroundDataLoads() {
    try {
      setTimeout(async () => {
        ...
        try {
          setBackgroundLoadStatus("dados completos", 2, total, false);
          await bootTimerMeasure("Background: deals/tarefas completos", () => loadDeals({ forceFull: true, openOnly: false, deferPhotos: true }).catch(()=>{}), "Histórico, concluídos/perdidos e futuros além de 5 dias; não bloqueia READY");
          try { applyLocalDealPatches(); } catch(_) {}
          try { overlayPendingSyncState(); } catch(_) {}
          try { rebuildDealsOpen(); } catch(_) {}
        ...
        try {
          setBackgroundLoadStatus("leads inativos", 3, total, false);
  ...
  try {
    ...
    setInterval(async () => {
      try{
        ...
        try {
          await refreshData(true, { forceRecur: false, forceFullDeals: false, deferLeads: true, statusSilent: true });
