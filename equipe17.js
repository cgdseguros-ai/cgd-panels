...
${curObs}` : transferAlert;
  ...
  function makeDealCard(deal, context) {
    ...
    if (!isFollowupDeal(deal)) {
      ...
      if (proposalMeta) {
        const tipoTask = deal._tarefaTxt || taskTypeLabelFromValue(deal[UF_TAREFA] || deal._tarefaId || '') || proposalMeta.tipoTarefa || '';
        const tagsMeta = [
  ...
  async function openRecurrenceBatchModalForUser(user) {
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        openRecurrenceManager(String(user.userId));
  ...
  function openNewTaskModalForUser(user, opts) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (proposalTaskSimple && !tipoVal) throw new Error("Selecione o TIPO DA TAREFA.");
        ...
        if (recType === "NONE") {
          ...
          if (proposalMeta) proposalMeta.tipoTarefa = taskTypeLabelFromValue(tipoVal || '') || proposalMeta.tipoTarefa || '';
