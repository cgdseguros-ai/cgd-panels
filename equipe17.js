...
${curObs}` : transferAlert;
  ...
  function makeDealCard(deal, context) {
    ...
    if (!isFollowupDeal(deal)) {
      ...
      if (proposalMeta) {
        const wanted = [
          ['TIPO DA TAREFA', deal._tarefaTxt || proposalMeta.tipoTarefa || ''],
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        for (const line of lines) {
          ...
          const mt = timeVal.match(/^(\d{2}):(\d{2})$/);
          if (!mt) throw new Error('Horário inválido em uma das linhas.');
          const recType = String((line.querySelector('.rlType') || {}).value || 'DAILY_BUSINESS');
          const rule = {
            id: makeRuleId(), title, type: recType, hh: Number(mt[1]), mm: Number(mt[2]),
            ...
            urg: String((line.querySelector('.rlUrg') || {}).value || '').trim(),
            colab: String((line.querySelector('.rlColab') || {}).value || '').trim(),
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (recType === "NONE") {
          ...
          if (proposalMeta && tipoVal) {
            const items = getFieldItemsFromMeta(STATE.dealFieldsMeta || {}, UF_TAREFA);
            const item = (items || []).find((it) => String(it.ID) === String(tipoVal));
            proposalMeta.tipoTarefa = String(item && (item.VALUE ?? item.NAME ?? item.ID) || '');
          }
