  ...
  function endBootPending(label) {
    try {
      ...
      if (BOOT_PROGRESS_PENDING_TASKS <= 0) {
        setBootProgress(99, label || 'Finalizando carregamento...');
      } else {
        setBootProgress(Math.min(99, Math.max(90, 99 - BOOT_PROGRESS_PENDING_TASKS)), label || 'Finalizando carregamento...');
      }
    } catch (_) {}
  }
    ...
    try {
      ...
      if (!BOOT_PROGRESS_TIMER) {
        BOOT_PROGRESS_TIMER = setInterval(() => {
          ...
          if (el && el.meta) el.meta.textContent = `${BOOT_PROGRESS_LABEL || "Carregando painel..."}`;
  ...
  async function openRecurrenceBatchModalForUser(user) {
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        Array.from(rowsBox.querySelectorAll('.recurLine')).forEach((line) => line.remove());
        await addRow();
        setTimeout(async () => {
          ...
          try { await loadRecurrenceConfigForUser(String(user.userId)).catch(()=>{}); } catch (_) {}
...
function makeUserCard(u) {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    const showBootProgress = !INITIAL_BOOT_DONE;
    const progress = (pct, label) => { if (showBootProgress) setBootProgress(pct, label); };
    progress(5, "Preparando dados...");
    ...
    try {
      ...
      if (opts.deferRecur) {
        ...
        setTimeout(async () => {
          try {
            ...
            if (opts.forceRecur || !STATE.recurRulesByUser.size || (later - LAST_RECUR_LOAD_AT > 10 * 60 * 1000)) {
              ...
              progress(72, "Regras de recorrência carregadas...");
        ...
        if (opts.forceRecur || (now - LAST_RECUR_GEN_AT > 5 * 60 * 1000)) {
          await generateRecurringDealsWindow().catch(()=>{});
          progress(80, "Instâncias de recorrência geradas...");
      ...
      if (opts.deferLeads) {
        ...
        if (shouldRefreshLeads) {
          try { progress(86, "Carregando resumos e leads..."); await loadLeadsSnapshotAllUsers({ force: !!forceNetwork }); LAST_LEADS_REFRESH_AT = Date.now(); } catch (_) {}
        }
        overlayPendingSyncState();
      }
      renderFooterPeople();
      DEALS_FAIL_STREAK = 0;
      if (BOOT_PROGRESS_PENDING_TASKS > 0) {
        progress(94, 'Concluindo carregamentos em segundo plano...');
      } else {
        progress(100, 'Painel carregado');
        setTimeout(() => {
          try {
            if (BOOT_PROGRESS_PENDING_TASKS > 0) return;
            clearBootProgress();
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    let rows = [];
    try { await ensureProposalMeta(false); } catch (_) {}
    try {
      rows = await bxAll('crm.deal.list', {
        filter: { ...baseFilter, CATEGORY_ID: Number(CATEGORY_PROPOSALS) },
        order: { ID: 'DESC' },
        select,
      }).catch(() => []);
    } catch (_) { rows = []; }
    ...
    (rows || []).forEach((d) => {
      ...
      if (uid && String(d.ASSIGNED_BY_ID || '') !== uid) return;
      const mappedName = proposalStageNameFromId(d.STAGE_ID);
      const stageName = inferProposalStageNameFromAny(mappedName) || inferProposalStageNameFromAny(d.STAGE_ID) || String(mappedName || d.STAGE_ID || '');
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "recurManager") {
        ...
        if (STATE.recurManagerShownForUser === uid && (nowTs - Number(STATE.recurManagerShownAt || 0)) < 1200) return;
        STATE.recurModalOpeningForUser = uid;
        openModal('RECORRÊNCIA', `<div style="padding:18px;font-weight:900">Carregando recorrências...</div>`, { wide: true, noStack: true });
        try {
          await loadRecurrenceConfigDeals().catch(()=>{});
