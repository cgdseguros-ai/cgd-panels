...
(function () {
  ...
  async function loadLeadsSnapshotAllUsers(opts = {}) {
    await loadLeadStages();
    const now = Date.now();
    if (!opts.force && (now - LAST_ALL_LEADS_SNAPSHOT_AT < 12 * 1000) && STATE.leadsByUser && STATE.leadsByUser.size) return STATE.leadsByUser;
...
${curObs}` : transferAlert;
  ...
  function openModal(title, bodyHTML, opts) {
    ...
    if (el.modalOverlay.style.display === "flex" && !(opts && opts.noStack) && !sameCurrent) {
      MODAL_STACK.push({
...
restoreSyncQueue();
  ...
  function openSelectRecentOldModal(n, onPick) {
    const host = el && el.modalBody ? el.modalBody : document.body;
    if (!host) return;
    try { host.querySelector('#fuPickOverlayInline')?.remove(); } catch(_) {}
    const wrap = document.createElement('div');
    wrap.id = 'fuPickOverlayInline';
    wrap.style.cssText = 'position:absolute;inset:0;background:rgba(10,16,28,.18);display:flex;align-items:center;justify-content:center;z-index:20;';
    wrap.innerHTML = `
      <div style="min-width:min(420px,92vw);max-width:92vw;background:#fff;border-radius:18px;padding:16px;box-shadow:0 18px 50px rgba(0,0,0,.22);display:flex;flex-direction:column;gap:12px;border:1px solid rgba(0,0,0,.08)">
        <div style="font-size:13px;font-weight:950;opacity:.86">Escolha como deseja selecionar os ${n} follow-ups.</div>
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap">
          <button class="eqd-btn" id="pickCloseBtn">Cancelar</button>
          <button class="eqd-btn" id="pickOldBtn">MAIS PRÓXIMOS</button>
          <button class="eqd-btn eqd-btnPrimary" id="pickRecentBtn">MAIS DISTANTES</button>
        </div>
      </div>`;
    ...
    const closeBtn = wrap.querySelector('#pickCloseBtn');
    const oldBtn = wrap.querySelector('#pickOldBtn');
    const recentBtn = wrap.querySelector('#pickRecentBtn');
    if (closeBtn) closeBtn.onclick = () => finish();
    if (oldBtn) oldBtn.onclick = () => finish(() => onPick(false));
    if (recentBtn) recentBtn.onclick = () => finish(() => onPick(true));
  ...
  function openNewTaskModalForUser(user, opts) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        renderCurrentView();
        setTimeout(() => {
          materializeRecurringRuleForUser(user.userId, rule, {
...
function makeUserCard(u) {
  ...
  function renderUserPanel(userId) {
    ...
    el.main.innerHTML = `
      ...
      <div class="panelCols">
        <div class="panelCol">
          <div class="panelColHead">${String(user.userId) === "1" ? "TAREFAS" : "NEGÓCIOS DO DIA + ATRASADOS (exceto FOLLOW-UP)"}</div>
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          closeModal();
          renderCurrentView();
          setTimeout(() => { refreshData(true, { deferLeads:false }).catch(()=>{}); }, 220);
          return;
        }
  ...
  function openSystemConfirmModal(title, message, okText = "Confirmar") {
    return new Promise((resolve) => {
      openModal(title || "Confirmar", `
        <div style="display:flex;flex-direction:column;gap:12px">
          <div style="font-size:14px;font-weight:950;line-height:1.4">${escHtml(String(message || ''))}</div>
          <div style="display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap">
            <button class="eqd-btn" id="sysConfirmNo">Cancelar</button>
            <button class="eqd-btn eqd-btnPrimary" id="sysConfirmYes">${escHtml(okText)}</button>
          </div>
        </div>
      `, { wide:true });
      document.getElementById('sysConfirmNo').onclick = () => { closeModal(); resolve(false); };
      document.getElementById('sysConfirmYes').onclick = () => { closeModal(); resolve(true); };
    });
  }
;