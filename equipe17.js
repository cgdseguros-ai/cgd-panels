  ...
  function endBootPending(label) {
    try {
      ...
      if (BOOT_PROGRESS_PENDING_TASKS <= 0) {
        ...
        setBootProgress(Math.min(99, Math.max(90, 100 - BOOT_PROGRESS_PENDING_TASKS)), label || 'Finalizando carregamento...');
      }
    } catch (_) {}
  }
  ...
  async function openRecurrenceBatchModalForUser(user) {
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        STATE.recurRulesByUser.set(String(user.userId), nextRules);
        if (warn) { warn.style.display = 'block'; warn.textContent = 'Recorrências salvas com sucesso.'; }
        clearBusy();
        closeModal();
        ...
        if (warn) {
          warn.style.display = 'block';
          warn.textContent = 'Falha: ' + (e.message || e);
        }
      } finally {
...
function makeUserCard(u) {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      DEALS_FAIL_STREAK = 0;
      if (BOOT_PROGRESS_PENDING_TASKS > 0) {
        setBootProgress(94, 'Concluindo carregamentos em segundo plano...');
      } else {
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const baseFilter = { CATEGORY_ID: Number(CATEGORY_PROPOSALS) };
    ...
    (rows || []).forEach((d) => {
      ...
      const stageName = inferProposalStageNameFromAny(proposalStageNameFromId(d.STAGE_ID)) || inferProposalStageNameFromAny(d.STAGE_ID) || String(proposalStageNameFromId(d.STAGE_ID) || d.STAGE_ID || '');
    ...
    if (uid) {
      ...
      const keep = Array.isArray(STATE.proposalDealsAll) ? (STATE.proposalDealsAll || []).filter((d) => String(d.ASSIGNED_BY_ID || '') !== uid) : [];
...
Operadora fechada: ${proposalFieldValueLabel(PROP_UF_OPER_FECHADA, deal[PROP_UF_OPER_FECHADA])}</textarea></div>
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    if (!sourceDeals.length || !!opts.forceFresh) {
      ...
      const nextHtml = sourceDeals.length ? renderProposalsModalContent(sourceDeals, LAST_PROPOSALS_CTX) : `<div style="padding:18px;font-weight:900">Nenhuma proposta encontrada para este filtro.</div><div style="display:flex;justify-content:flex-end;padding:0 18px 18px"><button class="eqd-btn" data-action="modalClose">Fechar</button></div>`;
      if (el.modalTitle && String(el.modalTitle.textContent||'').trim() === title && el.modalBody) {
        el.modalBody.innerHTML = nextHtml;
      } else {
        openModal(title, nextHtml, { full: true, noStack: true });
      }
    }
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "recurManager") {
        ...
        const uid = String(userId||"");
        if (!uid) return;
        if (STATE.recurModalOpeningForUser === uid) return;
        STATE.recurModalOpeningForUser = uid;
        try {
          setBusy('Abrindo recorrência…');
          await loadRecurrenceConfigDeals().catch(()=>{});
  ...
  setInterval(async () => {
    try{
      ...
      if (BOOT_PROGRESS_TIMER || BOOT_PROGRESS_PENDING_TASKS > 0 || BOOT_PROGRESS_TARGET < 100) return;
      if (BX_INFLIGHT > 0 || SYNC_QUEUE_RUNNING || (Array.isArray(SYNC_QUEUE) && SYNC_QUEUE.length)) return;
