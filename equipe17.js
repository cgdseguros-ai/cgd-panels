...
function makeUserCard(u) {
  ...
  function renderUserPanel(userId) {
    ...
    const user1AdminToggleBtn = String(user.userId) === "1" ? `<div id="user1AdminToggle" class="eqd-adminToggle" data-action="toggleAdminResources" style="display:flex;align-items:center;gap:6px;cursor:pointer;font-weight:950;padding:6px 2px;margin-left:auto"><span id="user1AdminArrow">▸</span><span>RECURSOS ADM</span></div>` : ``;
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    if (!force && !uid && Array.isArray(STATE.proposalDealsAll) && !stale) return STATE.proposalDealsAll;
    ...
    const allowedStageIds = proposalStageOrder()
      .map((nm) => String((STATE.proposalStageIdByName && STATE.proposalStageIdByName.get(norm(nm))) || '').trim())
      .filter(Boolean);

    let rows = [];
    if (allowedStageIds.length) {
      const perStage = await Promise.all(allowedStageIds.map((sid) => bxAll('crm.deal.list', {
    ...
    (rows || []).forEach((d) => {
      ...
      if (allowedStageIds.length && !allowedStageIds.includes(String(d.STAGE_ID || '').trim()) && !inferred) return;
      if (!proposalAllowedStage(inferred || proposalStageNameFromId(d.STAGE_ID))) return;
      byId.set(String(d.ID), { ...d, _proposalStageName: inferred || proposalStageNameFromId(d.STAGE_ID) || String(d.STAGE_ID || '') });
    });
  ...
  function renderProposalCard(deal, opts = {}) {
    ...
    const amount = String(deal.OPPORTUNITY || '').trim();
    const currency = String(deal.CURRENCY_ID || '').trim();
    const ownerName = dealUserNameByAssigned(deal.ASSIGNED_BY_ID);
    return `
      <div class="eqd-card" style="border-left:6px solid ${stageColor};padding:12px">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
          <div style="min-width:260px;flex:1">
            <div style="font-weight:950;font-size:14px">${escHtml(bestTitleFromText(deal.TITLE || ''))}</div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:6px">
              ...
              ${amount ? `<span class="eqd-tag">VALOR PRODUZIDO: <strong>${escHtml(amount)} ${escHtml(currency)}</strong></span>` : ''}
              <span class="eqd-tag">USER: <strong>${escHtml(ownerName)}</strong></span>
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px;margin-top:10px">${proposalSummaryRows(deal)}</div>
      </div>`;
  }

  async function openProposalStageMoveModal(dealId, returnCtx = {}) {
    await ensureProposalMeta(false);
    await loadProposalDeals(true);
    const deal = (STATE.proposalDealsAll || []).find((d) => String(d.ID) === String(dealId));
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const idx = (STATE.proposalDealsAll || []).findIndex((x) => String(x.ID) === String(deal.ID));
        if (idx >= 0) STATE.proposalDealsAll[idx] = { ...STATE.proposalDealsAll[idx], STAGE_ID: nextStage, _proposalStageName: proposalStageNameFromId(nextStage) };
        closeModal();
  ...
  async function openProposalEditor(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const idx = (STATE.proposalDealsAll || []).findIndex((x) => String(x.ID) === String(deal.ID));
        if (idx >= 0) STATE.proposalDealsAll[idx] = { ...STATE.proposalDealsAll[idx], ...patch, _proposalStageName: proposalStageNameFromId(patch.STAGE_ID || STATE.proposalDealsAll[idx].STAGE_ID) };
        closeModal();
        return openProposalsModal(String(returnCtx.userId || deal.ASSIGNED_BY_ID || ''), returnCtx);
      } catch (e) {
        warn.style.display = 'block';
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    let sourceDeals = [];
    if (adminView) {
      sourceDeals = await loadProposalDeals(true, {});
    } else {
      sourceDeals = await loadProposalDeals(true, { userId: uid });
    }
    ...
    let grouped = proposalStageOrder().map((nm) => ({ name: nm, deals: filteredDeals.filter((d) => proposalStageLabelForDeal(d) === nm) }));
    const knownCount = grouped.reduce((n,g) => n + g.deals.length, 0);
    if (knownCount < filteredDeals.length) {
      grouped.push({ name: 'OUTROS STAGES', deals: filteredDeals.filter((d) => !proposalStageOrder().includes(proposalStageLabelForDeal(d))) });
    }
    ...
    openModal(title, `
      ...
      <div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:10px">
        <div style="font-size:12px;font-weight:900;opacity:.82">Pipeline 0 • Total exibido: <strong>${filteredDeals.length}</strong></div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
      ...
      ${adminView ? `<div style="display:grid;grid-template-columns:minmax(260px,360px) 1fr;gap:14px;margin-bottom:14px"><div style="border:1px solid rgba(0,0,0,.1);border-radius:16px;padding:12px;background:rgba(255,255,255,.75)"><div style="font-weight:950;margin-bottom:8px">Ranqueamento</div>${rankingRows.length ? rankingRows.map((r, i) => `<div style="display:flex;justify-content:space-between;gap:8px;margin:6px 0"><span style="font-size:12px;font-weight:900">${i+1}. ${escHtml(trunc(r.label, 28))}</span><span style="font-size:12px;font-weight:900">${escHtml(String(r.value))}</span></div>`).join('') : `<div class="eqd-empty">Sem dados.</div>`}</div><div style="border:1px solid rgba(0,0,0,.1);border-radius:16px;padding:12px;background:rgba(255,255,255,.75)"><div style="font-weight:950;margin-bottom:8px">Gráfico</div>${rankingRows.length ? renderProposalChart(rankingRows) : `<div class="eqd-empty">Sem dados.</div>`}</div></div>` : ''}
      <div style="display:flex;flex-direction:column;gap:12px">${grouped.map((g) => `<div style="border:1px solid rgba(0,0,0,.1);border-radius:16px;padding:12px;background:rgba(255,255,255,.72)"><div style="font-weight:950;margin-bottom:10px;color:${proposalStageBadgeColor(g.name)}">${escHtml(g.name)} (${g.deals.length})</div><div style="display:flex;flex-direction:column;gap:10px">${g.deals.length ? g.deals.map((d) => renderProposalCard(d, { userId: d.ASSIGNED_BY_ID })).join('') : `<div class="eqd-empty">Sem contratos neste stage.</div>`}</div></div>`).join('')}</div>
    `, { full: adminView, wide: !adminView });
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "proposalDelete") { if (!confirm("Excluir este contrato/proposta?")) return; await bx("crm.deal.delete", { id: String(id||"") }); STATE.proposalDealsAll = (STATE.proposalDealsAll || []).filter((d) => String(d.ID) !== String(id||"")); return openProposalsModal(String(userId||""), { adminView: String(currentView && currentView.userId || '') === '1' && String(userId||'') === '1' }); }
