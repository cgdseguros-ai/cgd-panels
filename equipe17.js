...
(function () {
  ...
  injectCSS(`
    ...
    .userLine{font-size:11px;font-weight:950;opacity:.90;white-space:nowrap}
    ...
    .eqd-navArrow{font-size:18px;padding:8px 14px;min-width:46px;justify-content:center;background:#312e81;border-color:#4c3fda;color:#fff;box-shadow:0 4px 12px rgba(76,63,218,.28);}
    ...
    @media (orientation: landscape) and (max-width: 1024px){
      .eqd-footer{display:none !important;}
  ...
  async function openManualLeadCreateModal(user, defaultStatusId) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const fields = { ASSIGNED_BY_ID: Number(user.userId), NAME, TITLE: NAME };
  ...
  function openLeadAnalysisModal() {
    const rows = USERS.map((u) => {
      const arr = STATE.leadsByUser.get(String(u.userId)) || [];
      const novo = leadStageId("NOVO LEAD");
      const at = leadStageId("EM ATENDIMENTO");
      const atend = leadStageId("ATENDIDO");
      const qual = leadStageId("QUALIFICADO");
      const emAt = arr.filter((l) => String(l.STATUS_ID) === String(at)).length;
      const atd = arr.filter((l) => String(l.STATUS_ID) === String(atend)).length;
      const q = arr.filter((l) => String(l.STATUS_ID) === String(qual)).length;
      const semFu = arr.filter((l) => [String(atend), String(qual)].includes(String(l.STATUS_ID)) && !hasFutureFollowupForLeadOrig(leadOrigemId(l) || l.ID)).length;
      ...
      const novoCt = novo ? arr.filter((l) => String(l.STATUS_ID) === String(novo)).length : 0;
