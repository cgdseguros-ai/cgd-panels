...
(function () {
  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal) && linkedLeadOrig) {
      if (linkedLead && leadStageNameById(linkedLead.STATUS_ID) === "QUALIFICADO") tags.push(`<span class="eqd-tag" style="background:#dbeafe;color:#1d4ed8">QUALIFICADO</span>`);
...
restoreSyncQueue();
  ...
  function openFollowUpModal(user, prefillName, opts) {
    ...
    openModal(`FOLLOW-UP — ${user.name}`, `
      ...
      ${alreadyFuture ? `<div style="font-size:12px;font-weight:950;color:#92400e;background:#fef3c7;border:1px solid rgba(146,64,14,.18);padding:10px;border-radius:12px;margin-bottom:10px">Este lead já possui <strong>${alreadyFuture}</strong> follow-up agendado para hoje ou data futura.</div>` : ``}
  ...
  function openFollowupListModalForUser(user) {
    ...
    const render = (kwRaw) => {
      ...
      const filtered = kw ? list.filter((d) => norm([d.TITLE || "", d._obs || "", d._colabTxt || "", d._etapaTxt || "", d._urgTxt || ""].join(" ")).includes(kw)) : list.slice();
    ...
    const pickFirstNFollowList = (n) => { const arr = [...document.querySelectorAll("#fuListBox .eqd-batch")]; arr.forEach((x, idx) => { x.checked = idx < n; }); };
    const s10 = document.getElementById("fuListSelect10"); if (s10) s10.onclick = () => pickFirstNFollowList(10);
    const s20 = document.getElementById("fuListSelect20"); if (s20) s20.onclick = () => pickFirstNFollowList(20);
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    const byAtendidoDesc = (a,b) => String((b && b[LEAD_UF_ATENDIDO_DIA]) || '').localeCompare(String((a && a[LEAD_UF_ATENDIDO_DIA]) || '')) || String((b && b.DATE_MODIFY)||'').localeCompare(String((a && a.DATE_MODIFY)||''));
  ...
  const MARKETING_MEMBERS = ["1","3079","3085","3081","3387"];
  const MARKETING_LOGO_URL = "https://bitrix24public.com/b24-6iyx5y.bitrix24.com.br/docs/pub/0cbf888ac4208e72b8a959449178ee83/showFile?token=0184dc06wr8n";

  function makeMarketingCard() {
    return `
      <div class="userCard" data-action="openMktPanel">
        <div class="userEmoji" style="order:-1;align-self:center;min-width:22px;text-align:center">🟠</div>
        <div class="userInfoLeft">
          <div class="userName">MARKETING</div>
          <div class="userTeam">Agenda coletiva</div>
        </div>
        <div class="userPhotoWrap">
          <img class="userPhoto" src="${MARKETING_LOGO_URL}" alt="Marketing" referrerpolicy="no-referrer" style="width:100%;height:100%;object-fit:cover;object-position:center;transform:scale(1.30)" onerror="try{this.onerror=null;this.src='';this.style.display='none'}catch(e){}" />
        </div>
        <div class="userRight">
          <div class="userLine">Users: <strong>${MARKETING_MEMBERS.length}</strong></div>
          <div class="userLine">Painel MKT</div>
          <div class="userLine">Card 19</div>
        </div>
      </div>`;
  }

  async function openMarketingPanelStub(selectedDateStr = "") {
    const allowedTypes = ["GRAVAÇÃO","EDIÇÃO DE VÍDEO","ROTEIRO","REUNIÃO","OUTROS"];
    const typeEmojiMap = { "GRAVAÇÃO":"📹", "EDIÇÃO DE VÍDEO":"📱", "ROTEIRO":"📓", "REUNIÃO":"👥", "OUTROS":"" };
    const memberColorMap = { "1":"#111827", "3079":"#7c3aed", "3085":"#0f766e", "3081":"#b45309", "3387":"#be185d" };
    const members = USERS.filter((u) => MARKETING_MEMBERS.includes(String(u.userId)));
    const selected = selectedDateStr ? (tryParseDateAny(`${selectedDateStr} 00:00`) || new Date()) : new Date();
    const year = selected.getFullYear();
    const month = selected.getMonth();
    const selectedKey = `${year}-${String(month+1).padStart(2,'0')}-${String(selected.getDate()).padStart(2,'0')}`;
    const monthLabel = selected.toLocaleDateString('pt-BR', { month:'long', year:'numeric' });
    const tipoMapObj = await enums(UF_TAREFA).catch(() => ({}));
    const tipoMap = new Map(Object.entries(tipoMapObj || {}));
    const normType = (s) => norm(String(s||"").replace('VIDEO','VÍDEO'));
    const resolveTypeText = (d) => {
      const raw = String(d._tarefaTxt || tipoMap.get(String(d[UF_TAREFA] || d._tarefaId || '')) || '').trim();
      const n = normType(raw);
      const exact = allowedTypes.find((tt) => normType(tt) === n);
      if (exact) return exact;
      if (n.includes('GRAV')) return 'GRAVAÇÃO';
      if (n.includes('EDI')) return 'EDIÇÃO DE VÍDEO';
      if (n.includes('ROTEIR')) return 'ROTEIRO';
      if (n.includes('REUNI')) return 'REUNIÃO';
      if (n.includes('OUTR')) return 'OUTROS';
      return raw || '';
    };
    const mktDeals = (STATE.dealsOpen || []).filter((d) => String(d.TITLE || '').trim().startsWith('[MKT]')).sort((a,b) => {
      const ta = a._prazo ? new Date(a._prazo).getTime() : 0;
      const tb = b._prazo ? new Date(b._prazo).getTime() : 0;
      return ta - tb;
    });
    const filteredDeals = mktDeals.filter((d) => {
      if (!d._prazo) return false;
    ...
    for (let i=0;i<35;i++) {
      ...
      const dayDeals = mktDeals.filter((x) => {
        if (!x._prazo) return false;
        const dx = new Date(x._prazo);
        const xkey = `${dx.getFullYear()}-${String(dx.getMonth()+1).padStart(2,'0')}-${String(dx.getDate()).padStart(2,'0')}`;
        return xkey === key;
      });
      const typeSet = [...new Set(dayDeals.map((x) => resolveTypeText(x)).filter(Boolean))];
      const emojiLine = typeSet.map((tt) => typeEmojiMap[tt] || '').filter(Boolean).join(' ');
      const participantIds = [...new Set(dayDeals.map((x) => String(x.ASSIGNED_BY_ID || x._assigned || '')).filter(Boolean))].slice(0,4);
      ...
      const borderColor = participantIds.length ? (memberColorMap[participantIds[0]] || '#475569') : '#cbd5e1';
      const inMonth = d.getMonth() === month;
      days.push(`<button class="eqd-btn" data-action="mktPickDay" data-day="${key}" style="min-height:80px;border-radius:14px;justify-content:flex-start;padding:8px 8px;background:${key===selectedKey?'#111827':'#fff'};color:${key===selectedKey?'#fff':(inMonth?'#111':'#94a3b8')};border:2px solid ${key===selectedKey?'#111827':borderColor};flex-direction:column;align-items:flex-start;gap:5px;box-shadow:none"><div style="display:flex;width:100%;justify-content:space-between;align-items:center"><span style="font-weight:950">${d.getDate()}</span><span style="font-size:14px">${emojiLine || ''}</span></div><div style="display:flex;gap:4px;flex-wrap:wrap">${chips || '<span style="font-size:10px;opacity:.55">—</span>'}</div></button>`);
    }
    openModal("Painel MKT", `
      <div style="display:grid;grid-template-columns:minmax(408px,52%) 1fr;gap:12px;min-height:72vh">
        <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:12px">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div style="font-weight:950">Calendário MKT</div><div style="display:flex;gap:8px;align-items:center"><button class="eqd-btn" id="mktPrevMonth">←</button><div style="font-size:12px;opacity:.72;min-width:130px;text-align:center">${escHtml(monthLabel)}</div><button class="eqd-btn" id="mktNextMonth">→</button></div></div>
          <img src="${MARKETING_LOGO_URL}" style="width:108px;height:108px;object-fit:cover;transform:scale(1.08);border-radius:16px;background:#fff;border:1px solid rgba(0,0,0,.08);padding:0;align-self:flex-start" />
          <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:10px"><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center">SEG</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center">TER</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center">QUA</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center">QUI</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center">SEX</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center">SÁB</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center">DOM</div>${days.join('')}</div>
          <div style="font-size:12px;font-weight:900">Participantes</div>
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px">${allowedTypes.map((x)=>`<span class="eqd-tag">${escHtml((typeEmojiMap[x] ? typeEmojiMap[x] + ' ' : '') + x)}</span>`).join('')}</div>
        </div>
        <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:10px">
          ...
          <div style="font-size:12px;opacity:.78">Dia selecionado: <strong>${escHtml(fmtDateOnly(selected))}</strong></div>
          <div style="display:flex;flex-direction:column;gap:8px;min-height:260px;max-height:58vh;overflow:auto">${filteredDeals.length ? filteredDeals.map((d) => { const owner = USERS.find((u) => String(u.userId) === String(d.ASSIGNED_BY_ID || d._assigned || '')); const ownerUid = String(d.ASSIGNED_BY_ID || d._assigned || ''); const ownerBg = memberColorMap[ownerUid] || '#475569'; const typeText = resolveTypeText(d); const emoji = typeEmojiMap[typeText] || ''; return `<div class="eqd-card"><div class="eqd-inner"><div class="eqd-task">${escHtml(String(d.TITLE || '').replace(/^\[MKT\]\s*/,'').trim())}</div><div class="eqd-meta">${typeText ? `<span>Tipo: <strong>${escHtml(typeText)}</strong></span>` : ''}${d._prazo ? `<span>Prazo: <strong>${escHtml(fmt(d._prazo))}</strong></span>` : ''}${owner ? `<span>USER: <strong>${escHtml(owner.name)}</strong></span>` : ''}${d._colabTxt ? `<span>Participantes: <strong>${escHtml(d._colabTxt)}</strong></span>` : ''}</div><div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap"><span class="eqd-tag" style="background:${ownerBg};color:#fff;border-color:transparent">${escHtml(owner ? owner.name : ownerUid)}</span>${typeText ? `<span class="eqd-tag">${escHtml((emoji ? emoji + ' ' : '') + typeText)}</span>` : ''}</div><div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap"><button class="eqd-btn" data-action="doneOnly" data-id="${d.ID}">FEITA</button><button class="eqd-btn" data-action="reagendarDireto" data-id="${d.ID}">REAGENDAR</button></div></div></div>`; }).join('') : `<div class="eqd-empty">Nenhuma tarefa MKT para este dia.</div>`}</div>
        </div>
      </div>
    `, { full:true, wide:true });
    document.querySelectorAll('[data-action="mktPickDay"]').forEach((btn) => btn.onclick = () => openMarketingPanelStub(String(btn.getAttribute('data-day') || '')));
    const prevBtn = document.getElementById('mktPrevMonth');
    const nextBtn = document.getElementById('mktNextMonth');
    if (prevBtn) prevBtn.onclick = () => { const d = new Date(year, month - 1, 1); openMarketingPanelStub(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-01`); };
    if (nextBtn) nextBtn.onclick = () => { const d = new Date(year, month + 1, 1); openMarketingPanelStub(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-01`); };
    const createBtn = document.getElementById('mktCreateStub');
    if (createBtn) createBtn.onclick = async () => {
      const meta = await ensureDealFieldsMeta().catch(() => null);
      let tipoItems = (meta ? getFieldItemsFromMeta(meta, UF_TAREFA) : []).filter((x) => {
        const txt = String(x.text || '').trim();
        const n = normType(txt);
        return allowedTypes.some((tt) => normType(tt) === n) || n.includes('GRAV') || n.includes('EDI') || n.includes('ROTEIR') || n.includes('REUNI') || n.includes('OUTR');
      });
      if (!tipoItems.length) tipoItems = (meta ? getFieldItemsFromMeta(meta, UF_TAREFA) : []).slice();
      const base = tryParseDateAny(`${selectedKey} 11:00`) || new Date();
      const localDefault = new Date(base.getTime() - base.getTimezoneOffset()*60000).toISOString().slice(0,16);
      openModal("Nova tarefa MKT", `<div class="eqd-warn" id="mktWarn"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">TÍTULO</div><input id="mktTitle" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)" /></div><div><div style="font-size:11px;font-weight:900;margin-bottom:6px">TIPO</div><select id="mktType" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)">${tipoItems.map((x)=>`<option value="${x.value}">${escHtml(x.text)}</option>`).join('')}</select></div><div><div style="font-size:11px;font-weight:900;margin-bottom:6px">DATA E HORA</div><input id="mktPrazo" type="datetime-local" value="${localDefault}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)" /></div><div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">PARTICIPANTES</div><div style="display:flex;flex-wrap:wrap;gap:8px">${members.map((u)=>`<label style="display:inline-flex;gap:6px;align-items:center;padding:6px 10px;border:1px solid rgba(0,0,0,.08);border-radius:999px;background:${memberColorMap[String(u.userId)] || '#475569'};color:#fff"><input class="mktChk" type="checkbox" value="${u.userId}" checked /> ${escHtml(u.name)}</label>`).join('')}</div></div><div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">OBS</div><textarea id="mktObs" rows="4" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)"></textarea></div><div style="grid-column:1 / -1;display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="mktSave">CRIAR</button></div></div>`, { wide:true });
      document.getElementById('mktSave').onclick = async () => {
        ...
        try {
          ...
          for (const uid of checked) {
            ...
            const fields = { CATEGORY_ID:Number(CATEGORY_MAIN), STAGE_ID:String(STATE.todoStageId || ''), TITLE:`[MKT] ${title}`, ASSIGNED_BY_ID:Number(uid), [UF_PRAZO]: prazoIso };
          ...
          closeModal();
          openMarketingPanelStub(selectedKey);
        } catch (e) {
          warn.style.display='block';
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    openModal("Concluir e reagendar", `
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:10px">
        ...
        ${!isFollowupDeal(d) ? `<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">URGÊNCIA</div><select id="drUrg" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Carregando…</option></select></div>` : ``}<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">OBS</div><textarea id="drObs" rows="5" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px;font-weight:850;outline:none">${escHtml(String(d[UF_OBS] || ""))}</textarea></div>
    ...
    btn.onclick = async () => {
      try {
        ...
        if (d[UF_ETAPA]) fields[UF_ETAPA] = d[UF_ETAPA]; if (d[UF_COLAB]) fields[UF_COLAB] = d[UF_COLAB]; if (d[DEAL_UF_LEAD_ORIGEM]) fields[DEAL_UF_LEAD_ORIGEM] = d[DEAL_UF_LEAD_ORIGEM]; else if (resolvedLead) fields[DEAL_UF_LEAD_ORIGEM] = String(leadOrigemId(resolvedLead) || resolvedLead.ID || ""); fields[UF_OBS] = obs;
        const created = await bx("crm.deal.add", { fields });
        const realId = String((created && (created.result || created)) || '').trim() || makeTempId('TMP_DEAL');
        upsertDealLocal(parseLocalDealFromFields(realId, fields));
        closeModal(); await refreshData(true); renderCurrentView();
  ...
  function openLeadStageListModal(userId, stageId, title, kwRaw = "") {
    const user = USERS.find((u) => String(u.userId) === String(userId));
    const arr = (STATE.leadsByUser.get(String(userId)) || []).filter((l) => String(l.STATUS_ID || '') === String(stageId || ''));
    const kw = norm(String(kwRaw || '').trim());
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadShowLost") { const uid = String(a.getAttribute("data-userid")||""); return openLeadStageListModal(uid, leadStageId("LEAD DESCARTADO") || leadStageId("PERDIDO"), 'Perdidos'); }
      if (act === "leadShowConv") { const uid = String(a.getAttribute("data-userid")||""); return openLeadStageListModal(uid, leadStageId("LEAD CONVERTIDO") || leadStageId("CONVERTIDO"), 'Convertidos'); }
