...
(function () {
  ...
  injectCSS(`
    ...
    .eqd-logo{width:52px;height:52px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.92);object-fit:contain;padding:2px;flex:0 0 auto;}
...
function makeMarketingCard() {
  return `<div class="userCard" data-action="openMktPanel"><div class="userEmoji" style="order:-1;align-self:center;min-width:22px;text-align:center">🟠</div><div class="userInfoLeft"><div class="userName">MARKETING</div><div class="userTeam">Agenda coletiva</div></div><div class="userPhotoWrap"><img class="userPhoto" src="${MARKETING_LOGO_URL}" alt="Marketing" referrerpolicy="no-referrer" onerror="try{this.onerror=null;this.src='';this.style.display='none'}catch(e){}" /></div><div class="userRight"><div class="userLine">Users: <strong>${MARKETING_MEMBERS.length}</strong></div><div class="userLine">Em implantação</div></div></div>`;
}

function getMarketingDeals() {
  return (STATE.dealsOpen || []).filter((d) => String(d.TITLE || '').startsWith(MKT_DEAL_PREFIX)).sort((a,b) => {
    const ta = dDate(a); const tb = dDate(b); return ta - tb;
  });
  function dDate(d){ const x = d && d._prazo ? new Date(d._prazo).getTime() : Number.MAX_SAFE_INTEGER; return Number.isFinite(x) ? x : Number.MAX_SAFE_INTEGER; }
}

function openMarketingCreateModal() {
  const members = USERS.filter((u) => MARKETING_MEMBERS.includes(String(u.userId)));
  const types = ["EDIÇÃO DE VIDEO","ROTEIRO","GRAVAÇÃO","REUNIÃO","OUTROS"];
  const base = new Date(); base.setHours(11,0,0,0);
  const localDefault = new Date(base.getTime() - base.getTimezoneOffset()*60000).toISOString().slice(0,16);
  openModal("Novo deal MKT", `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
      ...
      <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">TIPO</div><select id="mktType" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)">${types.map((x)=>`<option>${escHtml(x)}</option>`).join('')}</select></div>
      ...
      <div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">PARTICIPANTES</div><div style="display:flex;flex-wrap:wrap;gap:8px">${members.map((u)=>`<label class="eqd-tag" style="display:inline-flex;gap:6px;align-items:center"><input type="checkbox" class="mktMember" value="${u.userId}" checked> ${escHtml(u.name)}</label>`).join('')}</div></div>
      <div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">OBS</div><textarea id="mktObs" rows="4" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px"></textarea></div>
      <div style="grid-column:1 / -1;display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="mktSave">SALVAR</button></div>
    </div>
  `, { wide:true });
  document.getElementById('mktSave').onclick = async () => {
    ...
    try {
      ...
      const tipo = String(document.getElementById('mktType').value || '').trim();
      ...
      const participants = [...document.querySelectorAll('.mktMember:checked')].map((x) => String(x.value || '').trim()).filter(Boolean);
      if (!title) throw new Error('Preencha o título.');
      if (!prazoIso) throw new Error('Preencha a data.');
      if (!participants.length) throw new Error('Selecione ao menos um participante.');
      if (!await confirmWeekendModal(prazoIso, 'esta tarefa')) return;
      const fields = { CATEGORY_ID:Number(CATEGORY_MAIN), STAGE_ID:String(STATE.defaultStageId || ''), TITLE:`${MKT_DEAL_PREFIX} ${tipo} — ${title}`, ASSIGNED_BY_ID:1, [UF_PRAZO]:prazoIso, [UF_OBS]:`MKT_PARTICIPANTS=${participants.join(',')}\n${obs}`.trim() };
      const created = await bx('crm.deal.add', { fields });
      const realId = String((created && (created.result || created)) || '').trim() || makeTempId('TMP_DEAL');
      upsertDealLocal(parseLocalDealFromFields(realId, fields));
      closeModal();
      openMarketingPanelStub();
    } catch (e) {
      if (warn) { warn.style.display='block'; warn.textContent='Falha: ' + (e.message || e); }
    }
  };
...
function openMarketingPanelStub() {
  const typeTags = ["EDIÇÃO DE VIDEO","ROTEIRO","GRAVAÇÃO","REUNIÃO","OUTROS"];
  const members = USERS.filter((u) => MARKETING_MEMBERS.includes(String(u.userId)));
  const deals = (STATE.dealsOpen || []).filter((d) => String(d.TITLE || '').startsWith(MKT_DEAL_PREFIX)).sort((a,b)=> {
    const ta = a._prazo ? new Date(a._prazo).getTime() : Number.MAX_SAFE_INTEGER;
    const tb = b._prazo ? new Date(b._prazo).getTime() : Number.MAX_SAFE_INTEGER;
    return ta - tb;
  });
  const dateCounts = new Map();
  deals.forEach((d) => {
    if (!d._prazo) return;
    const x = new Date(d._prazo);
    const k = `${x.getFullYear()}-${x.getMonth()+1}-${x.getDate()}`;
    const arr = dateCounts.get(k) || [];
    arr.push(d);
    dateCounts.set(k, arr);
  });
  const now = new Date();
  const days = Array.from({length: 35}, (_,i) => new Date(now.getFullYear(), now.getMonth(), now.getDate()+i));
  openModal("Painel MKT — implantação contínua", `
    <div style="display:grid;grid-template-columns:minmax(300px,42%) 1fr;gap:12px;min-height:72vh">
      <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:12px">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div style="font-weight:950">Calendário MKT</div><button class="eqd-btn eqd-btnPrimary" id="mktNewBtn">NOVO DEAL MKT</button></div>
        <img src="${MARKETING_LOGO_URL}" style="width:84px;height:84px;object-fit:contain;border-radius:16px;background:#fff;border:1px solid rgba(0,0,0,.08);padding:6px" />
        <div style="font-size:12px;font-weight:900">Participantes</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">${members.map((u) => `<span class="eqd-tag">${escHtml(u.name)}</span>`).join('')}</div>
        ...
        <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;overflow:auto">
          ${days.map((d) => {
            const k = `${d.getFullYear()}-${d.getMonth()+1}-${d.getDate()}`;
            const arr = dateCounts.get(k) || [];
            const hasCamera = arr.some((x) => String(x.TITLE || '').toUpperCase().includes('GRAVAÇÃO'));
            return `<button class="eqd-btn" style="justify-content:flex-start;border-radius:14px;padding:10px;background:#fff;color:#111827;border:1px solid rgba(0,0,0,.08)"><span>${fmtDateOnly(d)}</span><span style="margin-left:auto">${hasCamera ? '🎥' : (arr.length ? arr.length : '')}</span></button>`;
          }).join('')}
        </div>
      </div>
      <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:10px">
        <div style="font-weight:950">Agenda MKT</div>
        <div style="font-size:12px;opacity:.78">Aqui já aparecem apenas deals do MKT criados com o prefixo próprio do painel.</div>
        <div style="display:flex;flex-direction:column;gap:8px;overflow:auto;min-height:40vh;max-height:62vh">${deals.length ? deals.map((d) => `<div class="eqd-card"><div class="eqd-inner"><div class="eqd-task">${escHtml(bestTitleFromText(d.TITLE || ''))}</div><div class="eqd-foot"><span>Prazo: <strong>${escHtml(d._prazo ? fmt(d._prazo) : '—')}</strong></span><span>${String(d[UF_OBS] || '').includes('MKT_PARTICIPANTS=') ? escHtml(String(d[UF_OBS]).split('\n')[0].replace('MKT_PARTICIPANTS=','Participants: ')) : ''}</span></div></div></div>`).join('') : `<div class="eqd-empty">Nenhum deal de MKT criado ainda.</div>`}</div>
      </div>
    </div>
  `, { full:true, wide:true });
  document.getElementById('mktNewBtn')?.addEventListener('click', openMarketingCreateModal);
}
...
function makeUserCard(u) {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      if (!(opts && opts.skipRecurLoad) && (opts.forceRecur || !STATE.recurRulesByUser.size || (now - LAST_RECUR_LOAD_AT > 10 * 60 * 1000))) {
        await loadRecurrenceConfigDeals().catch(()=>{});
        LAST_RECUR_LOAD_AT = now;
      }

      if (!(opts && opts.skipRecurGen) && (opts.forceRecur || (now - LAST_RECUR_GEN_AT > 5 * 60 * 1000))) {
  ...
  async function revalidateLead(userId, leadId) {
    ...
    const nowIso = new Date().toISOString();
    const fields = { ASSIGNED_BY_ID:Number(uid), NAME:leadTitle(lead), TITLE:leadTitle(lead), STATUS_ID:String(leadStageId("EM ATENDIMENTO") || leadStageId("NOVO LEAD") || "") };
    ...
    upsertLeadLocal(uid, { ID: tempId, ASSIGNED_BY_ID:Number(uid), NAME:leadTitle(lead), TITLE:leadTitle(lead), STATUS_ID:String(leadStageId("NOVO LEAD") || leadStageId("EM ATENDIMENTO") || ""), DATE_CREATE:nowIso, DATE_MODIFY:nowIso, [LEAD_UF_DATAHORA]:nowIso, [LEAD_UF_LEAD_ORIGEM]: tempId, [LEAD_UF_OPERADORA]:lead[LEAD_UF_OPERADORA], [LEAD_UF_IDADE]:lead[LEAD_UF_IDADE], [LEAD_UF_TELEFONE]:lead[LEAD_UF_TELEFONE], [LEAD_UF_BAIRRO]:lead[LEAD_UF_BAIRRO], [LEAD_UF_FONTE]:lead[LEAD_UF_FONTE], [LEAD_UF_OBS]:lead[LEAD_UF_OBS], [LEAD_UF_HELENA]:lead[LEAD_UF_HELENA], [LEAD_UF_PESSOAL]:lead[LEAD_UF_PESSOAL], [LEAD_UF_POSSUI_PLANO]:lead[LEAD_UF_POSSUI_PLANO] });
    reopenLeadsModalSafe();
    renderCurrentView();
    const created = await bx('crm.lead.add', { fields });
    const newLeadId = String((created && (created.result || created)) || '').trim() || tempId;
    if (newLeadId !== tempId && LEAD_UF_LEAD_ORIGEM) await bx('crm.lead.update', { id:newLeadId, fields:{ [LEAD_UF_LEAD_ORIGEM]: newLeadId, STATUS_ID: String(leadStageId("EM ATENDIMENTO") || leadStageId("NOVO LEAD") || "") } }).catch(()=>{});
    const arr = (STATE.leadsByUser.get(uid) || []).slice();
    const idx = arr.findIndex((l) => String(l.ID) === tempId);
    if (idx >= 0) arr[idx] = { ...arr[idx], ID:newLeadId, STATUS_ID:String(leadStageId("EM ATENDIMENTO") || leadStageId("NOVO LEAD") || ""), [LEAD_UF_LEAD_ORIGEM]: newLeadId };
    STATE.leadsByUser.set(uid, arr);
    await moveLeadStage(uid, leadId, lostLeadStageIds()[0] || leadStageId("PERDIDO"), { skipLostPrompt: true, skipClose: true });
    reopenLeadsModalSafe();
    renderCurrentView();
  }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadShowLost") { const uid = String(a.getAttribute("data-userid")||""); await loadLeadsForOneUser(uid).catch(()=>{}); return openLeadStageListModal(uid, lostLeadStageIds(), 'Perdidos'); }
      if (act === "leadShowConv") { const uid = String(a.getAttribute("data-userid")||""); await loadLeadsForOneUser(uid).catch(()=>{}); return openLeadStageListModal(uid, convertedLeadStageIds(), 'Convertidos'); }
  ...
  (async function init(){
    ...
    renderGeneral();
    setSoftStatus("JS: ok");
    await refreshData(false, { forceRecur: false, forceFullDeals: false, skipLeads: true, skipRecurLoad: true, skipRecurGen: true });
    renderCurrentView();
    setTimeout(async () => {
      try {
