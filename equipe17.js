...
(function () {
  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal) && linkedLeadOrig) {
      ...
      if (linkedLead && isLostLead(linkedLead)) tags.push(`<span class="eqd-tag" style="background:#fee2e2;color:#b91c1c">LEAD PERDIDO</span>`);
...
restoreSyncQueue();
  ...
  function openNoFutureFollowupLeadBatchModal(user, leads, title) {
    ...
    document.getElementById('noFuBatch').onclick = async () => {
      ...
      const defaultBase = String((opts && opts.prefillPrazoIso) || "").trim() ? new Date(String((opts && opts.prefillPrazoIso) || "").trim()) : ((existingFutureFu && existingFutureFu._prazo) ? new Date(existingFutureFu._prazo) : base);
    const localDefault = new Date(defaultBase.getTime() - (defaultBase.getTimezoneOffset() * 60000)).toISOString().slice(0, 16);
  ...
  function mktTypeMatchesAllowed(text) {
    const n = norm(String(text || ""));
    return ["gravacao","edicao de video","roteiro","outros","reuniao"].some((k) => n.includes(k));
  }
...
function makeMarketingCard() {
  return `<div class="userCard" data-action="openMktPanel"><div class="userEmoji" style="order:-1;align-self:center;min-width:22px;text-align:center">🟠</div><div class="userInfoLeft"><div class="userName">MARKETING</div><div class="userTeam">Agenda coletiva</div></div><div class="userPhotoWrap"><img class="userPhoto" src="${MARKETING_LOGO_URL}" alt="Marketing" referrerpolicy="no-referrer" style="object-fit:cover;transform:scale(1.5)" onerror="try{this.onerror=null;this.src='';this.style.display='none'}catch(e){}" /></div><div class="userRight"><div class="userLine">Users: <strong>${MARKETING_MEMBERS.length}</strong></div><div class="userLine">Em implantação</div></div></div>`;
}

function openMarketingPanelStub(selectedDateStr = "", monthCursorStr = "") {
  const typeTags = ["EDIÇÃO DE VIDEO","ROTEIRO","GRAVAÇÃO","REUNIÃO","OUTROS"];
  ...
  const today = new Date();
  const monthCursor = monthCursorStr ? (tryParseDateAny(`${monthCursorStr}-01 00:00`) || new Date(today.getFullYear(), today.getMonth(), 1)) : new Date(today.getFullYear(), today.getMonth(), 1);
  const selected = selectedDateStr ? (tryParseDateAny(`${selectedDateStr} 00:00`) || new Date(monthCursor)) : new Date(today);
  ...
  for (let i=0;i<42;i++) {
    ...
    const hasGrav = dayDeals.some((x) => /GRAVAÇÃO/i.test(String(x._tarefaTxt || x.TITLE || "")));
    ...
    const chips = participantIds.map((uid) => {
      ...
      return `<span style="display:inline-flex;align-items:center;justify-content:center;min-width:18px;height:18px;border-radius:999px;background:${bg};color:#fff;font-size:10px;font-weight:900;padding:0 6px;border:1px solid rgba(255,255,255,.75)">${escHtml((owner ? owner.name : uid).slice(0,2).toUpperCase())}</span>`;
    }).join("");
    const inMonth = d.getMonth() === monthCursor.getMonth();
    const selectedDay = key === selectedKey;
    const borderColor = participantIds.length ? (memberColorMap[participantIds[0]] || "#475569") : (inMonth ? "rgba(59,130,246,.24)" : "rgba(148,163,184,.18)");
    const bgColor = selectedDay ? "#111827" : (dayDeals.length ? "#f8fbff" : (inMonth ? "#fff" : "#f8fafc"));
    const extraShadow = dayDeals.length ? "0 0 0 2px rgba(59,130,246,.10) inset" : "none";
    days.push(`<button class="eqd-btn" data-action="mktPickDay" data-day="${key}" data-month="${monthKey}" style="min-height:84px;border-radius:16px;justify-content:flex-start;padding:8px 8px;background:${bgColor};color:${selectedDay?'#fff':(inMonth?'#111':'#94a3b8')};border:2px solid ${selectedDay?'#111827':borderColor};flex-direction:column;align-items:flex-start;gap:6px;box-shadow:${extraShadow};white-space:normal"><div style="display:flex;width:100%;justify-content:space-between;align-items:center"><span style="font-weight:950;font-size:13px">${d.getDate()}</span>${hasGrav?'<span style="font-size:16px">📹</span>':''}</div><div style="display:flex;gap:4px;flex-wrap:wrap">${chips || '<span style="font-size:10px;opacity:.55">—</span>'}</div><div style="font-size:10px;opacity:${selectedDay?'.9':'.62'}">${dayDeals.length ? `${dayDeals.length} item(ns)` : '&nbsp;'}</div></button>`);
  }
  openModal("Painel MKT — implantação", `<div style="display:grid;grid-template-columns:minmax(460px,52%) 1fr;gap:14px;min-height:76vh"><div style="border:1px solid rgba(0,0,0,.08);border-radius:18px;padding:14px;background:#fff;display:flex;flex-direction:column;gap:12px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div style="font-weight:950">Calendário MKT</div><div style="display:flex;align-items:center;gap:8px"><button class="eqd-btn" data-action="mktPrevMonth" data-month="${monthKey}">←</button><div style="font-size:12px;font-weight:900;opacity:.78;min-width:120px;text-align:center">${escHtml(monthLabel)}</div><button class="eqd-btn" data-action="mktNextMonth" data-month="${monthKey}">→</button></div></div><img src="${MARKETING_LOGO_URL}" style="width:92px;height:92px;object-fit:cover;transform:scale(1.22);border-radius:16px;background:#fff;border:1px solid rgba(0,0,0,.08);padding:0;align-self:flex-start" /><div style="display:grid;grid-template-columns:repeat(7,1fr);gap:8px"><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center;color:#1d4ed8">SEG</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center;color:#7c3aed">TER</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center;color:#0f766e">QUA</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center;color:#b45309">QUI</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center;color:#be185d">SEX</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center;color:#dc2626">SÁB</div><div style="font-size:11px;font-weight:900;opacity:.65;text-align:center;color:#ea580c">DOM</div>${days.join("")}</div><div style="font-size:12px;font-weight:900">Participantes</div><div style="display:flex;flex-wrap:wrap;gap:6px">${members.map((u) => `<span class="eqd-tag" style="background:${memberColorMap[String(u.userId)] || '#475569'};color:#fff;border-color:transparent">${escHtml(u.name)}</span>`).join('')}</div><div style="font-size:12px;font-weight:900">Tipos permitidos</div><div style="display:flex;flex-wrap:wrap;gap:6px">${typeTags.map((x)=>`<span class="eqd-tag">${escHtml(x)}</span>`).join('')}</div></div><div style="border:1px solid rgba(0,0,0,.08);border-radius:18px;padding:14px;background:#fff;display:flex;flex-direction:column;gap:10px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div style="font-weight:950">Agenda MKT</div><button class="eqd-btn eqd-btnPrimary" id="mktCreateStub">Criar tarefa MKT</button></div><div style="font-size:12px;opacity:.78">Dia selecionado: <strong>${escHtml(fmtDateOnly(selected))}</strong></div><div style="display:flex;flex-direction:column;gap:8px;min-height:280px;max-height:60vh;overflow:auto">${filteredDeals.length ? filteredDeals.map((d) => { const owner = USERS.find((u) => String(u.userId) === String(d.ASSIGNED_BY_ID || d._assigned || "")); const ownerUid = String(d.ASSIGNED_BY_ID || d._assigned || ""); const ownerBg = memberColorMap[ownerUid] || "#475569"; return `<div class="eqd-card"><div class="eqd-inner"><div class="eqd-task">${escHtml(String(d.TITLE || '').replace(/^\[MKT\]\s*/, '').trim())}</div><div class="eqd-meta">${d._tarefaTxt ? `<span>Tipo: <strong>${escHtml(d._tarefaTxt)}</strong></span>` : ''}${d._prazo ? `<span>Prazo: <strong>${escHtml(fmt(d._prazo))}</strong></span>` : ''}${owner ? `<span>USER: <strong>${escHtml(owner.name)}</strong></span>` : ''}${d._colabTxt ? `<span>Participantes: <strong>${escHtml(d._colabTxt)}</strong></span>` : ''}</div><div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap"><span class="eqd-tag" style="background:${ownerBg};color:#fff;border-color:transparent">${escHtml(owner ? owner.name : ownerUid)}</span>${d._tarefaTxt ? `<span class="eqd-tag">${escHtml(d._tarefaTxt)}</span>` : ''}</div></div></div>`; }).join('') : `<div class="eqd-empty">Nenhuma tarefa MKT para este dia.</div>`}</div></div></div>`, { full:true, wide:true });
  document.querySelectorAll('[data-action="mktPickDay"]').forEach((btn) => btn.onclick = () => openMarketingPanelStub(String(btn.getAttribute('data-day') || ''), String(btn.getAttribute('data-month') || monthKey)));
  document.querySelectorAll('[data-action="mktPrevMonth"]').forEach((btn) => btn.onclick = () => { const cur = String(btn.getAttribute('data-month') || monthKey); const d = tryParseDateAny(`${cur}-01 00:00`) || monthCursor; const prev = new Date(d.getFullYear(), d.getMonth()-1, 1); const prevKey = `${prev.getFullYear()}-${String(prev.getMonth()+1).padStart(2,'0')}`; openMarketingPanelStub(`${prev.getFullYear()}-${String(prev.getMonth()+1).padStart(2,'0')}-01`, prevKey); });
  document.querySelectorAll('[data-action="mktNextMonth"]').forEach((btn) => btn.onclick = () => { const cur = String(btn.getAttribute('data-month') || monthKey); const d = tryParseDateAny(`${cur}-01 00:00`) || monthCursor; const nxt = new Date(d.getFullYear(), d.getMonth()+1, 1); const nextKey = `${nxt.getFullYear()}-${String(nxt.getMonth()+1).padStart(2,'0')}`; openMarketingPanelStub(`${nxt.getFullYear()}-${String(nxt.getMonth()+1).padStart(2,'0')}-01`, nextKey); });
  const createBtn = document.getElementById('mktCreateStub');
  if (createBtn) createBtn.onclick = async () => {
    const meta = await ensureDealFieldsMeta().catch(() => null);
    let tipoItems = (meta ? getFieldItemsFromMeta(meta, UF_TAREFA) : []).map((x)=>({ value:String(x.ID||''), text:String(x.VALUE ?? x.NAME ?? x.ID) })).filter((x) => mktTypeMatchesAllowed(x.text));
    if (!tipoItems.length && meta) tipoItems = (getFieldItemsFromMeta(meta, UF_TAREFA) || []).map((x)=>({ value:String(x.ID||''), text:String(x.VALUE ?? x.NAME ?? x.ID) }));
    const base = tryParseDateAny(`${selectedKey} 11:00`) || new Date();
    const localDefault = new Date(base.getTime() - base.getTimezoneOffset()*60000).toISOString().slice(0,16);
    openModal("Nova tarefa MKT", `<div class="eqd-warn" id="mktWarn"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">TÍTULO</div><input id="mktTitle" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)" /></div><div><div style="font-size:11px;font-weight:900;margin-bottom:6px">TIPO</div><select id="mktType" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)">${tipoItems.length ? tipoItems.map((x)=>`<option value="${escHtml(x.value)}">${escHtml(x.text)}</option>`).join('') : `<option value="">Sem opções carregadas</option>`}</select></div><div><div style="font-size:11px;font-weight:900;margin-bottom:6px">DATA E HORA</div><input id="mktPrazo" type="datetime-local" value="${localDefault}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)" /></div><div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">PARTICIPANTES</div><div style="display:flex;flex-wrap:wrap;gap:8px">${members.map((u)=>`<label style="display:inline-flex;gap:6px;align-items:center;padding:6px 10px;border:1px solid rgba(0,0,0,.08);border-radius:999px;background:${memberColorMap[String(u.userId)] || '#475569'};color:#fff"><input class="mktChk" type="checkbox" value="${u.userId}" checked /> ${escHtml(u.name)}</label>`).join('')}</div></div><div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">OBS</div><textarea id="mktObs" rows="4" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)"></textarea></div><div style="grid-column:1 / -1;display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="mktSave">CRIAR</button></div></div>`, { wide:true });
    document.getElementById('mktSave').onclick = async () => {
      ...
      try {
        ...
        if (!typeId) throw new Error('Selecione o tipo de tarefa.');
        if (!prazoIso) throw new Error('Informe data e hora.');
        if (!checked.length) throw new Error('Selecione ao menos um participante.');
        const stageId = String(STATE.todoStageId || '');
        for (const uid of checked) {
          const participantNames = members.filter((u)=>checked.includes(String(u.userId))).map((u)=>u.name).join(', ');
          const fields = { CATEGORY_ID:Number(CATEGORY_MAIN), STAGE_ID:String(stageId), TITLE:`[MKT] ${title}`, ASSIGNED_BY_ID:Number(uid), [UF_PRAZO]: prazoIso, [UF_TAREFA]: typeId };
          if (obs) fields[UF_OBS] = obs;
        ...
        closeModal();
        openMarketingPanelStub(selectedKey, monthKey);
      } catch (e) {
        warn.style.display = 'block';
...
function makeUserCard(u) {
  ...
  async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    if (target === String(sConv) && uid === "815") {
      openModal("Escolher pipeline", `<div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap"><button class="eqd-btn" id="pl0">SAÚDE (0)</button><button class="eqd-btn eqd-btnPrimary" id="pl11">SEGUROS (11)</button></div>`);
      document.getElementById("pl0").onclick = async () => { await routeLeadToCategory(uid, leadId, 0, target); reopenLeadsModalSafe(); };
      document.getElementById("pl11").onclick = async () => { await routeLeadToCategory(uid, leadId, 11, target); reopenLeadsModalSafe(); };
      return;
    }
