...
(function () {
  ...
  function sanitizeDealUpdateFields(fields) {
    ...
    Object.entries(fields || {}).forEach(([k, v]) => {
      ...
      if (typeof v === 'string') {
        const t = v.trim();
        if (!t) {
          if ([UF_OBS, DEAL_UF_LEAD_ORIGEM, 'TITLE', 'STAGE_ID'].includes(key)) out[key] = '';
          return;
        }
        out[key] = t;
        return;
    ...
    if (Object.prototype.hasOwnProperty.call(out, 'OPPORTUNITY')) {
      let raw = String(out.OPPORTUNITY || '').trim();
      if (/^-?[\d\.]+,\d+$/.test(raw)) raw = raw.replace(/\./g, '').replace(',', '.');
      else if (/^-?[\d,]+\.\d+$/.test(raw)) raw = raw.replace(/,/g, '');
      else raw = raw.replace(',', '.');
      const n = Number(raw);
      if (Number.isFinite(n)) out.OPPORTUNITY = String(n);
      else delete out.OPPORTUNITY;
    }
    if (Object.prototype.hasOwnProperty.call(out, 'CURRENCY_ID') && !String(out.CURRENCY_ID || '').trim()) delete out.CURRENCY_ID;
    return out;
  }

  async function safeDealUpdate(dealId, fields) {
    const patch = sanitizeDealUpdateFields(fields);
    if (!Object.keys(patch).length) return true;
    try {
      return await bx("crm.deal.update", { id: String(dealId), fields: patch });
    } catch (e) {
      const msg = String((e && e.message) || e || "");
      if (!/HTTP 400\s+em\s+crm\.deal\.update/i.test(msg)) throw e;
      try {
        const fresh = await bx("crm.deal.get", { id: String(dealId) });
        if (dealFieldsMatchRequested(fresh, patch)) return true;
      } catch (_) {}
      throw e;
    }
  }
  ...
  async function openSystemConfirmModal(title, message, confirmLabel = 'Confirmar') {
    return await new Promise((resolve) => {
      openModal(title || 'Confirmar', `
        <div style="display:flex;flex-direction:column;gap:14px;min-width:min(420px,90vw)">
          <div style="font-size:14px;font-weight:950">${escHtml(String(message || 'Tem certeza?'))}</div>
          <div style="display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap">
            <button class="eqd-btn" id="sysConfirmCancel">Cancelar</button>
            <button class="eqd-btn eqd-btnPrimary" id="sysConfirmOk">${escHtml(confirmLabel)}</button>
          </div>
        </div>
      `, { wide:true });
      const cancelBtn = document.getElementById('sysConfirmCancel');
      const okBtn = document.getElementById('sysConfirmOk');
    ...
    if (document.visibilityState === 'hidden') {
      ...
      setTimeout(async () => {
        try {
          ...
          if (el.modalOverlay.style.display !== "flex" && Date.now() >= UI_REFRESH_HOLD_UNTIL) renderCurrentView();
        } catch(_) {}
      }, 450);
    }
  });
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "proposalDelete") { const ok = await openSystemConfirmModal('Excluir proposta', 'Tem certeza que deseja excluir este contrato/proposta?', 'Excluir'); if (!ok) return; await bx("crm.deal.delete", { id: String(id||"") }); PROPOSALS_REFRESH_HOLD_UNTIL = Date.now() + 10000; UI_REFRESH_HOLD_UNTIL = Date.now() + 4000; removeProposalDealCaches(String(id||"")); return openProposalsModal(String((LAST_PROPOSALS_CTX&&LAST_PROPOSALS_CTX.userId)||userId||""), { ...(LAST_PROPOSALS_CTX||{}), noStack: true }); }
