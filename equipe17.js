...
(function () {
  ...
  const STATE = {
    ...
    recurModalOpeningForUser: "",
    recurManagerShownForUser: "",
    recurManagerShownAt: 0,
  };
  ...
  async function loadDeals(opts = {}) {
    ...
    setBootProgress(18, "Lendo catálogos...");
    ...
    if (openOnly) {
      setBootProgress(28, "Buscando itens abertos...");
      ...
      setBootProgress(32, "Buscando tarefas e eventos...");
      const deals = await bxAll("crm.deal.list", {
      ...
      setBootProgress(34, "Sincronizando alterações recentes...");
      const deals = await bxAll("crm.deal.list", {
    ...
    setBootProgress(46, "Processando tarefas e eventos...");
    normalizeDealsState();
    STATE.lastDealsSyncAt = new Date().toISOString();
    STATE.lastOkAt = new Date();
    STATE.offline = false;
    saveCache();
    setBootProgress(54, "Montando resumo do painel...");
  ...
  async function materializeRecurringRuleForUser(userId, rule, opts = {}) {
    ...
    for (const d of createdDates) {
      ...
      try {
        const missing = validateProposalRequiredFields(fields, fields.STAGE_ID);
  ...
  let BOOT_PROGRESS = 0;
  let BOOT_PROGRESS_TARGET = 0;
  let BOOT_PROGRESS_TIMER = null;
  let BOOT_PROGRESS_LABEL = "Carregando painel...";
  let BOOT_PROGRESS_PENDING_TASKS = 0;
  function beginBootPending(count = 1) {
    try { BOOT_PROGRESS_PENDING_TASKS += Math.max(0, Number(count)||0); } catch (_) {}
  }
  function endBootPending(label) {
    try {
      BOOT_PROGRESS_PENDING_TASKS = Math.max(0, BOOT_PROGRESS_PENDING_TASKS - 1);
      if (BOOT_PROGRESS_PENDING_TASKS <= 0) {
        setBootProgress(100, label || 'Painel carregado');
        clearBootProgress();
        if (el && el.meta) el.meta.textContent = `Atualizado: ${fmt(new Date())}`;
      } else {
        setBootProgress(96, label || 'Finalizando carregamento...');
      }
    } catch (_) {}
  }
  function setBootProgress(pct, label) {
    try {
      BOOT_PROGRESS_TARGET = Math.max(0, Math.min(100, Number(pct) || 0));
      if (label) BOOT_PROGRESS_LABEL = label;
      if (!BOOT_PROGRESS_TIMER) {
        BOOT_PROGRESS_TIMER = setInterval(() => {
          const diff = BOOT_PROGRESS_TARGET - BOOT_PROGRESS;
          if (Math.abs(diff) < 0.25) {
            BOOT_PROGRESS = BOOT_PROGRESS_TARGET;
          } else {
            const step = diff > 0 ? Math.max(0.6, diff * 0.12) : Math.min(-0.6, diff * 0.25);
            BOOT_PROGRESS = Math.max(0, Math.min(100, BOOT_PROGRESS + step));
          }
          if (el && el.meta) el.meta.textContent = `${Math.round(BOOT_PROGRESS)}% • ${BOOT_PROGRESS_LABEL || "Carregando painel..."}`;
          if (BOOT_PROGRESS === BOOT_PROGRESS_TARGET && BOOT_PROGRESS_TARGET >= 100) {
            clearInterval(BOOT_PROGRESS_TIMER);
  ...
  function clearBootProgress() {
    try {
      BOOT_PROGRESS_TARGET = 100;
      if (BOOT_PROGRESS_TIMER) clearInterval(BOOT_PROGRESS_TIMER);
      BOOT_PROGRESS_TIMER = null;
      BOOT_PROGRESS = 100;
      BOOT_PROGRESS_PENDING_TASKS = 0;
    } catch (_) {}
  }
  ...
  function renderRecurrenceLineEditor(index) {
    ...
    return `
      <div class="recurLine" data-row="${index}" style="display:grid;grid-template-columns:minmax(220px,2fr) 110px 130px minmax(240px,2fr) 150px 120px;gap:8px;align-items:start;border:1px solid rgba(0,0,0,.10);padding:10px;border-radius:14px;background:rgba(255,255,255,.78);margin-bottom:8px">
        ...
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">TIPO TAREFA</div>
        ...
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">PARÂMETROS</div>
        ...
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">AÇÃO</div>
  ...
  async function openRecurrenceBatchModalForUser(user) {
    openModal(`Nova recorrência em lote — ${user.name}`, `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px">
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">CRIAR FUTURAS</div>
        ...
        <div id="rbBatchCountBox">
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">QUANTIDADE MÁXIMA</div>
        ...
        <div id="rbUntilBox" style="display:none">
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">ATÉ A DATA</div>
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        const cur = (STATE.recurRulesByUser.get(String(user.userId)) || []).slice();
        const nextRules = cur.concat(builtRules);
        await saveRulesForUser(user.userId, nextRules);
        if (warn) { warn.style.display = 'block'; warn.textContent = 'Recorrências salvas com sucesso. Gerando futuras em segundo plano…'; }
        setTimeout(async () => {
          ...
          try { openRecurrenceManager(String(user.userId)); } catch (_) {}
          setTimeout(async () => {
            for (const rule of builtRules) {
              try {
                await materializeRecurringRuleForUser(user.userId, rule, { countLimit: batchModeVal === 'COUNT' ? batchCountVal : 999, untilDate: batchModeVal === 'UNTIL' ? untilDateVal : '' });
            ...
            try { await generateRecurringDealsWindow().catch(()=>{}); } catch (_) {}
            try { renderCurrentView(); } catch (_) {}
          }, 120);
        }, 20);
      } catch (e) {
...
function makeUserCard(u) {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    setBootProgress(5, "Preparando dados...");
    ...
    try {
      setBootProgress(24, "Carregando tarefas e eventos...");
      const fastOpenOnly = false;
      await loadDeals({ forceFull: true, openOnly: false, deferPhotos: true });
      setBootProgress(48, "Tarefas e eventos carregados...");
      ...
      if (opts.deferRecur) {
        ...
        setTimeout(async () => {
          try {
            ...
            if (opts.forceRecur || !STATE.recurRulesByUser.size || (later - LAST_RECUR_LOAD_AT > 10 * 60 * 1000)) {
              ...
              setBootProgress(72, "Regras de recorrência carregadas...");
          ...
          finally { endBootPending('Recorrências carregadas'); }
        }, 700);
      } else {
        ...
        if (opts.forceRecur || (now - LAST_RECUR_GEN_AT > 5 * 60 * 1000)) {
          await generateRecurringDealsWindow().catch(()=>{});
          setBootProgress(80, "Instâncias de recorrência geradas...");
      ...
      if (opts.deferLeads) {
        if (shouldRefreshLeads) {
          ...
          setTimeout(async () => {
            try {
              setBootProgress(86, "Carregando resumos e leads...");
              await loadLeadsSnapshotAllUsers({ force: !!forceNetwork });
              LAST_LEADS_REFRESH_AT = Date.now();
              overlayPendingSyncState();
              if (el.modalOverlay.style.display !== "flex") renderCurrentView();
            } catch (_) {}
            finally { endBootPending('Resumos carregados'); }
          }, 1200);
        }
      } else {
        if (shouldRefreshLeads) {
          try { setBootProgress(86, "Carregando resumos e leads..."); await loadLeadsSnapshotAllUsers({ force: !!forceNetwork }); LAST_LEADS_REFRESH_AT = Date.now(); } catch (_) {}
        }
        overlayPendingSyncState();
      }
      renderFooterPeople();
      DEALS_FAIL_STREAK = 0;
      if (BOOT_PROGRESS_PENDING_TASKS > 0) {
        setBootProgress(96, 'Concluindo carregamentos em segundo plano...');
      } else {
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalAmountNumber(deal) {
    const raw = deal && deal.OPPORTUNITY;
    if (typeof raw === 'number') return Number.isFinite(raw) ? raw : 0;
    const s = String(raw == null ? '' : raw).trim();
    if (!s) return 0;
    if (/^-?\d+(\.\d+)?$/.test(s)) return Number(s);
    return parseMoneyPtBr(s);
  }
  ...
  function parseMoneyPtBr(raw) {
    const s = String(raw == null ? '' : raw).trim();
    if (!s) return 0;
    const clean = s.replace(/\s+/g,'').replace(/R\$/gi,'').replace(/[^\d,.-]/g,'');
    if (!clean) return 0;
    ...
    if (lastComma >= 0 && lastDot >= 0) {
      if (lastComma > lastDot) {
        normVal = clean.replace(/\./g,'').replace(',', '.');
      } else {
        normVal = clean.replace(/,/g,'');
      }
    } else if (lastComma >= 0) {
      normVal = clean.replace(/\./g,'').replace(',', '.');
    } else if ((clean.match(/\./g) || []).length > 1) {
      const idx = clean.lastIndexOf('.');
      normVal = clean.slice(0, idx).replace(/\./g, '') + '.' + clean.slice(idx + 1);
    }
    const n = Number(normVal);
    return Number.isFinite(n) ? n : 0;
  }
  function proposalStageRank(stageNameOrId) {
    const name = inferProposalStageNameFromAny(proposalStageNameFromId(stageNameOrId) || stageNameOrId) || String(stageNameOrId || '');
    const arr = proposalStageOrder();
    const idx = arr.findIndex((nm) => nm === name);
    return idx >= 0 ? idx : 999;
  }
  function proposalStageRequiredMap() {
    return [
      { code: 'OPPORTUNITY', label: 'VALOR PRODUZIDO', stage: 'AGUARDANDO DOCUMENTOS' },
      { code: PROP_UF_ATIVACAO, label: 'DATA DA ATIVAÇÃO', stage: 'NEGÓCIOS FECHADOS' },
      { code: PROP_UF_IDADE, label: 'IDADE', stage: 'EM DIGITAÇÃO DE PROPOSTA' },
      { code: PROP_UF_BAIRRO, label: 'BAIRRO', stage: 'EM DIGITAÇÃO DE PROPOSTA' },
      { code: PROP_UF_PLANO, label: 'PLANO ESCOLHIDO', stage: 'EM DIGITAÇÃO DE PROPOSTA' },
      { code: PROP_UF_TELEFONE, label: 'TELEFONE', stage: 'EM DIGITAÇÃO DE PROPOSTA' },
      { code: PROP_UF_OPER_LEAD, label: 'OPERADORA DO LEAD', stage: 'EM DIGITAÇÃO DE PROPOSTA' },
      { code: PROP_UF_OPER_FECHADA, label: 'OPERADORA FECHADA', stage: 'EM DIGITAÇÃO DE PROPOSTA' },
      { code: DEAL_UF_POSSUI_PLANO, label: 'POSSUI PLANO', stage: 'EM DIGITAÇÃO DE PROPOSTA' },
      { code: PROP_UF_LEAD_FONTE, label: 'LEAD FONTE', stage: 'AGUARDANDO DOCUMENTOS' },
    ];
  }
  function validateProposalRequiredFields(dealLike, targetStage) {
    const meta = STATE.proposalFieldsMeta || {};
    const missing = [];
    const rank = proposalStageRank(targetStage || (dealLike && dealLike.STAGE_ID) || '');
    proposalFieldDefs().forEach((fd) => {
      const fm = meta[fd.code] || {};
      if (!(fm.isRequired || fm.required)) return;
      const v = dealLike ? dealLike[fd.code] : '';
      if (String(v == null ? '' : v).trim() === '' && !missing.includes(fd.label)) missing.push(fd.label);
    });
    proposalStageRequiredMap().forEach((req) => {
      if (rank < proposalStageRank(req.stage)) return;
      const v = dealLike ? dealLike[req.code] : '';
      if (String(v == null ? '' : v).trim() === '' && !missing.includes(req.label)) missing.push(req.label);
    });
    return missing;
  }
  function proposalMatchesKeyword(deal, keyword) {
    const q = norm(String(keyword || ''));
    if (!q) return true;
    const vals = [deal && deal.TITLE, deal && deal.ID, deal && deal[PROP_UF_TELEFONE], deal && deal[PROP_UF_OPER_LEAD], deal && deal[PROP_UF_OPER_FECHADA], deal && deal[PROP_UF_PLANO], proposalStageLabelForDeal(deal), dealUserNameByAssigned(deal && deal.ASSIGNED_BY_ID)];
    return vals.some((v) => norm(String(v || '')).includes(q));
  }
  function proposalDealsCacheGet(uid) {
    const rec = STATE.proposalDealsByUser.get(String(uid || ''));
    if (!rec) return null;
    if ((Date.now() - Number(rec.at || 0)) > 300000) return null;
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const stale = (Date.now() - Number(STATE.proposalLastLoadedAt || 0)) > 120000;
    if (!force) {
    ...
    await ensureProposalMeta(false).catch(() => {});
    const select = [
      ...
      PROP_UF_TELEFONE, PROP_UF_LEAD_FONTE, PROP_UF_OPER_LEAD, PROP_UF_OPER_FECHADA, PROP_UF_LEAD_ORIGEM, PROP_UF_DATAHORA_LEAD, DEAL_UF_POSSUI_PLANO
    ];
    const allowedStageIds = proposalStageOrder().map((nm) => String(STATE.proposalStageIdByName.get(norm(nm)) || '')).filter(Boolean);
    const baseFilter = {};
    if (uid) baseFilter.ASSIGNED_BY_ID = Number(uid);
    baseFilter.CATEGORY_ID = Number(CATEGORY_PROPOSALS);

    let rows = [];
    if (allowedStageIds.length) {
      const chunks = [];
      const maxParallel = uid ? 4 : 3;
      for (let i = 0; i < allowedStageIds.length; i += maxParallel) chunks.push(allowedStageIds.slice(i, i + maxParallel));
      for (const chunk of chunks) {
        const partial = await Promise.all(chunk.map((stageId) => bxAll('crm.deal.list', {
          filter: { ...baseFilter, STAGE_ID: String(stageId) },
          order: { ID: 'DESC' },
          select,
        }).catch(() => [])));
        partial.forEach((arr) => { if (Array.isArray(arr)) rows.push(...arr); });
      }
    } else {
      rows = await bxAll('crm.deal.list', {
        filter: baseFilter,
        order: { ID: 'DESC' },
        select,
      }).catch(() => []);
    ...
    if (uid) {
      ...
      if (Array.isArray(STATE.proposalDealsAll)) {
        const keep = (STATE.proposalDealsAll || []).filter((d) => String(d.ASSIGNED_BY_ID || '') !== uid);
        STATE.proposalDealsAll = [...mapped, ...keep];
      }
      return mapped;
    }
  ...
  function proposalFieldInputHtml(fd, deal) {
    ...
    if (fd.code === 'OPPORTUNITY') return `<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">${escHtml(label)}</div><input data-propfield="${escHtml(fd.code)}" type="number" step="0.01" value="${escHtml(String(value || ''))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>`;
    if (fd.kind === 'datetime') {
      let vv = String(value || '').trim();
      if (vv && vv.includes('T')) vv = vv.slice(0,16);
      return `<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">${escHtml(label)}</div><input data-propfield="${escHtml(fd.code)}" type="datetime-local" value="${escHtml(vv)}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>`;
    }
    if (items.length) {
      ...
      return `<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">${escHtml(label)}</div><select data-propfield="${escHtml(fd.code)}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">${opts.join('')}</select></div>`;
    }
    return `<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">${escHtml(label)}</div><input data-propfield="${escHtml(fd.code)}" value="${escHtml(String(value || ''))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>`;
  }

  function proposalSummaryRows(deal) {
    return proposalFieldDefs().map((fd) => {
      const txt = fd.code === 'OPPORTUNITY' ? formatBRL(proposalAmountNumber(deal)) : proposalFieldValueLabel(fd.code, deal ? deal[fd.code] : '');
      if (!txt || txt === '—') return '';
      return `<div style="font-size:11px;line-height:1.35"><strong>${escHtml(fd.label)}:</strong> ${escHtml(trunc(String(txt), 84))}</div>`;
    }).filter(Boolean).join('');
  }
  ...
  function renderProposalCard(deal, opts = {}) {
    ...
    const ownerName = dealUserNameByAssigned(deal.ASSIGNED_BY_ID);
    const rows = proposalSummaryRows(deal);
    return `
      <div class="eqd-card" style="border-left:4px solid ${stageColor};padding:6px 8px;border-radius:12px">
        <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:start">
          <div style="min-width:0">
            <div style="display:flex;gap:5px;align-items:center;flex-wrap:wrap;margin-bottom:2px">
              <span class="eqd-tag" style="background:${stageColor};color:#fff;border-color:${stageColor};padding:2px 7px">${escHtml(stageName || 'Sem stage')}</span>
              <span class="eqd-tag" style="padding:2px 7px">${escHtml(ownerName)}</span>
              <span class="eqd-tag" style="padding:2px 7px"><strong>${escHtml(formatBRL(amount))}</strong></span>
            </div>
            <div style="font-weight:950;font-size:12px;line-height:1.15">${escHtml(bestTitleFromText(deal.TITLE || ''))}</div>
          </div>
          <div style="display:flex;gap:5px;flex-wrap:wrap;justify-content:flex-end;align-items:center">
            <select class="proposal-inline-stage" data-id="${escHtml(String(deal.ID))}" style="padding:5px 7px;border-radius:10px;border:1px solid rgba(30,40,70,.16);font-weight:900;max-width:200px">${proposalCardStageOptions(deal.STAGE_ID)}</select>
            ...
            <button class="eqd-btn" data-action="proposalTask" data-id="${escHtml(String(deal.ID))}" data-userid="${escHtml(String(opts.userId || deal.ASSIGNED_BY_ID || ''))}">Tarefa</button>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(135px,1fr));gap:3px 6px;margin-top:5px">${rows}</div>
      </div>`;
  }


  async function openProposalTaskModal(dealId, returnCtx = {}) {
    const deal = findProposalDealById(String(dealId));
    if (!deal) throw new Error('Proposta não encontrada.');
    const uid = String(deal.ASSIGNED_BY_ID || returnCtx.userId || '');
    const owner = USERS.find((u) => String(u.userId) === uid);
    if (!owner) throw new Error('User não encontrada.');
    const dt0 = new Date();
    dt0.setMinutes(dt0.getMinutes() + 60);
    const localDefault = new Date(dt0.getTime() - dt0.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
    openModal(`Nova tarefa vinculada — ${bestTitleFromText(deal.TITLE || '')}`, `
      <div class="eqd-warn" id="proposalTaskWarn"></div>
      <div style="font-size:12px;font-weight:900;opacity:.82;margin-bottom:10px">A tarefa será criada no painel da ${escHtml(owner.name)} com referência a esta proposta.</div>
      <div style="display:grid;grid-template-columns:1fr 220px;gap:10px;align-items:end">
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">NOME DA TAREFA</div><input id="proposalTaskTitle" value="Consultar proposta — ${escAttr(bestTitleFromText(deal.TITLE || ''))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">PRAZO</div><input id="proposalTaskPrazo" type="datetime-local" value="${localDefault}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">OBS</div><textarea id="proposalTaskObs" rows="4" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px;font-weight:900;outline:none">Proposta vinculada: ${bestTitleFromText(deal.TITLE || '')}
ID proposta: ${String(deal.ID || '')}
Valor produzido: ${formatBRL(proposalAmountNumber(deal))}
Stage atual: ${proposalStageLabelForDeal(deal)}
Telefone: ${String(deal[PROP_UF_TELEFONE] || '')}
Operadora fechada: ${proposalFieldValueLabel(PROP_UF_OPER_FECHADA, deal[PROP_UF_OPER_FECHADA])}</textarea></div>
      </div>
      <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:12px"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="proposalTaskSave">Criar tarefa</button></div>
    `, { wide:true });
    const btn = document.getElementById('proposalTaskSave');
    const warn = document.getElementById('proposalTaskWarn');
    btn.onclick = async () => {
      btn.disabled = true;
      try {
        const stageId = await stageIdForUserName(owner.name);
        if (!stageId) throw new Error(`Não encontrei a coluna ${owner.name} na pipeline.`);
        const title = String((document.getElementById('proposalTaskTitle') || {}).value || '').trim();
        const prazoLocal = String((document.getElementById('proposalTaskPrazo') || {}).value || '').trim();
        const prazoIso = localInputToIsoWithOffset(prazoLocal);
        const obs = String((document.getElementById('proposalTaskObs') || {}).value || '').trim();
        if (!title) throw new Error('Preencha o nome da tarefa.');
        if (!prazoIso) throw new Error('Prazo inválido.');
        const fields = { CATEGORY_ID: Number(CATEGORY_MAIN), STAGE_ID: String(stageId), TITLE: title, ASSIGNED_BY_ID: Number(uid), [UF_PRAZO]: prazoIso, [UF_OBS]: `${obs}${obs ? '\n' : ''}[PROPOSTA_ID=${String(deal.ID||'')}]` };
        const created = await bx('crm.deal.add', { fields });
        const realId = String((created && (created.result || created)) || '').trim() || makeTempId('TMP_DEAL');
        upsertDealLocal(parseLocalDealFromFields(realId, fields));
        closeModal();
  ...
  async function openProposalStageMoveModal(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const missing = validateProposalRequiredFields(deal, nextStage);
        if (missing.length) throw new Error('Preencha os campos obrigatórios para este stage: ' + missing.join(', '));
        ...
        closeModal();
        return openProposalsModal(String(returnCtx.userId || deal.ASSIGNED_BY_ID || ''), { ...(LAST_PROPOSALS_CTX||returnCtx), noStack: true });
      } catch (e) {
        warn.style.display = 'block';
        warn.textContent = 'Falha:\n' + (e.message || e);
      } finally { btn.disabled = false; }
  ...
  async function openProposalEditor(dealId, returnCtx = {}) {
    ...
    openModal(`Editar proposta — ${bestTitleFromText(deal.TITLE || '')}`, `
      ...
      <div style="font-size:12px;font-weight:900;opacity:.82;margin-bottom:10px">Stage atual: <strong>${escHtml(stageName)}</strong>. Campos com * são os obrigatórios gerais do campo. Este painel também valida as obrigatoriedades por stage configuradas para PROPOSTAS antes de salvar ou mover stage.</div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px">
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">STAGE</div><select id="proposalStageSel" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">${proposalStageOrder().map((nm) => `<option value="${escHtml(STATE.proposalStageIdByName.get(norm(nm)) || '')}" ${String(STATE.proposalStageIdByName.get(norm(nm)) || '')===String(deal.STAGE_ID || '')?'selected':''}>${escHtml(nm)}</option>`).join('')}</select></div>
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const missing = validateProposalRequiredFields({ ...deal, ...patch }, patch.STAGE_ID || deal.STAGE_ID);
        if (missing.length) throw new Error('Preencha os campos obrigatórios: ' + missing.join(', '));
  ...
  function getProposalsCtxKey(ctx) {
    return JSON.stringify({ u: String(ctx.userId||''), a: !!ctx.adminView, fu: String(ctx.filterUserId||''), cm: String(ctx.closedMonth||''), rf: String(ctx.rankField||'OPPORTUNITY') });
  }
  ...
  function renderProposalsModalContent(sourceDeals, opts = {}) {
    ...
    const viewMode = String(opts.viewMode || 'LIST').toUpperCase() === 'KANBAN' ? 'KANBAN' : 'LIST';
    const keyword = String(opts.keyword || '').trim();
    let filteredDeals = filterUid ? (sourceDeals || []).filter((d) => String(d.ASSIGNED_BY_ID || '') === filterUid) : (sourceDeals || []).slice();
    filteredDeals = filteredDeals.filter((d) => proposalMatchesKeyword(d, keyword));
    const availableClosedMonths = [...new Set(filteredDeals.filter((d) => proposalStageLabelForDeal(d) === 'NEGÓCIOS FECHADOS').map(proposalActivationMonthKey).filter(Boolean))].sort().reverse();
    const currentMonth = (() => { const n = new Date(); return `${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,'0')}`; })();
    const closedMonth = String(opts.closedMonth || (availableClosedMonths.includes(currentMonth) ? currentMonth : ''));
    ...
    const gridStyle = viewMode === 'KANBAN'
      ? 'display:grid;grid-template-columns:repeat(9,minmax(160px,1fr));gap:8px;align-items:start;overflow:auto;padding-bottom:6px'
    ...
    return `
      ...
      <div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:10px">
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ...
          <span class="eqd-tag">Fechados${closedMonth ? ' (' + proposalMonthLabel(closedMonth) + ')' : ''}: <strong>${escHtml(formatBRL(closedTotal))}</strong></span>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ${adminView ? `<select id="proposalAdminUserFilter" style="padding:9px 10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">${filterOptions}</select>` : ``}
          ${adminView ? `<select id="proposalRankField" style="padding:9px 10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900">${rankOptions}</select>` : ``}
          <input id="proposalKeyword" value="${escAttr(keyword)}" placeholder="Buscar por nome, telefone, operadora..." style="min-width:240px;padding:9px 10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" />
          <select id="proposalViewMode" style="padding:9px 10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="LIST" ${viewMode==='LIST'?'selected':''}>Lista</option><option value="KANBAN" ${viewMode==='KANBAN'?'selected':''}>Kanban</option></select>
      ...
      <div style="${gridStyle}">${grouped.map((g) => {
        const stageTotal = g.deals.reduce((s,d) => s + proposalAmountNumber(d), 0);
        const closedToolbar = g.name === 'NEGÓCIOS FECHADOS' ? `<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><label style="font-size:12px;font-weight:900">Competência</label><select id="proposalClosedMonth" style="padding:7px 10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Todas</option>${availableClosedMonths.map((mk) => `<option value="${escHtml(mk)}" ${mk===closedMonth?'selected':''}>${escHtml(proposalMonthLabel(mk))}</option>`).join('')}</select></div>` : '';
        const cardsHtml = g.deals.length ? g.deals.map((d) => renderProposalCard(d, { userId: d.ASSIGNED_BY_ID })).join('') : `<div class="eqd-empty">Sem contratos.</div>`;
        const inner = viewMode === 'KANBAN'
          ? `<div style="display:flex;flex-direction:column;gap:6px;max-height:calc(100vh - 250px);overflow:auto">${cardsHtml}</div>`
          : `<div style="display:flex;flex-direction:column;gap:6px">${cardsHtml}</div>`;
        return `<div style="border:1px solid rgba(0,0,0,.1);border-radius:14px;padding:8px;background:rgba(255,255,255,.84);min-width:0"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:6px"><div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><span style="font-weight:950;color:${proposalStageBadgeColor(g.name)}">${escHtml(g.name)}</span><span class="eqd-tag">${g.deals.length} card(s)</span><span class="eqd-tag">${escHtml(formatBRL(stageTotal))}</span></div>${closedToolbar}</div>${inner}</div>`;
      }).join('')}</div>`;
  }
  ...
  function renderProposalChart(rows) {
    ...
    return rows.map((r) => {
      ...
      return `<div style="display:grid;grid-template-columns:180px 1fr 70px;gap:8px;align-items:center;margin:6px 0"><div style="font-size:12px;font-weight:900">${escHtml(trunc(r.label, 22))}</div><div style="height:12px;background:rgba(0,0,0,.08);border-radius:999px;overflow:hidden"><div style="height:100%;width:${w}%;background:rgba(37,99,235,.85)"></div></div><div style="font-size:12px;font-weight:900;text-align:right">${escHtml(fdMoney('OPPORTUNITY') ? formatBRL(r.value) : String(r.value))}</div></div>`;
    }).join('');
  }
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    const baseCtx = { userId: uid, adminView, filterUserId: String(opts.filterUserId || ''), rankField: String(opts.rankField || 'OPPORTUNITY'), closedMonth: String(opts.closedMonth || ''), keyword: String(opts.keyword || ''), viewMode: String(opts.viewMode || 'LIST') };
    LAST_PROPOSALS_CTX = baseCtx;

    const immediateRows = adminView
      ? collectAllProposalDeals().filter((d) => proposalDealAllowed(d))
      : (proposalDealsCacheGet(uid) || []).filter((d)=>proposalDealAllowed(d));

    openModal(title, immediateRows.length
      ? renderProposalsModalContent(immediateRows, LAST_PROPOSALS_CTX)
      : `<div style="padding:18px;font-weight:900">Carregando propostas...</div>`, { full: true, noStack: true });

    ensureProposalMeta(false).catch(()=>{});

    let sourceDeals = immediateRows;
    if (!sourceDeals.length || !!opts.forceFresh) {
      sourceDeals = adminView ? await loadProposalDeals(true, {}) : await loadProposalDeals(true, { userId: uid });
      sourceDeals = (sourceDeals || []).filter((d) => proposalDealAllowed(d));
      if (!sourceDeals.length && el.modalBody) el.modalBody.innerHTML = `<div style="padding:18px;font-weight:900">Nenhuma proposta encontrada para este filtro.</div>`;
      if (el.modalTitle && String(el.modalTitle.textContent||'').trim() === title && el.modalBody) {
        el.modalBody.innerHTML = renderProposalsModalContent(sourceDeals, LAST_PROPOSALS_CTX);
      } else {
    ...
    const applyBindings = () => {
      const getCtx = () => ({
        ...
        closedMonth: String((document.getElementById('proposalClosedMonth') || {}).value || ''),
        keyword: String((document.getElementById('proposalKeyword') || {}).value || ''),
      ...
      if (apply) apply.onclick = () => openProposalsModal(uid, getCtx());
      const closedSel = document.getElementById('proposalClosedMonth');
      if (closedSel) closedSel.onchange = () => openProposalsModal(uid, getCtx());
      const kw = document.getElementById('proposalKeyword');
      if (kw) kw.onkeydown = (ev) => { if (ev.key === 'Enter') openProposalsModal(uid, getCtx()); };
      document.querySelectorAll('.proposal-inline-stage').forEach((sel) => {
        sel.onchange = async () => {
          ...
          const missing = validateProposalRequiredFields(prev || {}, nextStage);
          if (missing.length) {
            sel.value = String((prev && prev.STAGE_ID) || '');
            const warn = document.getElementById('proposalWarn');
            if (warn) { warn.style.display = 'block'; warn.textContent = 'Preencha antes de mover: ' + missing.join(', '); }
            return;
          }
    ...
    const hasFreshEnough = adminView
      ? (Array.isArray(STATE.proposalDealsAll) && STATE.proposalDealsAll.length && (Date.now() - Number(STATE.proposalLastLoadedAt || 0)) < 120000)
      : !!proposalDealsCacheGet(uid);

    if (!hasFreshEnough || !!opts.forceFresh) {
      setTimeout(async () => {
        try {
          ...
          if (el.modalTitle && String(el.modalTitle.textContent||'').trim() === title && el.modalBody) el.modalBody.innerHTML = renderProposalsModalContent(freshRows, LAST_PROPOSALS_CTX);
          else openModal(title, renderProposalsModalContent(freshRows, LAST_PROPOSALS_CTX), { full: true, noStack: true });
          applyBindings();
        } catch (_) {}
      }, 60);
    }
  ...
  async function transferRecurringRuleToUser(fromUserId, ruleId, toUserId) {
    const fromUid = String(fromUserId || '');
    const toUid = String(toUserId || '');
    const rid = String(ruleId || '');
    if (!fromUid || !toUid || !rid) throw new Error('Dados inválidos para transferência.');
    if (fromUid === toUid) return 0;
    const fromRules = (STATE.recurRulesByUser.get(fromUid) || []).slice();
    const rule = fromRules.find((r) => String(r.id) === rid);
    if (!rule) throw new Error('Regra não encontrada.');
    const toRules = (STATE.recurRulesByUser.get(toUid) || []).slice();
    if (toRules.some((r) => String(r.id) === rid)) rule.id = makeRuleId();
    await saveRulesForUser(fromUid, fromRules.filter((r) => String(r.id) !== rid));
    await saveRulesForUser(toUid, [...toRules, { ...rule }]);
    const toUser = USERS.find((u) => String(u.userId) === toUid);
    const stageId = toUser ? await stageIdForUserName(toUser.name) : '';
    const todayKey = ymdKey(new Date());
    const targets = (STATE.dealsAll || []).filter((d) => {
      const obs = String(d[UF_OBS] || d._obs || '');
      const m = obs.match(/\[RUID=([^:\]]+):(\d{4}-\d{2}-\d{2})\]/);
      return !!(m && String(m[1]) === rid && String(m[2]) >= todayKey);
    });
    for (const d of targets) {
      const patch = { ASSIGNED_BY_ID: Number(toUid) };
      if (stageId) patch.STAGE_ID = String(stageId);
      try { await safeDealUpdate(String(d.ID), patch); } catch(_) {}
      Object.assign(d, patch, { _assigned: String(toUid) });
    }
    return targets.length;
  }
  ...
  function openRecurrenceManager(userId){
    ...
    if (STATE.recurManagerShownForUser === uid && (nowTs - Number(STATE.recurManagerShownAt || 0)) < 800) return;
    STATE.recurManagerShownForUser = uid;
    STATE.recurManagerShownAt = nowTs;
    const user = USERS.find(u => String(u.userId) === uid);
    if (!user) return;
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "proposalTask") { return openProposalTaskModal(String(id||""), { userId: String(userId||"") }); }
      ...
      if (act === "recurManager") {
        ...
        const uid = String(userId||"");
        if (!uid) return;
        if (STATE.recurModalOpeningForUser === uid || (STATE.recurManagerShownForUser === uid && (Date.now() - Number(STATE.recurManagerShownAt||0)) < 1500)) return;
        STATE.recurModalOpeningForUser = uid;
        openRecurrenceManager(uid);
        setTimeout(async () => {
          try {
            await loadRecurrenceConfigDeals().catch(()=>{});
            await loadRecurrenceConfigForUser(uid).catch(()=>{});
            if (STATE.recurModalOpeningForUser === uid) openRecurrenceManager(uid);
          } finally {
            if (STATE.recurModalOpeningForUser === uid) STATE.recurModalOpeningForUser = "";
          }
        }, 20);
        return;
      }
      ...
      if (act === "recurTransfer") {
        ...
        const rid = String(a.getAttribute("data-ruleid")||"");
        if (!uid || !rid) return;
        const options = USERS.filter((u) => String(u.userId) !== uid).map((u) => `<option value="${escHtml(String(u.userId))}">${escHtml(u.name)}</option>`).join('');
        openModal('Transferir recorrência', `
          <div class="eqd-warn" id="recurTransferWarn"></div>
          <div style="font-size:12px;font-weight:900;opacity:.82;margin-bottom:8px">Selecione a USER que vai receber essa recorrência e todos os cards futuros dela.</div>
          <select id="recurTransferUserSel" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Selecione a USER…</option>${options}</select>
          <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:12px"><button class="eqd-btn" id="recurTransferCancel">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="recurTransferGo">Transferir</button></div>
        `, { wide: true });
        const cancel = document.getElementById('recurTransferCancel');
        if (cancel) cancel.onclick = () => openRecurrenceManager(uid);
        const go = document.getElementById('recurTransferGo');
        const warn = document.getElementById('recurTransferWarn');
        if (go) go.onclick = async () => {
          const toUid = String((document.getElementById('recurTransferUserSel') || {}).value || '').trim();
          if (!toUid) { if (warn) { warn.style.display = 'block'; warn.textContent = 'Selecione a USER de destino.'; } return; }
          setBusy('Transferindo recorrência…');
          try {
            await transferRecurringRuleToUser(uid, rid, toUid);
            clearBusy();
            return openRecurrenceManager(toUid);
          } catch (e) {
            clearBusy();
            if (warn) { warn.style.display = 'block'; warn.textContent = 'Falha: ' + (e.message || e); }
          }
