...
restoreSyncQueue();
  ...
  function makeMarketingCard() {
    return `
      <div class="userCard" data-action="openMktPanel">
        ...
        <div class="userPhotoWrap">
          <img class="userPhoto" src="${MARKETING_LOGO_URL}" alt="Marketing" referrerpolicy="no-referrer" style="width:100%;height:100%;object-fit:cover;object-position:center;transform:scale(1.18)" onerror="try{this.onerror=null;this.src='';this.style.display='none'}catch(e){}" />
        </div>
        <div class="userRight">
  ...
  function openMarketingPanelStub(selectedDateStr = "") {
    ...
    const tipoMap = STATE.enumCache.get(UF_TAREFA) || new Map();
    ...
    const resolveTypeText = (d) => {
      ...
      return allowedTypes.find((tt) => normType(tt) === n) || raw || '';
    };
    ...
    if (createBtn) createBtn.onclick = async () => {
      ...
      const tipoItems = (meta ? getFieldItemsFromMeta(meta, UF_TAREFA) : []).filter((x) => allowedTypes.some((tt) => normType(tt) === normType(x.text || '')));
