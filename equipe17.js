...
(function () {
  ...
  async function safeDealUpdate(dealId, fields) {
    ...
    try {
      ...
      if (fields && Object.prototype.hasOwnProperty.call(fields, UF_PRAZO)) {
        try {
          const fresh = await bx("crm.deal.get", { id });
          if (fresh && Object.prototype.hasOwnProperty.call(fresh, UF_PRAZO)) {
            updateDealInState(id, { [UF_PRAZO]: fresh[UF_PRAZO], DATE_MODIFY: fresh.DATE_MODIFY || new Date().toISOString() });
          }
        } catch (_) {}
      }
      return res;
...
restoreSyncQueue();
  ...
  function stripQueuedDealFields(dealId, fieldNames) {
    const id = String(dealId || '');
    const keys = new Set((fieldNames || []).map((k) => String(k || '')).filter(Boolean));
    if (!id || !keys.size) return;
    const kept = [];
    for (const job of (SYNC_QUEUE || [])) {
      if (!job || job.type !== 'dealUpdate' || String(job.dealId || '') !== id || !job.fields) {
        kept.push(job);
        continue;
      }
      const nextFields = { ...(job.fields || {}) };
      for (const k of keys) delete nextFields[k];
      if (Object.keys(nextFields).length) kept.push({ ...job, fields: nextFields });
    }
    SYNC_QUEUE.length = 0;
    kept.forEach((j) => SYNC_QUEUE.push(j));
    persistSyncQueue();
  }
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (existingFutureDeal) {
          stripQueuedDealFields(String(existingFutureDeal.ID), [UF_PRAZO]);
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          stripQueuedDealFields(String(dealId), [UF_PRAZO]);
