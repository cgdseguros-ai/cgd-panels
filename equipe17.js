...
restoreSyncQueue();
  ...
  function openFollowupListModalForUser(user) {
    ...
    openModal(`Lista de FOLLOW-UP — ${user.name}`, `
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:space-between">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <input id="fuListOper" class="eqd-searchInput" style="width:min(180px,34vw);color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Operadora" />
    ...
    const applyQuickDayFilter = (offsetDays) => {
      ...
      currentDateFilter = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
    ...
    const render = (kwRaw) => {
      ...
      currentDateFilter = String(document.getElementById('fuListDate')?.value || currentDateFilter || '').trim();
    ...
    document.getElementById("fuListClear").onclick = () => { document.getElementById("fuListSearch").value = ""; if (fuListOperEl) fuListOperEl.value=''; if (fuListDateEl) fuListDateEl.value=''; currentDateFilter=''; currentOperFilter=''; render(""); };
    ...
    const pickFollowListByMode = (n) => {
      const pickRecent = window.confirm(`OK = selecionar ${n} mais recentes por Atendido em.
Cancelar = selecionar ${n} mais antigos.`);
    ...
    const s10 = document.getElementById("fuListSelect10"); if (s10) s10.onclick = () => pickFollowListByMode(10);
    const s20 = document.getElementById("fuListSelect20"); if (s20) s20.onclick = () => pickFollowListByMode(20);
