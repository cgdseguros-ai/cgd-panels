...
restoreSyncQueue();
  ...
  async function openAdminLinkFupModal(adminUserId, selectedUid = '', preloadedOrphans = null) {
    ...
    const fillSelect = async (dealId, idx, kw='') => {
      ...
      const rows = getLikelyLeadCandidatesForFollowup(deal, { kw }).slice(0, 60);
      sel.innerHTML = `<option value="">Selecione…</option>` + (rows.length
        ? rows.map((l) => `<option value="${escHtml(String(l.ID))}">${escHtml(leadTitle(l))} ${countOpenFutureFollowupsForLeadOrig(leadOrigemId(l)||l.ID, dealId) ? '• já tem FUP' : '• sem FUP'}</option>`).join('')
