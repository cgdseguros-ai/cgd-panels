...
restoreSyncQueue();
    ...
    try {
      ...
      try {
        ...
        for (const [, m] of PENDING_MUTATIONS) {
          if (m && m.status === 'pending' && Number(m.expiresAt || 0) > n) pendingMut++;
        }
      } catch (_) {}
  ...
  function openSelectRecentOldModal(n, onPick) {
    openModal(`Selecionar ${n}`, `<div style="display:flex;flex-direction:column;gap:12px"><div style="font-size:13px;font-weight:950;opacity:.86">Escolha como deseja selecionar os follow-ups.</div><div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap"><button class="eqd-btn" id="pickOldBtn">MAIS PRÓXIMOS</button><button class="eqd-btn eqd-btnPrimary" id="pickRecentBtn">MAIS DISTANTES</button></div></div>`, { allowStack:true, noStack:true });
...
function makeUserCard(u) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (failures.length) {
          ...
          showDiscreetToast(`Reagendamento em lote concluído. Itens processados: ${successes.length}.`);
        }
      } catch (e) {
  ...
  async function confirmRemoteEntityFields(entityType, id, expectedFields, opts = {}) {
    ...
    const method = String(entityType) === "lead" ? "crm.lead.get" : "crm.deal.get";
  ...
  async function rescheduleDealAtomic(oldDealId, newFields, originalDeal) {
    ...
    try {
      ...
      if (realId) registerLocalDealPatch(String(realId), { ...newFields, _pendingNewDeal: false, _rescheduledFrom: oldId }, 120000);
