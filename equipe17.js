...
function openMarketingPanelStub() {
  ...
  openModal("Painel MKT — implantação", `<div style="display:grid;grid-template-columns:minmax(300px,42%) 1fr;gap:12px;min-height:70vh"><div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:12px"><div style="font-weight:950">Calendário MKT</div><div style="font-size:12px;opacity:.78">Estrutura inicial do painel coletivo.</div><img src="${MARKETING_LOGO_URL}" style="width:96px;height:96px;object-fit:contain;border-radius:16px;background:#fff;border:1px solid rgba(0,0,0,.08);padding:6px" /><div style="font-size:12px;font-weight:900">Participantes</div><div style="display:flex;flex-wrap:wrap;gap:6px">${members.map((u) => `<span class="eqd-tag">${escHtml(u.name)}</span>`).join('')}</div><div style="font-size:12px;font-weight:900">Tipos permitidos</div><div style="display:flex;flex-wrap:wrap;gap:6px">${typeTags.map((x)=>`<span class="eqd-tag">${escHtml(x)}</span>`).join('')}</div><div style="font-size:12px;opacity:.78">Próximo passo: criação de deal MKT com múltiplos participantes e calendário clicável.</div></div><div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:10px"><div style="font-weight:950">Agenda MKT</div><div style="font-size:12px;opacity:.78">A estrutura visual do painel foi iniciada. Nesta etapa ainda não filtra deals exclusivos de MKT.</div><div style="border:1px dashed rgba(0,0,0,.18);border-radius:16px;min-height:220px;display:flex;align-items:center;justify-content:center;font-size:12px;opacity:.6">Calendário vertical e lista cronológica entram na próxima rodada</div></div></div>`, { full:true, wide:true });
}
