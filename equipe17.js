...
${curObs}` : transferAlert;
  ...
  function makeDealCard(deal, context) {
    ...
    if (!isFollowupDeal(deal)) {
      const proposalMeta = String(deal._obs || deal[UF_OBS] || '').trim();
      if (/CONTRATO\/DEAL:/i.test(proposalMeta) || /STAGE DA PROPOSTA:/i.test(proposalMeta)) {
        const lines = proposalMeta.split(/\r?\n/).map((x) => String(x || '').trim()).filter(Boolean);
        const wanted = ['CONTRATO/DEAL:', 'STAGE DA PROPOSTA:', 'VALOR PRODUZIDO:', 'DATA DA ATIVAÇÃO:', 'TELEFONE:', 'OPERADORA FECHADA:', 'PLANO ESCOLHIDO:'];
        const kept = lines.filter((ln) => wanted.some((w) => ln.startsWith(w))).slice(0, 7);
        if (kept.length) extraMetaInside += `<div style="font-size:11px;font-weight:900;opacity:.82;display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:4px 8px">${kept.map((ln) => {
          const idx = ln.indexOf(':');
          const label = idx >= 0 ? ln.slice(0, idx) : ln;
          const val = idx >= 0 ? ln.slice(idx + 1).trim() : '';
          return `<span><strong>${escHtml(label)}:</strong> ${escHtml(val)}</span>`;
        }).join('')}</div>`;
      }
    }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalMetaPayloadFromDeal(deal) {
    return {
      ...
      ativacao: proposalFieldValueLabel(PROP_UF_ATIVACAO, deal && deal[PROP_UF_ATIVACAO]),
