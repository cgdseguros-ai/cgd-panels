...
(function () {
  ...
  injectCSS(`
    ...
    .leadIconBtns{display:flex;gap:6px;align-items:center;justify-content:flex-start;margin-top:8px}
    ...
    const cards = visibleRows.length ? visibleRows.map((l) => `
      <div class="eqd-waitingLeadCard">
        ...
        <div style="display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap;margin-top:10px">
          <button class="eqd-btn" data-action="leadEdit" data-userid="${uid}" data-leadid="${escHtml(String(l.ID || ""))}">EDITAR</button>
...
restoreSyncQueue();
  ...
  function makeDealListRow(deal, context) {
    ...
    const actions = isFollowupDeal(deal)
      ? `${linkedLeadOrig ? `` : `<button class="eqd-smallBtn" data-action="linkFollowup" data-id="${deal.ID}">Vincular</button>`}<button class="eqd-smallBtn" data-action="reagendarDireto" data-id="${deal.ID}">Reagendar</button><button class="eqd-smallBtn eqd-smallBtnDanger" data-action="followupPerdido" data-id="${deal.ID}">Perdido</button>${leadIsAtendido ? `<button class="eqd-smallBtn eqd-smallBtnDanger" data-action="followupSemResposta" data-id="${deal.ID}">SEM RESPOSTA</button>` : ``}<button class="eqd-smallBtn eqd-smallBtnDanger leadBtnIcon" title="Excluir" aria-label="Excluir" data-action="delete" data-id="${deal.ID}">❌</button><button class="eqd-smallBtn eqd-smallBtnPrimary leadBtnIcon" title="Converter" aria-label="Converter" data-action="followupConverter" data-id="${deal.ID}">✅</button>`
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    openModal(`Leads — ${user.name}`, `
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;font-size:12px;font-weight:950">
        <span class="eqd-pill" style="background:#fff;color:#111;border-color:rgba(0,0,0,.12)">Próximos de vender: <strong style="margin-left:4px">${q.length}</strong></span>
    ...
    function cardLead(l, column) {
      ...
      const baseBtns = `${mkBtn("PERDIDO",sPerdido)}${column !== "AT" ? mkBtn("CONVERTER",sConv) : ``}${semRespostaBtn}<button class="leadBtn" data-action="leadTransferModal" data-leadid="${l.ID}" data-userid="${user.userId}">TRANSFERIR</button>${column !== "AT" ? `<button class="leadBtn" data-action="leadFollowupModal" data-leadid="${l.ID}" data-userid="${user.userId}">FUP</button><button class="leadBtn" data-action="leadRevalidate" data-leadid="${l.ID}" data-userid="${user.userId}">+60 DIAS</button>` : ``}`;
      const iconBtns = `${mkIcon('❌','Excluir','leadDelete','', 'leadBtnD')}${mkIcon('🖊️','Editar','leadEdit')}${column !== "AT" ? mkIcon('✅','Converter','leadMove', `data-tostatus="${escHtml(sConv||'')}"`, 'leadBtnP') : ``}`;
      const btns = baseBtns;

      return `
        <div class="leadCard" data-lead-col="${escHtml(column)}" data-fonte="${escHtml(norm(fonte))}" data-operadora="${escHtml(norm(op))}">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
            <div class="leadTitle">${escHtml(leadTitle(l))}${leadStageLabelChipHtml(l)}</div>
            ${obsChip}
          </div>
          ...
          <div style="display:flex;justify-content:space-between;gap:8px;align-items:center;flex-wrap:wrap"><div class="leadBtns">${btns}</div>${stageChips}</div>
          <div class="leadIconBtns">${iconBtns}</div>
        </div>
      `;
    }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const rows = await bxAll('crm.deal.list', {
      filter: { ...(uid ? { ASSIGNED_BY_ID: Number(uid) } : {}) },
      order: { ID: 'DESC' },
      select,
    }).catch(() => []);
...
${marker}` : marker;
  ...
  async function ensureLeadFunnelDataLoaded(userId, opts = {}) {
    ...
    try {
      ...
      await loadProposalDeals(false, { userId: uid }).catch(()=>{});
    } catch (_) {}
  }
