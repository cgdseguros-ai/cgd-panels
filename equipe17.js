...
(function () {
  ...
  function selectedOperationalWindowKey() {
    try { return ymdKey(selectedDate); } catch (_) { return fmtDateOnly(selectedDate); }
  }
  ...
  async function ensureOperationalWindowForSelectedDate(opts = {}) {
    ...
    OPERATIONAL_WINDOW_PROMISE = (async () => {
      try {
        if (!silent) setSoftStatus(`Carregando tarefas de ${fmtDateOnly(selectedDate)}...`);
  ...
  // PATCH V5.2.1 — carga operacional rápida do painel geral
  // Em vez de carregar TODAS as tarefas abertas da pipeline 17 no boot (que pode travar por vários minutos),
  // carrega apenas o que o painel geral precisa para o dia selecionado: abertas vencidas/até o fim do dia
  // e concluídas do próprio dia para a métrica "TF Concluídas". Histórico/futuras entram em background.
  async function loadDealsOperationalWindow(opts = {}) {
    ...
    const ds = dayStart(selectedDate);
    const de = dayEnd(selectedDate);
    const dsIso = ds.toISOString();
    const deIso = de.toISOString();
    const includeOverdue = !selectedDateIsFuture();

    const openFilter = { CATEGORY_ID: CATEGORY_MAIN };
    if (STATE.doneStageId) openFilter["!STAGE_ID"] = String(STATE.doneStageId);

    if (includeOverdue) {
      // Hoje/passado: painel precisa de tudo aberto até o fim do dia selecionado (inclui atrasadas).
      openFilter["<=" + UF_PRAZO] = deIso;
    } else {
      // Futuro: painel precisa só do dia selecionado.
      openFilter[">=" + UF_PRAZO] = dsIso;
      openFilter["<=" + UF_PRAZO] = deIso;
    }

    const requests = [
      bxAll("crm.deal.list", { filter: openFilter, select, order: { ID: "DESC" } })
    ];

    // Mantém TF Concluídas do dia sem precisar carregar todo o histórico concluído.
    if (STATE.doneStageId) {
      requests.push(bxAll("crm.deal.list", {
        filter: { CATEGORY_ID: CATEGORY_MAIN, STAGE_ID: String(STATE.doneStageId), [">=" + UF_PRAZO]: dsIso, ["<=" + UF_PRAZO]: deIso },
        select,
        order: { ID: "DESC" },
      }).catch(() => []));
    }
    ...
    STATE.dealsLoadedMode = "operational";
...
function makeUserCard(u) {
  ...
  async function loadMetricsCore(opts = {}) {
    ...
    await bootTimerMeasure("Métricas: janela operacional", () => loadDealsOperationalWindow({ deferPhotos: true }), "Cards operacionais: abertas vencidas/até o dia e concluídas do dia; histórico/futuras em background");
  ...
  function schedulePostReadyBackgroundDataLoads() {
    try {
      setTimeout(async () => {
        try {
          await bootTimerMeasure("Background: deals/tarefas antigos", () => loadDeals({ forceFull: true, openOnly: false, deferPhotos: true, oldOnly: true, olderThanDays: 5, append: true }).catch(()=>{}), "Histórico antigo/concluído para buscas e auditoria; não deve alterar a contagem operacional aberta");
          try { applyLocalDealPatches(); } catch(_) {}
          try { overlayPendingSyncState(); } catch(_) {}
          try { rebuildDealsOpen(); } catch(_) {}
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      if (opts && opts.forceFullDeals === true) {
        await loadDeals({ forceFull: true, openOnly: false, deferPhotos: true, recentOnly: true, daysBack: 10 });
      } else {
        await loadDealsOperationalWindow({ deferPhotos: true });
      }
