...
function makeUserCard(u) {
  ...
  const PENDING_LEAD_PATCHES = new Map();
  const PENDING_DEAL_PATCHES = new Map();

  function holdUiRefresh(ms = 5000) {
    const until = Date.now() + Math.max(0, Number(ms) || 0);
    UI_REFRESH_HOLD_UNTIL = Math.max(UI_REFRESH_HOLD_UNTIL || 0, until);
    return UI_REFRESH_HOLD_UNTIL;
  }
  function rememberPendingLeadPatch(leadId, patch, ms = 8000) {
    const id = String(leadId || '').trim();
    if (!id) return;
    PENDING_LEAD_PATCHES.set(id, { ...(PENDING_LEAD_PATCHES.get(id) || {}), ...(patch || {}), _expiresAt: Date.now() + Math.max(1000, Number(ms) || 8000) });
  }
  function rememberPendingDealPatch(dealId, patch, ms = 8000) {
    const id = String(dealId || '').trim();
    if (!id) return;
    PENDING_DEAL_PATCHES.set(id, { ...(PENDING_DEAL_PATCHES.get(id) || {}), ...(patch || {}), _expiresAt: Date.now() + Math.max(1000, Number(ms) || 8000) });
  }
  function prunePendingEntityPatches() {
    const now = Date.now();
    for (const [id, patch] of Array.from(PENDING_LEAD_PATCHES.entries())) if (!patch || Number(patch._expiresAt || 0) <= now) PENDING_LEAD_PATCHES.delete(id);
    for (const [id, patch] of Array.from(PENDING_DEAL_PATCHES.entries())) if (!patch || Number(patch._expiresAt || 0) <= now) PENDING_DEAL_PATCHES.delete(id);
  }
  function overlayPendingEntityPatches() {
    prunePendingEntityPatches();
    for (const [uid, list] of Array.from((STATE.leadsByUser || new Map()).entries())) {
      if (!Array.isArray(list) || !list.length) continue;
      STATE.leadsByUser.set(String(uid), list.map((lead) => {
        const patch = PENDING_LEAD_PATCHES.get(String(lead && lead.ID || ''));
        return patch ? { ...lead, ...patch } : lead;
      }));
    }
    if (Array.isArray(STATE.globalLeadsAll) && STATE.globalLeadsAll.length) {
      STATE.globalLeadsAll = STATE.globalLeadsAll.map((lead) => {
        const patch = PENDING_LEAD_PATCHES.get(String(lead && lead.ID || ''));
        return patch ? { ...lead, ...patch } : lead;
      });
    }
    if (Array.isArray(STATE.dealsAll) && STATE.dealsAll.length) {
      STATE.dealsAll = STATE.dealsAll.map((deal) => {
        const patch = PENDING_DEAL_PATCHES.get(String(deal && deal.ID || ''));
        return patch ? { ...deal, ...patch } : deal;
      });
      rebuildDealsOpen();
    }
  }
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    const refreshHoldActive = Date.now() < (UI_REFRESH_HOLD_UNTIL || 0);
    try {
      await loadDeals({ forceFull: !!(opts && opts.forceFullDeals) });
      overlayPendingSyncState();
      overlayPendingEntityPatches();
      ...
      if (!(opts && opts.skipLeads) && !refreshHoldActive) {
  ...
  async function markDone(dealId) {
    ...
    const patch = { STAGE_ID: String(STATE.doneStageId), DATE_MODIFY: new Date().toISOString() };
    holdUiRefresh(5000);
    rememberPendingDealPatch(dealId, patch, 8000);
    updateDealInState(dealId, patch);
    rebuildDealsOpen();
    ...
    btn.onclick = async () => {
      try {
        ...
        if (isFollowupDeal(d) && followMode === 'NAO_FEITO') {
          const patch = { TITLE: title, [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString(), _late:false, DATE_MODIFY:new Date().toISOString(), [UF_OBS]: obs };
          holdUiRefresh(7000);
          rememberPendingDealPatch(dealId, patch, 10000);
          updateDealInState(dealId, patch);
          rebuildDealsOpen();
          closeModal();
          renderCurrentView();
          await bx("crm.deal.update", { id: String(dealId), fields: { TITLE: title, [UF_PRAZO]: prazoIso, [UF_OBS]: obs } });
          setTimeout(() => { if (Date.now() >= UI_REFRESH_HOLD_UNTIL) refreshData(true, { skipLeads: true }).catch(()=>{}); }, 1600);
          return;
        }
        const tempNewId = makeTempId('TMP_DEAL');
        holdUiRefresh(7000);
        rememberPendingDealPatch(dealId, { STAGE_ID: String(STATE.doneStageId), DATE_MODIFY: new Date().toISOString() }, 10000);
  ...
  async function routeLeadToCategory(userId, leadId, categoryId, finalLeadStatus) {
    ...
    const tempId = makeTempId("TMP_DEAL");
    holdUiRefresh(9000);
    rememberPendingLeadPatch(leadId, { STATUS_ID: String(finalLeadStatus || "") }, 12000);
    upsertDealLocal(parseLocalDealFromFields(tempId, fields));
    enqueueSync({ type: "dealAdd", tempId, fields });
  ...
  async function convertLinkedLeadFromFollowup(dealId) {
    ...
    const okConvert = await openSystemConfirmModal('Confirmar conversão', 'Tem certeza que deseja converter este lead?', 'Converter');
    if (!okConvert) return;
  ...
  async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    holdUiRefresh(target === String(sAtendido) ? 9000 : 7000);
    rememberPendingLeadPatch(leadId, patch, target === String(sAtendido) ? 12000 : 10000);
    if (i >= 0) {
      arr[i] = { ...(arr[i] || {}), ...patch };
      STATE.leadsByUser.set(uid, arr);
    }
