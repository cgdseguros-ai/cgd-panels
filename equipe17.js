...
(function () {
  ...
  async function loadDeals(opts = {}) {
    ...
    if (!canIncremental) {
      let deals = await bxAll("crm.deal.list", {
        filter: { CATEGORY_ID: CATEGORY_MAIN },
        select,
        order: { ID: "DESC" },
      });
      if (!Array.isArray(deals) || !deals.length) {
        deals = await bxAll("crm.deal.list", {
          filter: {},
          select,
          order: { ID: "DESC" },
        });
      ...
      if ((!Array.isArray(deals) || !deals.length) && Array.isArray(STATE.dealsAll) && !STATE.dealsAll.length) {
        deals = await bxAll("crm.deal.list", {
          filter: {},
          select,
          order: { ID: "DESC" },
...
restoreSyncQueue();
  ...
  function isFollowupDeal(d) {
    ...
    if (title.startsWith(norm("FOLLOW-UP")) || title.includes(norm("FOLLOW-UP"))) return true;
    if (String((d && d[DEAL_UF_LEAD_ORIGEM]) || "").trim() && d && d._prazo) return true;
    return false;
  }
  ...
  function openFollowupListModalForUser(user) {
    const baseSource = Array.isArray(STATE.dealsOpen) && STATE.dealsOpen.length ? STATE.dealsOpen : STATE.dealsAll;
    const all = (baseSource || []).filter((d) => String(d.ASSIGNED_BY_ID || d._assigned || "") === String(user.userId));
  ...
  function openLeadBatchCreateModal(user, defaultStatusId) {
    const rows = 8;
    const now = new Date();
    const localNow = new Date(now.getTime() - now.getTimezoneOffset()*60000).toISOString().slice(0,16);
    const rowTpl = (i) => `<tr><td><input data-r="${i}" data-f="name" style="width:220px;padding:8px" /></td><td><input data-r="${i}" data-f="op" style="width:130px;padding:8px" /></td><td><input data-r="${i}" data-f="idade" style="width:70px;padding:8px" /></td><td><input data-r="${i}" data-f="tel" style="width:130px;padding:8px" /></td><td><input data-r="${i}" data-f="bairro" style="width:120px;padding:8px" /></td><td><input data-r="${i}" data-f="fonte" style="width:120px;padding:8px" /></td><td><input data-r="${i}" data-f="dh" type="datetime-local" value="${localNow}" style="width:170px;padding:8px" /></td><td><input data-r="${i}" data-f="obs" style="width:220px;padding:8px" /></td></tr>`;
    openModal(`Lead em lote — ${user.name}`, `<div class="eqd-warn" id="lbWarn"></div><div style="overflow:auto;max-height:68vh"><table style="border-collapse:collapse;width:max-content;min-width:100%"><thead><tr><th>Nome</th><th>Operadora</th><th>Idade</th><th>Telefone</th><th>Bairro</th><th>Fonte</th><th>Data/Hora</th><th>Obs</th></tr></thead><tbody>${Array.from({length:rows},(_,i)=>rowTpl(i)).join('')}</tbody></table></div><div style="display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Fechar</button><button class="eqd-btn eqd-btnPrimary" id="lbSave">SALVAR LOTE</button></div>`, { full:true });
    document.getElementById('lbSave').onclick = async () => {
      const warn = document.getElementById('lbWarn');
      try {
        setBusy('Salvando lote de leads…');
        for (let i=0;i<rows;i++) {
          const gv=(f)=>String((document.querySelector(`[data-r="${i}"][data-f="${f}"]`)||{}).value||'').trim();
          const name=gv('name');
          if (!name) continue;
          const fields = { ASSIGNED_BY_ID:Number(user.userId), NAME:name, TITLE:name, STATUS_ID:String(defaultStatusId || leadStageId("NOVO LEAD") || "") };
          if (gv('op')) fields[LEAD_UF_OPERADORA]=gv('op');
          if (gv('idade')) fields[LEAD_UF_IDADE]=gv('idade');
          if (gv('tel')) fields[LEAD_UF_TELEFONE]=gv('tel');
          if (gv('bairro')) fields[LEAD_UF_BAIRRO]=gv('bairro');
          if (gv('fonte')) fields[LEAD_UF_FONTE]=gv('fonte');
          if (gv('dh')) fields[LEAD_UF_DATAHORA]=localInputToIsoWithOffset(gv('dh'));
          if (gv('obs')) fields[LEAD_UF_OBS]=gv('obs');
          const created = await bx("crm.lead.add", { fields });
          const newLeadId = String((created && (created.result || created)) || "").trim();
          const localLead = { ID:newLeadId || makeTempId('TMP_LEAD'), ASSIGNED_BY_ID:Number(user.userId), NAME:name, TITLE:name, STATUS_ID:String(defaultStatusId || leadStageId("NOVO LEAD") || ""), DATE_CREATE:new Date().toISOString(), DATE_MODIFY:new Date().toISOString(), [LEAD_UF_OPERADORA]:gv('op'), [LEAD_UF_IDADE]:gv('idade'), [LEAD_UF_TELEFONE]:gv('tel'), [LEAD_UF_BAIRRO]:gv('bairro'), [LEAD_UF_FONTE]:gv('fonte'), [LEAD_UF_DATAHORA]:localInputToIsoWithOffset(gv('dh')), [LEAD_UF_OBS]:gv('obs'), [LEAD_UF_LEAD_ORIGEM]: newLeadId || '' };
          const arr = (STATE.leadsByUser.get(String(user.userId)) || []).slice();
          arr.unshift(localLead);
          STATE.leadsByUser.set(String(user.userId), arr);
        }
        clearBusy();
        reopenLeadsModalSafe();
      } catch(e) { clearBusy(); warn.style.display='block'; warn.textContent='Falha: '+(e.message||e); }
    };
  }
  ...
  function renderUserPanel(userId) {
    ...
    const lostFupsBtn = String(user.userId) === "1" ? `<button class="eqd-btn" data-action="lostFollowupsLocator">FUPs DE LEADS PERDIDOS</button>` : ``;
      ...
      <div class="panelHead">
        <div class="panelUserInfo">
          ...
          <div>
            <div class="panelUserName">${escHtml(user.name)}</div>
            <div class="panelUserTeam">Equipe ${escHtml(user.team || "")}</div>
            <div style="font-size:11px;font-weight:950;opacity:.82">Dia selecionado: ${fmtDateOnly(selectedDate)}</div>
          </div>
        </div>
  ...
  async function revalidateLead(userId, leadId) {
    ...
    const targetStatus = String(leadStageId('EM ATENDIMENTO') || leadStageId('NOVO LEAD') || lead.STATUS_ID || '');
    const discardStatus = String(leadStageId('LEAD DESCARTADO') || leadStageId('PERDIDO') || '');
    const nowIso = new Date().toISOString();
    const fields = { ASSIGNED_BY_ID:Number(uid), NAME:leadTitle(lead), TITLE:leadTitle(lead), STATUS_ID:targetStatus, [LEAD_UF_OPERADORA]:leadOperadora(lead), [LEAD_UF_IDADE]:leadIdade(lead), [LEAD_UF_TELEFONE]:leadTelefone(lead), [LEAD_UF_BAIRRO]:leadBairro(lead), [LEAD_UF_FONTE]:leadFonte(lead), [LEAD_UF_DATAHORA]:nowIso, [LEAD_UF_OBS]:leadObs(lead), [LEAD_UF_HELENA]:leadHelena(lead), [LEAD_UF_PESSOAL]:leadPessoal(lead) };
    setBusy('Revalidando lead…');
    try {
      const created = await bx('crm.lead.add', { fields });
      const newLeadId = String((created && (created.result || created)) || '').trim() || makeTempId('TMP_LEAD');
      const localLead = { ...lead, ID:newLeadId, STATUS_ID:targetStatus, DATE_CREATE:nowIso, DATE_MODIFY:nowIso, [LEAD_UF_DATAHORA]:nowIso, [LEAD_UF_LEAD_ORIGEM]: newLeadId };
      const filtered = arr.filter((l) => String(l.ID) !== String(leadId));
      filtered.unshift(localLead);
      if (discardStatus) await bx('crm.lead.update', { id:String(leadId), fields:{ STATUS_ID:discardStatus } }).catch(()=>{});
  ...
  function openLeadStageListModal(userId, stageId, title, kwRaw = "") {
    const user = USERS.find((u) => String(u.userId) === String(userId));
    const stageIds = Array.isArray(stageId) ? stageId.map((x)=>String(x||'')).filter(Boolean) : [String(stageId || '')].filter(Boolean);
    const arr = (STATE.leadsByUser.get(String(userId)) || []).filter((l) => String((l.ASSIGNED_BY_ID || l._ownerUserId || '')) === String(userId) && stageIds.includes(String(l.STATUS_ID || '')));
    const kw = norm(String(kwRaw || '').trim());
    const rows = kw ? arr.filter((l) => leadMatchesKw(l, kw)) : arr;
    const card = (l) => `
      <div class="leadCard">
        <div class="leadTitle">${escHtml(leadTitle(l))}</div>
        <div class="leadMeta"><span>USER: <strong>${escHtml((USERS.find((u)=>String(u.userId)===String(l.ASSIGNED_BY_ID||l._ownerUserId))||{}).name || String(l.ASSIGNED_BY_ID||''))}</strong></span>${leadOperadora(l) ? `<span>Operadora: <strong>${escHtml(leadOperadora(l))}</strong></span>` : ``}${leadTelefone(l) ? `<span>Telefone: <strong>${escHtml(leadTelefone(l))}</strong></span>` : ``}${leadDataHora(l) ? `<span>Data/Hora: <strong>${escHtml(fmt(leadDataHora(l)))}</strong></span>` : ``}</div>
        <div class="leadBtns"><button class="leadBtn leadBtnP" data-action="leadMove" data-leadid="${l.ID}" data-tostatus="${escHtml(String(leadStageId('EM ATENDIMENTO')||''))}" data-userid="${userId}">VOLTAR PARA EM ATENDIMENTO</button><button class="leadBtn" data-action="leadRevalidate" data-leadid="${l.ID}" data-userid="${userId}">REVALIDAR +60 DIAS</button></div>
      </div>`;
  ...
  async function loadAdminOrphanFollowupsForUser(userId) {
    ...
    let filter = { CATEGORY_ID: CATEGORY_MAIN, ASSIGNED_BY_ID: uid };
    if (followupTaskId) filter[UF_TAREFA] = followupTaskId;
    let deals = await bxAll('crm.deal.list', { filter, select, order: { ID: 'DESC' } });
    if (!Array.isArray(deals) || !deals.length) {
      filter = { ASSIGNED_BY_ID: uid };
      if (followupTaskId) filter[UF_TAREFA] = followupTaskId;
      deals = await bxAll('crm.deal.list', { filter, select, order: { ID: 'DESC' } });
    }
    return (deals || [])
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function openLostFollowupsLocatorModal() {
    const rows = (STATE.dealsOpen || []).filter((d) => isFollowupDeal(d)).map((d) => ({ deal:d, lead:getLeadByOrigemId(getDealLeadOrigemId(d)) })).filter((x) => x.lead && isLostLead(x.lead));
    openModal('FUPs de leads perdidos', `
      <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:center">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap"><button class="eqd-btn" id="lfSelAll">Selecionar todas</button><button class="eqd-btn eqd-btnDanger" id="lfDelSel">Excluir selecionadas</button><button class="eqd-btn" id="lfRescueSel">Voltar lead para EM ATENDIMENTO</button></div>
      </div>
      <div style="display:flex;flex-direction:column;gap:8px;min-height:50vh;max-height:72vh;overflow:auto;margin-top:10px">
        ${rows.length ? rows.map(({deal,lead}) => `<label style="display:block;border:1px solid rgba(0,0,0,.08);border-radius:12px;padding:10px;background:#fff"><div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><div style="display:flex;gap:10px;align-items:flex-start"><input type="checkbox" class="lfChk" data-dealid="${deal.ID}" data-leadid="${lead.ID}" /><div><div style="font-weight:950">${escHtml(bestTitleFromText(deal.TITLE || ''))}</div><div style="font-size:12px;opacity:.8">Prazo: <strong>${escHtml(deal._prazo ? fmt(deal._prazo) : '—')}</strong></div><div style="font-size:12px;opacity:.8">USER: <strong>${escHtml((USERS.find(u=>String(u.userId)===String(lead.ASSIGNED_BY_ID||lead._ownerUserId))||{}).name || String(lead.ASSIGNED_BY_ID||''))}</strong> • Lead: <strong>${escHtml(leadTitle(lead))}</strong> • Operadora: <strong>${escHtml(leadOperadora(lead) || '—')}</strong> • Telefone: <strong>${escHtml(leadTelefone(lead) || '—')}</strong></div></div></div><span class="eqd-tag" style="background:#fef2f2;color:#b91c1c;border:1px solid rgba(185,28,28,.18);padding:2px 8px;line-height:1.2">LEAD PERDIDO</span></div></label>`).join('') : `<div class="eqd-empty">Nenhum FUP agendado de lead perdido.</div>`}
      </div>
    `, { full:true });
    ...
    document.getElementById('lfDelSel')?.addEventListener('click', async () => { const ids = checks().map((c)=>String(c.getAttribute('data-dealid')||'')); if (!ids.length) return alert('Selecione ao menos uma FUP.'); if (!confirm('Excluir as FUPs selecionadas?')) return; for (const id of ids) { removeDealLocal(id); enqueueSync({ type:'dealDelete', dealId:id }); } closeModal(); renderCurrentView(); });
    document.getElementById('lfRescueSel')?.addEventListener('click', async () => { const rowsSel = checks(); if (!rowsSel.length) return alert('Selecione ao menos um lead.'); for (const c of rowsSel) { const leadId = String(c.getAttribute('data-leadid')||''); const lead = Array.from(STATE.leadsByUser.values()).flat().find((l)=>String(l.ID)===leadId); if (lead) await moveLeadStage(String(lead.ASSIGNED_BY_ID || lead._ownerUserId || ''), String(lead.ID), leadStageId('EM ATENDIMENTO'), { skipAtendidoModal:true }); } closeModal(); renderCurrentView(); });
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadRevalidate") {
        ...
        const leadId = String(a.getAttribute("data-leadid")||"");
        if (!uid || !leadId) return;
        if (!confirm("Revalidar este lead por mais 60 dias? O atual será descartado e um novo será criado.")) return;
