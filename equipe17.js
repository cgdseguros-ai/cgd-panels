...
${curObs}` : transferAlert;
  ...
  function openModal(title, bodyHTML, opts) {
    if (el.modalOverlay.style.display === "flex" && !(opts && opts.noStack)) {
      MODAL_STACK.push({
...
function makeUserCard(u) {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      const holdLeadReload = Date.now() < UI_REFRESH_HOLD_UNTIL;
      const shouldRefreshLeads = !holdLeadReload && !(opts && opts.skipLeads) && ((Date.now() - LAST_LEADS_REFRESH_AT > LEADS_REFRESH_MS) || !STATE.leadsByUser.size || !!forceNetwork);
      if (opts.deferLeads) {
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          rebuildDealsOpen();
          LAST_DEALS_LOAD_AT = Date.now();
          MODAL_STACK = [];
          closeModal();
          renderCurrentView();
          setTimeout(() => { refreshData(true).catch(()=>{}); }, 900);
          return;
        }
  ...
  async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    if (i >= 0) {
      arr[i] = { ...(arr[i] || {}), ...patch, DATE_MODIFY: new Date().toISOString() };
      STATE.leadsByUser.set(uid, arr);
      LAST_LEADS_REFRESH_AT = Date.now();
    }
    if (target === String(sAtendido)) {
    ...
    reopenLeadsModalSafe({ noBackgroundReload: true });
    renderCurrentView();
    setTimeout(() => { refreshData(true, { forceRecur:false, forceFullDeals:false, deferLeads:false }).catch(()=>{}); }, 900);
