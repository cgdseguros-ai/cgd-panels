...
restoreSyncQueue();
  ...
  function openAdminLinkFupModal(adminUserId, selectedUid = "") {
    ...
    const row = (d, idx) => {
      ...
      return `<div class="adminFupRow" data-dealid="${escHtml(String(d.ID))}" style="border:1px solid rgba(0,0,0,.12);border-radius:14px;padding:12px;background:rgba(255,255,255,.70);display:grid;grid-template-columns:minmax(280px,1.1fr) minmax(340px,1fr) auto;gap:12px;align-items:center">
        <div><div style="font-weight:950">${escHtml(bestTitleFromText(d.TITLE||''))}</div><div style="font-size:11px;font-weight:900;opacity:.75">Prazo: ${escHtml(d._prazo ? fmt(d._prazo) : '—')} • ID: ${escHtml(String(d.ID))}</div></div>
        <div>
          <input class="eqd-searchInput adminFupLeadSearch" data-idx="${idx}" style="width:100%;color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Buscar lead provável" value="${escHtml(defaultKw)}" />
          <select class="adminFupLeadSelect" data-idx="${idx}" data-dealid="${escHtml(String(d.ID))}" size="6" style="width:100%;margin-top:8px;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"></select>
        </div>
    ...
    openModal(`Vincular FUP — administração`, `
      ...
      <div id="adminFupBox" style="margin-top:10px;display:flex;flex-direction:column;gap:10px;min-height:55vh;max-height:72vh;overflow:auto;padding-right:4px">${uid ? (orphan.length ? orphan.map(row).join('') : `<div class="eqd-empty">Nenhum FOLLOW-UP sem lead vinculado.</div>`) : ``}</div>
    `, { wide:true, full:true });
    ...
    if (loadBtn) loadBtn.onclick = async () => {
      ...
      if (!STATE.leadsByUser.get(v)) { try { await loadLeadsForOneUser(v); } catch(_) {} }
      openAdminLinkFupModal(adminUserId, v);
    };
    const fillSelect = (dealId, idx, kw='') => {
      const sel = document.querySelector(`.adminFupLeadSelect[data-idx="${idx}"]`);
      const deal = getDealById(String(dealId));
      const rows = getLikelyLeadCandidatesForFollowup(deal, { kw }).slice(0, 40);
      if (sel) sel.innerHTML = `<option value="">Selecione…</option>` + rows.map((l) => `<option value="${escHtml(String(l.ID))}">${escHtml(leadTitle(l))} ${countOpenFutureFollowupsForLeadOrig(leadOrigemId(l)||l.ID, dealId) ? '• já tem FUP' : '• sem FUP'}</option>`).join('');
    };
    document.querySelectorAll('.adminFupLeadSearch').forEach((inp) => {
      const idx = String(inp.getAttribute('data-idx') || '');
      const sel = document.querySelector(`.adminFupLeadSelect[data-idx="${idx}"]`);
      const dealId = String(sel && sel.getAttribute('data-dealid') || '');
      fillSelect(dealId, idx, inp.value || '');
      inp.oninput = (e) => fillSelect(dealId, idx, e.target.value || '');
    });
    document.querySelectorAll('.adminFupLinkOne').forEach((btn) => btn.onclick = async () => {
      ...
      const row = document.querySelector(`.adminFupRow[data-dealid="${dealId}"]`);
      if (row) row.remove();
    });
    document.querySelectorAll('.adminFupNewLead').forEach((btn) => btn.onclick = async () => {
      ...
      if (!user || !deal) return;
      openManualLeadCreateModal(user, leadStageId('EM ATENDIMENTO') || leadStageId('NOVO LEAD') || '', { prefillName: bestTitleFromText(String(deal.TITLE || '').replace(/^FOLLOW-UP\s*/i,'')), onSaved: () => openAdminLinkFupModal(adminUserId, v) });
    });
  }
