...
function makeUserCard(u) {
  ...
  function renderUserPanel(userId) {
    ...
    const user1AdminToggleBtn = String(user.userId) === "1" ? `<button class="eqd-btn" id="user1AdminToggle">RECURSOS ADM</button>` : ``;
    const user1AdminPanel = String(user.userId) === "1" ? `<div id="user1AdminPanel" style="display:none;gap:8px;flex-wrap:wrap">${analysisBtn}${globalNoFuBtn}${lostFupBtn}${proposalsAdminBtn}</div>` : ``;
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      const fastOpenOnly = !Boolean(opts && opts.forceFullDeals);
      await loadDeals({ forceFull: !!(opts && opts.forceFullDeals), openOnly: fastOpenOnly, deferPhotos: true });
      if (fastOpenOnly && STATE.dealsLoadedMode !== 'full') ensureFullDealsLoadedInBackground(2500);
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const allowedStageIds = new Set(proposalStageOrder().map((nm) => String(STATE.proposalStageIdByName.get(norm(nm)) || '').trim()).filter(Boolean));
    const baseFilter = {};
    if (uid) baseFilter.ASSIGNED_BY_ID = uid;
    let rows = await bxAll('crm.deal.list', {
    ...
    const mapped = (rows || [])
      .filter((d) => {
        const categoryRaw = d && d.CATEGORY_ID != null ? String(d.CATEGORY_ID).trim() : '';
        const categoryLooksDefault = categoryRaw === '' || categoryRaw === '0';
        const stageId = String((d && d.STAGE_ID) || '').trim();
        const stageName = proposalStageNameFromId(stageId) || String(stageId || '');
        return categoryLooksDefault || allowedStageIds.has(stageId) || proposalAllowedStage(stageName);
      })
      .map((d) => ({ ...d, _proposalStageName: proposalStageNameFromId(d.STAGE_ID) || String(d.STAGE_ID || '') }));
    if (!uid) {
  ...
  function renderProposalCard(deal, opts = {}) {
    const stageName = proposalStageNameFromId(deal.STAGE_ID);
  ...
  async function openProposalStageMoveModal(dealId, returnCtx = {}) {
    ...
    openModal(`Mover stage — ${bestTitleFromText(deal.TITLE || '')}`, `
      ...
      <div style="font-size:12px;font-weight:900;opacity:.82;margin-bottom:8px">Stage atual: <strong>${escHtml(proposalStageNameFromId(deal.STAGE_ID))}</strong></div>
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    const monthRef = String(opts.monthRef || '').trim();
    let sourceDeals = [];
    if (adminView) {
      sourceDeals = await loadProposalDeals(false, {});
      if ((!sourceDeals || !sourceDeals.length) && !opts.__retriedForce) {
        sourceDeals = await loadProposalDeals(true, {});
      }
    } else {
      sourceDeals = await loadProposalDeals(false, { userId: uid });
      if ((!sourceDeals || !sourceDeals.length) && !opts.__retriedForce) {
    ...
    if (monthRef) {
      filteredDeals = filteredDeals.filter((d) => {
        const raw = String(d[PROP_UF_ATIVACAO] || '').trim();
        return raw && raw.slice(0,7) === monthRef;
      });
    }
    let grouped = proposalStageOrder().map((nm) => ({ name: nm, deals: filteredDeals.filter((d) => proposalStageNameFromId(d.STAGE_ID) === nm) })).filter((g) => g.deals.length || adminView);
    const knownCount = grouped.reduce((n,g) => n + g.deals.length, 0);
    if (knownCount < filteredDeals.length) {
      grouped.push({ name: 'OUTROS STAGES', deals: filteredDeals.filter((d) => !proposalStageOrder().includes(proposalStageNameFromId(d.STAGE_ID))) });
    }
    ...
    openModal(title, `
      ...
      <div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:10px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ...
          <input id="proposalMonthRef" type="month" value="${escHtml(monthRef)}" style="padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" />
    ...
    if (apply) apply.onclick = () => openProposalsModal(uid, {
      ...
      monthRef: String((document.getElementById('proposalMonthRef') || {}).value || ''),
