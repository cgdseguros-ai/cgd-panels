...
restoreSyncQueue();
  ...
  function updateSyncStatusIndicator() {
    try {
      ...
      } else if (s.pending > 0 || s.pendingMut > 0 || s.inflight > 0 || s.running) {
        const n = Math.max(s.pending, s.pendingMut);
        txt = s.backoff > 0 ? `SYNC: aguardando ${n}` : `SYNC: sincronizando ${n || s.inflight || 1}`;
