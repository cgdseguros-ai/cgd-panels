...
(function () {
  ...
  function beginVisualHold(ms = 2500) {
    STATE.visualHoldUntil = Math.max(Number(STATE.visualHoldUntil || 0), Date.now() + Number(ms || 0));
  }

  async function safeDealUpdate(dealId, fields) {
    beginVisualHold(2500);
    try {
  ...
  async function openRecurrenceBatchModalForUser(user) {
    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        try {
          const current = (STATE.recurRulesByUser.get(String(user.userId)) || []).slice();
          current.push(rule);
          STATE.recurRulesByUser.set(String(user.userId), current);
        } catch (_) {}
        closeAllModals();
        await loadRecurrenceConfigDeals().catch(()=>{});
        refreshData(true, { ignoreHold: true }).catch(()=>{});
        renderCurrentView();
      } catch (e) {
        warn.style.display = 'block';
        warn.textContent = 'Falha:\n' + (e.message || e);
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const select = [
      ...
      PROP_UF_TELEFONE, PROP_UF_OPER_LEAD, PROP_UF_OPER_FECHADA, PROP_UF_LEAD_ORIGEM, PROP_UF_DATAHORA_LEAD, DEAL_UF_POSSUI_PLANO
    ];
  ...
  async function openProposalRequiredFieldsModal(deal, nextStageId, returnCtx = {}, patch = {}) {
    ...
    if (btn) btn.onclick = async () => {
      ...
      try {
        ...
        const stageLabel = miss.stageName || proposalStageLabelById(finalPatch.STAGE_ID || deal.STAGE_ID);
        patchProposalDealCaches(String(deal.ID), { ...deal, ...finalPatch, _proposalStageName: stageLabel });
  ...
  async function openProposalStageMoveModal(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const nextStageLabel = moveSel && moveSel.selectedOptions && moveSel.selectedOptions[0] ? String(moveSel.selectedOptions[0].textContent || '').trim() : proposalStageLabelById(nextStage);
        PROPOSALS_REFRESH_HOLD_UNTIL = Date.now() + 10000;
        patchProposalDealCaches(String(deal.ID), { STAGE_ID: nextStage, _proposalStageName: nextStageLabel });
        closeModal();
        openProposalsModal(String(returnCtx.userId || deal.ASSIGNED_BY_ID || ''), { ...returnCtx, noStack: true });
  ...
  async function openProposalEditor(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const nextStageLabel = stageSel && stageSel.selectedOptions && stageSel.selectedOptions[0] ? String(stageSel.selectedOptions[0].textContent || '').trim() : proposalStageLabelById(nextStageId);
        patchProposalDealCaches(String(deal.ID), { ...patch, _proposalStageName: nextStageLabel });
        closeModal();
        openProposalsModal(String(returnCtx.userId || deal.ASSIGNED_BY_ID || ''), { ...returnCtx, noStack: true });
...
${marker}` : marker;
  ...
  function renderProposalsModalContent(sourceDeals, opts = {}) {
    ...
    const rankOptions = [PROP_UF_OPER_FECHADA, PROP_UF_OPER_LEAD, DEAL_UF_POSSUI_PLANO].map((code) => {
    ...
    return `
      ...
      <div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:10px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ...
          ${adminView ? `<select id="proposalRankField" style="padding:8px 10px;border-radius:10px;border:1px solid rgba(30,40,70,.16);font-weight:900">${rankOptions}</select>` : ``}
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    const applyBindings = () => {
      ...
      if (apply) apply.onclick = () => openProposalsModal(uid, {
        ...
        rankField: adminView ? String((document.getElementById('proposalRankField') || {}).value || 'OPPORTUNITY') : 'OPPORTUNITY',
      ...
      document.querySelectorAll('.proposal-inline-stage').forEach((sel) => {
        sel.onchange = async () => {
          ...
          patchProposalDealCaches(dealId, { STAGE_ID: nextStage, _proposalStageName: proposalStageNameFromId(nextStage) });
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "proposalDelete") { if (!confirm("Excluir este contrato/proposta?")) return; await bx("crm.deal.delete", { id: String(id||"") }); PROPOSALS_REFRESH_HOLD_UNTIL = Date.now() + 10000; removeProposalDealCaches(String(id||"")); return openProposalsModal(String((LAST_PROPOSALS_CTX&&LAST_PROPOSALS_CTX.userId)||userId||""), { ...(LAST_PROPOSALS_CTX||{}), noStack: true }); }
      ...
      if (act === "recurTransfer") {
        ...
        closeAllModals();
        return openRecurrenceManager(target);
      }
