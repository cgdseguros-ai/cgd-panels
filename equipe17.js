...
(function () {
  ...
  injectCSS(`
    ...
    .eqd-titleWrap{display:flex;align-items:center;gap:10px;min-width:300px;}
    .eqd-logo{width:48px;height:48px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.92);object-fit:contain;padding:6px;flex:0 0 auto;}
    .eqd-titleBlock{display:flex;flex-direction:column;gap:1px;}
    .eqd-title{display:flex;align-items:center;gap:8px;font-weight:950;font-size:16px;color:#fff;}
    .eqd-dot{width:10px;height:10px;border-radius:999px;background:#16a34a;box-shadow:0 0 0 6px rgba(22,163,74,.12);}
    .eqd-meta{font-size:11px;color:rgba(255,255,255,.75);font-weight:800;}

    .eqd-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end;}
    .eqd-pill{font-size:11px;font-weight:900;padding:6px 9px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.12);color:#fff;white-space:nowrap;}
    .eqd-btn{border:1px solid rgba(255,255,255,.22);background:#1f2430;border-radius:999px;padding:6px 10px;font-size:11px;font-weight:950;cursor:pointer;white-space:nowrap;color:#fff;text-decoration:none;display:inline-flex;align-items:center;gap:6px;box-shadow:0 2px 8px rgba(0,0,0,.16);}
    ...
    .eqd-searchInput{width:min(340px,54vw);border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.10);border-radius:999px;padding:7px 10px;font-size:11px;font-weight:900;outline:none;color:#fff;}
    .eqd-searchInput::placeholder{color:rgba(255,255,255,.70)}
    .eqd-searchSelect{border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.10);border-radius:999px;padding:6px 9px;font-size:11px;font-weight:950;outline:none;min-width:170px;color:#fff;}
    ...
    @media (max-width:900px){
      ...
      .eqd-topbar{zoom:1; padding:5px 8px;}
    }
    @media (orientation: landscape) and (max-width: 1024px){
      ...
      #eqd-app{padding-bottom:18px !important;}
    }
  `);
  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal) && linkedLead) {
      ...
      tags.push(`<span class="eqd-tag eqd-tagClickable" data-action="linkedLeadToggleHelena" data-dealid="${deal.ID}" style="background:${leadHelena(linkedLead) ? "#ede9fe" : "#374151"};color:${leadHelena(linkedLead) ? "#6d28d9" : "#fff"}">${leadHelena(linkedLead) ? "HELENA" : "HELENA NÃO"}</span>`);
      tags.push(`<span class="eqd-tag eqd-tagClickable" data-action="linkedLeadTogglePessoal" data-dealid="${deal.ID}" style="background:${leadPessoal(linkedLead) ? "#dcfce7" : "#6b7280"};color:${leadPessoal(linkedLead) ? "#166534" : "#fff"}">${leadPessoal(linkedLead) ? "WPP DIRETO" : "WPP DIRETO NÃO"}</span>`);
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (newLeadId && LEAD_UF_LEAD_ORIGEM) {
          await bx("crm.lead.update", { id: newLeadId, fields: { [LEAD_UF_LEAD_ORIGEM]: newLeadId } }).catch(()=>{});
      ...
      return `
        <div class="leadCard">
          ...
          <div class="leadMeta">
            ...
            <span style="background:${leadPessoal(l) ? "#dcfce7" : "#6b7280"};color:${leadPessoal(l) ? "#166534" : "#fff"};padding:2px 6px;border-radius:999px">${leadPessoal(l) ? "WPP DIRETO" : "WPP DIRETO NÃO"}</span>
          </div>
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    btn.onclick = async () => {
      try {
        ...
        if (d[UF_TAREFA]) fields[UF_TAREFA] = d[UF_TAREFA]; if (d[UF_URGENCIA]) fields[UF_URGENCIA] = d[UF_URGENCIA]; if (d[UF_ETAPA]) fields[UF_ETAPA] = d[UF_ETAPA]; if (d[UF_COLAB]) fields[UF_COLAB] = d[UF_COLAB]; if (d[DEAL_UF_LEAD_ORIGEM]) fields[DEAL_UF_LEAD_ORIGEM] = d[DEAL_UF_LEAD_ORIGEM]; if (obs) fields[UF_OBS] = obs;
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "linkedLeadToggleHelena") {
        ...
        const d = getDealById(dealId);
        const origId = d ? getDealLeadOrigemId(d) : "";
        const lead = origId ? getLeadByOrigemId(origId) : null;
      ...
      if (act === "followupPerdido") {
        const d = getDealById(id); const origId = d ? getDealLeadOrigemId(d) : ""; let lead = origId ? getLeadByOrigemId(origId) : null; if (!lead && d) { const titleNeedle = norm(String(d.TITLE||"").replace(/^FOLLOW-UP\s*/i, "").trim()); for (const arr of STATE.leadsByUser.values()) { const found = (arr||[]).find((l) => String(l.ASSIGNED_BY_ID||"") === String(d.ASSIGNED_BY_ID||d._assigned||"") && norm(leadTitle(l)) === titleNeedle); if (found) { lead = found; break; } } } if (!lead) return alert("Este FOLLOW-UP não tem lead de origem vinculado.");
