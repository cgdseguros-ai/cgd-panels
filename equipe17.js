...
restoreSyncQueue();
  ...
  function openFollowUpModal(user, prefillName, opts) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if ((wd === 0 || wd === 6) && !confirm('Tem certeza que deseja agendar para final de semana?')) return;
  ...
  function openNoFutureFollowupLeadBatchModal(user, leads, title) {
    ...
    document.getElementById('noFuBatch').onclick = async () => {
      ...
      const localDefault = new Date(((existingFutureFu && existingFutureFu._prazo) ? new Date(existingFutureFu._prazo) : base).getTime() - (((existingFutureFu && existingFutureFu._prazo) ? new Date(existingFutureFu._prazo) : base).getTimezoneOffset() * 60000)).toISOString().slice(0, 16);
  ...
  async function revalidateLead(userId, leadId) {
    ...
    fields[LEAD_UF_DATAHORA] = new Date().toISOString();
    const created = await bx('crm.lead.add', { fields });
    const newLeadId = String((created && (created.result || created)) || '').trim();
    if (newLeadId && LEAD_UF_LEAD_ORIGEM) await bx('crm.lead.update', { id:newLeadId, fields:{ [LEAD_UF_LEAD_ORIGEM]: newLeadId } }).catch(()=>{});
    await moveLeadStage(uid, leadId, leadStageId("LEAD DESCARTADO") || leadStageId("PERDIDO"), { skipLostPrompt: true, skipDeleteFups: false, skipClose: true });
    await loadLeadsSnapshotAllUsers({ force:true }).catch(()=>{});
    reopenLeadsModalSafe();
    renderCurrentView();
  }
  ...
  async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    if (target === String(sPerdido) && !(opts && opts.skipLostPrompt)) {
      lostAsk = confirm('Deseja agendar um novo FOLLOW-UP futuro para este lead perdido?\n\nOs FOLLOW-UPs futuros já agendados serão excluídos.');
    }
    ...
    let lostFollowupChoice = null;
    if (target === String(sPerdido) && !(opts && opts.skipLostPrompt)) {
      lostFollowupChoice = confirm('Deseja agendar um novo FOLLOW-UP futuro para este lead perdido?\n\nOs FOLLOW-UPs futuros já agendados serão excluídos.');
    }
    ...
    if (target === String(sPerdido) && !(opts && opts.skipDeleteFups)) {
      const toDelete = (STATE.dealsOpen || []).filter((d) => isFollowupDeal(d) && getDealLeadOrigemId(d) === origemForDelete && d._prazo && new Date(d._prazo).getTime() >= dayStart(new Date()).getTime());
      for (const fu of toDelete) {
        removeDealLocal(fu.ID);
        enqueueSync({ type: "dealDelete", dealId: String(fu.ID) });
      }
    }
    const origemForDelete = leadOrigemId(cur || arr[i] || {}) || String(leadId);
    ...
    reopenLeadsModalSafe();
    if (target === String(sPerdido) && lostFollowupChoice) {
      const ownerUser = USERS.find((u) => String(u.userId) === uid);
      if (ownerUser) openFollowUpModal(ownerUser, leadTitle(cur || arr[i] || {}), { leadId: origemForDelete, returnToLeads: { userId: uid, kw: LAST_LEADS_CTX.kw || "" } });
    }
  }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function openLostFollowupsLocatorModal(requesterUserId) {
  ...
  function openLostFollowupsLocatorModal() {
    ...
    openModal('FUPs de leads perdidos', `
      ...
      <div style="display:flex;flex-direction:column;gap:8px;min-height:50vh;max-height:72vh;overflow:auto;margin-top:10px">
        ${rows.length ? rows.map(({deal,lead}) => `<label style="display:block;border:1px solid rgba(0,0,0,.08);border-radius:12px;padding:10px;background:#fff"><div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><div style="display:flex;gap:10px;align-items:flex-start"><input type="checkbox" class="lfChk" data-dealid="${deal.ID}" data-leadid="${lead.ID}" /><div><div style="font-weight:950">${escHtml(bestTitleFromText(deal.TITLE || ''))}</div><div style="font-size:12px;opacity:.8">Prazo: <strong>${escHtml(deal._prazo ? fmt(deal._prazo) : '—')}</strong></div><div style="font-size:12px;opacity:.8">Lead: <strong>${escHtml(leadTitle(lead))}</strong> • Operadora: <strong>${escHtml(leadOperadora(lead) || '—')}</strong> • Telefone: <strong>${escHtml(leadTelefone(lead) || '—')}</strong></div></div></div><span class="eqd-tag" style="background:#fee2e2;color:#b91c1c">LEAD PERDIDO</span></div></label>`).join('') : `<div class="eqd-empty">Nenhum FUP agendado de lead perdido.</div>`}
      </div>
    `, { full:true });
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadShowLost") { const uid = String(a.getAttribute("data-userid")||""); return openLeadStageListModal(uid, lostLeadStageIds().join(","), 'Perdidos'); }
      if (act === "leadShowConv") { const uid = String(a.getAttribute("data-userid")||""); return openLeadStageListModal(uid, convertedLeadStageIds().join(","), 'Convertidos'); }
