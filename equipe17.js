    ...
    if (saveBtn) saveBtn.onclick = async () => {
      ...
      try {
        ...
        setTimeout(async () => {
          ...
          try { await generateRecurringDealsWindow().catch(()=>{});
              setBootProgress(80, "Instâncias de recorrência geradas..."); } catch (_) {}
...
function makeUserCard(u) {
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      renderFooterPeople();
      DEALS_FAIL_STREAK = 0;
      setBootProgress(96, "Finalizando painel...");
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    if (uid) {
      rows = await bxAll('crm.deal.list', {
        filter: { ASSIGNED_BY_ID: Number(uid) },
        order: { ID: 'DESC' },
        select,
      }).catch(() => []);
    } else {
...
Operadora fechada: ${proposalFieldValueLabel(PROP_UF_OPER_FECHADA, deal[PROP_UF_OPER_FECHADA])}</textarea></div>
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    openModal(title, immediateRows.length
      ...
      : `<div style="padding:18px;font-weight:900">Carregando propostas...</div>`, { full: true, noStack: !!opts.noStack });
    ...
    if (sourceDeals.length) applyBindings();
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "recurManager") {
        const uid = String(userId||"");
        if (!uid) return;
        if (STATE.recurModalOpeningForUser === uid || (STATE.recurManagerShownForUser === uid && (Date.now() - Number(STATE.recurManagerShownAt||0)) < 800)) return;
