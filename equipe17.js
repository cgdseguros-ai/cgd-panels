...
function makeUserCard(u) {
  ...
  async function rescheduleDealAtomic(oldDealId, newFields, originalDeal) {
    ...
    try {
      ...
      const confirmedOld = await confirmRemoteEntityFields("deal", oldId, { STAGE_ID: doneStageId }, { attempts: 4, baseDelay: 450 });
      ...
      try {
        ...
        updateDealInState(realId, { _pendingNewDeal: false, _rescheduledFrom: oldId, DATE_MODIFY: new Date().toISOString(), __skipPending: true });
        await confirmRemoteEntityFields("deal", realId, { [UF_PRAZO]: newFields[UF_PRAZO] }, { attempts: 3, baseDelay: 400 }).catch(()=>null);
      } catch (addErr) {
        enqueueSync({ type: "dealAdd", tempId, fields: newFields }, { delayMs: 1200 });
        try { logSyncError({ entityType: "deal", entityId: tempId, operationId: opDone, source: "reschedule", action: "crm.deal.add", message: (addErr && addErr.message) ? addErr.message : String(addErr), payload: { tempId, fields: newFields, oldDealId: oldId }, retryAvailable: true, retryMethod: "crm.deal.add", retryParams: { fields: newFields }, retryLabel: "Retry criar novo" }); } catch (_) {}
        console.warn("[EQD] Reagendamento: criação remota ficará na fila:", addErr);
      }
      ...
      try { logSyncError({ entityType: "deal", entityId: oldId, operationId: opDone, source: "reschedule", action: "rescheduleDealAtomic", message: (e && e.message) ? e.message : String(e), payload: { oldDealId: oldId, tempId, newFields, oldStageId }, retryAvailable: false }); } catch (_) {}
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        try {
          ...
          forceCloseAllModals();
          renderCurrentView(true);
          setTimeout(() => { refreshData(true, { deferLeads:false }).catch(()=>{}); }, 300);
        } catch (err) {
          showRuntimeWarn(err);
        } finally {
          clearBusy();
