...
restoreSyncQueue();
  ...
  function purgePendingDealFieldUpdates(dealId, fieldNames) {
    const id = String(dealId || "");
    const wanted = new Set((Array.isArray(fieldNames) ? fieldNames : [fieldNames]).filter(Boolean).map(String));
    if (!id || !wanted.size) return;
    const kept = [];
    for (const job of (SYNC_QUEUE || [])) {
      ...
      for (const k of wanted) {
        if (Object.prototype.hasOwnProperty.call(nextFields, k)) delete nextFields[k];
      }
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (existingFutureDeal) {
          purgePendingDealFieldUpdates(String(existingFutureDeal.ID), [UF_PRAZO, "TITLE"]);
  ...
  async function openBatchRescheduleAdvanced(ids, opts = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const nowIso = new Date().toISOString();
        for (const id of ids) {
          purgePendingDealFieldUpdates(String(id), UF_PRAZO);
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          purgePendingDealFieldUpdates(String(dealId), UF_PRAZO);
