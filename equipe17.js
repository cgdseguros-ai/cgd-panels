...
(function () {
  ...
  const LOGO_FALLBACK = "data:image/svg+xml;utf8," + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120"><defs><linearGradient id="g" x1="0" x2="1"><stop offset="0%" stop-color="#111827"/><stop offset="100%" stop-color="#374151"/></linearGradient></defs><circle cx="60" cy="60" r="56" fill="white" stroke="#111827" stroke-width="4"/><path d="M60 16 88 64 60 52 32 64Z" fill="url(#g)"/><path d="M60 104 88 56 60 68 32 56Z" fill="#111827" opacity=".9"/></svg>`);
  ...
  injectCSS(`
    ...
    .eqd-logo{width:40px;height:40px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.92);object-fit:contain;padding:5px;flex:0 0 auto;}
    ...
    .panelTools{display:flex;gap:6px;flex-wrap:wrap;align-items:center;justify-content:flex-end;flex:1 1 auto;}
  ...
  function reopenLeadsModalSafe() {
    ...
    openLeadsModalForUser(LAST_LEADS_CTX.userId, LAST_LEADS_CTX.kw || "", { useCache: true });
  }
  ...
  function getUserDealViewMode(userId){
    ...
    if (!STATE.viewModeByUser.has(k)) STATE.viewModeByUser.set(k, "CARD");
    return "CARD";
  }
  ...
  function leadPossuiPlano(lead) {
    const rawVal = lead && lead[LEAD_UF_POSSUI_PLANO];
    if (rawVal == null) return "";
    const raw = Array.isArray(rawVal) ? String(rawVal[0] || "").trim() : String(rawVal || "").trim();
    if (!raw) return "";
    const up = norm(raw);
    if (["Y","SIM","S","1","TRUE"].includes(up)) return "SIM";
    if (["N","NAO","NÃO","0","FALSE"].includes(up)) return "NÃO";
    return raw;
  }
  function sameDay(a,b){ return !!(a&&b) && a.getFullYear()===b.getFullYear() && a.getMonth()===b.getMonth() && a.getDate()===b.getDate(); }
  function existingFutureFollowupForLeadOrig(origId){
    const needle = String(origId || "").trim();
    if (!needle) return null;
    const today0 = dayStart(new Date()).getTime();
    return (STATE.dealsOpen || []).find((d) => isFollowupDeal(d) && getDealLeadOrigemId(d) === needle && d._prazo && new Date(d._prazo).getTime() >= today0) || null;
  }
  ...
  function applyLeadOverrides(lead) {
    if (!lead) return lead;
    const id = String(lead.ID || "").trim();
    if (!id) return lead;
    const ov = STATE.leadFieldOverrides.get(id);
    return ov ? { ...lead, ...ov } : lead;
  }
  function rememberLeadOverride(leadId, patch) {
    const id = String(leadId || "").trim();
    if (!id || !patch || typeof patch !== "object") return;
    const cur = STATE.leadFieldOverrides.get(id) || {};
    STATE.leadFieldOverrides.set(id, { ...cur, ...patch });
  }
  ...
  function leadSortDateMs(v) {
    const dt = tryParseDateAny(v);
  ...
  async function loadLeadsForOneUser(userId) {
    ...
    const statusIds = [leadStageId("NOVO LEAD"), leadStageId("EM ATENDIMENTO"), leadStageId("ATENDIDO"), leadStageId("QUALIFICADO"), leadStageId("LEAD DESCARTADO"), leadStageId("PERDIDO"), leadStageId("LEAD PERDIDO"), leadStageId("LEAD CONVERTIDO"), leadStageId("CONVERTIDO")].filter(Boolean);
    let leads = [];
    let okCalls = 0;
    if (statusIds.length) {
  ...
  async function loadLeadsSnapshotAllUsers(opts = {}) {
    await loadLeadStages();
    const now = Date.now();
    if (!opts.force && (now - LAST_ALL_LEADS_SNAPSHOT_AT < 90 * 1000) && STATE.leadsByUser && STATE.leadsByUser.size) return STATE.leadsByUser;
  ...
  async function loadDeals(opts = {}) {
    ...
    const canIncremental = !forceFull && !!STATE.lastDealsSyncAt && (nowMs - Number(STATE.lastFullDealsSyncAt || 0) < 20 * 60 * 1000) && Array.isArray(STATE.dealsAll) && STATE.dealsAll.length;
  ...
  function isWeekendDate(v){
    const d = v instanceof Date ? v : tryParseDateAny(v);
    if (!d || Number.isNaN(d.getTime())) return false;
    const w = d.getDay();
    return w === 0 || w === 6;
  }
  function adjustedMonthlyDate(rule, year, monthIndex){
    const day = Number(rule && rule.monthDay || 0);
    if (!(day >= 1 && day <= 31)) return null;
    const lastDay = new Date(year, monthIndex + 1, 0).getDate();
    const chosen = Math.min(day, lastDay);
    const d = new Date(year, monthIndex, chosen, 0, 0, 0, 0);
    const wd = d.getDay();
    if (wd === 6) d.setDate(d.getDate() - 1);
    else if (wd === 0) d.setDate(d.getDate() - 2);
    return d;
  }
  async function confirmWeekendIfNeeded(isoOrDate, label){
    const d = isoOrDate instanceof Date ? isoOrDate : tryParseDateAny(isoOrDate);
    if (!d || Number.isNaN(d.getTime()) || !isWeekendDate(d)) return true;
    return !!confirm(`Tem certeza que deseja agendar ${label || 'este item'} para final de semana? (${fmt(d)})`);
  }
  async function deleteDealSilently(dealId){
    const id = String(dealId || '').trim();
    if (!id) return;
    removeDealLocal(id);
    enqueueSync({ type: "dealDelete", dealId: id });
  }
  async function deleteFutureFollowupsForLeadOrig(origId, exceptDealId){
    const needle = String(origId || '').trim();
    if (!needle) return 0;
    const today0 = dayStart(new Date()).getTime();
    const rows = (STATE.dealsOpen || []).filter((d) => isFollowupDeal(d) && getDealLeadOrigemId(d) === needle && String(d.ID || '') !== String(exceptDealId || '') && d._prazo && new Date(d._prazo).getTime() >= today0);
    for (const d of rows) await deleteDealSilently(d.ID);
    return rows.length;
  }
  function occursOn(rule, d){
    ...
    if (t === "MONTHLY") {
      const ad = adjustedMonthlyDate(rule, d.getFullYear(), d.getMonth());
      return !!ad && sameDay(ad, d);
    }
    if (t === "YEARLY") {
    ...
    <div id="eqd-app">
      <div class="eqd-topbar">
        <div class="eqd-titleWrap">
          <img class="eqd-logo" src="${LOGO_URL}" alt="CGD" referrerpolicy="no-referrer" onerror="try{if(this.dataset.fallbackDone)return;this.dataset.fallbackDone=1;this.src='${LOGO_FALLBACK}';this.style.padding='2px';}catch(e){}">
  ...
  function openSelectRecentOldModal(n, onPick) {
    openModal(`Selecionar ${n}`, `
      <div style="display:flex;flex-direction:column;gap:12px">
        <div style="font-size:13px;font-weight:950;opacity:.86">Escolha como deseja selecionar os follow-ups.</div>
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap">
          <button class="eqd-btn" id="pickOldBtn">MAIS ANTIGOS</button>
          <button class="eqd-btn eqd-btnPrimary" id="pickRecentBtn">MAIS RECENTES</button>
        </div>
      </div>
    `);
    const oldBtn = document.getElementById("pickOldBtn");
    const recentBtn = document.getElementById("pickRecentBtn");
    if (oldBtn) oldBtn.onclick = () => { closeModal(); onPick(false); };
    if (recentBtn) recentBtn.onclick = () => { closeModal(); onPick(true); };
  }
  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal) && linkedLead) {
      ...
      parts.push(`FUPs concluídos: <strong>${countDoneFollowupsForLeadOrig(linkedLeadOrig)}</strong>`);
    ...
    const actionButtons = isFollowupDeal(deal)
      ? `
            ...
            ${linkedLead && ["LEAD DESCARTADO","PERDIDO","LEAD PERDIDO"].includes(String(leadStageNameById(linkedLead.STATUS_ID) || "").trim().toUpperCase()) ? `<button class="eqd-smallBtn eqd-smallBtnPrimary" data-action="followupResgatar" data-id="${deal.ID}">RESGATAR LEAD</button>` : ``}
...
restoreSyncQueue();
  ...
  async function flushSyncQueue() {
    ...
    try {
      ...
      while (SYNC_QUEUE.length) {
        ...
        if (job.type === "dealUpdate") { if (isTempId(job.dealId)) { SYNC_QUEUE.push(SYNC_QUEUE.shift()); persistSyncQueue(); continue; } await bx("crm.deal.update", { id: String(job.dealId), fields: job.fields }); SYNC_QUEUE.shift(); persistSyncQueue(); continue; }
  ...
  async function createFollowUpDealForUser(user, negocioNome, prazoIso, extraFields) {
    ...
    if (BLOCKED_EXACT_DEAL_TITLES.has(String(fields.TITLE || "").trim())) throw new Error("Este item está bloqueado para não ser recriado.");
  ...
  function openFollowUpModal(user, prefillName, opts) {
    ...
    let localDefault = new Date(base.getTime() - base.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
    ...
    const existingFutureFu = origId ? existingFutureFollowupForLeadOrig(origId) : null;
    const alreadyFuture = existingFutureFu ? 1 : 0;
    if (existingFutureFu && existingFutureFu._prazo) {
      try {
        const dt = new Date(existingFutureFu._prazo);
        if (!Number.isNaN(dt.getTime())) localDefault = new Date(dt.getTime() - dt.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
      } catch (_) {}
    }

    openModal(`FOLLOW-UP — ${user.name}`, `
      <div class="eqd-warn" id="fuWarn"></div>
      ${alreadyFuture ? `<div style="font-size:12px;font-weight:950;color:#92400e;background:#fef3c7;border:1px solid rgba(146,64,14,.18);padding:10px;border-radius:12px;margin-bottom:10px">Este lead já possui <strong>${alreadyFuture}</strong> follow-up agendado para hoje ou data futura${existingFutureFu && existingFutureFu._prazo ? `: <strong>${escHtml(fmt(existingFutureFu._prazo))}</strong>` : ''}.</div>` : ``}
      <div style="display:grid;grid-template-columns:1fr;gap:10px">
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap;margin-top:6px">
          ${alreadyFuture && existingFutureFu ? `<button class="eqd-btn" id="fuOpenExisting">EDITAR FOLLOW-UP AGENDADO</button>` : ``}
    ...
    const btnExisting = document.getElementById("fuOpenExisting");
    if (btnExisting && existingFutureFu) btnExisting.onclick = () => openEditPrazoModal(existingFutureFu.ID, { onDone: () => reopenLeadsModalSafe() });
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (!await confirmWeekendIfNeeded(prazoIso, 'o FOLLOW-UP')) return;
        if (origId && alreadyFuture) {
          throw new Error('Este lead já possui FOLLOW-UP futuro. Reagende o existente em vez de criar outro.');
        }
  ...
  async function promptLinkLeadForOldFollowup(deal, actionLabel) {
    ...
    let candidates = getLikelyLeadCandidatesForFollowup(deal, {});
    ...
    return await new Promise((resolve) => {
      ...
      const render = async (kw='') => {
        ...
        const rows = getLikelyLeadCandidatesForFollowup(deal, { kw }).slice(0, 200);
        const base = rows.length ? rows : candidates;
  ...
  function openNoFutureFollowupLeadBatchModal(user, leads, title) {
    ...
    const render = () => {
      ...
      box.innerHTML = visible.length ? visible.map((l) => `
        <label style="display:flex;gap:10px;align-items:flex-start;padding:10px 12px;border:1px solid rgba(0,0,0,.10);border-radius:14px;background:#fff">
          ...
          <div style="display:flex;flex-direction:column;gap:4px">
            <div style="font-weight:950">${escHtml(leadTitle(l))}</div>
            <div style="font-size:12px;opacity:.8;display:flex;gap:10px;flex-wrap:wrap">${leadOperadora(l) ? `<span>Operadora: <strong>${escHtml(leadOperadora(l))}</strong></span>` : ``}${leadTelefone(l) ? `<span>Tel: <strong>${escHtml(leadTelefone(l))}</strong></span>` : ``}${leadDateOnly(l) ? `<span>Data: <strong>${escHtml(leadDateOnly(l))}</strong></span>` : ``}</div>
          </div>
        </label>`).join('') : `<div class="eqd-empty">Nenhum lead.</div>`;
  ...
  function openFollowupListModalForUser(user) {
    if (!STATE.globalLeadsLoadedAt || (Date.now() - Number(STATE.globalLeadsLoadedAt || 0) > 2 * 60 * 1000)) {
      loadGlobalLeadsBulk(true).then(() => { try { openFollowupListModalForUser(user); } catch (_) {} }).catch(() => {});
    }
    ...
    openModal(`Lista de FOLLOW-UP — ${user.name}`, `
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:space-between">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <span id="fuListDayLabel" style="padding:6px 10px;border-radius:999px;border:1px solid rgba(0,0,0,.12);font-size:11px;font-weight:900;background:#fff">Sem data</span>
          <input id="fuListDate" type="date" style="padding:6px 10px;border-radius:999px;border:1px solid rgba(0,0,0,.18);font-weight:900" />
          <div style="display:flex;align-items:center;gap:6px"><span style="font-size:11px;font-weight:900;opacity:.78">OPERADORA</span><input id="fuListOper" class="eqd-searchInput" style="width:min(180px,34vw);color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Filtrar por operadora" /></div>
          ...
          <button class="eqd-btn" id="fuListBatchDone">CONCLUIR EM LOTE</button>
    ...
    let currentOperFilter = "";
    let currentNavDate = null;
    const syncDayLabel = () => {
      const el = document.getElementById('fuListDayLabel');
      if (!el) return;
      if (!currentDateFilter) { el.textContent = "Sem data"; return; }
      const dt = tryParseDateAny(`${currentDateFilter} 00:00`);
      el.textContent = dt ? fmtDateOnly(dt) : currentDateFilter;
    };
    ...
    const render = (kwRaw) => {
      ...
      currentOperFilter = String(document.getElementById('fuListOper')?.value || currentOperFilter || '').trim();
      const kw = norm(currentKw);
      const operNorm = norm(currentOperFilter);
      let filtered = list.slice();
      if (kw) filtered = filtered.filter((d) => norm([d.TITLE || "", d._obs || "", d._colabTxt || "", d._etapaTxt || "", d._urgTxt || ""].join(" ")).includes(kw));
      if (currentDateFilter) filtered = filtered.filter((d) => {
      ...
      if (operNorm) filtered = filtered.filter((d) => {
        const linkedLeadOrig = getDealLeadOrigemId(d);
        const linkedLead = linkedLeadOrig ? getLeadByOrigemId(linkedLeadOrig) : null;
        return norm(String(linkedLead ? leadOperadora(linkedLead) : '')).includes(operNorm);
      });
    ...
    const fuListOperEl = document.getElementById('fuListOper'); if (fuListOperEl) fuListOperEl.onkeydown = (e) => { if (e.key === 'Enter') render(document.getElementById("fuListSearch").value); };
    ...
    document.getElementById("fuListClear").onclick = () => { document.getElementById("fuListSearch").value = ""; if (fuListOperEl) fuListOperEl.value=''; if (fuListDateEl) fuListDateEl.value=''; currentDateFilter=''; currentOperFilter=''; currentNavDate=null; syncDayLabel(); render(""); };
    ...
    document.getElementById("fuListBatchDone").onclick = async () => {
      const ids = [...document.querySelectorAll("#fuListBox .eqd-batch:checked")].map((x) => x.getAttribute("data-id"));
      if (!ids.length) return alert("Selecione os follow-ups marcando 'Lote' nos cards.");
  ...
  document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') { try { persistSyncQueue(); } catch(_) {} } else { flushSyncQueue().catch(()=>{}); } });
  ...
  function makeDealListRow(deal, context) {
    ...
    const actions = isFollowupDeal(deal)
      ...
      : `<button class="eqd-smallBtn eqd-smallBtnPrimary" data-action="doneOnly" data-id="${deal.ID}">Concluir</button><button class="eqd-smallBtn" data-action="reagendarDireto" data-id="${deal.ID}">Reagendar</button><button class="eqd-smallBtn" data-action="editPrazo" data-id="${deal.ID}">Prazo</button>`;
  ...
  async function openManualLeadBatchCreateModal(user, defaultStatusId) {
    ...
    const rowTpl = (i) => `
      <tr>
        ...
        <td style="padding:6px"><select data-k="possuiPlano" data-i="${i}" style="width:100%;padding:8px;border-radius:10px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value=""></option><option value="SIM">SIM</option><option value="NÃO">NÃO</option></select></td>
      </tr>`;
    openModal(`Novo Lead em lote — ${user.name}`, `
      ...
      <div style="overflow:auto;max-height:70vh">
        <table style="width:100%;border-collapse:collapse">
          <thead><tr><th style="text-align:left;padding:6px">NOME</th><th style="text-align:left;padding:6px">IDADE</th><th style="text-align:left;padding:6px">TELEFONE</th><th style="text-align:left;padding:6px">BAIRRO</th><th style="text-align:left;padding:6px">FONTE</th><th style="text-align:left;padding:6px">POSSUI PLANO</th></tr></thead>
          <tbody id="nlbBody"></tbody>
        </table>
      </div>
    `, { wide:true, full:true });
    ...
    let rowCount = 0;
    const appendBatchRows = (n) => {
      if (!body) return;
      let html = "";
      for (let j = 0; j < n; j++) html += rowTpl(rowCount++);
      body.insertAdjacentHTML('beforeend', html);
    };
    setTimeout(() => appendBatchRows(5), 0);
    document.getElementById('nlbAddRow').onclick = () => { appendBatchRows(5); };
    document.getElementById('nlbSave').onclick = async () => {
      ...
      try {
        ...
        for (let i=0;i<rowCount;i++) {
          ...
          const possuiPlano = String(q('possuiPlano')?.value || '').trim();
          if (!name && !idade && !tel && !bairro && !fonte && !possuiPlano) continue;
          if (!name) throw new Error(`Linha ${i+1}: preencha o nome.`);
          rows.push({ name, idade, tel, bairro, fonte, possuiPlano });
        }
        ...
        for (const r of rows) {
          ...
          if (r.possuiPlano) fields[LEAD_UF_POSSUI_PLANO] = r.possuiPlano;
          ...
          const localLead = applyLeadOverrides({ ID: newLeadId, ASSIGNED_BY_ID: Number(uid), NAME: r.name, TITLE: r.name, STATUS_ID: String(defaultStatusId || leadStageId('NOVO LEAD') || ''), DATE_CREATE: nowIso, DATE_MODIFY: nowIso, [LEAD_UF_IDADE]: r.idade, [LEAD_UF_TELEFONE]: r.tel, [LEAD_UF_BAIRRO]: r.bairro, [LEAD_UF_FONTE]: r.fonte, [LEAD_UF_POSSUI_PLANO]: r.possuiPlano, [LEAD_UF_DATAHORA]: nowIso, [LEAD_UF_LEAD_ORIGEM]: newLeadId });
  ...
  async function openManualLeadCreateModal(user, defaultStatusId, opts) {
    ...
    openModal(`Novo Lead — ${user.name}`, `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div>
          ...
          <select id="nlPossuiPlano" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Selecione…</option><option value="SIM">SIM</option><option value="NÃO">NÃO</option></select>
        </div>
        <div style="grid-column:1 / -1">
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const possuiPlano = getVal("nlPossuiPlano");
        ...
        if (possuiPlano) fields[LEAD_UF_POSSUI_PLANO] = possuiPlano;
        ...
        const localLead = {
          ...
          [LEAD_UF_POSSUI_PLANO]: possuiPlano,
          [LEAD_UF_OBS]: obs,
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    if (shouldLoadLeads) {
      ...
      setTimeout(async () => {
        try {
          ...
          if (modal.style.display === 'block' && LAST_LEADS_CTX.userId === String(user.userId)) openLeadsModalForUser(user.userId, LAST_LEADS_CTX.kw || '', { useCache: true, dateFilter: LAST_LEADS_CTX.dateFilter || '', operFilter: LAST_LEADS_CTX.operFilter || '' });
        } catch (_) {}
      }, 0);
    }
    ...
    const byAtendidoDesc = (a,b) => (leadSortDateMs((b && b[LEAD_UF_ATENDIDO_DIA]) || (b && b.DATE_MODIFY) || (b && b.DATE_CREATE)) - leadSortDateMs((a && a[LEAD_UF_ATENDIDO_DIA]) || (a && a.DATE_MODIFY) || (a && a.DATE_CREATE))) || String((b && b.ID)||'').localeCompare(String((a && a.ID)||''));
    ...
    function cardLead(l, column) {
      ...
      return `
        <div class="leadCard">
          ...
          <div class="leadBtns">${btns}<button class="leadBtn" data-action="leadRevalidate" data-leadid="${l.ID}" data-userid="${user.userId}">REVALIDAR +60 DIAS</button></div>
        </div>
      `;
    }
  ...
  async function loadGlobalLeadsBulk(forceReload = false) {
    ...
    const rows = (leads || [])
      ...
      .map((l) => applyLeadOverrides({ ...l, _ownerUserId: String(l.ASSIGNED_BY_ID || "") }));
  ...
  function renderGlobalLeadsModal(filterUserId = "__ALL__") {
    ...
    const card = (l) => { const owner = USERS.find((u) => String(u.userId) === String(l._ownerUserId)); const atendidoStamp = getLeadAtendidoStamp(l); return `<div class="leadCard"><div class="leadTitle">${escHtml(leadTitle(l))}</div><div class="leadMeta"><span>USER: <strong>${escHtml(owner ? owner.name : l._ownerUserId)}</strong></span><span>Criado em: <strong>${l.DATE_CREATE ? fmt(l.DATE_CREATE) : "—"}</strong></span>${leadOperadora(l) ? `<span>Operadora: <strong>${escHtml(leadOperadora(l))}</strong></span>` : ``}${leadTelefone(l) ? `<span>Telefone: <strong>${escHtml(leadTelefone(l))}</strong></span>` : ``}${leadPossuiPlano(l) ? `<span>Possui plano: <strong>${escHtml(leadPossuiPlano(l))}</strong></span>` : ``}${atendidoStamp ? `<span>Atendido em: <strong>${escHtml(atendidoStamp)}</strong></span>` : ``}</div></div>`; };
  ...
  function renderGeneral() {
    el.main.innerHTML = `
      <div style="margin-bottom:10px;font-weight:950;opacity:.75;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap"><span>Dia selecionado: ${fmtDateOnly(selectedDate)}</span><button class="eqd-btn" id="eqd-legendBtn">LEGENDA</button></div>
      <div class="userGrid">
  ...
  async function openBatchRescheduleAdvanced(ids, opts = {}) {
    ...
    openModal("Reagendar em lote", `
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:10px">
        <div style="font-size:12px;font-weight:950;opacity:.85">Selecionadas: <strong>${ids.length}</strong></div>
        <div>
          <div style="font-size:11px;font-weight:900;margin-bottom:6px">RESULTADO</div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button class="eqd-btn eqd-btnPrimary" id="brModeDone" type="button">FEITO</button>
            <button class="eqd-btn" id="brModeNoDone" type="button">NÃO FEITO</button>
          </div>
          <div id="brModeLabel" style="font-size:11px;font-weight:900;opacity:.75;margin-top:6px">FEITO: conclui o atual e cria um novo.</div>
        </div>
        <div>
    ...
    let mode = "done";
    const syncModeUi = () => {
      document.getElementById("brModeDone")?.classList.toggle("eqd-btnPrimary", mode === "done");
      document.getElementById("brModeNoDone")?.classList.toggle("eqd-btnPrimary", mode === "nodone");
      const lab = document.getElementById("brModeLabel");
      if (lab) lab.textContent = mode === "done"
        ? "FEITO: conclui o atual e cria um novo."
        : "NÃO FEITO: mantém o atual e só altera a nova data.";
    };
    document.getElementById("brModeDone").onclick = () => { mode = "done"; syncModeUi(); };
    document.getElementById("brModeNoDone").onclick = () => { mode = "nodone"; syncModeUi(); };
    syncModeUi();
      ...
      try {
        ...
        for (const id of ids) {
          ...
          if (mode === "nodone") {
            ...
            await markDone(id);
            const fields = { CATEGORY_ID: Number(CATEGORY_MAIN), STAGE_ID: String(d.STAGE_ID || ""), TITLE: String(d.TITLE || ""), ASSIGNED_BY_ID: Number(d.ASSIGNED_BY_ID || d._assigned || 0), [UF_PRAZO]: prazoIso };
  ...
  async function openDoneSearchModal(userId) {
    ...
    `, { wide:true });
    document.getElementById("doneSearchBtn").onclick = () => {
  ...
  function renderUserPanel(userId) {
    ...
    const viewBtn = ``;
    const vincularFupBtn = LEAD_USERS.has(String(user.userId)) ? `<button class="eqd-btn" data-action="adminLinkFup" data-userid="${user.userId}">VINCULAR FUP</button>` : ``;
      ...
      <div class="panelHead">
        <div class="panelUserInfo">
          ...
          <div>
            <div class="panelUserName">${escHtml(user.name)}</div>
            <div class="panelUserTeam">Equipe ${escHtml(user.team || "")}</div>
            <div style="font-size:12px;font-weight:950;opacity:.92;background:rgba(255,255,255,.14);border:1px solid rgba(255,255,255,.16);padding:4px 10px;border-radius:999px;display:inline-flex;align-items:center">Dia selecionado: ${fmtDateOnly(selectedDate)}</div>
          </div>
        </div>
    ...
    if (hasLeadsBtn && !STATE.leadsByUser.has(String(user.userId))) {
    ...
    el.main.innerHTML = `
      <div class="panelHead">
        ...
        <div class="panelTools">
          ...
          <button class="eqd-btn" data-action="followUpMulti">FOLLOW-UP</button>
          <button class="eqd-btn" data-action="backToPrevious">VOLTAR</button>
        </div>
      </div>
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      await loadDeals({ forceFull: !!(opts && opts.forceFullDeals) });
      ...
      if (!(opts && opts.skipLeads)) {
        try { await loadLeadsSnapshotAllUsers({ force: !!forceNetwork }); } catch (_) {}
      }
      overlayPendingSyncState();
      renderFooterPeople();
      DEALS_FAIL_STREAK = 0;
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    openModal("Reagendar", `
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:10px">
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">RESULTADO</div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button class="eqd-btn eqd-btnPrimary" id="drModeDone" type="button">FEITO</button>
            <button class="eqd-btn" id="drModeNoDone" type="button">NÃO FEITO</button>
          </div>
          <div id="drModeLabel" style="font-size:11px;font-weight:900;opacity:.75;margin-top:6px">FEITO: conclui o atual e cria um novo.</div>
        </div>
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">TÍTULO</div><input id="drTitle" value="${escHtml(String(d.TITLE || ""))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">NOVO PRAZO</div><input id="drPrazo" type="datetime-local" value="${escHtml(localDefault)}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        ${!isFollowupDeal(d) ? `<div><div style="font-size:11px;font-weight:900;margin-bottom:6px">URGÊNCIA</div><select id="drUrg" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Carregando…</option></select></div>` : ``}
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">OBS</div><textarea id="drObs" rows="5" style="width:100%;border-radius:14px;border:1px solid rgba(30,40,70,.16);padding:10px;font-weight:850;outline:none">${escHtml(String(d[UF_OBS] || ""))}</textarea></div>
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="drSave">SALVAR</button></div>
      </div>
    `);
    let mode = "done";
    const warn = document.getElementById("drWarn"); const btn = document.getElementById("drSave");
    const modeDone = document.getElementById("drModeDone");
    const modeNoDone = document.getElementById("drModeNoDone");
    const modeLabel = document.getElementById("drModeLabel");
    const syncModeUi = () => {
      if (modeDone) modeDone.classList.toggle("eqd-btnPrimary", mode === "done");
      if (modeNoDone) modeNoDone.classList.toggle("eqd-btnPrimary", mode === "nodone");
      if (modeLabel) modeLabel.textContent = mode === "done"
        ? "FEITO: conclui o atual e cria um novo."
        : "NÃO FEITO: mantém o atual e só altera a nova data.";
    };
    if (modeDone) modeDone.onclick = () => { mode = "done"; syncModeUi(); };
    if (modeNoDone) modeNoDone.onclick = () => { mode = "nodone"; syncModeUi(); };
    syncModeUi();
    (async () => { if (!isFollowupDeal(d)) { try { const meta = await ensureDealFieldsMeta(); const items = getFieldItemsFromMeta(meta, UF_URGENCIA); const sel = document.getElementById('drUrg'); if (sel) { sel.innerHTML = renderOptions(items, 'Selecione a urgência…'); if (d[UF_URGENCIA]) sel.value = String(d[UF_URGENCIA]); } } catch(_) {} } })();
    btn.onclick = async () => {
      try {
        btn.disabled = true; warn.style.display = "none";
        const title = String(document.getElementById("drTitle").value || "").trim();
        const prazoIso = localInputToIsoWithOffset(String(document.getElementById("drPrazo").value || "").trim());
        const obs = String(document.getElementById("drObs").value || "").trim();
        const originalStageId = String(d.STAGE_ID || "");
        if (!title) throw new Error("Título vazio.");
        if (!prazoIso) throw new Error("Prazo inválido.");
        let resolvedLead = null;
        if (isFollowupDeal(d) && !d[DEAL_UF_LEAD_ORIGEM]) {
        ...
        const drUrg = document.getElementById('drUrg');
        if (mode === "nodone") {
          const fields = { TITLE: title, [UF_PRAZO]: prazoIso, [UF_OBS]: obs };
          if (!isFollowupDeal(d) && drUrg && drUrg.value) fields[UF_URGENCIA] = String(drUrg.value);
          updateDealInState(dealId, { TITLE: title, [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString(), _late: false, [UF_OBS]: obs, DATE_MODIFY: new Date().toISOString() });
          enqueueSync({ type: "dealUpdate", dealId: String(dealId), fields });
        } else {
          await markDone(dealId);
          const fields = { CATEGORY_ID: Number(CATEGORY_MAIN), STAGE_ID: originalStageId, TITLE: title, ASSIGNED_BY_ID: Number(d.ASSIGNED_BY_ID || d._assigned || 0), [UF_PRAZO]: prazoIso };
          if (d[UF_TAREFA]) fields[UF_TAREFA] = d[UF_TAREFA];
          if (!isFollowupDeal(d) && drUrg && drUrg.value) fields[UF_URGENCIA] = String(drUrg.value); else if (d[UF_URGENCIA]) fields[UF_URGENCIA] = d[UF_URGENCIA];
          if (d[UF_ETAPA]) fields[UF_ETAPA] = d[UF_ETAPA];
          if (d[UF_COLAB]) fields[UF_COLAB] = d[UF_COLAB];
          if (d[DEAL_UF_LEAD_ORIGEM]) fields[DEAL_UF_LEAD_ORIGEM] = d[DEAL_UF_LEAD_ORIGEM];
  ...
  async function openChangeColabModal(dealId){
    ...
    btn.onclick = async () => {
      ...
      try{
        ...
        closeModal();
        await refreshData(true, { forceRecur: false, forceFullDeals: false });
    ...
    if (uid === "815") {
      return new Promise((resolve) => {
        ...
        document.getElementById("pl0").onclick = async () => {
          try { closeModal(); await routeLeadToCategory(uid, leadId, 0, sConv); await markDone(dealId); reopenLeadsModalSafe(); renderCurrentView(); }
          finally { resolve(); }
        };
        document.getElementById("pl11").onclick = async () => {
          try { closeModal(); await routeLeadToCategory(uid, leadId, 11, sConv); await markDone(dealId); reopenLeadsModalSafe(); renderCurrentView(); }
          finally { resolve(); }
        };
      });
    }
  ...
  async function openLeadAtendidoConfirmModal(userId, leadId, toStatus){
    ...
    return await new Promise((resolve) => {
      ...
      document.getElementById('attOk').onclick = async () => {
        ...
        if (!existingCount && !(await confirmWeekendIfNeeded(fuPrazoIso, 'o FOLLOW-UP'))) return;
        closeModal();
  ...
  async function askPerdidoFollowupOptions(lead) {
    const now = new Date();
    ...
    return await new Promise((resolve) => {
      openModal("Lead perdido", `
        ...
        <div style="display:flex;flex-direction:column;gap:12px;min-height:280px">
          <div style="font-size:15px;font-weight:950">Deseja agendar FOLLOW-UP para este lead perdido?</div>
          <div style="font-size:13px;font-weight:900;opacity:.82">${escHtml(leadTitle(lead))}</div>
          <label style="display:flex;gap:8px;align-items:center;font-weight:900"><input type="checkbox" id="lostCreate"> Agendar FOLLOW-UP</label>
          <div id="lostBox" style="display:none;padding:12px;border:1px solid rgba(0,0,0,.10);border-radius:14px;background:#fff">
            <input id="lostFuNome" value="${escHtml(leadTitle(lead))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);margin-bottom:8px" placeholder="Nome do follow-up" />
            <input id="lostFuPrazo" type="datetime-local" value="${localDefault}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)" />
          </div>
          <div style="display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap;margin-top:auto">
            <button class="eqd-btn" id="lostCancel">Cancelar</button>
            <button class="eqd-btn eqd-btnPrimary" id="lostOk">Confirmar</button>
          </div>
        </div>
      `, { wide:true });
      const chk = document.getElementById("lostCreate");
      const box = document.getElementById("lostBox");
      if (chk) chk.onchange = () => { if (box) box.style.display = chk.checked ? "block" : "none"; };
      document.getElementById("lostCancel").onclick = () => { closeModal(); resolve(false); };
      document.getElementById("lostOk").onclick = async () => {
        const createFu = !!document.getElementById("lostCreate")?.checked;
        ...
        if (createFu && !fuNome) { if (warn) { warn.style.display='block'; warn.textContent='Preencha o nome do FOLLOW-UP.'; } return; }
        if (createFu && !fuPrazoIso) { if (warn) { warn.style.display='block'; warn.textContent='Preencha a data e hora do FOLLOW-UP.'; } return; }
        if (createFu && !(await confirmWeekendIfNeeded(fuPrazoIso, 'o FOLLOW-UP'))) return;
        closeModal();
        resolve({ createFu, fuNome, fuPrazoIso });
      };
    });
  }
...
async function moveLeadStage(userId, leadId, toStatus, opts){
    ...
    if (target === String(sPerdido) && !(opts && opts.skipPerdidoModal) && cur) {
      const ans = await askPerdidoFollowupOptions(cur);
      if (!ans) return;
      return moveLeadStage(uid, leadId, target, { skipPerdidoModal: true, createFu: ans.createFu, fuNome: ans.fuNome, fuPrazoIso: ans.fuPrazoIso });
    }
    ...
    if (target === String(sConv) && uid === "815") {
      openModal("Escolher pipeline", `<div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap"><button class="eqd-btn" id="pl0">SAÚDE (0)</button><button class="eqd-btn eqd-btnPrimary" id="pl11">SEGUROS (11)</button></div>`);
      document.getElementById("pl0").onclick = async () => { closeModal(); await routeLeadToCategory(uid, leadId, 0, target); reopenLeadsModalSafe(); };
      document.getElementById("pl11").onclick = async () => { closeModal(); await routeLeadToCategory(uid, leadId, 11, target); reopenLeadsModalSafe(); };
      return;
    }
    ...
    if (target === String(sPerdido)) {
      const origem = leadOrigemId(cur || arr[i] || {}) || String(leadId);
      if (origem) await deleteFutureFollowupsForLeadOrig(origem);
      if (ownerUser && opts && opts.createFu) {
        const fuNome = String((opts && opts.fuNome) || leadTitle(cur || arr[i] || {})).trim();
        const fuPrazoIso = String((opts && opts.fuPrazoIso) || '').trim();
        if (fuPrazoIso) await createFollowUpDealForUser(ownerUser, fuNome, fuPrazoIso, origem ? { [DEAL_UF_LEAD_ORIGEM]: origem } : {});
      }
    }
    reopenLeadsModalSafe();
  }
  ...
  function openLeadStageListModal(userId, stageId, title, kwRaw = "") {
    const user = USERS.find((u) => String(u.userId) === String(userId));
    const stageIds = Array.isArray(stageId) ? stageId.map((v) => String(v || '')).filter(Boolean) : [String(stageId || '')].filter(Boolean);
    const arr = (STATE.leadsByUser.get(String(userId)) || []).filter((l) => stageIds.includes(String(l.STATUS_ID || '')));
    const kw = norm(String(kwRaw || '').trim());
    ...
    const card = (l) => `
      <div class="leadCard">
        ...
        <div class="leadBtns"><button class="leadBtn leadBtnP" data-action="leadMove" data-leadid="${l.ID}" data-tostatus="${escHtml(String(leadStageId('EM ATENDIMENTO')||''))}" data-userid="${userId}">VOLTAR PARA EM ATENDIMENTO</button></div>
      </div>`;
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function openAdminLinkFupModal(adminUserId, selectedUid = '', preloadedOrphans = null) {
    ...
    document.querySelectorAll('.adminFupLinkOne').forEach((btn) => btn.onclick = async () => {
      ...
      btn.disabled = true;
      try {
        await bx('crm.deal.update', { id: dealId, fields: { [DEAL_UF_LEAD_ORIGEM]: origem } });
  ...
  async function revalidateLead(userId, leadId) {
    ...
    const arr = (STATE.leadsByUser.get(uid) || []).slice();
    const lead = arr.find((l) => String(l.ID) === String(leadId));
    if (!lead) throw new Error("Lead não encontrado para revalidar.");
    const discardStatus = String(leadStageId("LEAD DESCARTADO") || leadStageId("PERDIDO") || "");
    const targetStatus = String(leadStageId("NOVO LEAD") || leadStageId("EM ATENDIMENTO") || lead.STATUS_ID || "");
    const nowIso = new Date().toISOString();
    const fields = {
      ASSIGNED_BY_ID: Number(uid),
      NAME: lead.NAME || leadTitle(lead),
      TITLE: leadTitle(lead),
      STATUS_ID: targetStatus,
      [LEAD_UF_OPERADORA]: leadOperadora(lead),
      [LEAD_UF_IDADE]: leadIdade(lead),
      [LEAD_UF_TELEFONE]: leadTelefone(lead),
      [LEAD_UF_BAIRRO]: leadBairro(lead),
      [LEAD_UF_FONTE]: leadFonte(lead),
      [LEAD_UF_DATAHORA]: nowIso,
      [LEAD_UF_HELENA]: leadHelena(lead),
      [LEAD_UF_PESSOAL]: leadPessoal(lead),
      [LEAD_UF_POSSUI_PLANO]: leadPossuiPlano(lead),
    };
    setBusy("Revalidando lead…");
    try {
      if (discardStatus) await bx("crm.lead.update", { id: String(leadId), fields: { STATUS_ID: discardStatus } });
      const created = await bx("crm.lead.add", { fields });
      const newLeadId = String((created && (created.result || created)) || "").trim() || `local_${Date.now()}`;
      rememberLeadOverride(leadId, { STATUS_ID: discardStatus || lead.STATUS_ID });
      const newLead = applyLeadOverrides({ ...lead, ID: newLeadId, STATUS_ID: targetStatus, DATE_CREATE: nowIso, DATE_MODIFY: nowIso, [LEAD_UF_DATAHORA]: nowIso, [LEAD_UF_LEAD_ORIGEM]: newLeadId });
      const filtered = arr.filter((l) => String(l.ID) !== String(leadId));
      filtered.unshift(newLead);
      STATE.leadsByUser.set(uid, filtered);
      if (Array.isArray(STATE.globalLeadsAll)) {
        STATE.globalLeadsAll = STATE.globalLeadsAll.filter((l) => String(l.ID) !== String(leadId));
        STATE.globalLeadsAll.unshift({ ...newLead, _ownerUserId: uid });
      }
    } finally { clearBusy(); }
    return openLeadsModalForUser(uid, LAST_LEADS_CTX.kw || "", { useCache: true, dateFilter: String((document.getElementById("leadDateFilter")||{}).value || '').trim(), operFilter: String((document.getElementById("leadOperFilter")||{}).value || '').trim() });
  }
  ...
  async function toggleLeadField(leadId, fieldName, userId, reopenMode) {
    ...
    rememberLeadOverride(leadId, { [fieldName]: next });
    const arr = (STATE.leadsByUser.get(ownerUid) || []).slice();
    const idx = arr.findIndex((l) => String(l.ID) === String(leadId));
    ...
    if (reopenMode === "leads") return openLeadsModalForUser(ownerUid, LAST_LEADS_CTX.kw || "", { useCache: true });
    renderCurrentView();
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadRevalidate") {
        ...
        const leadId = String(a.getAttribute("data-leadid")||"");
        if (!uid || !leadId) return;
        if (!confirm("Revalidar este lead por mais 60 dias? O atual irá para LEAD DESCARTADO/PERDIDO e um novo será criado com a data atual e os mesmos dados principais.")) return;
      ...
      if (act === "leadShowLost") { const uid = String(a.getAttribute("data-userid")||""); return openLeadStageListModal(uid, [leadStageId("LEAD DESCARTADO"), leadStageId("PERDIDO"), leadStageId("LEAD PERDIDO")].filter(Boolean), 'Perdidos'); }
      if (act === "leadShowConv") { const uid = String(a.getAttribute("data-userid")||""); return openLeadStageListModal(uid, [leadStageId("LEAD CONVERTIDO"), leadStageId("CONVERTIDO")].filter(Boolean), 'Convertidos'); }
      ...
      if (act === "followupPerdido") {
        ...
        if (!lead) return alert("Este FOLLOW-UP não tem lead de origem vinculado.");
        const uid = String(lead.ASSIGNED_BY_ID || lead._ownerUserId || "");
        await moveLeadStage(uid, String(lead.ID), leadStageId("LEAD DESCARTADO") || leadStageId("PERDIDO"));
        await markDone(id);
        return renderCurrentView();
      }
      if (act === "followupResgatar") {
        const d = getDealById(id);
        let lead = await resolveLeadForDeal(d);
        if (!lead) lead = await promptLinkLeadForOldFollowup(d, 'resgatar lead');
        if (!lead) return alert("Este FOLLOW-UP não tem lead de origem vinculado.");
        const uid = String(lead.ASSIGNED_BY_ID || lead._ownerUserId || "");
        await moveLeadStage(uid, String(lead.ID), leadStageId("EM ATENDIMENTO"), { skipAtendidoModal: true });
        return renderCurrentView();
      }
  ...
  (async function init(){
    ...
    renderGeneral();
    setSoftStatus("JS: ok");
    await refreshData(false, { forceRecur: false, forceFullDeals: false, skipLeads: true });
    renderCurrentView();
    setTimeout(async () => {
      try {
        await loadLeadsSnapshotAllUsers({ force: true });
        if (currentView.kind === "general") renderCurrentView();
      } catch (_) {}
    }, 0);
  })();
