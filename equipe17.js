...
(function () {
  ...
  async function recurringInstanceExistsRemote(uid, stageId, title, iso, markerText){
    ...
    return (list || []).some((d) => {
      ...
      return obs.includes(markerText) || true;
    });
  }
  ...
  async function loadRecurrenceConfigDeals() {
    ...
    (cfgDeals || []).forEach((d) => {
      const uid = String(d && d.ASSIGNED_BY_ID || "").trim();
      if (!uid) return;
    ...
    for (const [uid, deals] of byUser.entries()) {
      ...
      STATE.recurConfigDealIdByUser.set(uid, String(latest && latest.ID || ""));

      const merged = [];
      const seen = new Set();
      for (const d of ordered) {
        const raw = String(d && d[UF_OBS] || "").trim();
        const j = safeJsonParse(raw);
        const rules = (j && Array.isArray(j.rules)) ? j.rules : [];
        for (const rule of rules) {
          const rid = String(rule && rule.id || "").trim();
          const key = rid || `${String(rule && rule.title || "").trim()}|${String(rule && rule.type || "").trim()}|${String(rule && rule.hh || "")}|${String(rule && rule.mm || "")}`;
