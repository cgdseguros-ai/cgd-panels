...
(function () {
  ...
  const DIRECT_DEAL_PATCH_TTL_MS = 15000;
  const DIRECT_DEAL_PATCHES = new Map();
  ...
  async function safeDealUpdate(dealId, fields) {
    ...
    holdDirectDealPatch(id, fields, 20000);
    try {
      const res = await bx("crm.deal.update", { id, fields });
      holdDirectDealPatch(id, fields, 12000);
      return res;
    } catch (e) {
      ...
      if (await verify()) return true;
      try {
        const retry = await bx("crm.deal.update", { id, fields });
        holdDirectDealPatch(id, fields, 12000);
        return retry;
      } catch (_) {}
...
restoreSyncQueue();
  ...
  function holdDirectDealPatch(dealId, patchFields, ttlMs = DIRECT_DEAL_PATCH_TTL_MS) {
    const id = String(dealId || "").trim();
    if (!id || !patchFields) return;
    const prev = DIRECT_DEAL_PATCHES.get(id) || { fields: {}, until: 0 };
    DIRECT_DEAL_PATCHES.set(id, {
      fields: { ...(prev.fields || {}), ...(patchFields || {}) },
      until: Date.now() + Math.max(1000, Number(ttlMs || DIRECT_DEAL_PATCH_TTL_MS) || DIRECT_DEAL_PATCH_TTL_MS),
    });
  }

  function clearExpiredDirectDealPatches() {
    const now = Date.now();
    for (const [id, entry] of DIRECT_DEAL_PATCHES.entries()) {
      if (!entry || Number(entry.until || 0) <= now) DIRECT_DEAL_PATCHES.delete(id);
    }
  }

  function overlayDirectDealPatches() {
    clearExpiredDirectDealPatches();
    for (const [id, entry] of DIRECT_DEAL_PATCHES.entries()) {
      if (!entry || !entry.fields) continue;
      updateDealInState(String(id), { ...(entry.fields || {}), DATE_MODIFY: new Date().toISOString() });
    }
  }
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (existingFutureDeal) {
          UI_REFRESH_HOLD_UNTIL = Date.now() + 8000;
          holdDirectDealPatch(String(existingFutureDeal.ID), { TITLE: `FOLLOW-UP ${nm}`, [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString(), _late:false }, 20000);
          await safeDealUpdate(String(existingFutureDeal.ID), { TITLE: `FOLLOW-UP ${nm}`, [UF_PRAZO]: prazoIso });
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          UI_REFRESH_HOLD_UNTIL = Date.now() + 8000;
          holdDirectDealPatch(String(dealId), { [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString(), _late:false }, 20000);
  ...
  function openEditPrazoModal(dealId){
    ...
    btn.onclick = async () => {
      ...
      try{
        ...
        UI_REFRESH_HOLD_UNTIL = Date.now() + 8000;
        holdDirectDealPatch(String(dealId), { [UF_PRAZO]: iso, _prazo: new Date(iso).toISOString(), _late: false }, 20000);
