...
(function () {
  ...
  function getUserDealViewMode(userId){
    ...
    if (STATE.viewModeByUser.has(k)) return STATE.viewModeByUser.get(k);
    let v = "CARD";
    try { v = String(localStorage.getItem(`EQD_VIEWMODE_${k}`) || "CARD").toUpperCase(); } catch(_) {}
    v = v === "LIST" ? "LIST" : "CARD";
    STATE.viewModeByUser.set(k, v);
    return v;
  }
  ...
  function makeDealCard(deal, context) {
    ...
    if (isFollowupDeal(deal) && linkedLeadOrig) {
      if (linkedLead && leadStageNameById(linkedLead.STATUS_ID) === "QUALIFICADO") tags.push(`<span class="eqd-tag" style="background:#dcfce7;color:#166534">QUALIFICADO</span>`);
    ...
    const actionButtons = isFollowupDeal(deal)
      ? `
            <button class="eqd-smallBtn" data-action="linkFollowup" data-id="${deal.ID}">Vincular</button>
...
restoreSyncQueue();
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (origId && alreadyFuture) {
          if (!confirm("Este lead já possui FOLLOW-UP futuro. Deseja criar outro mesmo assim?")) return;
        }
    ...
    return await new Promise((resolve) => {
      openModal(`Vincular FOLLOW-UP ao lead`, `
        <div style="display:flex;flex-direction:column;gap:10px">
          ...
          <div style="font-size:11px;font-weight:900;opacity:.72">Prioridade de sugestão: leads sem FOLLOW-UP futuro vinculado.</div>
      ...
      const render = (kw='') => {
        ...
        sel.innerHTML = `<option value="">Selecione um lead…</option>` + base.map((l) => {
          const origem = leadOrigemId(l) || l.ID;
          const fu = countOpenFutureFollowupsForLeadOrig(origem, deal.ID);
          return `<option value="${escHtml(String(l.ID))}">${escHtml(leadTitle(l))} • ${escHtml(leadStageNameById(l.STATUS_ID)||'Sem etapa')}${fu ? ` • ${fu} FUP(s)` : ' • sem FUP'}</option>`;
        }).join('');
      };
      render(q && q.value || '');
  ...
  function renderDealsByMode(deals, context, userId) {
    const mode = getUserDealViewMode(userId);
    if (mode === 'LIST') return `<div class="eqd-listWrap">${deals.length ? deals.map((d) => makeDealListRow(d, context)).join('') : `<div class="eqd-empty">Sem itens</div>`}</div>`;
  ...
  async function openLeadsModalForUser(userId, kwRaw, opts) {
    ...
    const filtered = (kw ? leadsUser.filter((l) => leadMatchesKw(l, kw)) : leadsUser).filter((l) => {
      const okDate = !dateFilter || String((leadDataHora(l) || '')).slice(0,10) === dateFilter;
  ...
  function renderUserPanel(userId) {
    ...
    const viewBtn = `<button class="eqd-btn eqd-viewToggle" data-action="toggleDealView" data-userid="${user.userId}">${viewMode === 'LIST' ? 'MODO LISTA' : 'MODO CARD'}</button>`;
    const vincularFupBtn = String(user.userId) === '1' ? `<button class="eqd-btn" data-action="adminLinkFup" data-userid="${user.userId}">VINCULAR FUP</button>` : ``;
  ...
  async function moveLeadStage(userId, leadId, toStatus){
    ...
    if (target === String(sAtendido) && !confirm('Você realmente já atendeu o lead?')) return;
    ...
    if (i >= 0) {
      ...
      if (target === String(sAtendido) && !curMark) patch[LEAD_UF_ATENDIDO_DIA] = new Date().toISOString();
      if (target === String(sAtendido) && !leadHelena(cur) && !leadPessoal(cur)) throw new Error('Marque HELENA e/ou WPP DIRETO antes de mover para ATENDIDO.');
    }
  ...
  function openAdminLinkFupModal(adminUserId, selectedUid = "") {
    const userOpts = USERS.map((u) => `<option value="${u.userId}" ${String(selectedUid)===String(u.userId)?'selected':''}>${escHtml(u.name)}</option>`).join('');
    const uid = String(selectedUid || '');
    const orphan = uid ? (STATE.dealsOpen || []).filter((d) => String(d.ASSIGNED_BY_ID || d._assigned || '') === uid && isFollowupDeal(d) && !getDealLeadOrigemId(d)).sort((a,b) => String(a.TITLE||'').localeCompare(String(b.TITLE||''), 'pt-BR')) : [];
    const row = (d, idx) => {
      const suggestions = getLikelyLeadCandidatesForFollowup(d, {}).slice(0,20);
      return `<div style="border:1px solid rgba(0,0,0,.12);border-radius:14px;padding:10px;background:rgba(255,255,255,.62);display:grid;grid-template-columns:1.4fr 1fr;gap:10px;align-items:center"><div><div style="font-weight:950">${escHtml(bestTitleFromText(d.TITLE||''))}</div><div style="font-size:11px;font-weight:900;opacity:.75">Prazo: ${escHtml(d._prazo ? fmt(d._prazo) : '—')} • ID: ${escHtml(String(d.ID))}</div></div><div><input class="eqd-searchInput adminFupLeadSearch" data-idx="${idx}" style="width:100%;color:#111;background:#fff;border-color:rgba(0,0,0,.18)" placeholder="Buscar lead provável" value="${escHtml(firstNameFromText(bestTitleFromText(String(d.TITLE||'').replace(/^FOLLOW-UP\s*/i,''))))}" /><select class="adminFupLeadSelect" data-idx="${idx}" data-dealid="${escHtml(String(d.ID))}" style="width:100%;margin-top:8px;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900"><option value="">Selecione…</option>${suggestions.map((l) => `<option value="${escHtml(String(l.ID))}">${escHtml(leadTitle(l))}</option>`).join('')}</select></div></div>`;
    };
    openModal(`Vincular FUP — administração`, `
      ...
      <div style="margin-top:10px;font-size:12px;font-weight:900;opacity:.8">${uid ? `FOLLOW-UPs sem lead vinculado: <strong>${orphan.length}</strong>` : 'Selecione uma user para listar os FOLLOW-UPs sem vínculo.'}</div>
      <div id="adminFupBox" style="margin-top:10px;display:flex;flex-direction:column;gap:8px">${uid ? (orphan.length ? orphan.map(row).join('') : `<div class="eqd-empty">Nenhum FOLLOW-UP sem lead vinculado.</div>`) : ``}</div>
      ${uid && orphan.length ? `<div style="display:flex;justify-content:flex-end;margin-top:10px"><button class="eqd-btn eqd-btnPrimary" id="adminFupSave">SALVAR VÍNCULOS</button></div>` : ``}
    `, { wide:true, full:true });
    const loadBtn = document.getElementById('adminFupLoad'); if (loadBtn) loadBtn.onclick = async () => { const v = String((document.getElementById('adminFupUser')||{}).value || ''); if (!v) return alert('Selecione a user.'); if (!STATE.leadsByUser.get(v)) { try { await loadLeadsForOneUser(v); } catch(_) {} } openAdminLinkFupModal(adminUserId, v); };
    document.querySelectorAll('.adminFupLeadSearch').forEach((inp) => { inp.oninput = (e) => { const idx = String(e.target.getAttribute('data-idx') || ''); const sel = document.querySelector(`.adminFupLeadSelect[data-idx="${idx}"]`); const dealId = String(sel && sel.getAttribute('data-dealid') || ''); const deal = getDealById(dealId); const rows = getLikelyLeadCandidatesForFollowup(deal, { kw: e.target.value || '' }).slice(0, 40); if (sel) sel.innerHTML = `<option value="">Selecione…</option>` + rows.map((l) => `<option value="${escHtml(String(l.ID))}">${escHtml(leadTitle(l))}</option>`).join(''); }; });
    const saveBtn = document.getElementById('adminFupSave'); if (saveBtn) saveBtn.onclick = async () => { const sels = [...document.querySelectorAll('.adminFupLeadSelect')]; let done = 0; for (const sel of sels) { const leadId = String(sel.value || ''); const dealId = String(sel.getAttribute('data-dealid') || ''); if (!leadId || !dealId) continue; let lead = null; for (const arr of STATE.leadsByUser.values()) { lead = (arr || []).find((x) => String(x.ID) === leadId) || lead; if (lead) break; } if (!lead) continue; const origem = String(leadOrigemId(lead) || lead.ID || '').trim(); await bx('crm.deal.update', { id: dealId, fields: { [DEAL_UF_LEAD_ORIGEM]: origem } }); updateDealInState(dealId, { [DEAL_UF_LEAD_ORIGEM]: origem, DATE_MODIFY: new Date().toISOString() }); done++; } alert(done ? `${done} vínculo(s) salvo(s).` : 'Nenhum vínculo selecionado.'); if (uid) openAdminLinkFupModal(adminUserId, uid); };
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "adminLinkFup") { const uid = String(userId||""); if (!uid) return; if (!STATE.globalLeadsAll.length) { try { await loadLeadsSnapshotAllUsers({ force: true }); } catch(_) {} } return openAdminLinkFupModal(uid, ""); }
      ...
      if (act === "leadShowLost") { const uid = String(a.getAttribute("data-userid")||""); return openLeadStageListModal(uid, leadStageId("PERDIDO"), 'Perdidos'); }
      if (act === "leadShowConv") { const uid = String(a.getAttribute("data-userid")||""); return openLeadStageListModal(uid, leadStageId("CONVERTIDO"), 'Convertidos'); }
      ...
      if (act === "followupPerdido") {
        ...
        if (!lead) return alert("Este FOLLOW-UP não tem lead de origem vinculado.");
        await moveLeadStage(String(lead.ASSIGNED_BY_ID || lead._ownerUserId || ""), String(lead.ID), leadStageId("PERDIDO"));
        await markDone(id);
        return renderCurrentView();
      }
