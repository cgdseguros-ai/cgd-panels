...
(function () {
  ...
  function lostLeadStageIds() {
    const primary = leadStageId("LEAD DESCARTADO");
    if (primary) return [primary];
    return [leadStageId("PERDIDO"), leadStageId("LEAD PERDIDO")].filter(Boolean);
  }
...
function openMarketingPanelStub() {
  ...
  const days = Array.from({length:35}, (_,i) => `<div style="min-height:44px;border:1px solid rgba(0,0,0,.06);border-radius:10px;padding:6px;font-size:11px;background:#fff">${i+1 <= 31 ? i+1 : ''}</div>`).join('');
  openModal("Painel MKT — implantação", `<div style="display:grid;grid-template-columns:minmax(320px,42%) 1fr;gap:12px;min-height:72vh">
    <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:12px">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div style="font-weight:950">Calendário MKT</div><div style="font-size:12px;opacity:.72">${escHtml(monthLabel)}</div></div>
      <img src="${MARKETING_LOGO_URL}" style="width:112px;height:112px;object-fit:contain;border-radius:16px;background:#fff;border:1px solid rgba(0,0,0,.08);padding:6px" />
      <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:6px">${days}</div>
    ...
    <div style="border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:12px;background:#fff;display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div style="font-weight:950">Agenda MKT</div><button class="eqd-btn eqd-btnPrimary" id="mktCreateStub">Criar tarefa MKT (próxima etapa)</button></div>
      <div style="font-size:12px;opacity:.78">Estrutura de implantação avançada. Próximo passo: criar deals MKT com participantes múltiplos e filtrar apenas os criados no painel MKT.</div>
      <div style="border:1px dashed rgba(0,0,0,.18);border-radius:16px;min-height:260px;display:flex;align-items:center;justify-content:center;font-size:12px;opacity:.6">Lista cronológica dos deals MKT entra na próxima rodada</div>
    </div>
  </div>`, { full:true, wide:true });
  const btn = document.getElementById('mktCreateStub');
  if (btn) btn.onclick = () => alert('Na próxima rodada, este botão abrirá o modal de criação da tarefa MKT com participantes.');
}
...
function makeUserCard(u) {
  ...
  async function revalidateLead(userId, leadId) {
    ...
    const nowIso = new Date().toISOString();
    const emAtendimento = String(leadStageId("EM ATENDIMENTO") || leadStageId("NOVO LEAD") || "");
    const tempId = makeTempId('TMP_LEAD');
    upsertLeadLocal(uid, { ID: tempId, ASSIGNED_BY_ID:Number(uid), NAME:leadTitle(lead), TITLE:leadTitle(lead), STATUS_ID:emAtendimento, DATE_CREATE:nowIso, DATE_MODIFY:nowIso, [LEAD_UF_DATAHORA]: nowIso, [LEAD_UF_OPERADORA]:lead[LEAD_UF_OPERADORA], [LEAD_UF_IDADE]:lead[LEAD_UF_IDADE], [LEAD_UF_TELEFONE]:lead[LEAD_UF_TELEFONE], [LEAD_UF_BAIRRO]:lead[LEAD_UF_BAIRRO], [LEAD_UF_FONTE]:lead[LEAD_UF_FONTE], [LEAD_UF_OBS]:lead[LEAD_UF_OBS], [LEAD_UF_HELENA]:lead[LEAD_UF_HELENA], [LEAD_UF_PESSOAL]:lead[LEAD_UF_PESSOAL] });
    reopenLeadsModalSafe();
    renderCurrentView();
    const fields = { ASSIGNED_BY_ID:Number(uid), NAME:leadTitle(lead), TITLE:leadTitle(lead), STATUS_ID:emAtendimento, [LEAD_UF_DATAHORA]: nowIso };
    [LEAD_UF_OPERADORA, LEAD_UF_IDADE, LEAD_UF_TELEFONE, LEAD_UF_BAIRRO, LEAD_UF_FONTE, LEAD_UF_OBS, LEAD_UF_HELENA, LEAD_UF_PESSOAL].forEach((k)=>{ if (String(lead[k]||'').trim()) fields[k]=lead[k]; });
    const createdPromise = bx('crm.lead.add', { fields });
    const created = await createdPromise;
    const newLeadId = String((created && (created.result || created)) || '').trim() || tempId;
    const arr = (STATE.leadsByUser.get(uid) || []).slice();
    const idx = arr.findIndex((l) => String(l.ID) === tempId);
    if (idx >= 0) arr[idx] = { ...arr[idx], ID:newLeadId, STATUS_ID: emAtendimento, [LEAD_UF_LEAD_ORIGEM]: newLeadId };
    STATE.leadsByUser.set(uid, arr);
  ...
  async function openLeadStageListModal(userId, stageId, title, kwRaw = "") {
    const user = USERS.find((u) => String(u.userId) === String(userId));
    const wanted = Array.isArray(stageId) ? stageId.map((s) => String(s || '')).filter(Boolean) : String(stageId || '').split(',').map((s) => String(s||'').trim()).filter(Boolean);
    const select = ["ID","TITLE","NAME","LAST_NAME","STATUS_ID","ASSIGNED_BY_ID","DATE_CREATE","DATE_MODIFY","SOURCE_ID", LEAD_UF_OPERADORA, LEAD_UF_IDADE, LEAD_UF_TELEFONE, LEAD_UF_BAIRRO, LEAD_UF_FONTE, LEAD_UF_DATAHORA, LEAD_UF_OBS, LEAD_UF_ATENDIDO_DIA, LEAD_UF_LEAD_ORIGEM, LEAD_UF_HELENA, LEAD_UF_PESSOAL].filter(Boolean);
    let arr = [];
    if (wanted.length) {
      for (const sid of wanted) {
        const rows = await bxAll("crm.lead.list", { filter: { ASSIGNED_BY_ID: Number(userId), STATUS_ID: String(sid) }, select, order: { ID: "DESC" } }).catch(() => []);
        arr = arr.concat(rows || []);
      }
    } else {
      arr = await bxAll("crm.lead.list", { filter: { ASSIGNED_BY_ID: Number(userId) }, select, order: { ID: "DESC" } }).catch(() => []);
    }
    const seen = new Set();
    arr = arr.filter((l) => { const id = String((l && l.ID) || ""); if (!id || seen.has(id)) return false; seen.add(id); return String((l.ASSIGNED_BY_ID || l._ownerUserId || "")) === String(userId); }).sort((a,b)=>{ const ta=parseFlexDate((a && a[LEAD_UF_ATENDIDO_DIA]) || leadDataHora(a) || (a && a.DATE_CREATE) || 0); const tb=parseFlexDate((b && b[LEAD_UF_ATENDIDO_DIA]) || leadDataHora(b) || (b && b.DATE_CREATE) || 0); return (tb?tb.getTime():0)-(ta?ta.getTime():0); });
    const kw = norm(String(kwRaw || '').trim());
    ...
    const card = (l) => {
      ...
      <div class="leadCard">
        <div class="leadTitle">${escHtml(leadTitle(l))}</div>
        <div class="leadMeta">
          <span>USER: <strong>${escHtml(owner ? owner.name : String(l.ASSIGNED_BY_ID || ""))}</strong></span>
