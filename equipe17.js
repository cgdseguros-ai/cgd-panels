...
restoreSyncQueue();
  ...
  function openNoFutureFollowupLeadBatchModal(user, leads, title) {
    ...
    openModal(title, `
      <div style="display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <button class="eqd-btn" id="noFuBatchDone">Concluir em lote</button>
    ...
    document.getElementById('noFuBatchDone').onclick = async () => {
      ...
      if (!confirm(`Concluir ${picks.length} lead(s) em lote?`)) return;
      const sConv = leadStageId("LEAD CONVERTIDO") || leadStageId("CONVERTIDO");
      if (!sConv) return alert('Etapa CONVERTIDO não encontrada.');
      setBusy('Concluindo leads…');
      try {
        for (const leadId of picks) await moveLeadStage(String(user.userId), leadId, sConv, { skipLostPrompt: true });
        closeModal();
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    btn.onclick = async () => {
      try {
        ...
        if (isFollowupDeal(d) && doneMode === 'FEITO' && !d[DEAL_UF_LEAD_ORIGEM]) {
        ...
        if (isFollowupDeal(d) && doneMode === 'FEITO') {
