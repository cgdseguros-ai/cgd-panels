...
function makeUserCard(u) {
  ...
  function renderUserPanel(userId) {
    ...
    if (!isSpecial) {
      ...
      if (adminToggle2) adminToggle2.onclick = () => {
        const box = document.getElementById('user1AdminPanel');
        if (!box) return;
        box.style.display = box.style.display === 'none' ? 'flex' : 'none';
      };
      const adminToggle2 = document.getElementById('user1AdminToggle');
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function ensureProposalMeta(force) {
    ...
    const [fieldsMeta, catStages, allStatuses] = await Promise.all([
      bx("crm.deal.fields", {}).catch(() => ({})),
      bx("crm.dealcategory.stage.list", { id: Number(CATEGORY_PROPOSALS) }).catch(() => ([])),
      bxAll("crm.status.list", { filter: { ENTITY_ID: `DEAL_STAGE_${CATEGORY_PROPOSALS}` }, order: { SORT: "ASC" } }).catch(() => ([])),
    ]);
    STATE.proposalFieldsMeta = fieldsMeta || {};
    const stageRows = (Array.isArray(catStages) && catStages.length ? catStages : allStatuses) || [];
    const byId = new Map();
    const byName = new Map();
    stageRows.forEach((st) => {
      const nm = String((st && (st.NAME || st.name || st.VALUE)) || '').trim();
      const sid = String((st && (st.STATUS_ID || st.STATUS_ID || st.ID || st.STATUS_ID)) || '').trim();
      if (!nm || !sid) return;
      byId.set(sid, nm);
      byName.set(norm(nm), sid);
    });
  ...
  async function loadProposalDeals(force, opts = {}) {
    ...
    const select = [
      'ID','TITLE','STAGE_ID','ASSIGNED_BY_ID','OPPORTUNITY','CURRENCY_ID','DATE_CREATE','DATE_MODIFY','CLOSEDATE',
    ...
    const filter = { CATEGORY_ID: Number(CATEGORY_PROPOSALS) };
    if (uid) filter.ASSIGNED_BY_ID = uid;
    const rows = await bxAll('crm.deal.list', {
      filter,
      order: { ID: 'DESC' },
      select,
    }).catch(() => []);
    const mapped = (rows || []).map((d) => ({ ...d, _proposalStageName: proposalStageNameFromId(d.STAGE_ID) || String(d.STAGE_ID || '') }));
    if (!uid) {
  ...
  async function openProposalsModal(userId, opts = {}) {
    ...
    openModal(title, `
      ...
      <div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:10px">
        <div style="font-size:12px;font-weight:900;opacity:.82">Pipeline 5 • Total exibido: <strong>${filteredDeals.length}</strong></div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
