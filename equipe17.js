...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalMissingFieldDefs(nextStageId, patch, baseDeal) {
    const stageName = proposalStageLabelById(nextStageId) || proposalStageLabelForDeal(baseDeal);
  ...
  async function openProposalStageMoveModal(dealId, returnCtx = {}) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        const miss = proposalMissingFieldDefs(nextStage, { STAGE_ID: nextStage }, deal);
        if (miss.fields.length) return openProposalRequiredFieldsModal(deal, nextStage, returnCtx, {});
        PROPOSALS_REFRESH_HOLD_UNTIL = Date.now() + 10000;
    ...
    const applyBindings = () => {
      ...
      document.querySelectorAll('.proposal-inline-stage').forEach((sel) => {
        sel.onchange = async () => {
          ...
          const miss = proposalMissingFieldDefs(nextStage, { STAGE_ID: nextStage }, prev);
          if (miss.fields.length) {
            sel.value = String(prev && prev.STAGE_ID || '');
            await openProposalRequiredFieldsModal(prev, nextStage, { ...LAST_PROPOSALS_CTX, userId: uid }, {});
            return;
          }
          PROPOSALS_REFRESH_HOLD_UNTIL = Date.now() + 10000;
          UI_REFRESH_HOLD_UNTIL = Date.now() + 4000;
...
document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "recurTransfer") {
        ...
        const rid = String(a.getAttribute("data-ruleid") || "");
        if (!uid || !rid) return;
        const choices = USERS.filter((u) => String(u.userId) !== uid).map((u) => `${u.userId} - ${u.name}`).join('\n');
        const picked = prompt(`Transferir recorrência para qual USER?\n\n${choices}`,'');
        if (!picked) return;
        const target = String(picked).trim().split(/\s+/)[0];
        if (!target) return;
        await transferRecurrenceRule(uid, rid, target);
        MODAL_STACK = [];
        closeModal();
      ...
      if (act === "recurDelete") {
        ...
        const rid = String(a.getAttribute("data-ruleid")||"");
        if (!uid || !rid) return;
        if (!confirm("Excluir esta regra de recorrência e todas as futuras já criadas?")) return;
