...
(function () {
  ...
  const LEAD_UF_ATENDIDO_DIA = "UF_CRM_1771741018";
  ...
  async function loadLeadsForOneUser(userId) {
    ...
    const select = [
      ...
      LEAD_UF_OPERADORA, LEAD_UF_IDADE, LEAD_UF_TELEFONE, LEAD_UF_BAIRRO, LEAD_UF_FONTE, LEAD_UF_DATAHORA, LEAD_UF_OBS, LEAD_UF_ATENDIDO_DIA,
    ].filter(Boolean);
    ...
    const leads = await bxAll("crm.lead.list", {
      filter: { ASSIGNED_BY_ID: Number(userId), ...(statusIds.length ? { STATUS_ID: statusIds } : {}) },
      select,
      order: { ID: "DESC" },
    });
  ...
  function makeDealCard(deal, context) {
    ...
    if (deal._etapaTxt) { const etapaRun = norm(deal._etapaTxt).includes(norm("EM ANDAMENTO")) ? `<span class="eqd-etapaRunDot"></span>` : ``; tags.push(`<span class="eqd-tag eqd-tagClickable" data-action="quickEtapa" data-id="${deal.ID}">${etapaRun}ETAPA: ${trunc(deal._etapaTxt, 18)}</span>`); }
  ...
  function enqueueSync(job) { SYNC_QUEUE.push(job); scheduleSyncFlush(450); }
  ...
  async function loadGlobalLeadsBulk(forceReload = false) {
    ...
    const leads = await bxAll("crm.lead.list", {
      filter: statusIds.length ? { STATUS_ID: statusIds } : {},
      select,
      order: { ID: "DESC" },
    }).catch(() => []);
    clearBusy();
  ...
  function getLeadMetricDate(l) {
    const raw = (l && l[LEAD_UF_ATENDIDO_DIA]) || leadDataHora(l) || "";
  ...
  function openGlobalLeadsDayStats() {
    ...
    openModal(`LEADS • DIA (${fmtDateOnly(new Date())})`, `
      <div style="font-size:12px;font-weight:950;opacity:.82;margin-bottom:8px">
        * ATENDIDO NO DIA usa o campo UF_CRM_1771741018 do lead como referência, mesmo que hoje ele já esteja em outro stage.
      </div>
  ...
  function openGlobalLeadsHistoryModal(days) {
    ...
    openModal(`LEADS • ${days === 7 ? "SEMANA" : "MÊS"}`, `
      <div style="font-size:12px;font-weight:950;opacity:.82;margin-bottom:8px">
        Histórico de leads marcados como ATENDIDOS no dia pelo campo UF_CRM_1771741018, mesmo que depois mudem de stage.
      </div>
