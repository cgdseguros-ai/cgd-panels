    ...
    return `
      <div style="display:flex;flex-direction:column;gap:12px;color:#111">
        ...
        <div style="font-size:11px;font-weight:850;opacity:.72;line-height:1.45">
          Observação: se uma etapa aparecer muito alta, ela é a primeira candidata a ser carregada em background, paginada ou filtrada. Este relatório mede o tempo dentro do navegador; lentidão da API Bitrix aparece como tempo alto na etapa correspondente.
        </div>
      </div>
    `;
  }
  ...
  async function loadLeadsSnapshotAllUsers(opts = {}) {
    ...
    for (const [uid, currentArr] of (STATE.leadsByUser || new Map()).entries()) {
      ...
      (currentArr || []).forEach((lead) => {
        ...
        if (!lid || !isLeadMutationHeld(lid) || nextIds.has(lid)) return;
        nextArr.push({ ...(lead || {}) });
        nextIds.add(lid);
      });
...
function makeUserCard(u) {
  ...
  async function loadMetricsCore(opts = {}) {
    ...
    await bootTimerMeasure("Métricas: deals/tarefas", () => loadDeals({ forceFull: true, openOnly: false, deferPhotos: true }), "Deals usados nos cards dos USERS");
  ...
  async function loadOperationalDataAfterMetrics(opts = {}) {
    ...
    if (shouldRefreshLeads) {
      ...
      await bootTimerMeasure("Leads: snapshot global", () => loadLeadsSnapshotAllUsers({ force: !!opts.forceLeads }).catch(()=>{}), "Resumos globais de leads por USER/status");
