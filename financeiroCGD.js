/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v25: CARTÕES mostra lançamentos já realizados; valor informado permanece como VALOR DA PARCELA
...
(function () {
  ...
  function openCardModal() {
      ...
      '<div class="fin-modal-body">' +
        ...
        '<div class="fin-panel" style="margin-top:10px"><div class="fin-panel-inner">' +
          '<div class="fin-row" style="justify-content:space-between;gap:8px;flex-wrap:wrap;margin-bottom:8px">' +
            '<div><div style="font-weight:950">Lançamentos já realizados do cartão</div><div class="fin-muted" style="font-weight:800;font-size:12px">Mostra as compras/parcelas já criadas para o cartão selecionado. Não inclui a fatura consolidada.</div></div>' +
          ...
          '<div class="fin-table-wrap" style="max-height:240px;overflow:auto"><table class="fin-table" style="min-width:980px">' +
            '<thead><tr>' +
              '<th>DATA COMPRA</th><th>FATURA</th><th>CONTA/COMPRA</th><th>PARCELA</th><th>VALOR PARCELA</th><th>ETAPA</th><th>OBS</th>' +
    ...
    function renderCardLaunches() {
      ...
      for (var i = 0; i < list.length; i++) {
        var val = finAmount(list[i][CFG.F.VALOR_PREV] || list[i][CFG.F.VALOR_REAL]);
        totalAll += val;
        if (String(list[i].STAGE_ID || "") === CFG.STAGES.DESP_A_PAGAR) totalOpen += val;
      }
      sum.textContent = list.length + " lançamento(s) • Total geral: " + moneyBR(totalAll) + " • Em aberto: " + moneyBR(totalOpen);
      if (!list.length) {
        tb.innerHTML = '<tr><td colspan="7" class="fin-muted">Nenhum lançamento encontrado para este cartão.</td></tr>';
        return;
      }
      tb.innerHTML = list.slice(0, 200).map(function(d){
        var obs = String(d[CFG.F.OBS] || "");
        var cleanObs = obs.replace(/COMPRA_ISO:\s*\d{4}-\d{2}-\d{2}/ig, "").trim();
        return '<tr>' +
          '<td>' + esc(toDisplayDate(parseCardPurchaseDate(d))) + '</td>' +
          ...
          '<td>' + esc(stageLabel(d.STAGE_ID)) + '</td>' +
          '<td class="fin-muted" style="max-width:300px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="' + esc(cleanObs) + '">' + esc(cleanObs || "—") + '</td>' +
        '</tr>';
      }).join('');
    }
    ...
    m.q("#cc-card").addEventListener("change", function(){ updateCardSummary(); renderCardLaunches(); });
