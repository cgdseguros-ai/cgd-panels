/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v39: PATCH fallback API pelo proxy GET + diagnóstico de crm.deal.list
...
(function () {
  ...
  // PATCH v39: fallback bruto pelo mesmo proxy que o GET usa.
  // Objetivo: contornar o Worker financeiro quando ele quebra internamente em crm.deal.list.
  var ALT_BX_PROXY_BASE = "https://cgd-bx-proxy.cgdseguros.workers.dev/bx/";
  ...
  // =========================
  function finToPairs(prefix, value, out) {
    out = out || [];
    if (value === undefined || value === null) return out;
    if (Array.isArray(value)) {
      for (var i = 0; i < value.length; i++) finToPairs(prefix ? (prefix + "[" + i + "]") : String(i), value[i], out);
      return out;
    }
    if (typeof value === "object") {
      for (var k in value) if (value.hasOwnProperty(k)) finToPairs(prefix ? (prefix + "[" + k + "]") : k, value[k], out);
      return out;
    }
  ...
  function apiCallAltProxy(method, payload) {
    var body = new URLSearchParams();
    var pairs = finToPairs("", payload || {}, []);
    for (var i = 0; i < pairs.length; i++) if (pairs[i][0]) body.append(pairs[i][0], pairs[i][1]);
    return fetch(ALT_BX_PROXY_BASE + method, {
      method: "POST",
      ...
      return r.text().then(function (txt) {
        var j = parseJson(txt);
        if (!r.ok) throw new Error((j && (j.error_description || j.error)) || txt || ("ALT HTTP " + r.status));
  ...
  function apiCall(method, payload) {
    ...
    // crm.deal.list é exatamente a rota que o Worker financeiro passou a recusar/estourar.
    // Para ela, tentamos primeiro o proxy bruto do GET, que não faz paginação interna.
    if (String(method) === "crm.deal.list") {
      return apiCallAltProxy(method, payload)
        .then(function (j) { S.apiMode = "alt-cgd-proxy"; return j; })
        .catch(function (altErrFirst) {
          S.lastApiError = "ALT primeiro falhou: " + finErrText(altErrFirst);
          return req(API_BASE + "/" + method, "path")
            .catch(function (e1) { S.lastApiError += " | worker path: " + finErrText(e1); return req(API_BASE + "?method=" + encodeURIComponent(method), "query"); })
            .then(function (res) { S.apiMode = res.mode; return res.json; })
            .catch(function (workerErr) {
              S.lastApiError += " | worker final: " + finErrText(workerErr);
              throw workerErr;
            });
        });
    }
    ...
    return req(API_BASE + "/" + method, "path")
      ...
      .catch(function (workerErr) {
        S.lastApiError += " | worker final: " + finErrText(workerErr);
        // Para métodos de leitura, ainda tentamos o proxy alternativo como reserva.
        if (/^(crm\.deal\.fields|crm\.status\.list|user\.get)$/i.test(String(method))) {
          return apiCallAltProxy(method, payload).then(function (j) { S.apiMode = "alt-cgd-proxy"; return j; });
        }
        throw workerErr;
      });
  }
  ...
  function openDiagnosticsModal() {
    ...
    Promise.allSettled([
      ...
      '</tbody></table><div class="fin-muted" style="margin-top:10px;font-weight:900">v39 tenta crm.deal.list pelo proxy alternativo do GET antes do Worker financeiro.</div>';
    });
  }
    ...
    return listDealsAll()
      ...
      .catch(function (e) {
        ...
        if (/Maximum call stack size exceeded/i.test(msg)) {
          msg = "Worker/Bitrix ainda recusou a consulta financeira. v39 tentou proxy alternativo e Worker. Último erro: " + (S.lastApiError || msg);
        }
        toast("Falha ao carregar: " + msg, "err");
        var tb = el("#fin-tbody");
        if (tb) tb.innerHTML = '<tr><td colspan="12" class="fin-muted">Erro: ' + esc(msg) + '</td></tr>';
      })
      .finally(function () {
