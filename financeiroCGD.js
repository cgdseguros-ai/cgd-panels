/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v37: PATCH paginação por cursor ID + blindagem Bitrix init
...
(function () {
  ...
  function listDealsAll() {
    ...
    // PATCH v37:
    // O erro continuou porque o problema não era apenas recursão no front.
    // Em alguns ambientes o Bitrix/Worker passou a estourar pilha quando usamos paginação por "start/next".
    // Então deixamos de usar paginação por start e passamos a usar cursor por ID:
    // 1) busca ID DESC com start:-1;
    // 2) pega o menor ID retornado;
    // 3) próxima página usa filtro <ID;
    // 4) repete até vir menos de 50 registros.
    // Isso evita a rota de paginação do Bitrix/Worker que está instável.
    ...
    function isMaxStackErr(e) {
      return /Maximum call stack size exceeded/i.test(finErrText(e));
    }
    ...
    async function fetchPagesForFilter(filter, label) {
      var lastId = null;
      var seenCursors = {};
      var pageCount = 0;
      var MAX_PAGES = 800;
      var PAGE_SIZE_EXPECTED = 50; // limite padrão do Bitrix REST

      while (true) {
        pageCount++;
        if (pageCount > MAX_PAGES) throw new Error('Carga interrompida por segurança em ' + label + '.');

        var f = cloneFilter(filter);
        if (lastId !== null) f['<ID'] = String(lastId);

        var cursorKey = String(lastId === null ? 'FIRST' : lastId);
        if (seenCursors[cursorKey]) throw new Error('Cursor repetido em ' + label + ' (ID<' + cursorKey + ').');
        ...
        if (nextLastId === null || (lastId !== null && nextLastId >= lastId)) break;
        if (chunk.length < PAGE_SIZE_EXPECTED) break;
    ...
    async function mainLoad() {
      try {
        for (var i = 0; i < stageArr.length; i++) {
          await fetchPagesForFilter(
            { CATEGORY_ID: String(CFG.DEAL_CATEGORY_ID), STAGE_ID: String(stageArr[i]) },
            'stage ' + stageArr[i]
          );
        }
      } catch (e) {
        // Fallback adicional: se mesmo por stage o Worker cair, tenta categoria inteira
        // também por cursor ID, e filtra localmente. Não usa start/next.
        if (!isMaxStackErr(e)) throw e;
        out = [];
        var allowed = {};
        for (var a = 0; a < stageArr.length; a++) allowed[String(stageArr[a])] = true;
        await fetchPagesForFilter({ CATEGORY_ID: String(CFG.DEAL_CATEGORY_ID) }, 'categoria financeira por cursor');
        out = out.filter(function(d){ return allowed[String(d.STAGE_ID || '')]; });
      }
      ...
      for (var x = 0; x < out.length; x++) {
        var id = String(out[x] && out[x].ID || '');
        if (!id || byId[id]) continue;
        byId[id] = true;
        dedup.push(out[x]);
      }
      dedup.sort(function(a, b){ return Number(b.ID || 0) - Number(a.ID || 0); });
      return dedup;
    }
    ...
    return listDealsAll()
      ...
      .catch(function (e) {
        ...
        if (/Maximum call stack size exceeded/i.test(msg)) {
          msg = "Erro de paginação do Bitrix/Worker evitado parcialmente. Clique em ATUALIZAR; se persistir, abrir DIAGNÓSTICO.";
        }
