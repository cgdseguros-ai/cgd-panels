/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v38: PATCH modo seguro sem paginação REST + limpeza de erro externo Bitrix
...
(function () {
  ...
  function applyFilters() {
    ...
    // PATCH v41: após a atualização do Bitrix/Worker, há cenários em que os lançamentos
    // chegam em S.deals, mas a combinação de filtros/stages/metadados deixa S.filtered zerado.
    // Em VISÃO GLOBAL, sem busca/filtro específico e com Despesas+Receitas marcados,
    // entramos em modo recuperação e exibimos os lançamentos carregados em vez de deixar a tabela vazia.
    S.recoveryMode = false;
    var noSpecificFilter = !q && !S.filters.centro && !S.filters.competencia && !S.filters.stageId && S.filters.showPayables && S.filters.showReceivables;
    if (!S.filtered.length && (S.deals || []).length && noSpecificFilter) {
      S.filtered = (S.deals || []).filter(function(d){
        if (isBadFav(d[CFG.F.FAVORECIDO])) return false;
  ...
  function renderTable() {
    ...
    var rows = [];
    if (S.recoveryMode) {
      rows.push('<tr><td colspan="12" class="fin-muted"><b>Modo recuperação ativo:</b> lançamentos foram carregados, mas os filtros/stages do Bitrix retornaram inconsistentes. Exibindo dados carregados em VISÃO GLOBAL.</td></tr>');
    }
    if (S.lastRefreshPartialError && !S.recoveryMode) {
      rows.push('<tr><td colspan="12" class="fin-muted"><b>Aviso:</b> a última atualização teve erro parcial, mas os dados carregados foram preservados. Erro: ' + esc(S.lastRefreshPartialError).slice(0, 900) + '</td></tr>');
    }
  ...
  function openDiagnosticsModal() {
    ...
    Promise.allSettled([
      ...
      host.innerHTML = '<table class="fin-table" style="min-width:0"><tbody>' +
        ...
        line('Versão JS', 'v41 recuperação de dados carregados + não sobrescrever tabela em erro parcial', true) +
        ...
        line('Qtd. filtrados/exibíveis', String((S.filtered || []).length), (S.filtered || []).length > 0) +
        line('Modo recuperação', S.recoveryMode ? 'ATIVO' : 'inativo', !S.recoveryMode) +
    ...
    return listDealsAll()
      ...
      .catch(function (e) {
        ...
        if (/Maximum call stack size exceeded/i.test(msg)) {
          msg = "Worker/Bitrix ainda recusou parte da consulta financeira. Dados já carregados foram preservados.";
        }
        S.lastApiError = msg;
        S.lastRefreshPartialError = msg;

        // PATCH v41: se já há lançamentos carregados, não sobrescreve a tabela com erro.
        // Reaplica os filtros e mantém o financeiro utilizável.
        if ((S.deals || []).length) {
          toast("Atualização parcial falhou, mas os dados carregados foram preservados.", "err");
          applyFilters();
          return;
        }
