/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v17: edição de recorrências futuras não pagas + renovação de recorrências
...
(function () {
  ...
  function captureInstitutionalBundle() {
    return {
      ...
      meta: { savedAt: new Date().toISOString(), version: 'v17-recurring-edit-renew' }
    };
  }
  ...
  function safeRecurringBulkFields(fields) {
    var f = {};
    // Em edição futura, preserva datas/competências/status de cada parcela.
    // Isso evita transformar várias parcelas mensais em uma única data.
    ...
    m.q("#ed-save").addEventListener("click", function () {
      ...
      var op = scope === "future" && future.length > 1
        ? updateDealsSequential(future, function(d){
            if (String(d.ID) === String(deal.ID)) return fields;
            return safeRecurringBulkFields(fields);
          })
        : updateDeal(deal.ID, fields).then(function(){ mergeDealPatch(deal.ID, fields); return 1; });
    ...
    function renderTable(host) {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        html += '<tr data-i="' + i + '">' +
          ...
          '<td><input class="fin-batch-inp" data-k="favorecido" value="' + esc(r.favorecido) + '" placeholder="' + (isRec ? 'Ex.: Cliente, operadora...' : 'Ex.: Light, aluguel, pró-labore...') + '"></td>' +
      ...
      tbody.addEventListener("change", function(e){ var tr = safeClosest(e.target, "tr[data-i]"); if (!tr) return; var i = Number(tr.getAttribute("data-i")); var k = e.target.getAttribute("data-k"); if (!k) return; rows[i][k] = e.target.value; if (k === "categoria") { if (!socioGainTypeFromCategoryId(rows[i].categoria || "")) rows[i].socio = ""; renderTable(host); return; } if (k === "freq") renderTable(host); });
            ...
            p = p.then(function(){
              ...
              return createDeal(fields).then(function(newId){
                ...
                if (kind !== "RECEITA" && count > 1 && r.__recSerie) {
                  ...
                  if (!S.recurringIndex[r.__recSerie]) S.recurringIndex[r.__recSerie] = { id: r.__recSerie, kind: kind, freq: r.freq, total: count, createdAt: new Date().toISOString(), startISO: start, endISO: calcDate(start, r, count - 1), dealIds: [] };
  ...
  function recurrenceSeriesList() {
    ...
    for (var k in map) if (map.hasOwnProperty(k)) {
      ...
      item.value = Number(first[CFG.F.VALOR_PREV] || 0) || 0;
    ...
    while (nextDate <= untilDate) {
      (function(dt, idx){
        p = p.then(function(){
          ...
          fields[CFG.F.DATA_PREV] = dt;
          fields[CFG.F.VALOR_PREV] = Number(base[CFG.F.VALOR_PREV] || 0) || 0;
  ...
  function openRecurringManagerModal() {
    var m = modal(
      ...
      '<div class="fin-modal-body"><div class="fin-muted" style="font-weight:900;margin-bottom:10px">Use esta tela para renovar séries que terminam em 12/2026 ou próximas do vencimento. A renovação cria novas despesas a partir da próxima data após o fim atual.</div><div id="rec-series-host"></div></div>',
      { full: true }
    );
    function draw(){
      var list = recurrenceSeriesList();
      ...
      for (var i=0;i<list.length;i++) {
        var it = list[i];
        rows.push('<tr>' +
          '<td class="fin-mono">' + esc(it.id) + (recurrenceNeedsRenewal(it) ? '<div class="fin-small" style="color:#b45309;font-weight:950">Vence/planejar renovação</div>' : '') + '</td>' +
  ...
  function openExpensesManagerModal() {
    ...
    function renderExpenses() {
      ...
      if (alertHost) {
        ...
        alertHost.innerHTML = recList.length ? '<div class="fin-rec-alert">Há <b>' + recList.length + '</b> recorrência(s) próximas do fim ou planejadas até 12/2026. Clique em <b>RECORRÊNCIAS</b> para renovar.</div>' : '';
      }
    ...
    m.q("#cc-save").addEventListener("click", function () {
      ...
      rows.forEach(function (r) {
        ops = ops.then(function () {
          ...
          var cat = kind === "RECEITA" ? "" : (r.categoria || "");
          var centroSelecionado = String(r.centro || "").trim();
          if (kind === "RECEITA" && !centroSelecionado) throw new Error("Selecione o CENTRO DE CUSTO RECEBEDOR da receita: " + fav);
          var socioGainType = socioGainTypeFromCategoryId(cat);
          var socioSelecionado = String(r.socio || "").trim();
          if ((socioGainType === "PRO_LABORE" || socioGainType === "BONIFICACAO") && !socioSelecionado) {
            throw new Error("Para PRÓ-LABORE ou BONIFICAÇÃO, selecione o SÓCIO: " + fav);
