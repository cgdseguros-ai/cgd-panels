  ...
  function renderChartsPlaceholders() {
    ...
    if (b) {
      ...
      for (i = 0; i < (S.deals || []).length; i++) {
        ...
        if (st2 === CFG.STAGES.DESP_A_PAGAR || st2 === CFG.STAGES.DESP_PAGA) monthMap[m[2]].exp += amt;
        if (st2 === CFG.STAGES.REC_A_RECEBER || st2 === CFG.STAGES.REC_RECEBIDA) monthMap[m[2]].rec += amt;
      }
      var evo = [];
      for (i = 1; i <= 12; i++) evo.push(monthMap[String(i).padStart(2,'0')]);
      b.innerHTML = '<div class="fin-chart-box"><div class="fin-chart-title">Receitas x despesas por competência</div>' + buildHorizontalCompareSvg(evo, 'Despesas', 'Receitas') + '</div>';
    }
          ...
          '<div class="fin-body">' +
            '<aside class="fin-side">' +
              ...
              '<div class="fin-side-block fin-side-chart">' +
                '<div class="fin-side-h">Receitas x despesas por competência</div>' +
