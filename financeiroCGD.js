...
(function () {
  ...
  function displayLabel(v) {
    ...
    return t.replace(/\bDIOGO\s+PF\b/gi, "PESSOA FÍSICA");
  }
  ...
  function openPayReceiveModal(deal) {
    ...
    m.q("#pr-save").addEventListener("click", function () {
      ...
      if (centroSel) fields[CFG.F.CENTRO_CUSTO] = centroSel;
    ...
    function renderTable(host) {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        html += '<tr data-i="' + i + '">' +
          ...
          '<td><input class="fin-batch-inp" data-k="start" value="' + esc(toDisplayDate(r.start)) + '" placeholder="DD/MM/AAAA"></td>' +
        ...
        ops = ops.then(function () {
          ...
          for (var k = 0; k < count; k++) (function (idx) {
            p = p.then(function () {
              ...
              fields[CFG.F.FAVORECIDO] = fav;
              fields[CFG.F.OBS] = (count > 1 ? ("Recorrência: " + r.freq + " • Item " + (idx + 1) + "/" + count + (obs ? " • " + obs : "")) : obs);
  ...
  function openExpensesManagerModal() {
    ...
    var m = modal(
      ...
      '<div class="fin-modal-body">' +
        '<div class="fin-row" style="gap:10px;flex-wrap:wrap;align-items:flex-end">' +
          ...
          '<button class="fin-btn" id="exp-refresh">Atualizar lista</button>' +
        '</div>' +
        ...
        '<div class="fin-table-wrap" style="margin-top:10px;max-height:70vh;overflow:auto">' +
          '<table class="fin-table"><thead><tr>' +
            '<th style="width:76px">ID</th><th>Favorecido</th><th style="width:160px">Centro</th><th style="width:160px">Conta</th><th style="width:120px">Data Prev.</th><th style="width:120px">Valor</th><th style="width:150px">Status</th><th style="width:240px">Ações</th>' +
    ...
    function renderExpenses() {
      ...
      if (!list.length) { tb.innerHTML = '<tr><td colspan="8" class="fin-muted">Nenhuma despesa encontrada.</td></tr>'; return; }
      var rows = [];
      for (var i = 0; i < list.length; i++) {
        var d = list[i];
    ...
    function renderRows() {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        html += '<tr data-i="' + i + '">' +
          ...
          '<td><input class="fin-batch-inp" data-k="valor" value="' + esc(r.valor) + '" placeholder="1500,00"></td>' +
          '<td><input class="fin-batch-inp" data-k="data" value="' + esc(toDisplayDate(r.data)) + '" placeholder="DD/MM/AAAA"></td>' +
    ...
    m.q("#cc-save").addEventListener("click", function () {
      ...
      rows.forEach(function (r) {
        ops = ops.then(function () {
          ...
          if (!dt0) throw new Error("Linha sem data (DD/MM/AAAA). Fav: " + fav);
