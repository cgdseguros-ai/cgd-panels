    ...
    function openCardLaunchEdit(id) {
      ...
      var em = modal(
        ...
        '<div class="fin-modal-body">' +
          ...
          '<div class="fin-row fin-row--right" style="margin-top:14px"><button class="fin-btn" data-close="1">Cancelar</button><button type="button" class="fin-btn fin-btn--primary fin-card-edit-save" id="ce-save" style="background:#2563eb!important;color:#fff!important;border-color:#2563eb!important;min-width:130px">Salvar edição</button></div>' +
        '</div>'
      );
      attachDateMasks(em.node);
      var ceSaveBtn = em.q('#ce-save');
      if (ceSaveBtn) {
        ceSaveBtn.__origText = 'Salvar edição';
        ceSaveBtn.textContent = 'Salvar edição';
        ceSaveBtn.style.background = '#2563eb';
        ceSaveBtn.style.color = '#fff';
        ceSaveBtn.style.borderColor = '#2563eb';
        ceSaveBtn.style.minWidth = '130px';
      }
      if (ceSaveBtn) ceSaveBtn.addEventListener('click', function(ev){
        ...
        function setEditBusy(busy, txt) {
          if (!saveBtn) return;
          saveBtn.disabled = !!busy;
          saveBtn.textContent = busy ? (txt || 'Salvando...') : 'Salvar edição';
          saveBtn.style.background = '#2563eb';
          saveBtn.style.color = '#fff';
          saveBtn.style.borderColor = '#2563eb';
          saveBtn.style.minWidth = '130px';
        }
        ...
        try {
          ...
          if (!isFinite(val) || val < 0) { setEditStatus('Informe um valor válido para a parcela.', true); toast('Informe um valor válido para a parcela.', 'err'); return; }
          ...
          var firstDue = calcCardDueDateISO(baseDate, cardName) || toISODate(source[CFG.F.DATA_PREV] || '') || baseDate;
          var okCount = 0;
          var failCount = 0;
          var firstError = null;

          setEditBusy(true, 'Salvando...');
          setEditStatus('Salvando ' + targets.length + ' lançamento(s)...', false);

          var chain = Promise.resolve();
          targets.forEach(function(d, idx){
            chain = chain.then(function(){
              var parc = applyFuture ? (parcStart + idx) : parcStart;
              if (parc > total) parc = total;
              var dueDt = applyFuture ? (addMonthsISO(firstDue, idx, null) || firstDue) : (toISODate(d[CFG.F.DATA_PREV] || '') || firstDue);
              ...
              if (comp) fields[CFG.F.COMPETENCIA] = comp;
              fields[CFG.F.OBS] = 'Cartão: ' + cardName + ' • Compra: ' + toDisplayDate(baseDate) + ' • Venc. fatura: ' + toDisplayDate(dueDt) + ' • Parcela ' + parc + '/' + total + ' • COMPRA_ISO:' + baseDate + ' • CARD_SERIE:' + series;
              setEditStatus('Salvando parcela ' + (idx + 1) + ' de ' + targets.length + '...', false);
              return updateDeal(d.ID, fields).then(function(){
                okCount++;
                try { mergeDealPatch(d.ID, fields); } catch (e) { console.warn('mergeDealPatch falhou', e); }
              }).catch(function(err){
                failCount++;
          ...
          chain.then(function(){
            ...
            if (okCount > 0) {
              ...
              em.close();
              // Não sincroniza fatura automaticamente aqui para não travar a edição.
              // Use o botão "Atualizar fatura recorrente"/"Atualizar lista" depois, se necessário.
            } else {
            ...
            toast('Falha ao editar: ' + ((e && (e.message || String(e))) || 'erro'), 'err');
          });
        } catch (e) {
          setEditBusy(false);
          setEditStatus('Falha ao editar: ' + ((e && (e.message || String(e))) || 'erro'), true);
          toast('Falha ao editar: ' + ((e && (e.message || String(e))) || 'erro'), 'err');
          console.error('Erro síncrono na edição de cartão', e);
        }
      });
    }
