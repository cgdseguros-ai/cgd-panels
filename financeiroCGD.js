...
(function () {
  ...
  function listDealsAll() {
    ...
    // PATCH v36:
    // A versão anterior ainda fazia paginação recursiva (return fetchPage()).
    // Em alguns ambientes do Bitrix/Worker isso termina em "Maximum call stack size exceeded".
    // Agora a paginação é iterativa com async/await e pequenos respiros entre páginas.
    ...
    async function fetchPagesForFilter(filter, label) {
      ...
      while (true) {
        pageCount++;
        if (pageCount > MAX_PAGES) throw new Error('Paginação interrompida por segurança em ' + label + '.');
        if (seenStarts[String(start)]) throw new Error('Paginação repetida em ' + label + ' (start=' + start + ').');
        ...
        // quebra a cadeia de microtasks/promises para evitar estouro de pilha em navegadores/worker
        await delay0();
      }
    }
    ...
    async function mainLoad() {
      try {
        ...
        // Fallback: se o Worker/Bitrix ainda estourar pilha usando STAGE_ID,
        // tenta uma consulta única por categoria e filtra localmente.
        if (!isMaxStackErr(e)) throw e;
        out = [];
        var allowed = {};
        for (var a = 0; a < stageArr.length; a++) allowed[String(stageArr[a])] = true;
        await fetchPagesForFilter({ CATEGORY_ID: String(CFG.DEAL_CATEGORY_ID) }, 'categoria financeira');
