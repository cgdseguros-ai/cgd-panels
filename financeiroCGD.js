/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v34: CARTÕES adiciona exclusão de lançamentos e parcelas futuras
...
(function () {
  ...
  // =========================
  // PATCH CGD 2026-05-31 — Blindagem contra erro externo do Bitrix
  // =========================
  // Após atualização recente do Bitrix, alguns portais/máquinas passaram a disparar erros
  // em bundles internos do Bitrix antes/durante o boot do painel, especialmente:
  // - Cannot read properties of undefined (reading 'Accessibility')
  // - Cannot read properties of undefined (reading 'init')
  // - falhas/404 em /bitrix/js/main/popup, ui.hint, core, sidepanel etc.
  //
  // Esses erros não devem derrubar o Financeiro. Eles são tratados como aviso externo.
  var FIN_BOOT_DONE = false;
  var FIN_EXTERNAL_WARNED = false;

  function finPatchErrText(err) {
    try {
      if (!err) return "";
  ...
  function finPatchIsBitrixExternalError(err) {
    var txt = finPatchErrText(err);
    return (
      /bitrix\/js\//i.test(txt) ||
      /main\.popup\.bundle/i.test(txt) ||
      /main\.core/i.test(txt) ||
      /core\.js/i.test(txt) ||
      /ui\.hint/i.test(txt) ||
      /sidepanel/i.test(txt) ||
      /getPages/i.test(txt)
    );
  }

  function finPatchIsKnownBitrixCrash(err) {
    var txt = finPatchErrText(err);
    return (
      ...
      /Cannot read properties of undefined/i.test(txt) && finPatchIsBitrixExternalError(err) ||
      /Script error\.?/i.test(txt) ||
      /Loading chunk \d+ failed/i.test(txt)
    );
  }

  function finPatchIsResourceEvent(ev) {
    try {
      var t = ev && ev.target;
      if (!t || t === window) return false;
      var tag = String(t.tagName || "").toUpperCase();
      return tag === "SCRIPT" || tag === "LINK" || tag === "IMG" || tag === "IFRAME";
    } catch (_) {
      return false;
    }
  }

  function finPatchShowWarn(msg) {
    try {
      if (FIN_EXTERNAL_WARNED) return;
      FIN_EXTERNAL_WARNED = true;
      var box = document.createElement("div");
      box.id = "fin-bitrix-external-warn";
      box.style.cssText = "position:fixed;right:14px;bottom:52px;z-index:999999;max-width:480px;background:#fff7ed;color:#111827;border:1px solid rgba(245,158,11,.35);border-radius:14px;padding:11px 13px;box-shadow:0 16px 40px rgba(0,0,0,.18);font:12px/1.35 system-ui,-apple-system,Segoe UI,Roboto,Arial";
      box.innerHTML = '<b>Financeiro mantido aberto.</b><br>' + String(msg || "Erro externo do Bitrix detectado. O painel continuará carregando.").replace(/[<>&"]/g, function(c){ return {"<":"&lt;",">":"&gt;","&":"&amp;",'"':"&quot;"}[c]; });
      (document.body || document.documentElement).appendChild(box);
      setTimeout(function(){ try { if (box && box.parentNode) box.parentNode.removeChild(box); } catch(_){} }, 12000);
    } catch (_) {}
  }

  window.addEventListener("error", function (ev) {
    try {
      // Erro de recurso externo não pode derrubar o boot do painel.
      if (finPatchIsResourceEvent(ev)) {
        var src = (ev.target && (ev.target.src || ev.target.href)) || "";
        if (/bitrix\/|bitrix24|ui\.hint|main\.popup|core/i.test(String(src))) {
          finPatchShowWarn("O Bitrix falhou ao carregar um recurso interno, mas o Financeiro foi mantido aberto.");
        }
        return;
      }

      var err = ev && (ev.error || ev.message || ev);
      if (finPatchIsKnownBitrixCrash(err)) {
        finPatchShowWarn("O Bitrix apresentou uma falha temporária em módulos internos. O Financeiro foi mantido aberto.");
  ...
  function finPatchIsTemporaryApiError(err) {
    var txt = finPatchErrText(err);
    return /HTTP\s+(429|502|503|504)/i.test(txt) || /timeout|tempor/i.test(txt);
  }

  function finPatchFatalHtml(err) {
    var txt = finPatchErrText(err).replace(/[<>&"]/g, function(c){ return {"<":"&lt;",">":"&gt;","&":"&amp;",'"':"&quot;"}[c]; });
    return '' +
      '<div style="padding:16px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;color:#111827">' +
      '<div style="font-weight:950;font-size:16px;margin-bottom:8px">Financeiro não concluiu o carregamento</div>' +
      '<div style="font-size:13px;line-height:1.45;margin-bottom:12px;max-width:920px">O painel entrou, mas uma etapa de inicialização falhou. O erro abaixo foi preservado para diagnóstico, sem deixar a tela totalmente em branco.</div>' +
      '<button onclick="location.reload()" style="border:0;background:#2563eb;color:#fff;border-radius:10px;padding:10px 14px;font-weight:900;cursor:pointer">Recarregar painel</button>' +
      '<pre style="white-space:pre-wrap;background:#fff;border:1px solid rgba(0,0,0,.12);border-radius:12px;padding:12px;max-width:1100px;overflow:auto;margin-top:12px;font-size:12px">' + txt.slice(0, 3000) + '</pre>' +
      '</div>';
  }
  ...
  function listDealsAll() {
    ...
    // Correção anti "Maximum call stack size exceeded":
    // antes o painel fazia UMA consulta com STAGE_ID em array. Após mudança no Bitrix/Worker,
    // esse filtro composto pode estourar a pilha no roteamento/serialização. Agora consultamos
    // um stage por vez, com STAGE_ID escalar, e ainda blindamos paginação repetida/inválida.
    function normalizeNext(next) {
      if (next === null || next === undefined || next === false || next === '') return null;
      var n = Number(next);
    ...
    function fetchStage(stageId) {
      ...
      var seenStarts = {};
      var pageCount = 0;
      var MAX_PAGES_PER_STAGE = 250;

      function fetchPage() {
        pageCount++;
        if (pageCount > MAX_PAGES_PER_STAGE) {
          throw new Error('Paginação interrompida por segurança no stage ' + stageId + '.');
        }
        if (seenStarts[String(start)]) {
          throw new Error('Paginação repetida no stage ' + stageId + ' (start=' + start + ').');
        }
        seenStarts[String(start)] = true;

        return apiCall("crm.deal.list", {
          select: selectFields,
          filter: { CATEGORY_ID: String(CFG.DEAL_CATEGORY_ID), STAGE_ID: String(stageId) },
          order: { ID: "DESC" },
          start: start
        }).then(function (res) {
          ...
          var next = normalizeNext(res && res.next);
    ...
    for (var i = 0; i < stageArr.length; i++) {
      (function(stageId){
        chain = chain.then(function(){ return fetchStage(stageId); });
      })(stageArr[i]);
    }
  ...
  function boot() {
    ...
    return loadInstitutionalState()
      ...
      .catch(function (e) {
        ...
        if (finPatchIsKnownBitrixCrash(e)) {
          finPatchShowWarn("Erro externo do Bitrix detectado durante o boot. O Financeiro tentará manter a tela aberta.");
          return;
        }

        if (finPatchIsTemporaryApiError(e)) {
          toast("Bitrix/Worker instável no carregamento. Tentando novamente em alguns segundos...", "err");
          setTimeout(function(){ try { boot(); } catch(_) {} }, 6000);
          return;
        }

        toast("Erro ao iniciar: " + (e && e.message ? e.message : String(e)), "err");
        root.innerHTML = finPatchFatalHtml(e);
      })
      .finally(function () {
        FIN_BOOT_DONE = true;
        setLoading(false);
