/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v21: faturas de cartão aparecem nas recorrências e podem ser renovadas/criadas por vencimento fixo
...
(function () {
  ...
  function recurrenceSeriesList() {
    ...
    var cardItems = buildCardInvoiceRecurringItems();
    for (var ci = 0; ci < cardItems.length; ci++) out.push(cardItems[ci]);
  ...
  function openRecurringManagerModal() {
    var m = modal(
      '<div class="fin-modal-head"><div class="fin-modal-title">RECORRÊNCIAS — Renovar séries e faturas de cartão</div><button class="fin-x" data-close="1">×</button></div>' +
      '<div class="fin-modal-body"><div class="fin-muted" style="font-weight:900;margin-bottom:10px">Use esta tela para renovar séries que terminam em 12/2026 ou próximas do vencimento. As faturas de cartão aparecem como recorrências fixas pelo vencimento cadastrado; ao renovar, o sistema cria as despesas mensais da fatura com valor previsto inicial igual à fatura aberta ou R$ 0,00.</div><div id="rec-series-host"></div></div>',
      { full: true }
    );
    function draw(){
      var list = recurrenceSeriesList();
      ...
      for (var i=0;i<list.length;i++) {
        ...
        rows.push('<tr>' +
          ...
          '<td><button class="fin-mini fin-mini--ok" data-renew-series="' + esc(it.id) + '">' + (it.isCardInvoiceSeries ? 'Criar faturas' : 'Renovar') + '</button></td>' +
        '</tr>');
      }
  ...
  function isCardInvoiceDeal(d) {
    ...
    return obs.indexOf("FIN_CARD_INVOICE:1") > -1 || /^FATURA CARTÃO\s*-/i.test(title);
  }
  ...
  function upsertCardInvoiceDeal(cardId, dueIso, total, closeIso, count) {
    ...
    fields[CFG.F.OBS] = "FIN_CARD_INVOICE:1 • FIN_CARD_INVOICE_DUE:" + dueIso + " • REC_SERIE:" + cardInvoiceSeriesId(cardId) + " • REC_FREQ:monthly • Cartão: " + cardName + " • Vencimento fixo: " + toDisplayDate(dueIso) + (closeIso ? " • Fechamento: " + toDisplayDate(closeIso) : "") + " • Itens: " + String(count || 0);
  ...
  function openCardModal() {
      ...
      '<div class="fin-modal-body">' +
        ...
        '<div class="fin-panel" style="margin-top:10px"><div class="fin-panel-inner">' +
          ...
          '<div class="fin-row" style="gap:10px;flex-wrap:wrap">' +
            ...
            '<button class="fin-btn" id="card-sync-invoice" style="align-self:flex-end">Atualizar despesas de fatura</button>' +
    ...
    m.q("#card-sync-invoice").addEventListener("click", function(){
      ...
      syncCardInvoicesForCard(card).then(function(n){
        applyFilters();
        updateCardSummary();
        toast("Despesas de fatura atualizadas ✅ (" + n + ")");
      }).catch(function(e){
