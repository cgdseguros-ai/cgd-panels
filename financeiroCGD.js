    ...
    try {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        html.push('<tr>' +
          '<td></td>' +
          '<td class="fin-mono">#' + esc(d.ID || '') + '</td>' +
          '<td><div class="fin-strong">' + esc(fav) + '</div><div class="fin-small fin-muted">fallback bruto v48</div></td>' +
      ...
      try { console.error('[Financeiro][fallback bruto v48] falhou', e); } catch(_) {}
      return false;
    }
  }
    ...
    return listDealsAll()
      .then(function (deals) {
        ...
        try {
          ...
          if ((S.deals || []).length && !(S.filtered || []).length) {
            renderRawDealsFallback(S.deals, "Filtragem normal zerou a lista; fallback bruto v48 ativado.");
          }
        } catch (eApply) {
      ...
      .catch(function (e) {
        ...
        if (/Maximum call stack size exceeded/i.test(msg)) {
          msg = "Maximum call stack evitado: mantendo lançamentos já carregados em modo bruto v48.";
        }
        S.lastApiError = msg;
