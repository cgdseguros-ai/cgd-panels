    ...
    try {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        html.push('<tr>' +
          '<td></td>' +
          '<td class="fin-mono">#' + esc(d.ID || '') + '</td>' +
          '<td><div class="fin-strong">' + esc(fav) + '</div><div class="fin-small fin-muted">fallback bruto v47</div></td>' +
      ...
      try { toast('Modo seguro v47 ativado: lançamentos carregados preservados.', 'err'); } catch(_) {}
      return true;
    } catch (e) {
      try { console.error('[Financeiro][fallback bruto v47] falhou', e); } catch(_) {}
      return false;
    }
  }
      ...
      .then(function (deals) {
        ...
        try {
          ...
          if ((S.deals || []).length && !(S.filtered || []).length) {
            renderRawDealsFallback(S.deals, "Filtragem normal zerou a lista; fallback bruto v47 ativado.");
          }
        } catch (eApply) {
