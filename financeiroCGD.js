/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v42: PATCH fallback bruto contra Maximum call stack + preserva lançamentos carregados
...
(function () {
  ...
  try { setInterval(finCleanExternalFatalPanels, 1500); } catch (_) {}
    ...
    try {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        html.push('<tr>' +
          '<td></td>' +
          '<td class="fin-mono">#' + esc(d.ID || '') + '</td>' +
          '<td><div class="fin-strong">' + esc(fav) + '</div><div class="fin-small fin-muted">fallback bruto v42</div></td>' +
      ...
      try { console.error('[Financeiro][fallback bruto v42] falhou', e); } catch(_) {}
      return false;
    }
  }
  ...
  function openDiagnosticsModal() {
    ...
    Promise.allSettled([
      ...
      host.innerHTML = '<table class="fin-table" style="min-width:0"><tbody>' +
        ...
        line('Versão JS', 'v42 fallback bruto sem stack', true) +
    ...
    return listDealsAll()
      .then(function (deals) {
        ...
        try {
          ...
          if ((S.deals || []).length && !(S.filtered || []).length) {
            renderRawDealsFallback(S.deals, "Filtragem normal zerou a lista; fallback bruto v42 ativado.");
          }
        } catch (eApply) {
      ...
      .catch(function (e) {
        ...
        if (/Maximum call stack size exceeded/i.test(msg)) {
          msg = "Maximum call stack evitado: mantendo lançamentos já carregados em modo bruto v42.";
        }
        S.lastApiError = msg;
  ...
  function boot() {
    ...
    return loadInstitutionalState()
      ...
      .catch(function (e) {
        ...
        if (/Maximum call stack size exceeded/i.test(bootMsg)) {
          if ((S.deals || []).length && renderRawDealsFallback(S.deals, bootMsg)) return;
          if (document.getElementById('fin-tbody')) {
            toast('Erro de stack evitado. Tela preservada em modo diagnóstico.', 'err');
            return;
          }
        }
        toast("Erro ao iniciar: " + bootMsg, "err");
        root.innerHTML = '<div style="padding:16px;font-family:system-ui"><b>Financeiro não concluiu o carregamento.</b><br><button onclick="location.reload()" style="margin-top:10px;padding:8px 12px;border-radius:10px;border:0;background:#2563eb;color:#fff;font-weight:900">Recarregar</button><pre style="white-space:pre-wrap;background:#fff;border:1px solid rgba(0,0,0,.12);border-radius:12px;padding:12px;margin-top:12px;max-width:1100px;overflow:auto">' + esc(bootMsg) + '</pre></div>';
      })
      .finally(function () {
