...
(function () {
  ...
  function listDealsAll() {
    ...
    // Bitrix normalmente pagina com res.next. Se o Worker/Bitrix devolver
    // next repetido, vazio, 0 indevido ou string estranha, a versão recursiva
    // antiga entrava em loop e podia gerar "Maximum call stack size exceeded".
    ...
    function fetchPage() {
      ...
      if (pageCount > MAX_PAGES) {
        throw new Error('Paginação interrompida por segurança: limite de páginas excedido. Verifique o retorno res.next do Bitrix/Worker.');
      }
      if (seenStarts[String(start)]) {
        throw new Error('Paginação interrompida por segurança: o Bitrix/Worker repetiu o start=' + start + '.');
      }
      ...
      return apiCall("crm.deal.list", {
        ...
        if (next === null || next === start) return out;
        start = next;
        return fetchPage();
      });
    }
