/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v14: LOTE RECEITAS usa Centro de Custo recebedor; Sócio aparece conforme categoria em despesas
...
(function () {
  ...
  function captureInstitutionalBundle() {
    return {
      ...
      meta: { savedAt: new Date().toISOString(), version: 'v13-socio-categoria-visivel' }
    };
  }
    ...
    function renderTable(host) {
      ...
      var html =
        '<div class="fin-row" style="gap:8px;flex-wrap:wrap;margin-bottom:10px;justify-content:space-between">' +
          '<div class="fin-muted" style="font-weight:900">' + (isRec ? 'Receitas são lançamentos manuais, sem recorrência. Informe o CENTRO DE CUSTO recebedor da receita.' : 'Despesas podem ser avulsas ou recorrentes. Para pró-labore, bonificação ou distribuição de lucro, use a CATEGORIA e a UF SÓCIO.') + '</div>' +
        ...
        '<div class="fin-table-wrap"><table class="fin-batch-table"><thead><tr>' +
          (isRec ? '<th style="min-width:220px">CENTRO DE CUSTO RECEBEDOR</th>' : '<th style="min-width:180px">CATEGORIA</th>') +
      ...
      for (var i = 0; i < rows.length; i++) {
        var r = rows[i];
        html += '<tr data-i="' + i + '">' +
          (isRec ? '<td><select class="fin-batch-sel" data-k="centro">' + centroOpts + '</select></td>' : '<td><select class="fin-batch-sel" data-k="categoria">' + catOpts + '</select></td>') +
      ...
      for (var ti = 0; ti < trs.length; ti++) {
        ...
        var sc = trs[ti].querySelector('select[data-k="centro"]'); if (sc) sc.value = rr.centro || "";
      }
