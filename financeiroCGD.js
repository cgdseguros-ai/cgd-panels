    ...
    function renderTable(host) {
      ...
      var html =
        '<div class="fin-row" style="gap:8px;flex-wrap:wrap;margin-bottom:10px;justify-content:space-between">' +
          '<div class="fin-muted" style="font-weight:900">' + (isRec ? 'Receitas são lançamentos manuais, sem recorrência.' : 'Despesas podem ser avulsas ou recorrentes. Para pró-labore, bonificação ou distribuição de lucro, use a CATEGORIA e a UF SÓCIO.') + '</div>' +
      ...
      if (!isRec) {
        html += '<th style="min-width:145px">RECORRÊNCIA</th><th style="min-width:145px">CONFIG.</th><th style="min-width:80px">QTD</th><th style="min-width:150px">SÓCIO</th>';
      }
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        if (!isRec) {
          html += '<td><select class="fin-batch-sel" data-k="freq"><option value="once" ' + (r.freq === "once" ? "selected" : "") + '>Avulsa</option><option value="weekly" ' + (r.freq === "weekly" ? "selected" : "") + '>Semanal</option><option value="monthly" ' + (r.freq === "monthly" ? "selected" : "") + '>Mensal</option><option value="yearly" ' + (r.freq === "yearly" ? "selected" : "") + '>Anual</option></select></td>' +
            ...
            '<td><select class="fin-batch-sel" data-k="socio">' + socioOptions(r.socio) + '</select></td>';
        }
      ...
      host.querySelector("#b-clean").addEventListener("click", function(){ rows = rows.filter(function(r){ return String(r.favorecido || "").trim() || String(r.start || "").trim() || String(r.valor || "").trim() || String(r.obs || "").trim(); }); if (!rows.length) for (var z=0; z<6; z++) rows.push(mkRow(kind)); renderTable(host); });
