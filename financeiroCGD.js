/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   PATCH v46 — overlay seguro: cobre erro/rodapé Bitrix sem esconder o Financeiro
...
(function () {
  ...
  // PATCH v46: em vez de esconder todos os elementos externos do Bitrix,
  // o Financeiro fica em um overlay próprio e seguro. Isso cobre a tela
  // "Falha ao carregar o painel" e o rodapé Bitrix, sem provocar tela branca.
  function finInstallOverlayCssV46() {
    try {
      if (document.getElementById("fin-v46-overlay-css")) return;
      var st = document.createElement("style");
      st.id = "fin-v46-overlay-css";
      st.textContent = [
        "body.fin-v46-active{overflow:hidden!important;background:#f4f5f7!important;}",
        "#fin-root.fin-fixed-root-v46{position:fixed!important;inset:0!important;z-index:900!important;display:block!important;visibility:visible!important;opacity:1!important;overflow:auto!important;background:#f4f5f7!important;}",
        "#fin-root.fin-fixed-root-v46 .fin-page{min-height:100vh!important;}",
        "#fin-root.fin-fixed-root-v46 .fin-modal-wrap{z-index:1000!important;}",
        "#fin-root.fin-fixed-root-v46 .fin-toast-host{z-index:1100!important;}"
      ].join("\n");
      (document.head || document.documentElement).appendChild(st);
    } catch (_) {}
  }
  function finActivateOverlayV46() {
    try { finInstallOverlayCssV46(); } catch (_) {}
    try { document.body && document.body.classList && document.body.classList.add("fin-v46-active"); } catch (_) {}
    try {
      var r = document.getElementById("fin-root");
      if (r && r.classList) r.classList.add("fin-fixed-root-v46");
    } catch (_) {}
  }
  try { finInstallOverlayCssV46(); } catch (_) {}
  function finShowPatchWarn(msg) {
    try {
      ...
      if (!box) {
        ...
        box.id = id;
        box.style.cssText = "position:fixed;right:12px;bottom:50px;z-index:1100;background:#fff7ed;color:#111827;border:1px solid rgba(245,158,11,.45);border-radius:12px;padding:10px 12px;box-shadow:0 14px 38px rgba(0,0,0,.18);font:12px/1.35 system-ui,-apple-system,Segoe UI,Roboto,Arial;max-width:460px";
  ...
  try { finActivateOverlayV46(); } catch (_) {}
  ...
  function renderMinimalFinanceShellV43(reason) {
    try {
      root.innerHTML =
        '<div class="fin-page">' +
          '<header class="fin-topbar">' +
            '<div class="fin-top-left"><img class="fin-top-logo" src="' + esc(CFG.LOGO_URL) + '" alt="CGD"><div><div class="fin-top-title">Financeiro CGD</div><div class="fin-top-sub">MODO SEGURO v46</div></div></div>' +
          ...
          '<main class="fin-shell"><section class="fin-panel"><div class="fin-panel-inner">' +
            ...
            '<div class="fin-table-wrap" style="margin-top:12px"><table class="fin-table"><thead><tr><th>ID</th><th>Favorecido/Pagador</th><th>Centro</th><th>Tipo</th><th>Competência</th><th>Data Prev.</th><th>Data Real</th><th>Previsto</th><th>Realizado</th><th>Etapa</th><th>Ações</th></tr></thead><tbody id="fin-tbody"><tr><td colspan="11" class="fin-muted">Carregando modo seguro v46...</td></tr></tbody></table></div>' +
      ...
      try { root.innerHTML = '<div style="padding:16px;font-family:system-ui">Financeiro em modo seguro v46. Recarregue a página.</div>'; } catch(_) {}
    }
  }
  ...
  function listDealsAll() {
    ...
    function isWorkerPaginationErr(e) {
      return /Maximum call stack size exceeded|paginação|pagination|stack size/i.test(finErrText(e));
    }
    ...
    function apiListOnePage(filter, label) {
      // PATCH v38: não envia START. O Worker/Bitrix começou a quebrar com paginação
      // start/next em alguns ambientes. Uma única página por filtro deixa o painel utilizável.
      return apiCall("crm.deal.list", {
        ...
        var msg = finErrText(e);
        try { console.warn("[Financeiro][safe-page] falhou", label, e); } catch(_) {}
        if (isWorkerPaginationErr(e)) return [];
        throw e;
      });
    }

    function apiListOnePageNarrow(filter, label) {
      return apiCall("crm.deal.list", {
        select: selectFields,
        filter: filter || {},
        order: { ID: "DESC" }
        ...
        .catch(function(e){
          try { console.warn("[Financeiro][safe-narrow] falhou", label, e); } catch(_) {}
          return [];
        });
    }
    ...
    function currentSafeYear() {
      try {
        var n = document.getElementById("fin-chart-year");
      ...
      comps.forEach(function(compId){
        stageArr.forEach(function(stageId){
          p = p.then(function(){
            ...
            return apiListOnePageNarrow(f, "comp " + compId + " / stage " + stageId)
              .then(function(chunk){ rows = rows.concat(chunk || []); });
          });
        });
      });
    ...
    // Estratégia v38:
    // 1) tenta carregar uma página por stage, SEM paginação/START;
    // 2) se vier vazio, tenta quebrar por competência do ano atual;
    // 3) se ainda vier vazio, não derruba o painel: retorna [] e o diagnóstico fica disponível.
    ...
    try {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        html.push('<tr>' +
          '<td></td>' +
          '<td class="fin-mono">#' + esc(d.ID || '') + '</td>' +
          '<td><div class="fin-strong">' + esc(fav) + '</div><div class="fin-small fin-muted">fallback bruto v46</div></td>' +
      ...
      try { toast('Modo seguro v46 ativado: lançamentos carregados preservados.', 'err'); } catch(_) {}
      return true;
    } catch (e) {
      try { console.error('[Financeiro][fallback bruto v46] falhou', e); } catch(_) {}
      return false;
    }
  }
  ...
  function openDiagnosticsModal() {
    ...
    Promise.allSettled([
      ...
      host.innerHTML = '<table class="fin-table" style="min-width:0"><tbody>' +
        ...
        line('Versão JS', 'v45 rollback seguro sem isolamento agressivo', true) +
    ...
    return listDealsAll()
      .then(function (deals) {
        ...
        try {
          ...
          if ((S.deals || []).length && !(S.filtered || []).length) {
            renderRawDealsFallback(S.deals, "Filtragem normal zerou a lista; fallback bruto v46 ativado.");
          }
        } catch (eApply) {
      ...
      .catch(function (e) {
        ...
        if (/Maximum call stack size exceeded/i.test(msg)) {
          msg = "Maximum call stack evitado: mantendo lançamentos já carregados em modo bruto v43.";
        }
        S.lastApiError = msg;
    ...
    return loadInstitutionalState()
      ...
      .then(function () {
        try { render(); } catch (eRender) {
          ...
          renderMinimalFinanceShellV43("Render principal falhou; modo seguro v46 ativado.");
        }
      ...
      .catch(function (e) {
        ...
        if (!document.getElementById('fin-tbody')) renderMinimalFinanceShellV43("Boot interrompido; modo seguro v46 preservou a tela.");
