...
(function () {
  ...
  function captureInstitutionalBundle() {
    return {
      ...
      meta: { savedAt: new Date().toISOString(), version: 'v9-cloud' }
    };
  }
  ...
  function openBatch(kind) {
    ...
    function calcDate(startISO, r, idx) {
      ...
      // A recorrência usa exclusivamente a DATA INICIAL.
      // Mensal: mantém o mesmo dia da data inicial.
      // Anual: mantém o mesmo dia/mês da data inicial.
      if (freq === "monthly") return addMonthsISO(startISO, idx, null);
      if (freq === "yearly") return addYearsISO(startISO, idx, null, null);
    ...
    function renderTable(host) {
      ...
      var html =
        '<div class="fin-row" style="gap:8px;flex-wrap:wrap;margin-bottom:10px;justify-content:space-between">' +
          '<div class="fin-muted" style="font-weight:900">' + (isRec ? 'Receitas são lançamentos manuais, sem recorrência.' : 'Despesas usam DATA INICIAL em DD/MM/AAAA. Se houver recorrência, o sistema calcula as próximas datas automaticamente.') + '</div>' +
      ...
      if (!isRec) {
        html += '<th style="min-width:150px">RECORRÊNCIA</th>';
      }
