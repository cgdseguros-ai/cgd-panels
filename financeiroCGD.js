/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   v27: CARTÕES com abas, faturas por mês, edição de parcelas e valor permanece como VALOR DA PARCELA
...
(function () {
  ...
  function openCardModal() {
    ...
    var m = modal(
      ...
      '<div class="fin-modal-body">' +
        ...
        '<div class="fin-row" style="gap:8px;flex-wrap:wrap;margin-top:10px">' +
          '<button class="fin-btn fin-btn--primary" id="card-mode-launches">Ver lançamentos/faturas</button>' +
          '<button class="fin-btn" id="card-mode-add">Adicionar novos lançamentos</button>' +
        '</div>' +
    ...
    function setCardMode(mode) {
      ...
      if (b1) b1.className = "fin-btn" + (mode === "add" ? "" : " fin-btn--primary");
      if (b2) b2.className = "fin-btn" + (mode === "add" ? " fin-btn--primary" : "");
    }
    ...
    function findCardRelatedDeals(source, applyFuture, newParcelNo, newTotal) {
      ...
      var startParcel = Number(newParcelNo || meta.parcel || 1);
      var out = [];
      for (var i = 0; i < (S.deals || []).length; i++) {
        var d = S.deals[i];
        if (!isCardPurchaseDetailDeal(d)) continue;
        ...
        if (Number(dm.parcel || 1) < startParcel) continue;
        out.push(d);
      }
      out.sort(function(a,b){ return Number(cardMeta(a).parcel || 1) - Number(cardMeta(b).parcel || 1); });
    ...
    function openCardLaunchEdit(id) {
      ...
      var em = modal(
        ...
        '<div class="fin-modal-body">' +
          ...
          '<div class="fin-row fin-row--right" style="margin-top:14px"><button class="fin-btn" data-close="1">Cancelar</button><button class="fin-btn fin-btn--primary" id="ce-save" data-busylock="1">Salvar edição</button></div>' +
        '</div>'
      );
      attachDateMasks(em.node);
      em.q('#ce-save').addEventListener('click', function(){
        ...
        if (!fav) { toast('Informe a conta/compra.', 'err'); return; }
        if (!baseDate) { toast('Informe a data da compra.', 'err'); return; }
        var targets = findCardRelatedDeals(source, applyFuture, parcStart, total);
        if (applyFuture && !confirm('Serão alteradas ' + targets.length + ' parcela(s) ainda não pagas. Parcelas pagas serão ignoradas. Continuar?')) return;
        ...
        updateDealsSequential(targets, function(d){
          var dm = cardMeta(d);
          var parc = applyFuture ? Number(dm.parcel || parcStart) : parcStart;
          if (parc < parcStart) parc = parcStart;
          var purchaseDt = addMonthsISO(baseDate, parc - parcStart, null);
          ...
          return syncCardInvoicesForCard(source[CFG.F.CONTA]).then(function(){ return n; });
        }).then(function(n){
          applyFilters();
          updateCardSummary();
          renderCardLaunches();
          toast('Lançamento(s) atualizado(s) ✅ (' + n + ')');
          em.close();
        }).catch(function(e){
          toast('Falha ao editar: ' + (e.message || String(e)), 'err');
        }).finally(function(){ setModalBusy(em.node, false); });
      });
    }
