/* financeiro.js — CGD Financeiro (Bitrix via Worker)
   PATCH v45 — rollback seguro: remove isolamento agressivo da v44 e evita tela branca
   v43: PATCH estabilidade raiz + sem sobrescrever tela + render protegido
...
(function () {
  ...
  // =========================
  // PATCH v36 — blindagem precoce contra erro externo do Bitrix
  // =========================
  var FIN_PATCH_BOOT_DONE = false;
  ...
  function finIsBitrixExternalError(e) {
    var t = finErrText(e);
    return /Cannot read properties of undefined \(reading ['"]init['"]\)/i.test(t)
      || /Cannot read properties of undefined \(reading ['"]Accessibility['"]\)/i.test(t)
      || (/Cannot read properties of undefined/i.test(t) && /(bitrix\/js\/main\/core|core\.min\.js|main\.popup|ui\.hint|getPages|sidepanel)/i.test(t))
      || (/Script error\.?/i.test(t) && !FIN_PATCH_BOOT_DONE);
  }
  function finShowPatchWarn(msg) {
    try {
      var id = "fin-patch-warn";
      var box = document.getElementById(id);
      if (!box) {
        box = document.createElement("div");
        box.id = id;
        box.style.cssText = "position:fixed;right:12px;bottom:50px;z-index:999999;background:#fff7ed;color:#111827;border:1px solid rgba(245,158,11,.45);border-radius:12px;padding:10px 12px;box-shadow:0 14px 38px rgba(0,0,0,.18);font:12px/1.35 system-ui,-apple-system,Segoe UI,Roboto,Arial;max-width:460px";
        (document.body || document.documentElement).appendChild(box);
      }
      box.innerHTML = "<b>Financeiro mantido aberto.</b><br>" + String(msg || "Erro externo do Bitrix ignorado.");
      clearTimeout(box._t);
      box._t = setTimeout(function(){ try { box.remove(); } catch(_) {} }, 9000);
    } catch (_) {}
  }
  window.addEventListener("error", function(ev) {
    try {
      ...
      if (target && target !== window && (target.src || target.href)) {
        var u = String(target.src || target.href || "");
        if (/bitrix\/|main\.popup|core\.min|ui\.hint/i.test(u)) {
          finShowPatchWarn("Falha de recurso interno do Bitrix ignorada.");
          try { ev.preventDefault && ev.preventDefault(); ev.stopImmediatePropagation && ev.stopImmediatePropagation(); } catch(_) {}
          return false;
        }
        return;
      }
      var er = ev && (ev.error || ev.message || ev);
      if (finIsBitrixExternalError(er)) {
        finShowPatchWarn("Falha de inicialização do Bitrix ignorada.");
        try { ev.preventDefault && ev.preventDefault(); ev.stopImmediatePropagation && ev.stopImmediatePropagation(); } catch(_) {}
        return false;
      }
    } catch (_) {}
  ...
  // Watchdog: algumas páginas do Bitrix têm um tratador próprio que escreve
  // "Falha ao carregar o painel" por erro do core.js antes/de fora do Financeiro.
  // Se o Financeiro renderizar, esse bloco externo é apenas ruído visual e deve sumir.
  function finCleanExternalFatalPanels() {
    try {
      var nodes = Array.prototype.slice.call(document.body ? document.body.children : []);
      for (var i = 0; i < nodes.length; i++) {
        var n = nodes[i];
        if (!n || n.id === "fin-root" || (n.querySelector && n.querySelector(".fin-page"))) continue;
        var tx = String(n.innerText || n.textContent || "");
        // PATCH v45: limpeza seletiva. Não esconde o body inteiro, nem todos os filhos.
        // Só oculta blocos externos claramente criados pelo Bitrix/tela sentinela.
        if (/Falha ao carregar o painel/i.test(tx) && /(Erro JS|reading ['"]init['"]|core\.min\.js)/i.test(tx)) {
          n.style.display = "none";
          n.setAttribute("data-fin-hidden-bitrix-fatal", "1");
        }
        if (/Fornecido por Bitrix24|Denunciar abuso|Crie sites online/i.test(tx)) {
          n.style.display = "none";
          n.setAttribute("data-fin-hidden-bitrix-footer", "1");
        }
      }
    } catch (_) {}
  }
  try { setInterval(finCleanExternalFatalPanels, 500); } catch (_) {}

  function finAntiBlankWatchdogV45() {
    try {
      var r = document.getElementById("fin-root");
      if (!r) return;
      var hasPage = !!(r.querySelector && r.querySelector(".fin-page"));
      var txt = String(r.innerText || r.textContent || "").trim();
      // Se algum patch visual deixou a página sem conteúdo, devolve um shell mínimo em vez de branco.
      if (!hasPage && txt.length < 5) {
        r.style.display = "block";
        r.style.visibility = "visible";
        r.innerHTML = '<div style="padding:16px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;color:#111827"><b>Financeiro em recuperação visual v45</b><br><button onclick="location.reload()" style="margin-top:10px;border:0;background:#2563eb;color:#fff;border-radius:10px;padding:10px 14px;font-weight:900;cursor:pointer">Recarregar</button><div style="margin-top:10px;font-size:12px;opacity:.75">A tela branca foi bloqueada. Recarregue uma vez ou volte para a versão v43/v42 se necessário.</div></div>';
      }
    } catch (_) {}
  }
  try { setTimeout(finAntiBlankWatchdogV45, 1800); setTimeout(finAntiBlankWatchdogV45, 4500); } catch (_) {}
  ...
  // PATCH v40: se a página do Bitrix inserir uma tela externa de "Falha ao carregar",
  // não deixamos ela cobrir o Financeiro quando .fin-page já existe.
  setInterval(function(){
    try { if (document.querySelector('.fin-page')) finHideExternalBitrixFatalBlocks(); } catch(_) {}
  }, 1200);
  ...
  // Root
  // =========================
  // PATCH v43: isola o Financeiro em #fin-root. Antes, em páginas sem #fin-root,
  // o JS podia usar document.body como root. Com a atualização do Bitrix, isso
  // aumentou o risco de conflito com o rodapé/tela fatal externa do Bitrix.
  function ensureFinanceRootV43() {
    var r = document.getElementById("fin-root");
    if (!r) {
      r = document.createElement("div");
      r.id = "fin-root";
      try { (document.body || document.documentElement).insertBefore(r, (document.body || document.documentElement).firstChild || null); }
      catch (_) { (document.body || document.documentElement).appendChild(r); }
    }
    return r;
  }
  var root = ensureFinanceRootV43();
  ...
  function finSafeSetBodyClassV43() {
    try { document.body && document.body.classList && document.body.classList.add("fin-v43-active"); } catch (_) {}
  }

  function renderMinimalFinanceShellV43(reason) {
    try {
      root.innerHTML =
        '<div class="fin-page">' +
          '<header class="fin-topbar">' +
            '<div class="fin-top-left"><img class="fin-top-logo" src="' + esc(CFG.LOGO_URL) + '" alt="CGD"><div><div class="fin-top-title">Financeiro CGD</div><div class="fin-top-sub">MODO SEGURO v45</div></div></div>' +
            '<div class="fin-top-actions"><button class="fin-btn" onclick="location.reload()">RECARREGAR</button><button class="fin-btn" id="btn-diagnostics">DIAGNÓSTICO</button><button class="fin-btn" id="btn-refresh">ATUALIZAR</button></div>' +
          '</header>' +
          '<main class="fin-shell"><section class="fin-panel"><div class="fin-panel-inner">' +
            '<div class="fin-kpis"><div class="fin-kpi"><div class="fin-kpi-k">Total Previsto</div><div class="fin-kpi-v">—</div></div><div class="fin-kpi"><div class="fin-kpi-k">Total Realizado</div><div class="fin-kpi-v">—</div></div><div class="fin-kpi"><div class="fin-kpi-k">Qtd. Itens</div><div class="fin-kpi-v">' + esc(String((S.deals || []).length || 0)) + '</div></div></div>' +
            '<div class="fin-table-wrap" style="margin-top:12px"><table class="fin-table"><thead><tr><th>ID</th><th>Favorecido/Pagador</th><th>Centro</th><th>Tipo</th><th>Competência</th><th>Data Prev.</th><th>Data Real</th><th>Previsto</th><th>Realizado</th><th>Etapa</th><th>Ações</th></tr></thead><tbody id="fin-tbody"><tr><td colspan="11" class="fin-muted">Carregando modo seguro v45...</td></tr></tbody></table></div>' +
            '<div class="fin-muted fin-small" style="margin-top:8px">' + esc(reason || 'Render normal protegido.') + '</div>' +
          '</div></section></main>' +
          '<div class="fin-toast-host" id="fin-toasts"></div>' +
        '</div>';
      bindBasicActions();
    } catch (e) {
      try { root.innerHTML = '<div style="padding:16px;font-family:system-ui">Financeiro em modo seguro v45. Recarregue a página.</div>'; } catch(_) {}
    }
  }
  ...
  // Compatibilidade: algumas rotinas antigas chamavam attachDateMasks.
  // Mantemos alias para evitar ReferenceError e impedir quebra do painel/modal.
  function attachDateMasks(scope) {
    return bindDateMasks(scope);
  }
  ...
  function renderAfterFilter() {
    // PATCH v43: renderização por partes. Um erro em gráfico/sidebar/pager não pode
    // derrubar a tabela nem substituir o painel inteiro.
    try {
      renderTable();
    } catch (eTable) {
      S.lastApiError = finErrText(eTable) || String(eTable);
      renderRawDealsFallback(S.filtered && S.filtered.length ? S.filtered : S.deals, "Tabela normal falhou: " + (eTable && eTable.message ? eTable.message : String(eTable)));
    }
    try { renderTotals(); } catch (eTotals) { S.lastApiError = finErrText(eTotals) || String(eTotals); }
    try { renderChartsPlaceholders(); } catch (eCharts) { S.lastApiError = finErrText(eCharts) || String(eCharts); }
    try { renderSidebarCenters(); } catch (eSide) { S.lastApiError = finErrText(eSide) || String(eSide); }
    try { renderPager(); } catch (ePager) { S.lastApiError = finErrText(ePager) || String(ePager); }
    try { syncTopbarFilters(); } catch (_) {}
  }
  ...
  function apiCall(method, payload) {
    ...
    return req(API_BASE + "/" + method)
      .catch(function (e1) {
        S.lastApiError = "path " + method + ": " + finErrText(e1);
        return req(API_BASE + "?method=" + encodeURIComponent(method));
      })
      .then(function (res) { S.apiMode = res.mode; return res.json; })
      .catch(function(e2){
        S.lastApiError = (S.lastApiError ? S.lastApiError + " | " : "") + "query " + method + ": " + finErrText(e2);
        throw e2;
      });
  }
  ...
  function listDealsAll() {
    ...
    function cloneFilter(filter) {
      var f = {};
      filter = filter || {};
      for (var k in filter) if (filter.hasOwnProperty(k)) f[k] = filter[k];
      return f;
    }
    ...
    function dedupAndSort(rows) {
      ...
      out.sort(function(a, b){ return Number(b.ID || 0) - Number(a.ID || 0); });
      return out;
    }

    function minNumericId(chunk) {
      var min = null;
      chunk = chunk || [];
      for (var i = 0; i < chunk.length; i++) {
        var n = Number(chunk[i] && chunk[i].ID);
    ...
    function isWorkerHardErr(e) {
      return /Maximum call stack size exceeded|paginação|pagination|stack size|Worker\/Bitrix|recusou/i.test(finErrText(e));
    }

    // PATCH v49 — carga completa sem usar paginação start/next.
    // O Bitrix/Worker estava devolvendo só os primeiros 50 quando não usávamos paginação,
    // mas a paginação REST tradicional causou stack no Worker. Para contornar, fazemos
    // paginação manual por ID: sempre busca os 50 maiores; depois busca ID menor que o
    // menor ID recebido. Isso evita start/next e consegue varrer todos os lançamentos.
    async function fetchAllByIdCursor(baseFilter, label) {
      var rows = [];
      var lastId = null;
      var seen = {};
      var maxPages = 300; // 300 x 50 = 15.000 registros por filtro/stage
      var page = 0;

      while (page < maxPages) {
        page++;
        var f = cloneFilter(baseFilter);
        if (lastId !== null) f["<ID"] = String(lastId);

        var cursor = String(lastId === null ? "FIRST" : lastId);
        if (seen[cursor]) break;
        seen[cursor] = true;

        var res = await apiCall("crm.deal.list", {
          select: selectFields,
          filter: f,
          order: { ID: "DESC" }
        });
        ...
        var nextLastId = minNumericId(chunk);
        if (nextLastId === null) break;
        if (lastId !== null && nextLastId >= lastId) break;

        // Se veio menos que 50, não há próxima página nesse filtro.
    ...
    function apiListOnePage(filter, label) {
      return apiCall("crm.deal.list", {
        select: selectFields,
        filter: filter || {},
        order: { ID: "DESC" }
      }).then(function(res){ return (res && res.result) ? res.result : []; })
        .catch(function(e){
          try { console.warn("[Financeiro][safe-page] falhou", label, e); } catch(_) {}
          return [];
        });
    }

    function competenciaIdsForYear(year) {
      var y = String(year || new Date().getFullYear());
      var list = S.enums[CFG.F.COMPETENCIA] || [];
      var out = [];
      for (var i = 0; i < list.length; i++) {
        var val = String(list[i].VALUE || "");
        if (val.indexOf(y) > -1) out.push(String(list[i].ID));
      }
      return out;
    }

    function currentSafeYear() {
      try {
        var n = document.getElementById("chart-year");
        if (n && n.value) return String(n.value);
      } catch (_) {}
      return String(new Date().getFullYear());
    }

    async function loadFullByStageCursor() {
      var rows = [];
      for (var i = 0; i < stageArr.length; i++) {
        var stageId = String(stageArr[i]);
        var f = { CATEGORY_ID: String(CFG.DEAL_CATEGORY_ID), STAGE_ID: stageId };
        var chunk = await fetchAllByIdCursor(f, "stage " + stageId);
        rows = rows.concat(chunk || []);
      }
      return dedupAndSort(rows);
    }

    function loadSafeFirstPages() {
      var rows = [];
      var p = Promise.resolve();
      stageArr.forEach(function(stageId){
        p = p.then(function(){
          return apiListOnePage({ CATEGORY_ID: String(CFG.DEAL_CATEGORY_ID), STAGE_ID: String(stageId) }, "stage " + stageId)
            .then(function(chunk){ rows = rows.concat(chunk || []); });
        });
      });
      return p.then(function(){ return dedupAndSort(rows); });
    }

    function loadSafeByCompetenciaYear(year) {
      var rows = [];
      var comps = competenciaIdsForYear(year);
      if (!comps.length) return Promise.resolve([]);
      var p = Promise.resolve();
      comps.forEach(function(compId){
        stageArr.forEach(function(stageId){
          p = p.then(function(){
            var f = { CATEGORY_ID: String(CFG.DEAL_CATEGORY_ID), STAGE_ID: String(stageId) };
            f[CFG.F.COMPETENCIA] = String(compId);
            return apiListOnePage(f, "comp " + compId + " / stage " + stageId)
              .then(function(chunk){ rows = rows.concat(chunk || []); });
          });
        });
      });
    ...
    return loadFullByStageCursor()
      .then(function(rows){
        if (rows && rows.length) return rows;
        return loadSafeFirstPages();
      })
      .then(function(rows){
        if (rows && rows.length) return rows;
        return loadSafeByCompetenciaYear(currentSafeYear());
      })
      .catch(function(e){
        // Se a carga completa por cursor for recusada pelo Worker, não derruba o painel:
        // volta ao modo seguro de 1 página por stage, que já funcionou no seu ambiente.
        S.lastApiError = "Carga completa v49 falhou; fallback seguro ativado: " + finErrText(e);
        return loadSafeFirstPages().then(function(rows){
          if (rows && rows.length) return rows;
          return loadSafeByCompetenciaYear(currentSafeYear());
        });
      });
  ...
  function applyFilters() {
    ...
    var trRows = [];
    try { trRows = buildTransferRows(); } catch (eTR) {
      S.lastApiError = finErrText(eTR) || String(eTR);
      trRows = [];
    }
  ...
  function isCardCreditInvoiceDeal(d) {
    // PATCH v48: removida chamada recursiva para isCardPurchaseDetailDeal().
    // Fluxo antigo: isCardPurchaseDetailDeal -> isCardInvoiceDeal -> isCardCreditInvoiceDeal -> isCardPurchaseDetailDeal,
    // causando "Maximum call stack size exceeded" em alguns lançamentos de cartão.
    if (!d) return false;
    ...
    if (obs.indexOf("COMPRA_ISO:") > -1 && obs.toLowerCase().indexOf("cartão:") > -1) return false;
    return true;
  }
  ...
  function openCardModal() {
      ...
      try {
        ...
        for (var ri = 0; ri < Math.min(filtered.length, 400); ri++) {
          ...
          htmlRows.push('<tr>' +
            ...
            '<td><div class="fin-row" style="gap:4px;flex-wrap:nowrap"><button class="fin-mini" data-card-edit="' + esc(d.ID || "") + '">Editar</button><button class="fin-mini fin-card-del-btn" data-card-delete="' + esc(d.ID || "") + '">Excluir</button></div></td>' +
          '</tr>');
        }
    ...
    function openCardLaunchDelete(id) {
      var source = null;
      for (var i = 0; i < (S.deals || []).length; i++) {
        if (String(S.deals[i].ID) === String(id)) { source = S.deals[i]; break; }
      }
      if (!source) { toast("Lançamento não encontrado.", "err"); return; }
      if (String(source.STAGE_ID || "") !== CFG.STAGES.DESP_A_PAGAR) { toast("Só excluo pelo módulo CARTÕES parcelas ainda não pagas.", "err"); return; }

      var meta = cardMeta(source);
      var scope = "one";
      if (meta && meta.total && Number(meta.total) > 1) {
        var msg = "Excluir lançamento do cartão:\n\n" +
          String(source[CFG.F.FAVORECIDO] || source.TITLE || "") + "\n" +
          "Parcela " + String(meta.parcel || 1) + "/" + String(meta.total || 1) + " • Fatura " + toDisplayDate(source[CFG.F.DATA_PREV] || "") + "\n\n" +
          "OK = excluir esta e as próximas parcelas ainda não pagas da mesma compra.\n" +
          "Cancelar = excluir somente esta parcela.";
        scope = window.confirm(msg) ? "future" : "one";
      } else {
        if (!window.confirm("Excluir este lançamento do cartão?")) return;
      }

      var targets = scope === "future" ? findCardRelatedDeals(source, true, meta.parcel || 1, meta.total || 1) : [source];
      targets = (targets || []).filter(function(d){ return d && String(d.STAGE_ID || "") === CFG.STAGES.DESP_A_PAGAR; });
      if (!targets.length) { toast("Nenhuma parcela em aberto para excluir.", "err"); return; }

      var okText = scope === "future" ? ("Confirma excluir " + targets.length + " parcela(s) em aberto desta compra? Parcelas pagas serão mantidas.") : "Confirma excluir esta parcela?";
      ...
      targets.forEach(function(d){
        p = p.then(function(){
          return deleteDeal(d.ID).then(function(){
            deleted++;
            removeDealLocal(d.ID);
            logAudit('Excluir cartão', 'Deal #' + d.ID + ' excluído via módulo CARTÕES');
          });
        });
      });
      p.then(function(){
        toast("Lançamento(s) excluído(s) ✅ (" + deleted + ")");
        try { renderCardLaunches(); } catch (e1) { console.warn('renderCardLaunches pós-exclusão falhou', e1); }
        try { updateCardSummary(); } catch (e2) { console.warn('updateCardSummary pós-exclusão falhou', e2); }
        try { applyFilters(); } catch (e3) { console.warn('applyFilters pós-exclusão falhou', e3); }
      }).catch(function(e){
        toast("Falha ao excluir lançamento: " + ((e && (e.message || String(e))) || "erro"), "err");
        console.error('Falha ao excluir lançamento do cartão', e);
      }).finally(function(){ setModalBusy(m.node, false); });
    }
    ...
    function openCardLaunchEdit(id) {
      ...
      var em = modal(
        ...
        '<div class="fin-modal-body">' +
          ...
          '<div id="ce-status" class="fin-muted" style="font-weight:900;margin-top:10px;min-height:22px">Pronto para salvar.</div>' +
          '<div class="fin-row fin-row--right" style="margin-top:14px;gap:8px">' +
            '<button type="button" class="fin-mini" data-close="1" style="padding:10px 14px;border-radius:12px">Cancelar</button>' +
            '<div id="ce-save" class="fin-card-edit-save-v32" role="button" tabindex="0" aria-label="Salvar edição">Salvar edição</div>' +
          '</div>' +
        '</div>'
      );
      bindDateMasks(em.node);

      var saving = false;
      var saveBtn = em.q('#ce-save');
      ...
      function setEditButtonBusy(busy, label) {
        if (!saveBtn) return;
        saveBtn.setAttribute('aria-busy', busy ? 'true' : 'false');
        saveBtn.setAttribute('data-saving', busy ? '1' : '0');
        saveBtn.textContent = busy ? (label || 'Salvando...') : 'Salvar edição';
        saveBtn.className = 'fin-card-edit-save-v32' + (busy ? ' is-busy' : '');
        saveBtn.style.background = '#2563eb';
        saveBtn.style.color = '#fff';
        saveBtn.style.border = '0';
        saveBtn.style.minWidth = '170px';
        saveBtn.style.minHeight = '38px';
        saveBtn.style.display = 'inline-flex';
        saveBtn.style.alignItems = 'center';
        saveBtn.style.justifyContent = 'center';
        saveBtn.style.visibility = 'visible';
        saveBtn.style.opacity = busy ? '0.75' : '1';
        saveBtn.style.pointerEvents = 'auto';
        saveBtn.style.cursor = busy ? 'wait' : 'pointer';
      }
      function localPatchNoThrow(id, fields) {
        try { mergeDealPatch(id, fields); } catch (e) { console.warn('mergeDealPatch cartão falhou', e); }
      }
      function buildFieldsForTarget(d, idx, data) {
        var parc = data.applyFuture ? (data.parcStart + idx) : data.parcStart;
        if (parc > data.total) parc = data.total;
        var dueDt = data.applyFuture ? (addMonthsISO(data.firstDue, idx, null) || data.firstDue) : (toISODate(d[CFG.F.DATA_PREV] || '') || data.firstDue);
        var comp = guessCompetenciaIdFromISO(dueDt);
        var fields = {};
        fields.TITLE = fullDealTitle('DESPESA', data.fav, 'Cartão ' + parc + '/' + data.total);
        fields[CFG.F.FAVORECIDO] = data.fav;
        fields[CFG.F.VALOR_PREV] = data.val;
        fields[CFG.F.DATA_PREV] = dueDt;
        if (comp) fields[CFG.F.COMPETENCIA] = comp;
        fields[CFG.F.OBS] = 'Cartão: ' + data.cardName + ' • Compra: ' + toDisplayDate(data.baseDate) + ' • Venc. fatura: ' + toDisplayDate(dueDt) + ' • Parcela ' + parc + '/' + data.total + ' • COMPRA_ISO:' + data.baseDate + ' • CARD_SERIE:' + data.series;
        return fields;
      }

      function runSaveCardEdit() {
        if (saving) return;
        try {
          var fav = String((em.q('#ce-fav') && em.q('#ce-fav').value) || '').trim();
          var val = parseMoneyBR((em.q('#ce-valor') && em.q('#ce-valor').value) || '');
          var baseDate = toISODate((em.q('#ce-data') && em.q('#ce-data').value) || '');
          var parcStart = Math.max(1, parseInt((em.q('#ce-parcela') && em.q('#ce-parcela').value) || '1', 10) || 1);
          var total = Math.max(parcStart, parseInt((em.q('#ce-total') && em.q('#ce-total').value) || String(parcStart), 10) || parcStart);
          var applyFuture = !!(em.q('#ce-scope') && em.q('#ce-scope').value === 'future');
          if (!fav) { setEditStatus('Informe a conta/compra.', true); return; }
          if (!baseDate) { setEditStatus('Informe a data da compra.', true); return; }
          if (!isFinite(val) || val < 0) { setEditStatus('Informe um valor válido para a parcela.', true); return; }

          var srcMeta = cardMeta(source);
          var series = srcMeta.series || ('CARD_' + Date.now() + '_' + Math.floor(Math.random()*10000));
          var firstDue = applyFuture ? (toISODate(source[CFG.F.DATA_PREV] || '') || calcCardDueDateISO(baseDate, cardName) || baseDate) : (calcCardDueDateISO(baseDate, cardName) || toISODate(source[CFG.F.DATA_PREV] || '') || baseDate);
          var targets = findCardRelatedDeals(source, applyFuture, parcStart, total) || [source];
          if (!targets.length) targets = [source];

          var data = { fav: fav, val: val, baseDate: baseDate, parcStart: parcStart, total: total, applyFuture: applyFuture, cardName: cardName, series: series, firstDue: firstDue };
          saving = true;
          setEditButtonBusy(true, 'Salvando...');
          setEditStatus('Salvando 0 de ' + targets.length + ' lançamento(s)...', false);

          var p = Promise.resolve();
          targets.forEach(function(d, idx){
            p = p.then(function(){
              var fields = buildFieldsForTarget(d, idx, data);
              setEditStatus('Salvando ' + (idx + 1) + ' de ' + targets.length + ' lançamento(s)...', false);
              return updateDeal(d.ID, fields).then(function(){ localPatchNoThrow(d.ID, fields); });
            });
          });
          ...
          p.then(function(){
            ...
            setEditStatus('Edição salva com sucesso. Atualizando tela...', false);
            toast('Lançamento(s) atualizado(s) ✅ (' + targets.length + ')');
            setTimeout(function(){
              try { em.close(); } catch (closeErr) { console.warn('Falha ao fechar modal de edição', closeErr); }
              setTimeout(function(){
                try { renderCardLaunches(); } catch (e3) { console.warn('renderCardLaunches pós-edição falhou', e3); }
                try { updateCardSummary(); } catch (e2) { console.warn('updateCardSummary pós-edição falhou', e2); }
                try { applyFilters(); } catch (e1) { console.warn('applyFilters pós-edição falhou', e1); }
              }, 50);
            }, 250);
          }).catch(function(err){
            saving = false;
            ...
            setEditStatus('Falha ao salvar: ' + msg, true);
            toast('Falha ao salvar edição do cartão.', 'err');
            console.error('Falha ao salvar edição do cartão', err);
          });
        } catch (e) {
          saving = false;
          setEditButtonBusy(false);
          var msg2 = (e && (e.message || String(e))) || 'erro desconhecido';
          setEditStatus('Falha ao salvar: ' + msg2, true);
          toast('Falha ao salvar edição do cartão.', 'err');
          console.error('Erro síncrono ao salvar edição do cartão', e);
        }
      }

      function ceSaveHandler(ev) {
        var t = ev && ev.target;
        var hit = t && ((t.id === 'ce-save') || (t.closest && t.closest('#ce-save')));
        if (!hit) return;
        if (ev && ev.preventDefault) ev.preventDefault();
        if (ev && ev.stopPropagation) ev.stopPropagation();
        setEditStatus('Comando recebido. Preparando salvamento...', false);
        runSaveCardEdit();
        return false;
      }
      if (saveBtn) {
        saveBtn.onclick = ceSaveHandler;
        saveBtn.onmousedown = function(ev){ if (ev && ev.stopPropagation) ev.stopPropagation(); };
        saveBtn.onkeydown = function(ev){ if (ev && (ev.key === 'Enter' || ev.key === ' ')) return ceSaveHandler(ev); };
      }
      if (em && em.node) {
        em.node.addEventListener('click', ceSaveHandler, true);
        em.node.addEventListener('touchend', ceSaveHandler, true);
      }
    }
    ...
    if (m.q("#card-launches-tb")) m.q("#card-launches-tb").addEventListener("click", function(e){
      var delBtn = safeClosest(e.target, "[data-card-delete]");
      if (delBtn) {
        if (e && e.preventDefault) e.preventDefault();
        openCardLaunchDelete(delBtn.getAttribute("data-card-delete"));
        return;
      }
  ...
  // PATCH v42 — fallback bruto: se applyFilters/renderização normal estourar stack,
  // o sistema não pode substituir a tela por erro. Mostra os lançamentos carregados
  // diretamente no tbody, sem depender de filtros, gráficos, sidebar ou enum helpers complexos.
  function renderRawDealsFallback(rows, msg) {
    try {
      rows = (rows || []).slice(0, CFG.PAGE_SIZE || 50);
      S.filtered = rows.slice();
      S.page = 1;
      var tb = el("#fin-tbody");
      if (!tb) return false;
      if (!rows.length) {
        tb.innerHTML = '<tr><td colspan="12" class="fin-muted">Nenhum lançamento bruto carregado. ' + esc(msg || '') + '</td></tr>';
        return true;
      }
      var html = [];
      for (var i = 0; i < rows.length; i++) {
        ...
        html.push('<tr>' +
          '<td></td>' +
          '<td class="fin-mono">#' + esc(d.ID || '') + '</td>' +
          '<td><div class="fin-strong">' + esc(fav) + '</div><div class="fin-small fin-muted">fallback bruto v49</div></td>' +
          '<td>' + esc(d[CFG.F.CENTRO_CUSTO] || '') + '</td>' +
          '<td>' + esc(d[CFG.F.TIPO_FIN] || '') + '</td>' +
          '<td>' + esc(d[CFG.F.COMPETENCIA] || '') + '</td>' +
          '<td class="fin-mono">' + esc(toDisplayDate(d[CFG.F.DATA_PREV] || '')) + '</td>' +
          ...
          '<td>' + esc(d.STAGE_ID || '') + '</td>' +
          '<td><span class="fin-muted">modo seguro</span></td>' +
        '</tr>');
      }
      tb.innerHTML = html.join('');
      var count = el('#tot-count'); if (count) count.textContent = String((S.deals || []).length || rows.length);
      var prev = el('#tot-prev'); if (prev) prev.textContent = '—';
      var real = el('#tot-real'); if (real) real.textContent = '—';
      var pager = el('#fin-pager-info'); if (pager) pager.textContent = '1/1';
      var range = el('#fin-range-info'); if (range) range.textContent = 'Exibindo 1–' + rows.length + ' de ' + ((S.deals || []).length || rows.length);
      S.lastApiError = msg || S.lastApiError || 'Fallback bruto v42 ativado';
      try { toast('Modo seguro v48 ativado: lançamentos carregados preservados.', 'err'); } catch(_) {}
      return true;
    } catch (e) {
      try { console.error('[Financeiro][fallback bruto v49] falhou', e); } catch(_) {}
      return false;
    }
  }
  ...
  function openDiagnosticsModal() {
    ...
    Promise.allSettled([
      ...
      host.innerHTML = '<table class="fin-table" style="min-width:0"><tbody>' +
        ...
        line('Versão JS', 'v48 correção recursão cartões + render normal', true) +
        line('Bitrix API mode', S.apiMode || 'Ainda não testado', !!S.apiMode) +
        line('Último erro API', S.lastApiError || '—', !S.lastApiError) +
        line('Qtd. lançamentos carregados', String((S.deals || []).length), (S.deals || []).length > 0) +
  ...
  function refresh() {
    ...
    return listDealsAll()
      .then(function (deals) {
        ...
        try {
          applyFilters();
          if ((S.deals || []).length && !(S.filtered || []).length) {
            renderRawDealsFallback(S.deals, "Filtragem normal zerou a lista; fallback bruto v49 ativado.");
          }
        } catch (eApply) {
          S.lastApiError = finErrText(eApply) || String(eApply);
          renderRawDealsFallback(S.deals, "Renderização normal falhou: " + (eApply && eApply.message ? eApply.message : String(eApply)));
        }
        try { scheduleFinanceReminderSync(); } catch(eSync) { try { console.warn('sync lembretes ignorado', eSync); } catch(_) {} }
      })
      .catch(function (e) {
        if (token !== S.refreshToken) return;
        var msg = e && e.message ? e.message : String(e);
        if (/Maximum call stack size exceeded/i.test(msg)) {
          msg = "Maximum call stack evitado: mantendo lançamentos já carregados em modo bruto v49.";
        }
        S.lastApiError = msg;
        if ((S.deals || []).length && renderRawDealsFallback(S.deals, msg)) return;
        toast("Falha ao carregar: " + msg, "err");
        var tb = el("#fin-tbody");
        if (tb) tb.innerHTML = '<tr><td colspan="12" class="fin-muted">Erro: ' + esc(msg) + '<br><b>Ação:</b> abrir DIAGNÓSTICO e copiar o campo Último erro API. O JS foi mantido estável; se persistir, a correção deve ser no Worker financeiro.</td></tr>';
      })
      .finally(function () {
  ...
  // =========================
  function boot() {
    finSafeSetBodyClassV43();
    setLoading(true);
    return loadInstitutionalState()
      .catch(function(eState){ S.lastApiError = finErrText(eState) || String(eState); return null; })
      .then(function(){ return loadMeta(); })
      .then(function () {
        try { render(); } catch (eRender) {
          S.lastApiError = finErrText(eRender) || String(eRender);
          renderMinimalFinanceShellV43("Render principal falhou; modo seguro v45 ativado.");
        }
        try { updateStorageBadge(); } catch (_) {}
        return loadPartners().catch(function(ePartners){ S.partners = []; S.lastApiError = finErrText(ePartners) || String(ePartners); });
      })
      .then(function () {
        try { renderPartnersAvatars(); } catch (_) {}
        return refresh();
      })
      .catch(function (e) {
        try { console.error("Financeiro boot error:", e); } catch(_) {}
        if (finIsBitrixExternalError(e)) {
          finShowPatchWarn("Erro externo do Bitrix ignorado durante o boot.");
          return;
        }
        var bootMsg = e && e.message ? e.message : String(e);
        S.lastApiError = bootMsg;
        if (!document.getElementById('fin-tbody')) renderMinimalFinanceShellV43("Boot interrompido; modo seguro v45 preservou a tela.");
        if ((S.deals || []).length && renderRawDealsFallback(S.deals, bootMsg)) return;
        var tb = document.getElementById('fin-tbody');
        if (tb) tb.innerHTML = '<tr><td colspan="11" class="fin-muted">Modo seguro v43 preservou o painel. Erro: ' + esc(bootMsg) + '</td></tr>';
        try { toast("Modo seguro v43: " + bootMsg, "err"); } catch (_) {}
      })
      .finally(function () {
        FIN_PATCH_BOOT_DONE = true;
        setLoading(false);
      });
  }
