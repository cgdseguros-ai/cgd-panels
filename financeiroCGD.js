...
(function () {
  ...
  var CFG = {
    ...
    REMINDER_UF_URGENCIA: "",
    REMINDER_UF_URGENCIA_VALOR: "URGENTE",
    REMINDER_UF_TIPO_TAREFA: "",
    ...
    // Centro virtual local: não depende de UF/enum no Bitrix.
    // Usado para reservar valores que não podem sair da conta.
  ...
  function centersWithReserve() {
    var arr = (S.enums && S.enums[CFG.F.CENTRO_CUSTO]) ? S.enums[CFG.F.CENTRO_CUSTO].slice() : [];
    var exists = false;
    for (var i = 0; i < arr.length; i++) if (String(arr[i].ID) === CFG.RESERVE_CENTER_ID) exists = true;
    if (!exists) arr.push({ ID: CFG.RESERVE_CENTER_ID, VALUE: CFG.RESERVE_CENTER_NAME });
    return arr;
  }
  ...
  function computeCCBalance(centroId) {
    ...
    // RESERVA é virtual/local: saldo manual + transferências recebidas/enviadas.
    // Não depende de lançamento de receita/despesa no Bitrix.
    if (centroId === CFG.RESERVE_CENTER_ID) {
      initial += Number(S.reserve && S.reserve.balance || 0) || 0;
    }
  ...
  function renderTotals() {
    ...
    if (el("#reserve-balance")) el("#reserve-balance").textContent = moneyBR(computeCCBalance(CFG.RESERVE_CENTER_ID));
      ...
      var html =
        '<div class="fin-row" style="gap:8px;flex-wrap:wrap;margin-bottom:10px;justify-content:space-between">' +
          ...
          '<div class="fin-row" style="gap:8px;flex-wrap:wrap">' +
            '<button class="fin-btn" id="b-import-rec">IMPORTAR CSV</button>' + (isRec ? '<button class="fin-btn" id="b-export-rec">EXPORTAR CSV</button>' : '') +
          '</div>' +
        '</div>' +
        ...
        ops = ops.then(function(){
          ...
          for (var k = 0; k < count; k++) (function(idx){
            p = p.then(function(){
              ...
              return createDeal(fields).then(function(newId){ created++; addDealLocal(fields, newId); logAudit('Criar lote', 'Deal #' + newId + ' criado via lote'); });
            });
          })(k);
          return p;
        });
    ...
    var m = modal(
      ...
      '<div class="fin-modal-body">' +
        ...
        '<div class="fin-row" style="justify-content:space-between;margin-top:10px;flex-wrap:wrap">' +
          '<div class="fin-row" style="gap:8px;flex-wrap:wrap"><button class="fin-btn" id="cc-add">+ Linha</button><button class="fin-btn" id="cc-import">IMPORTAR CSV</button></div>' +
      ...
      if (isTransferRow(d)) {
        ...
        rows.push(
          "<tr>" +
            ...
            '<td><span class="fin-muted">Lançamento local</span></td>' +
          '</tr>'
        );
        continue;
      }
