...
(function () {
  ...
  function buildDealUpdateDelta(currentDeal, fields) {
    const src = currentDeal || {};
    const wanted = fields || {};
    const delta = {};
    for (const [k, v] of Object.entries(wanted)) {
      if (v === undefined) continue;
      if (normalizeCompareValue(src[k]) !== normalizeCompareValue(v)) delta[k] = v;
    }
    return delta;
  }
  ...
  async function safeDealUpdate(dealId, fields) {
    ...
    try {
      return await bx("crm.deal.update", { id, fields: requested });
    } catch (e) {
      ...
      const getFresh = async () => {
        try {
          return await bx("crm.deal.get", { id });
        } catch (_) {
          return null;
        }
      };
      const verify = async () => {
        ...
        return fresh ? dealFieldsMatchRequested(fresh, requested) : false;
      };

      let fresh = await getFresh();
      if (fresh && dealFieldsMatchRequested(fresh, requested)) return true;

      await new Promise((r) => setTimeout(r, 180));
      fresh = await getFresh();
      if (fresh && dealFieldsMatchRequested(fresh, requested)) return true;

      let delta = buildDealUpdateDelta(fresh, requested);
      if (!Object.keys(delta).length) return true;

      try {
        return await bx("crm.deal.update", { id, fields: delta });
      } catch (_) {}
      ...
      delta = buildDealUpdateDelta(fresh, requested);
      if (!Object.keys(delta).length) return true;

      if (delta.STAGE_ID !== undefined && delta.ASSIGNED_BY_ID !== undefined) {
        try {
          const { STAGE_ID, ASSIGNED_BY_ID, ...rest } = delta;
          if (Object.keys(rest).length) await bx("crm.deal.update", { id, fields: rest });
          await bx("crm.deal.update", { id, fields: { ASSIGNED_BY_ID } });
          await bx("crm.deal.update", { id, fields: { STAGE_ID } });
          return true;
        } catch (_) {}
      }
...
${marker}` : marker;
  ...
  async function transferRecurringInstancesForRule(fromUid, toUid, ruleId) {
    ...
    for (const d of targets) {
      await safeDealUpdate(String(d.ID), { ASSIGNED_BY_ID: Number(toUid), STAGE_ID: String(stageId) }).catch(() => null);
