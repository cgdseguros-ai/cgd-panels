...
(function () {
  ...
  const BUSINESS_RULES = Object.freeze({
    segurosUsers: SEGUROS_USERS,
    looseFollowupUsers: OMEGA_FOLLOWUP_USERS,
    leadUsers: LEAD_USERS,
    proposalCategoryId: CATEGORY_PROPOSALS,
    mainCategoryId: CATEGORY_MAIN,
    refresh: Object.freeze({
      fullDealsMaxAgeMs: 20 * 60 * 1000,
      incrementalAfterMutationMs: 900,
      fallbackFullRefreshMs: 4500
    })
  });
  ...
  const DEAL_MUTATION_HOLD_UNTIL = new Map();
  ...
  const EQD_LOG_BUFFER = [];
  function pushLogEntry(level, scope, err, extra) {
    try {
      EQD_LOG_BUFFER.push({
      ...
      if (EQD_LOG_BUFFER.length > 80) EQD_LOG_BUFFER.splice(0, EQD_LOG_BUFFER.length - 80);
    } catch (_) {}
  }
  function logSoftError(scope, err, extra) {
    try { pushLogEntry('soft', scope, err, extra); console.warn('[EQD]', scope, err, extra || ''); } catch (_) {}
  }
  function logHardError(scope, err, extra) {
    try { pushLogEntry('hard', scope, err, extra); console.error('[EQD]', scope, err, extra || ''); } catch (_) {}
  }
  window.__EQD_LOG_BUFFER = EQD_LOG_BUFFER;
  ...
  async function safeDealUpdate(dealId, fields) {
    ...
    const cleanFields = sanitizeDealFields(fields);
    try {
      return await bx("crm.deal.update", { id, fields: cleanFields });
    } catch (e) {
      ...
      const verify = async () => {
        try {
          ...
          return dealFieldsMatchRequested(fresh, cleanFields);
        } catch (_) {
          return false;
        }
      };
  ...
  function holdDealMutation(dealId, ms = 15000) {
    const id = String(dealId || '').trim();
    if (!id) return 0;
    const until = Date.now() + Math.max(1000, Number(ms) || 0);
    DEAL_MUTATION_HOLD_UNTIL.set(id, until);
    return until;
  }
  function isDealMutationHeld(dealId) {
    const id = String(dealId || '').trim();
    if (!id) return false;
    const until = Number(DEAL_MUTATION_HOLD_UNTIL.get(id) || 0);
    if (!until) return false;
    if (Date.now() >= until) { DEAL_MUTATION_HOLD_UNTIL.delete(id); return false; }
    return true;
  }
  function hasPendingMutationHolds() {
    for (const [id, until] of DEAL_MUTATION_HOLD_UNTIL.entries()) {
      if (Date.now() < Number(until || 0)) return true;
      DEAL_MUTATION_HOLD_UNTIL.delete(id);
    }
    for (const [id, until] of LEAD_MUTATION_HOLD_UNTIL.entries()) {
      if (Date.now() < Number(until || 0)) return true;
      LEAD_MUTATION_HOLD_UNTIL.delete(id);
    }
    return false;
  }
  ...
  async function loadDeals(opts = {}) {
    ...
    if (openOnly) {
      ...
      const fetched = (deals || []).map((d) => parseDeal(d, maps)).filter(Boolean);
      const byId = new Map(fetched.map((d) => [String(d.ID), d]));
      (STATE.dealsAll || []).forEach((localDeal) => {
        const lid = String(localDeal && localDeal.ID || '');
        if (!lid || !isDealMutationHeld(lid)) return;
        byId.set(lid, { ...localDeal });
      });
      STATE.dealsAll = Array.from(byId.values()).sort((a,b) => Number(b && b.ID || 0) - Number(a && a.ID || 0));
      STATE.dealsLoadedMode = "full";
      ...
      if (Array.isArray(deals) && deals.length) {
        ...
        for (const raw of deals) {
          ...
          if (isDealMutationHeld(id)) continue;
          const i = byId.get(id);
  ...
  function recurrenceUserIdFromDeal(d){
    ...
    const parsed = safeJsonParse(rawObs);
    const fromObs = String(parsed && (parsed.userId || parsed.uid || "") || "").trim();
  ...
  async function ensureConfigDealForUser(userId) {
    ...
    const cached = String(STATE.recurConfigDealIdByUser.get(uid) || '').trim();
    if (cached) return cached;
    try {
      const existingRules = await loadRecurrenceConfigForUser(uid);
      const existingId = String(STATE.recurConfigDealIdByUser.get(uid) || '').trim();
      if (existingId) {
        if (!STATE.recurRulesByUser.has(uid)) STATE.recurRulesByUser.set(uid, Array.isArray(existingRules) ? existingRules : []);
        return existingId;
      }
    } catch (_) {}
  ...
  async function deleteFutureRecurringInstancesForRule(userId, ruleId) {
    ...
    const ids = new Set();
    const collectFrom = (rows) => {
      (rows || []).forEach((d) => {
        if (String(d.ASSIGNED_BY_ID || d._assigned || '') !== uid) return;
        const obs = String(d[UF_OBS] || d._obs || '');
        const m = obs.match(/\[RUID=([^:\]]+):(\d{4}-\d{2}-\d{2})\]/);
        if (!m) return;
        if (String(m[1]) !== rid || String(m[2]) < todayKey) return;
        const id = String(d.ID || '').trim();
        if (id) ids.add(id);
      });
    };
    collectFrom(STATE.dealsAll || []);
    try {
      const remote = await bxAll('crm.deal.list', {
        filter: { CATEGORY_ID: CATEGORY_MAIN, ASSIGNED_BY_ID: Number(uid) },
        select: ['ID','ASSIGNED_BY_ID', UF_OBS],
        order: { ID: 'DESC' },
      }).catch(() => []);
      collectFrom(remote || []);
    } catch (_) {}
...
${curObs}` : transferAlert;
  ...
  function captureModalSnapshot() {
    return {
      ...
      bodyNodes: Array.from(el.modalBody.childNodes || []),
      wide: el.modalEl.classList.contains("wide"),
      full: el.modalEl.classList.contains("full")
    };
  }
  function restoreModalSnapshot(prev) {
    el.modalTitle.textContent = prev && prev.title ? prev.title : "—";
    el.modalBody.onclick = null;
    el.modalBody.innerHTML = "";
    if (prev && Array.isArray(prev.bodyNodes) && prev.bodyNodes.length) {
      prev.bodyNodes.forEach((node) => { try { el.modalBody.appendChild(node); } catch (err) { logSoftError('modal.restore.node', err); } });
    } else {
      el.modalBody.innerHTML = (prev && prev.bodyHTML) ? prev.bodyHTML : "";
    }
    el.modalOverlay.style.display = "flex";
    el.modalOverlay.setAttribute("aria-hidden", "false");
    el.modalEl.classList.remove("wide");
    el.modalEl.classList.remove("full");
    if (prev && prev.wide) el.modalEl.classList.add("wide");
    if (prev && prev.full) el.modalEl.classList.add("full");
  }
  function openModal(title, bodyHTML, opts) {
    ...
    if (el.modalOverlay.style.display === "flex" && allowStack && !sameCurrent) {
      MODAL_STACK.push(captureModalSnapshot());
    }
...
restoreSyncQueue();
  ...
  function scheduleSyncFlush(delayMs = 450) {
    ...
    SYNC_FLUSH_TIMER = setTimeout(() => { flushSyncQueue().catch((err) => { logSoftError('sync.flush.schedule', err); }); }, delayMs);
  }

  function buildSyncDedupeKey(job) {
    if (!job || !job.type) return '';
    if (job.type === 'dealUpdate') return `dealUpdate:${String(job.dealId || '')}`;
    if (job.type === 'leadUpdate') return `leadUpdate:${String(job.leadId || '')}`;
    if (job.type === 'dealDelete') return `dealDelete:${String(job.dealId || '')}`;
    if (job.type === 'leadDelete') return `leadDelete:${String(job.leadId || '')}`;
    if (job.type === 'dealAdd') return `dealAdd:${String(job.tempId || '')}`;
    return '';
  }
  function normalizeSyncJob(job) {
    if (!job) return null;
    const out = { ...job, _queuedAt: job._queuedAt || Date.now() };
    if (out.fields && typeof out.fields === 'object') out.fields = { ...out.fields };
    out._dedupeKey = buildSyncDedupeKey(out);
    return out;
  }
  function tryMergeSyncJob(existing, incoming) {
    if (!existing || !incoming) return false;
    if (existing.type === 'dealUpdate' && incoming.type === 'dealUpdate' && String(existing.dealId || '') === String(incoming.dealId || '')) {
      existing.fields = { ...(existing.fields || {}), ...(incoming.fields || {}) };
      existing._queuedAt = incoming._queuedAt || existing._queuedAt;
      return true;
    }
    if (existing.type === 'leadUpdate' && incoming.type === 'leadUpdate' && String(existing.leadId || '') === String(incoming.leadId || '')) {
      existing.fields = { ...(existing.fields || {}), ...(incoming.fields || {}) };
      existing._queuedAt = incoming._queuedAt || existing._queuedAt;
      return true;
    }
  ...
  function enqueueSync(job, opts = {}) {
    ...
    for (let i = SYNC_QUEUE.length - 1; i >= 0; i--) {
      if (tryMergeSyncJob(SYNC_QUEUE[i], normalized)) { merged = true; break; }
    }
    if (!merged) SYNC_QUEUE.push(normalized);
    persistSyncQueue();
    const delay = opts && typeof opts.delayMs === 'number' ? opts.delayMs : (normalized && (normalized.type === "dealDelete" || normalized.type === "leadDelete") ? 80 : 450);
  ...
  async function flushSyncQueue() {
    ...
    try {
      ...
      logSoftError('sync.flush', err, { queueSize: SYNC_QUEUE.length });
      persistSyncQueue();
  ...
  async function deleteFutureFollowupsForLeadOrig(origId, ignoreDealId) {
    const needle = String(origId || "").trim();
    if (!needle) return 0;
    const todayMs = dayStart(new Date()).getTime();
    const localRows = (STATE.dealsOpen || []).filter((d) => isFollowupDeal(d) && String(d.ID || "") !== String(ignoreDealId || "") && getDealLeadOrigemId(d) === needle && d._prazo && new Date(d._prazo).getTime() >= todayMs);
    const ids = new Set(localRows.map((d) => String(d.ID)).filter(Boolean));
    try {
      const remoteRows = await bxAll('crm.deal.list', {
        filter: { CATEGORY_ID: CATEGORY_MAIN, [DEAL_UF_LEAD_ORIGEM]: needle },
        select: ['ID','TITLE','STAGE_ID','ASSIGNED_BY_ID', UF_TAREFA, UF_PRAZO, UF_OBS, DEAL_UF_LEAD_ORIGEM],
        order: { ID: 'DESC' },
      }).catch(() => []);
      (remoteRows || []).forEach((raw) => {
        const parsed = parseDeal(raw, {
          urgMap: STATE.enumCache.get(UF_URGENCIA) || {},
        ...
        if (!parsed || !id || id === String(ignoreDealId || '')) return;
        if (!isFollowupDeal(parsed)) return;
        if (!parsed._prazo || new Date(parsed._prazo).getTime() < todayMs) return;
        ids.add(id);
      });
    } catch (_) {}
    ids.forEach((id) => { removeDealLocal(String(id)); enqueueSync({ type: "dealDelete", dealId: String(id) }); });
    rebuildDealsOpen();
    saveCache();
    return ids.size;
  }
...
function makeUserCard(u) {
  ...
  function shouldForceFullDealsRefresh(forceNetwork) {
    if (!forceNetwork) return false;
    const lastFull = Number(STATE.lastFullDealsSyncAt || 0);
    if (!lastFull) return true;
    if (STATE.dealsLoadedMode !== 'full') return true;
    return (Date.now() - lastFull) >= BUSINESS_RULES.refresh.fullDealsMaxAgeMs;
  }
  function scheduleDataRefresh(forceNetwork, opts = {}, delayMs) {
    const delay = typeof delayMs === 'number' ? delayMs : BUSINESS_RULES.refresh.incrementalAfterMutationMs;
    setTimeout(() => {
      refreshData(!!forceNetwork, opts).catch((err) => { logSoftError('refresh.scheduled', err, { forceNetwork: !!forceNetwork, opts }); });
    }, delay);
  }
  function scheduleFallbackFullRefresh(delayMs) {
    scheduleDataRefresh(true, { deferLeads: false }, typeof delayMs === 'number' ? delayMs : BUSINESS_RULES.refresh.fallbackFullRefreshMs);
  }
  ...
  async function refreshData(forceNetwork, opts = {}) {
    ...
    try {
      ...
      renderFooterPeople();
      DEALS_FAIL_STREAK = 0;
      setBootProgress(100, "Carregando...");
      clearBootProgress();
      el.meta.textContent = `Atualizado: ${fmt(new Date())}`;
      STATE.offline = false;
    } catch (err) {
      logSoftError('refresh.data', err, { forceNetwork: !!forceNetwork, opts });
      DEALS_FAIL_STREAK++;
  ...
  async function applyDealMutation(dealId, fields, meta = {}) {
    const id = String(dealId || '');
    const patch = sanitizeDealFields({ ...(fields || {}) });
    if (!id || !Object.keys(patch).length) return false;
    holdDealMutation(id, 15000);
    UI_REFRESH_HOLD_UNTIL = Math.max(UI_REFRESH_HOLD_UNTIL, Date.now() + 5000);
    const localPatch = { ...patch, DATE_MODIFY: new Date().toISOString() };
    updateDealInState(id, localPatch);
    rebuildDealsOpen();
    saveCache();
    try {
      await safeDealUpdate(id, patch);
      return true;
    } catch (err) {
      logSoftError('deal.mutation', err, { dealId: id, patch, meta });
      throw err;
    }
  }
  ...
  async function markDone(dealId) {
    ...
    await applyDealMutation(String(dealId), { STAGE_ID: String(STATE.doneStageId) }, { reason: 'markDone' });
  }
    ...
    btn.onclick = async () => {
      try {
        ...
        if (doneMode === 'NAO_FEITO') {
          ...
          await applyDealMutation(String(dealId), { [UF_PRAZO]: prazoIso }, { reason: 'reschedule:not_done' });
          ...
          scheduleDataRefresh(false, { deferLeads:false }, 180);
          return;
        }
        ...
        try {
          ...
          await applyDealMutation(String(dealId), { STAGE_ID: String(STATE.doneStageId) }, { reason: 'reschedule:done_close_current' });
          const created = await bx("crm.deal.add", { fields });
          ...
          scheduleFallbackFullRefresh(180);
          throw netErr;
        }
        scheduleDataRefresh(false, { deferLeads:false }, 260);
      } catch (e) {
        warn.style.display = "block";
        warn.textContent = "Falha:\n" + (e.message || e);
  ...
  async function convertLinkedLeadFromFollowup(dealId) {
    ...
    await deleteFutureFollowupsForLeadOrig(origemForDelete, String(dealId));
    ...
    await moveLeadStage(uid, leadId, sConv, { skipConvertConfirm: true });
    await markDone(dealId);
    renderCurrentView();
  }
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  function proposalFieldInputHtml(fd, deal) {
    ...
    if (fd.code === 'OPPORTUNITY') return `<div><div style="font-size:11px;font-weight:900;margin-bottom:4px">${escHtml(label)}</div><input data-propfield="${escHtml(fd.code)}" type="number" step="0.01" value="${escHtml(String((Math.round(proposalAmountNumber({ OPPORTUNITY: value }) * 100) / 100).toFixed(2) || ''))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>`;
    if (fd.kind === 'datetime') {
  ...
  function proposalValueForRequirement(code, patch, baseDeal) {
    const raw = patch && Object.prototype.hasOwnProperty.call(patch, code) ? patch[code] : (baseDeal ? baseDeal[code] : '');
    if (code === 'OPPORTUNITY') return proposalAmountNumber({ OPPORTUNITY: raw });
    return raw;
  }
  ...
  function validateProposalRequiredFields(nextStageId, patch, baseDeal) {
    ...
    const missing = reqs.filter(([_, code]) => {
      ...
      if (code === 'OPPORTUNITY') return !(Number(v || 0) > 0);
...
document.addEventListener("click", async (e) => {
  ...
  setInterval(async () => {
    try{
      ...
      if (BX_INFLIGHT > 0 || SYNC_QUEUE_RUNNING || (Array.isArray(SYNC_QUEUE) && SYNC_QUEUE.length)) return;
      if (hasPendingMutationHolds() || Date.now() < UI_REFRESH_HOLD_UNTIL || Date.now() < PROPOSALS_REFRESH_HOLD_UNTIL) return;
