...
(function () {
    ...
    <header>
      <div class="brand">
        ...
        <div class="titles">
          ...
          <div class="sub" id="systemVersion">Versão: V6.1.1 • Última versão criada em 04/06/2026 às 09:30</div>
        </div>
      </div>
      ...
      <div class="right">
        ...
        <div class="rightRow row2">
          ...
          <button class="btn btnAdm" id="admUser1Prod">USER 1 (ADM)</button>
          <button class="btn" id="admAnalise">ANÁLISE</button>
          <select class="sel" id="compFilter" title="Filtrar cards por competência">
          ...
          <div class="hideWrap">
            ...
            <div class="hidePanel" id="hidePanel">
              <div class="hpHead">
                ...
                <button class="btn" id="hpClose" style="padding:4px 8px;font-size:12px;min-height:auto;">✕</button>
              </div>
  ...
  /* ================= CONFIG ================= */
  const PAINEL_CGD_VERSAO = "V6.1.5";
  const PAINEL_CGD_ULTIMA_VERSAO = "27/08/2026";
  const $systemVersion = document.getElementById("systemVersion");
  if ($systemVersion) $systemVersion.textContent = `Versão: ${PAINEL_CGD_VERSAO} • Última versão criada em ${PAINEL_CGD_ULTIMA_VERSAO}`;
  console.log("Painel CGD carregado:", PAINEL_CGD_VERSAO, PAINEL_CGD_ULTIMA_VERSAO);
  ...
  const OPERATOR_FIELD = "UF_CRM_1771388342";
  ...
  const ADMIN_VIEW_PASS = "8123";
  const BLOCKED_USER_IDS = new Set([843, 269, 3389, 3387, 3085]);
  const BLOCKED_USER_NAMES = new Set([
    "NICOLLE BELMONTE",
    "ANNA CLARA GUEDES",
    "BEATRIZ LEAL"
  ]);
  function isBlockedSellerRef(s){
    const uid = Number(s?.userId || s?.ID || 0);
    const label = normalizeText(s?.label || s?.matchFullName || `${s?.NAME || ""} ${s?.LAST_NAME || ""}`);
    if (uid && BLOCKED_USER_IDS.has(uid)) return true;
    if (label && BLOCKED_USER_NAMES.has(label)) return true;
    return false;
  }
  ...
  const MAIN_SELLERS = [
    ...
    { label: "BRUNA", userId: 3081 },
    { label: "FERNANDA", userId: 3083 },
    { label: "GABRIEL", userId: 815 },
    { label: "GILDA", userId: 4945 },
    { label: "JOSIANE", userId: 811 },
    { label: "LIVIA", userId: 3079 },
    { label: "MANUELA", userId: 813 },
    { label: "MARCELA", userId: 5265 },
    { label: "MARIA CLARA", userId: 841 },
    { label: "MARIANA", userId: 23 },
    { label: "MARIANA PLASA", userId: 6353 },
    { label: "NICOLE", userId: 4741 },
  ];
  ...
  function isBlockedSeller(s){
    const uid = Number(s?.userId || 0);
    if (uid && BLOCKED_USER_IDS.has(uid)) return true;

    const labelN = normalizeText(s?.label || "");
    const fullN = normalizeText(s?.matchFullName || "");
    for (const blockedName of BLOCKED_USER_NAMES){
      const bn = normalizeText(blockedName);
      if (labelN === bn || fullN === bn) return true;
    }
    return false;
  }
  ...
  function checkAdminPass(input){
    ...
    if (p === normalizePass(ADMIN_PASS)) return { ok:true, canEdit:true, readOnly:false, isAdmin:true };
    if (p === normalizePass(ADMIN_VIEW_PASS)) return { ok:true, canEdit:false, readOnly:true, isAdmin:true };
    return { ok:false, canEdit:false, readOnly:false, isAdmin:false };
  }
  function promptAdminAccess(label="Senha ADM:"){
    const pass = prompt(label);
    if (pass == null) return { ok:false, cancelled:true, canEdit:false, readOnly:false, isAdmin:false };
  ...
  function canonicalLeadLabel(v){
    const raw = String(v ?? "").trim();
    if (!raw || raw === "—") return "—";
    const key = normalizeText(raw).replace(/\s+/g, " ").trim();
    const map = {
      "INDICACAO": "INDICAÇÃO",
      "INDICACAO FAMILIAR": "INDICAÇÃO FAMILIAR",
    ...
    return map[key] || key || raw;
  }

  function compareTextBR(a, b){
  ...
  async function loadSources(){
    return loadDealFieldMap(SOURCE_FIELD);
  }
  ...
  async function ensureUsersAndResolve(){
    ...
    const resolveArray = (arr) => {
      for (const s of arr){
        ...
        if (!s.userId && s.matchFullName){
          ...
          if (found && !isBlockedSellerRef(found)) s.userId = Number(found.ID);
        }
    ...
    for (const arr of [MAIN_SELLERS, OTHER_SELLERS, SEARCH_ONLY_USERS]){
      for (let i = arr.length - 1; i >= 0; i--){
        if (isBlockedSellerRef(arr[i])) arr.splice(i, 1);
      }
    }
  }
  ...
  async function fetchDeals({userId, categoryId, extraFilter={}, select=[]}){
    ...
    while (true){
      const data = await bxRaw("crm.deal.list", {
        ...
        select: ["ID","TITLE","STAGE_ID",OPERATOR_FIELD,SOURCE_FIELD,"OPPORTUNITY","CURRENCY_ID", CLOSEDATE_FIELD, BONIF_FIELD, ...select],
        start
      });
  ...
  async function loadConfigIfExists(yyyymm){
    const ym = normYYYYMM(yyyymm) || nowYYYYMM();
    if (cfgCacheByYM[ym]) return cfgCacheByYM[ym];
    ...
    return { dealId:null, sorteioMeta:0, metasObj:{}, estornosObj:{} };
  }
  ...
  function currentPanelCompetence(){
    return selectedCompetence === "current" ? nowYYYYMM() : (normYYYYMM(selectedCompetence) || nowYYYYMM());
  }

  function getStatsForSellerCompetence(sellerId, yyyymm){
    const ym = normYYYYMM(yyyymm) || nowYYYYMM();
    const base = statsBySellerId[sellerId] || { andamentoQtd:0, andamentoValor:0, docsQtd:0, pagQtd:0, metaMes:0, metaPct:0, produzidoMes:0 };
    const list = (closedWonBySellerId[sellerId] || []).filter(x => normYYYYMM(x.competence) === ym);
    const produzido = list.reduce((sum, it)=> sum + (Number(it.opportunity)||0), 0);
    const meta = getMeta(ym, sellerId);
    const pct = meta > 0 ? (produzido / meta) * 100 : 0;
    return {
      andamentoQtd: base.andamentoQtd || 0,
      andamentoValor: base.andamentoValor || 0,
      docsQtd: base.docsQtd || 0,
      pagQtd: base.pagQtd || 0,
      metaMes: meta,
      metaPct: pct,
      produzidoMes: produzido
    };
  }

  function buildPanelCompetenceOptions(){
    const set = new Set();
    ...
    for (let i=-12;i<=2;i++) set.add(addMonthsYYYYMM(base, i));
    return Array.from(set).map(normYYYYMM).filter(Boolean).sort().reverse();
  }

  function updateCompFilterOptions(){
    if (!$compFilter) return;
    const cur = selectedCompetence || "current";
    const opts = [`<option value="current" ${cur==="current"?"selected":""}>Mês atual (${nowYYYYMM()})</option>`]
      .concat(buildPanelCompetenceOptions().map(c => `<option value="${c}" ${cur===c?"selected":""}>${c}</option>`));
    $compFilter.innerHTML = opts.join("");
  }

  function mapDealToItem(d, sellerLabel, sellerId){
    const stageName = stageMap[d.STAGE_ID]||"";
    const rawOperator = d[OPERATOR_FIELD] || "";
    const opName = (operatorMap && rawOperator && operatorMap[rawOperator]) ? operatorMap[rawOperator] : (rawOperator || "—");
    const opportunity = parseBRNumber(d.OPPORTUNITY);
    const rawSource = d[SOURCE_FIELD] || "";
    const srcNameRaw = (sourceMap && rawSource && sourceMap[rawSource]) ? sourceMap[rawSource] : (rawSource || "—");
    const srcName = canonicalLeadLabel(srcNameRaw);
  ...
  function getAllSellersForCalc(){
    const all = [...MAIN_SELLERS, ...OTHER_SELLERS, ...SEARCH_ONLY_USERS].filter(s=>s.userId && !isBlockedSeller(s));
    const map = new Map();
    for (const s of all){
      if (isBlockedSeller(s)) continue;
      map.set(Number(s.userId), s);
    }
    return Array.from(map.values());
  }
  function getAllVisibleUsersForCards(){
    const all = [...MAIN_SELLERS, ...OTHER_SELLERS].filter(s=>s.userId && !isBlockedSeller(s));
    const byId = new Map();
  ...
  function getSellerById(id){
    ...
    const found = all.find(s => Number(s.userId) === Number(id)) || null;
    return found && !isBlockedSeller(found) ? found : null;
  }
  ...
  function buildRankingMap(){
    ...
    sellers.sort((a,b)=>{
      const sa = getStatsForSellerCompetence(a.userId, currentPanelCompetence());
      const sb = getStatsForSellerCompetence(b.userId, currentPanelCompetence());
      const pa = (sa.produzidoMes || 0);
      const pb = (sb.produzidoMes || 0);
      const ia = (sa.andamentoValor || 0);
      const ib = (sb.andamentoValor || 0);
  ...
  function renderCard(container, seller, rankMap){
    ...
    const st = getStatsForSellerCompetence(sid, currentPanelCompetence());
    ...
    card.innerHTML = `
      ...
      <div class="head">
        ...
        <div class="stats">
          ...
          <div class="stat"><b>Produzido no mês</b><span class="money">${fmtMoney(st.produzidoMes || 0,"BRL")}</span></div>
  ...
  function renderAll(){
    ...
    const eligibleTop = sellers.filter(s=>{
      const st = getStatsForSellerCompetence(s.userId, currentPanelCompetence());
      return (st.andamentoQtd||0) > 0 || (st.metaPct||0) > 0 || (st.produzidoMes||0) > 0;
    });

    const nonZero = sellers.filter(s => (getStatsForSellerCompetence(s.userId, currentPanelCompetence()).andamentoQtd || 0) > 0 || (getStatsForSellerCompetence(s.userId, currentPanelCompetence()).produzidoMes || 0) > 0);
    const zero = sellers.filter(s => ((getStatsForSellerCompetence(s.userId, currentPanelCompetence()).andamentoQtd || 0) === 0) && ((getStatsForSellerCompetence(s.userId, currentPanelCompetence()).produzidoMes || 0) === 0));
  ...
  function requireSellerOrAdminPass(seller){
    ...
    const p = normalizePass(input);
    const adm = checkAdminPass(input);
    if (adm.ok) return adm;
    const userPass = normalizePass(`${seller.userId}${firstNameLower(seller.label)}`);
    return { ok: p === userPass, isAdmin:false, canEdit:false, readOnly:false };
  }
  ...
  async function openConcluidasForSeller(sellerId){
    ...
    await renderConcluidasSellerModal(sellerId, initial, auth.isAdmin && auth.canEdit);
  }
  ...
  async function renderConcluidasSellerModal(sellerId, yyyymm, isAdmin){
    ...
    if (isAdmin){
      document.getElementById("admBonif").addEventListener("click", async ()=>{
        const auth = promptAdminAccess("Senha ADM:");
        if (auth.cancelled) return;
        if (!auth.ok || !auth.canEdit){
          alert(auth.readOnly ? "Essa senha permite apenas visualizar." : "Senha incorreta.");
          return;
        }
  ...
  async function openConcluidasAdm(){
    ...
    await renderConcluidasAdmModal(initial, "ALL", "ALL", "date");
  }
  ...
  async function renderConcluidasAdmModal(yyyymm, sellerFilter, sourceFilter, sortKey="date"){
    const ym = normYYYYMM(yyyymm) || nowYYYYMM();
    ...
    if (sourceFilter !== "ALL"){
      filtered = filtered.filter(x => canonicalLeadLabel(x.source||"—") === canonicalLeadLabel(sourceFilter));
    }
    ...
    for (const it of filtered){
      const k = canonicalLeadLabel(it.source||"—");
      it.source = k;
    ...
    const sourceOpts = [`<option value="ALL" ${sourceFilter==="ALL"?"selected":""}>Todas</option>`]
      .concat(Object.keys(totalsBySource).sort((a,b)=>(totalsBySource[b]-totalsBySource[a]) || a.localeCompare(b,"pt-BR")).map(src=>{
        return `<option value="${escHtml(src)}" ${canonicalLeadLabel(sourceFilter)===canonicalLeadLabel(src)?"selected":""}>${escHtml(src)} • ${fmtMoney(totalsBySource[src]||0,"BRL")}</option>`;
      })).join("");

    const sortOptions = [
      { value:"date", label:"Data de implantação" },
      { value:"source", label:"Fonte" },
      { value:"operator", label:"Operadora" },
      { value:"seller", label:"Vendedora" },
      { value:"value", label:"Valor produzido" },
    ];
    if (!sortOptions.some(o => o.value === sortKey)) sortKey = "date";
    const sortOpts = sortOptions.map(o=>`<option value="${o.value}" ${o.value===sortKey?"selected":""}>${o.label}</option>`).join("");

    filtered.sort((a,b)=>{
      if (sortKey === "source") return compareTextBR(a.source, b.source) || compareTextBR(a.operator, b.operator) || String(b.closeDate||"").localeCompare(String(a.closeDate||""));
      if (sortKey === "operator") return compareTextBR(a.operator, b.operator) || compareTextBR(a.source, b.source) || String(b.closeDate||"").localeCompare(String(a.closeDate||""));
      if (sortKey === "seller") return compareTextBR(a.sellerName, b.sellerName) || String(b.closeDate||"").localeCompare(String(a.closeDate||""));
      if (sortKey === "value") return (Number(b.opportunity)||0) - (Number(a.opportunity)||0) || String(b.closeDate||"").localeCompare(String(a.closeDate||""));
      return String(b.closeDate||"").localeCompare(String(a.closeDate||"")) || compareTextBR(a.sellerName, b.sellerName);
    });
    ...
    $modalTop.innerHTML = `
      <div class="rowFlex">
        <div class="inlineCtrl">
          ...
          <span class="pillSmall">Ordenar por</span>
          <select class="sel" id="admSort">${sortOpts}</select>
    ...
    document.getElementById("admComp").addEventListener("change", async (e)=>{
      await renderConcluidasAdmModal(e.target.value, sellerFilter, sourceFilter, sortKey);
    });
    document.getElementById("admSeller").addEventListener("change", async (e)=>{
      await renderConcluidasAdmModal(ym, e.target.value, sourceFilter, sortKey);
    });
    document.getElementById("admSource").addEventListener("change", async (e)=>{
      await renderConcluidasAdmModal(ym, sellerFilter, e.target.value, sortKey);
    });
    document.getElementById("admSort").addEventListener("change", async (e)=>{
      await renderConcluidasAdmModal(ym, sellerFilter, sourceFilter, e.target.value);
    });
  ...
  /* ================= USER 1 ADM ================= */
  async function openUser1ProducaoADM(){
    const auth = promptAdminAccess("Senha ADM:");
    if (auth.cancelled) return;
    if (!auth.ok){
      alert("Senha incorreta.");
    ...
    const compSet = new Set();
    const listUser1 = closedWonBySellerId[1] || [];
    for (const it of listUser1){
      const c = normYYYYMM(it.competence);
      if (c) compSet.add(c);
    }
    const base = nowYYYYMM();
    for (let i=-12;i<=2;i++) compSet.add(addMonthsYYYYMM(base, i));

    const comps = Array.from(compSet).map(normYYYYMM).filter(Boolean).sort().reverse();
    const initial = comps[0] || nowYYYYMM();
    await renderUser1ProducaoADM(initial);
  }

  async function renderUser1ProducaoADM(yyyymm){
    const ym = normYYYYMM(yyyymm) || nowYYYYMM();
    await loadConfigIfExists(ym);

    const seller = getSellerById(1) || { userId:1, label:"USER 1" };
    openModal(`USER 1 (ADM) • PRODUÇÃO • ${ym}`, seller);
    ...
    const list = (closedWonBySellerId[1] || [])
      .filter(x => normYYYYMM(x.competence) === ym)
      .sort((a,b)=> String(b.closeDate||"").localeCompare(String(a.closeDate||"")) || a.client.localeCompare(b.client,"pt-BR"));

    const totalUser1 = list.reduce((s,it)=> s + (Number(it.opportunity)||0), 0);
    const metaGeral = getSorteioMeta(ym) || 0;
    const pctMetaGeral = metaGeral > 0 ? (totalUser1 / metaGeral) * 100 : 0;

    const totalSorteio = getAllSellersForCalc().reduce((sum, s)=>{
      const sid = Number(s.userId||0);
      const arr = (closedWonBySellerId[sid]||[]).filter(x => normYYYYMM(x.competence) === ym);
      return sum + arr.reduce((a,x)=>a+(Number(x.opportunity)||0),0);
    }, 0);
    const pctProducaoSorteio = totalSorteio > 0 ? (totalUser1 / totalSorteio) * 100 : 0;
    ...
    $modalTop.innerHTML = `
      <div class="rowFlex">
        <div class="inlineCtrl">
          <span class="pillSmall">Competência</span>
          <select class="sel" id="user1AdmComp">${compOpts}</select>
          <span class="pillSmall">Total USER 1: <b>${fmtMoney(totalUser1,"BRL")}</b></span>
          <span class="pillSmall">Meta geral sorteio: <b>${metaGeral ? fmtMoney(metaGeral,"BRL") : "—"}</b></span>
          <span class="pillSmall">% da meta geral: <b>${metaGeral > 0 ? pctMetaGeral.toFixed(1) + "%" : "—"}</b></span>
          <span class="pillSmall">% da produção do sorteio: <b>${totalSorteio > 0 ? pctProducaoSorteio.toFixed(1) + "%" : "—"}</b></span>
        </div>
        <div class="btnLine">
          <button class="btn" id="user1PdfBtn">Exportar PDF</button>
          <button class="btn" id="user1CsvBtn">Exportar CSV</button>
        </div>
      </div>
      <div class="smallNote" style="margin-bottom:10px;">
        Lista exclusiva dos fechamentos do USER 1 na competência ${ym}. O USER 1 entra no cálculo do sorteio, mas não ganha card no ranking.
      </div>
    `;
    ...
    $modalContent.innerHTML = `
      <table class="table">
        ...
        <tbody>${rows || `<tr><td colspan="5">Sem fechamentos para o USER 1 nesta competência.</td></tr>`}</tbody>
      </table>
    `;

    document.getElementById("user1AdmComp").addEventListener("change", async (e)=>{
      await renderUser1ProducaoADM(e.target.value);
    });
    document.getElementById("user1PdfBtn").addEventListener("click", ()=>exportModalToPDF(`user1_producao_adm_${ym}`));
    document.getElementById("user1CsvBtn").addEventListener("click", ()=>exportUser1ProducaoCSV(list, ym, totalUser1, metaGeral, pctMetaGeral, totalSorteio, pctProducaoSorteio));
  }

  function exportUser1ProducaoCSV(list, yyyymm, totalUser1, metaGeral, pctMetaGeral, totalSorteio, pctProducaoSorteio){
    const ym = normYYYYMM(yyyymm) || nowYYYYMM();
    const lines = [];
    lines.push(["COMPETENCIA","TOTAL_USER_1","META_GERAL_SORTEIO","PCT_META_GERAL","TOTAL_PRODUCAO_SORTEIO","PCT_PRODUCAO_SORTEIO"].join(";"));
    lines.push([`"${ym}"`,`"${Number(totalUser1)||0}"`,`"${Number(metaGeral)||0}"`,`"${(Number(pctMetaGeral)||0).toFixed(2)}"`,`"${Number(totalSorteio)||0}"`,`"${(Number(pctProducaoSorteio)||0).toFixed(2)}"`].join(";"));
    lines.push("");
    lines.push(["IMPLANTADA_EM","NEGOCIO","OPERADORA","FONTE","VLR_PRODUZIDO"].join(";"));
    for (const it of list){
      lines.push([
        `"${formatImplantada(it.closeDate)}"`,
        `"${String(it.client||"").replace(/"/g,'""')}"`,
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `user1_producao_adm_${ym}.csv`;
    document.body.appendChild(a);
    a.click();
  ...
  document.getElementById("hideBtn").addEventListener("click", (e)=>{
    ...
    renderHidePanel();
    updateHideButton();
    $hidePanel.style.display = ($hidePanel.style.display === "block") ? "none" : "block";
  });
  ...
  async function openAdmMetasByCompetence(yyyymm, readOnly=false){
    const ym = normYYYYMM(yyyymm) || nowYYYYMM();
    const cfg = readOnly ? await loadConfigIfExists(ym) : await ensureConfigLoaded(ym);

    openModal(`${readOnly ? "ADM Metas • VISUALIZAÇÃO" : "ADM Metas"} • ${ym}`, null);
    ...
    const rows = sellers.map(s=>{
      ...
      return `
        <tr>
          ...
          <td>#${sid}</td>
          <td>
            <input class="inpt" data-meta="${sid}" value="${cur ? String(cur).replace(".",",") : ""}" placeholder="Meta (R$)" style="width:180px;" ${readOnly ? "disabled" : ""}>
          </td>
        </tr>
      `;
    }).join("");
    ...
    $modalTop.innerHTML = `
      <div class="rowFlex">
        <div class="inlineCtrl">
          ...
          <input class="inpt" id="metaSorteio" value="${sorteioCur ? String(sorteioCur).replace(".",",") : ""}" placeholder="Meta Sorteio (R$)" style="width:180px;" ${readOnly ? "disabled" : ""}>
        </div>
        <div class="btnLine">
          ${readOnly ? "" : `<button class="btn btnAdm" id="saveMetas">Salvar</button>`}
        </div>
      </div>
      <div class="pillSmall">${readOnly ? "Senha 8123: visualização sem edição" : `Salva no Bitrix (Deal ${CFG_TITLE_PREFIX}${ym})`}</div>
    `;
    ...
    document.getElementById("metaComp").addEventListener("change", async (e)=>{
      await openAdmMetasByCompetence(e.target.value, readOnly);
    });
    document.getElementById("metaPrev").addEventListener("click", async ()=>{
      await openAdmMetasByCompetence(addMonthsYYYYMM(ym, -1), readOnly);
    });
  ...
  function getAvailableAnalysisYears(){
    const set = new Set();
    for (const sid of Object.keys(closedWonBySellerId)){
      for (const it of (closedWonBySellerId[sid] || [])){
        const c = normYYYYMM(it.competence);
        if (c) set.add(c.slice(0,4));
      }
    }
  ...
  function getAnnualStatsForSeller(sellerId, year){
    const y = String(year || new Date().getFullYear());
    const base = statsBySellerId[sellerId] || { andamentoQtd:0, andamentoValor:0, docsQtd:0, pagQtd:0 };
    const list = (closedWonBySellerId[sellerId] || []).filter(x => String(normYYYYMM(x.competence) || "").startsWith(y + "-"));
    const produzido = list.reduce((sum, it)=> sum + (Number(it.opportunity)||0), 0);

    let metaAno = 0;
    for (let m=1; m<=12; m++){
      const ym = `${y}-${String(m).padStart(2,"0")}`;
      metaAno += getMeta(ym, sellerId);
    }
    const pct = metaAno > 0 ? (produzido / metaAno) * 100 : 0;
  ...
  async function openAnaliseModal(mode="monthly", refValue=null){
    const monthly = mode !== "annual";
    const ym = normYYYYMM(refValue) || currentPanelCompetence();
    const year = String(refValue || ym.slice(0,4) || new Date().getFullYear()).slice(0,4);
    const titleRef = monthly ? ym : year;

    openModal(`ANÁLISE • ${monthly ? "MENSAL" : "ANUAL"} • ${titleRef}`, null);
    ...
    for (const s of sellers){
      const sid = Number(s.userId||0);
      const st = monthly ? getStatsForSellerCompetence(sid, ym) : getAnnualStatsForSeller(sid, year);
      totalProduzido += Number(st.produzidoMes)||0;
      totalAndamento += Number(st.andamentoValor)||0;
      totalCards += Number(st.andamentoQtd)||0;
      rows.push({ seller:s.label, sid, ...st });
    }

    rows.sort((a,b)=> (b.produzidoMes-a.produzidoMes) || (b.andamentoValor-a.andamentoValor) || a.seller.localeCompare(b.seller,"pt-BR"));

    const metaGeral = monthly ? (getSorteioMeta(ym) || 0) : rows.reduce((s,r)=>s+(Number(r.metaMes)||0),0);
    const pctGeral = metaGeral > 0 ? (totalProduzido/metaGeral)*100 : 0;

    const compOpts = buildPanelCompetenceOptions().map(c=>`<option value="${c}" ${c===ym?"selected":""}>${c}</option>`).join("");
    const yearOpts = getAvailableAnalysisYears().map(y=>`<option value="${y}" ${String(y)===String(year)?"selected":""}>${y}</option>`).join("");

    $modalTop.innerHTML = `
      <div class="rowFlex">
        <div class="inlineCtrl">
          <span class="pillSmall">Tipo de ranking</span>
          <select class="sel" id="analiseMode">
            <option value="monthly" ${monthly ? "selected" : ""}>Mensal</option>
            <option value="annual" ${!monthly ? "selected" : ""}>Anual</option>
          </select>
          <span class="pillSmall">Competência</span>
          <select class="sel" id="analiseComp" ${monthly ? "" : "disabled"}>${compOpts}</select>
          <span class="pillSmall">Ano</span>
          <select class="sel" id="analiseYear" ${monthly ? "disabled" : ""}>${yearOpts}</select>
          <span class="pillSmall">Cards visíveis</span>
          <span class="pillSmall"><b>${sellers.length}</b></span>
        </div>
        <div class="btnLine">
          <button class="btn" id="analiseCsv">Exportar CSV</button>
          <button class="btn" id="pdfBtn">Exportar PDF</button>
        </div>
      </div>
      <div class="kpiGrid">
        <div class="kpiCard"><b>Produzido ${monthly ? "na competência" : "no ano"}</b><span>${fmtMoney(totalProduzido,"BRL")}</span></div>
        <div class="kpiCard"><b>Valor a implantar</b><span>${fmtMoney(totalAndamento,"BRL")}</span></div>
        <div class="kpiCard"><b>Propostas em andamento</b><span>${totalCards}</span></div>
        <div class="kpiCard"><b>% meta ${monthly ? "sorteio" : "anual"}</b><span>${metaGeral ? pctGeral.toFixed(1)+"%" : "—"}</span></div>
      </div>
    `;
    ...
    const tableRows = rows.map((r, idx)=>`
      <tr>
        <td><b>#${idx+1}</b></td>
        <td>${escHtml(r.seller)}</td>
        <td class="tdRight"><b>${fmtMoney(r.produzidoMes,"BRL")}</b></td>
        <td class="tdRight">${r.metaMes ? fmtMoney(r.metaMes,"BRL") : "—"}</td>
        <td class="tdRight"><b>${r.metaMes ? (r.metaPct||0).toFixed(1)+"%" : "—"}</b></td>
        <td class="tdRight">${r.andamentoQtd||0}</td>
        <td class="tdRight">${fmtMoney(r.andamentoValor||0,"BRL")}</td>
      </tr>
    `).join("");
      ...
      <table class="table">
        <thead>
          <tr>
            ...
            <th>User</th>
            <th class="tdRight">Produzido</th>
            <th class="tdRight">Meta ${monthly ? "do mês" : "do ano"}</th>
            <th class="tdRight">% Meta</th>
            <th class="tdRight">Em andamento</th>
            <th class="tdRight">Valor a implantar</th>
          </tr>
        </thead>
    ...
    document.getElementById("analiseMode").addEventListener("change", async (e)=>{
      const nextMode = e.target.value;
      await openAnaliseModal(nextMode, nextMode === "annual" ? document.getElementById("analiseYear").value : document.getElementById("analiseComp").value);
    });
    document.getElementById("analiseComp").addEventListener("change", async (e)=>{
      await openAnaliseModal("monthly", e.target.value);
    });
    document.getElementById("analiseYear").addEventListener("change", async (e)=>{
      await openAnaliseModal("annual", e.target.value);
    });

    document.getElementById("pdfBtn").addEventListener("click", ()=>exportModalToPDF(`analise_${monthly ? ym : year}`));
    document.getElementById("analiseCsv").addEventListener("click", ()=>{
      const periodo = monthly ? ym : year;
      const header = ["RANK","USER","ID","TIPO","PERIODO","PRODUZIDO","META","PCT_META","EM_ANDAMENTO","VALOR_A_IMPLANTAR"];
      const lines = [header.join(";")];
      rows.forEach((r, idx)=>{
        lines.push([idx+1, `"${String(r.seller).replace(/"/g,'""')}"`, r.sid, monthly ? "MENSAL" : "ANUAL", periodo, r.produzidoMes||0, r.metaMes||0, (r.metaPct||0).toFixed(2), r.andamentoQtd||0, r.andamentoValor||0].join(";"));
      });
      const blob = new Blob(["\ufeff"+lines.join("\n")], { type:"text/csv;charset=utf-8" });
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `analise_${periodo}.csv`;
      document.body.appendChild(a);
      a.click();
      setTimeout(()=>{ URL.revokeObjectURL(a.href); a.remove(); }, 600);
    });
  ...
  document.getElementById("admMetas").addEventListener("click", async ()=>{
    ...
    await openAdmMetasByCompetence(nowYYYYMM(), auth.readOnly);
  });

  document.getElementById("admDoneAll").addEventListener("click", openConcluidasAdm);
  document.getElementById("admUser1Prod").addEventListener("click", openUser1ProducaoADM);
  document.getElementById("admBonifRanking").addEventListener("click", openBonifRankingADM);
  document.getElementById("admAnalise").addEventListener("click", ()=>openAnaliseModal("monthly", currentPanelCompetence()));
  if ($compFilter){
    $compFilter.addEventListener("change", async (e)=>{
      selectedCompetence = e.target.value || "current";
      const ym = currentPanelCompetence();
      await ensureConfigLoaded(ym);
      renderAll();
      updateSorteioBar();
    });
  ...
  function updateSorteioBar(){
    const ym = currentPanelCompetence();
    $sorteioYm.textContent = selectedCompetence === "current" ? `${ym} (atual)` : ym;

    const meta = getSorteioMeta(ym) || 0;

    // Produção do sorteio: considera todos os usuários de cálculo
    // MAIN_SELLERS + OTHER_SELLERS + SEARCH_ONLY_USERS, incluindo o USER 1 oculto.
    let produzido = 0;
    for (const s of getAllSellersForCalc()){
      const sid = Number(s.userId||0);
      const list = (closedWonBySellerId[sid]||[]).filter(x => normYYYYMM(x.competence) === ym);
