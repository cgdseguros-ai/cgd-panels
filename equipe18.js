...
(function () {
  ...
  injectCSS(`
    ...
    #eqd-app.eqd-dark .leadCard .leadBtns,
    #eqd-app.eqd-dark .eqd-card,
    #eqd-app.eqd-dark .eqd-card .eqd-task,
    #eqd-app.eqd-dark .eqd-card .eqd-meta,
    #eqd-app.eqd-dark .eqd-listRow,
    #eqd-app.eqd-dark .eqd-listTitle,
    #eqd-app.eqd-dark .eqd-listMeta{color:#111;}
  ...
  async function safeDealUpdate(dealId, fields) {
    try {
      return await bx("crm.deal.update", { id: String(dealId), fields: fields || {} });
    } catch (e) {
      const msg = String((e && e.message) || e || "");
      if (!/HTTP 400 em crm\.deal\.update/i.test(msg)) throw e;
      try {
        const cur = await bx("crm.deal.get", { id: String(dealId) });
        let ok = true;
        for (const [k, v] of Object.entries(fields || {})) {
          if (String((cur && cur[k]) ?? "") !== String(v ?? "")) { ok = false; break; }
        }
        if (ok) return true;
      } catch (_) {}
      throw e;
  ...
  async function safeLeadUpdate(leadId, fields) {
    try {
      return await bx("crm.lead.update", { id: String(leadId), fields: fields || {} });
    } catch (e) {
      const msg = String((e && e.message) || e || "");
      if (!/HTTP 400 em crm\.lead\.update/i.test(msg)) throw e;
      try {
        const cur = await bx("crm.lead.get", { id: String(leadId) });
        let ok = true;
        for (const [k, v] of Object.entries(fields || {})) {
          if (String((cur && cur[k]) ?? "") !== String(v ?? "")) { ok = false; break; }
        }
  ...
  async function markFollowupAsLost(dealId) {
    const etapMap = await enums(UF_ETAPA).catch(() => ({}));
    const perdidoEtapaId =
      findEnumIdByLabel(etapMap, norm("PERDIDO")) ||
      findEnumIdByLabel(etapMap, norm("PERDIDA")) ||
      findEnumIdByLabel(etapMap, norm("SEM INTERESSE")) || "";
    const fields = {};
    if (perdidoEtapaId) fields[UF_ETAPA] = perdidoEtapaId;
    if (STATE.doneStageId) fields.STAGE_ID = String(STATE.doneStageId);
    updateDealInState(dealId, { ...fields, DATE_MODIFY: new Date().toISOString() });
    rebuildDealsOpen();
    return safeDealUpdate(dealId, fields);
  }

  async function moveLeadStageBatch(userId, leadIds, toStatus) {
    const ids = (leadIds || []).map(String).filter(Boolean);
    for (const leadId of ids) {
      await safeLeadUpdate(leadId, { STATUS_ID: String(toStatus) });
      const arr = (STATE.leadsByUser.get(String(userId)) || []).slice();
      const idx = arr.findIndex((l) => String(l.ID) === String(leadId));
      if (idx >= 0) arr[idx] = { ...arr[idx], STATUS_ID: String(toStatus), DATE_MODIFY: new Date().toISOString() };
...
restoreSyncQueue();
  ...
  function openNoFutureFollowupLeadBatchModal(user, leads, title) {
    ...
    openModal(title, `
      <div style="display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <button class="eqd-btn" id="noFuBatchConv">Concluir em lote</button>
          <button class="eqd-btn eqd-btnDanger" id="noFuBatchLost">Perder em lote</button>
        </div>
      </div>
    ...
    const render = () => {
      ...
      box.innerHTML = visible.length ? visible.map((l) => `
        <label style="display:flex;gap:10px;align-items:flex-start;padding:10px 12px;border:1px solid rgba(0,0,0,.10);border-radius:14px;background:#fff;color:#111">
          <input type="checkbox" class="noFuLeadChk" data-id="${escHtml(String(l.ID))}" data-name="${escHtml(leadTitle(l))}" style="margin-top:4px" />
          <div style="display:flex;flex:1;justify-content:space-between;gap:10px;flex-wrap:wrap">
            <div style="display:flex;flex-direction:column;gap:4px">
              <div style="font-weight:950">${escHtml(leadTitle(l))}</div>
              <div style="font-size:12px;opacity:.82;display:flex;gap:10px;flex-wrap:wrap">${leadOperadora(l) ? `<span>Operadora: <strong>${escHtml(leadOperadora(l))}</strong></span>` : ``}${leadTelefone(l) ? `<span>Tel: <strong>${escHtml(leadTelefone(l))}</strong></span>` : ``}${leadBairro(l) ? `<span>Bairro: <strong>${escHtml(leadBairro(l))}</strong></span>` : ``}${leadFonte(l) ? `<span>Fonte: <strong>${escHtml(leadFonte(l))}</strong></span>` : ``}${leadDataHora(l) ? `<span>Data/Hora: <strong>${escHtml(fmt(leadDataHora(l)))}</strong></span>` : ``}<span style="color:#92400e">Sem FOLLOW-UP futuro</span></div>
            </div>
            <div style="display:flex;gap:6px;flex-wrap:wrap"><button type="button" class="eqd-btn" data-action="leadEditModal" data-leadid="${l.ID}" data-userid="${user.userId}">Editar</button></div>
          </div>
    ...
    document.getElementById('noFuBatchConv').onclick = async () => {
      const ids = getChecks().filter((x) => x.checked).map((x) => String(x.getAttribute('data-id') || ''));
      if (!ids.length) return alert('Selecione ao menos um lead.');
      if (!sConv) return alert('Stage de convertido não encontrado.');
      await moveLeadStageBatch(user.userId, ids, sConv);
      openNoFutureFollowupLeadBatchModal(user, baseList.filter((l) => !ids.includes(String(l.ID))), title);
    };
    document.getElementById('noFuBatchLost').onclick = async () => {
      const ids = getChecks().filter((x) => x.checked).map((x) => String(x.getAttribute('data-id') || ''));
      if (!ids.length) return alert('Selecione ao menos um lead.');
      if (!sPerdido) return alert('Stage de perdido não encontrado.');
      await moveLeadStageBatch(user.userId, ids, sPerdido);
      openNoFutureFollowupLeadBatchModal(user, baseList.filter((l) => !ids.includes(String(l.ID))), title);
    };
  ...
  function openFollowupListModalForUser(user) {
    ...
    openModal(`Lista de FOLLOW-UP — ${user.name}`, `
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:space-between">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <button class="eqd-btn eqd-btnDanger" id="fuListBatchLost">PERDER EM LOTE</button>
    ...
    document.getElementById("fuListBatchLost").onclick = async () => {
      const ids = [...document.querySelectorAll("#fuListBox .eqd-batch:checked")].map((x) => x.getAttribute("data-id"));
      if (!ids.length) return alert("Selecione os follow-ups marcando 'Lote' nos cards.");
      for (const id of ids) await markFollowupAsLost(id);
      openFollowupListModalForUser(user);
    };
  ...
  function openLeadEditModal(userId, leadId) {
    const uid = String(userId || "");
    const leads = STATE.leadsByUser.get(uid) || [];
    const lead = leads.find((l) => String(l.ID) === String(leadId));
    if (!lead) return;
    const dt = tryParseDateAny(leadDataHora(lead) || lead.DATE_CREATE || "");
    const localNow = dt ? new Date(dt.getTime() - dt.getTimezoneOffset()*60000).toISOString().slice(0,16) : "";
    openModal(`Editar Lead — ${escHtml(leadTitle(lead))}`, `
      <div class="eqd-warn" id="leadEditWarn"></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div style="grid-column:1 / -1"><div style="font-size:11px;font-weight:900;margin-bottom:6px">NOME</div><input id="leadEditName" value="${escHtml(leadTitle(lead))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16);font-weight:900" /></div>
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">OPERADORA</div><input id="leadEditOp" value="${escHtml(leadOperadora(lead))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)" /></div>
        ...
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">DATA/HORA DO LEAD</div><input id="leadEditDataHora" type="datetime-local" value="${localNow}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)" /></div>
        <div><div style="font-size:11px;font-weight:900;margin-bottom:6px">POSSUI PLANO</div><input id="leadEditPossuiPlano" value="${escHtml(leadPossuiPlano(lead))}" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(30,40,70,.16)" /></div>
        ...
        <div style="grid-column:1 / -1;display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="leadEditSave">SALVAR</button></div>
      </div>
    `, { wide:true });
    document.getElementById('leadEditSave').onclick = async () => {
      const warn = document.getElementById('leadEditWarn');
      try {
        const fields = {
          TITLE: String(document.getElementById('leadEditName').value || '').trim(),
          NAME: String(document.getElementById('leadEditName').value || '').trim(),
          [LEAD_UF_OPERADORA]: String(document.getElementById('leadEditOp').value || '').trim(),
          [LEAD_UF_IDADE]: String(document.getElementById('leadEditIdade').value || '').trim(),
          [LEAD_UF_TELEFONE]: String(document.getElementById('leadEditTel').value || '').trim(),
          [LEAD_UF_BAIRRO]: String(document.getElementById('leadEditBairro').value || '').trim(),
          [LEAD_UF_FONTE]: String(document.getElementById('leadEditFonte').value || '').trim(),
          [LEAD_UF_OBS]: String(document.getElementById('leadEditObs').value || '').trim(),
          [LEAD_UF_POSSUI_PLANO]: String(document.getElementById('leadEditPossuiPlano').value || '').trim(),
        };
        const dh = localInputToIsoWithOffset(String(document.getElementById('leadEditDataHora').value || '').trim());
        if (dh) fields[LEAD_UF_DATAHORA] = dh;
        if (!fields.TITLE) throw new Error('Informe o nome do lead.');
        await safeLeadUpdate(String(leadId), fields);
        Object.assign(lead, fields, { DATE_MODIFY: new Date().toISOString() });
        closeModal();
        reopenLeadsModalSafe({ noBackgroundReload: true });
      } catch (e) {
        warn.style.display = 'block';
    ...
    function cardLead(l, column) {
      ...
      if (column === "AT") {
        btns = `${mkBtn("ATENDIDO",sAtendido)}${mkBtn("PERDIDO",sPerdido)}
                <button class="leadBtn" data-action="leadEditModal" data-leadid="${l.ID}" data-userid="${user.userId}">EDITAR</button>
...
function openMarketingPanelStub(selectedDateStr = "", monthAnchorStr = "") {
  ...
  const mktDeals = (STATE.dealsAll || []).filter((d) => String(d.TITLE || "").trim().startsWith("[MKT]")).sort((a,b) => {
    const ta = a._prazo ? new Date(a._prazo).getTime() : 0;
    const tb = b._prazo ? new Date(b._prazo).getTime() : 0;
    return ta - tb;
  });

  const groupedAll = groupMarketingDeals(mktDeals, members, memberColorMap);
  const filteredDeals = groupedAll.filter((g) => {
  ...
  document.querySelectorAll('[data-action="mktDoneGroup"]').forEach((btn) => btn.onclick = async () => {
    const g = groupMap.get(String(btn.getAttribute('data-group') || ''));
    if (!g || !Array.isArray(g.ids)) return;
    for (const id of g.ids) await markDone(id);
  ...
  async function openDoneAndRescheduleModal(dealId) {
    ...
    openModal("Concluir e reagendar", `
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:10px">
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap"><button class="eqd-btn" data-action="modalClose">Cancelar</button><button class="eqd-btn eqd-btnPrimary" id="drSave">SALVAR REAGENDAMENTO</button></div>
      </div>
    `);
    ...
    btn.onclick = async () => {
      try {
        ...
        if (isFollowupDeal(d)) {
          ...
          followMode = await new Promise((resolve) => {
            openModal("Este FOLLOW-UP foi feito?", `
              <div style="display:flex;flex-direction:column;gap:12px">
                <div style="font-size:14px;font-weight:950">${escHtml(bestTitleFromText(title))}</div>
                <div style="font-size:12px;font-weight:900;opacity:.82">Escolha como salvar o reagendamento.</div>
                <div style="display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap">
                  <button class="eqd-btn" id="drAskCancel">Voltar</button>
                  <button class="eqd-btn" id="drAskNo">NÃO FEITO</button>
                  <button class="eqd-btn eqd-btnPrimary" id="drAskYes">FEITO</button>
                </div>
              </div>
            `, { wide:true });
          ...
          if (!followMode) { openDoneAndRescheduleModal(dealId); return; }
        }
        ...
        if (isFollowupDeal(d) && followMode === 'NAO_FEITO') {
          updateDealInState(dealId, { TITLE: title, [UF_PRAZO]: prazoIso, _prazo: new Date(prazoIso).toISOString(), _late:false, DATE_MODIFY:new Date().toISOString(), [UF_OBS]: obs });
          rebuildDealsOpen();
          closeModal();
          renderCurrentView();
          enqueueSync({ type: 'dealUpdate', dealId: String(dealId), fields: { TITLE: title, [UF_PRAZO]: prazoIso, [UF_OBS]: obs } });
  ...
  function openLeadStageListModal(userId, stageId, title, kwRaw = "") {
    ...
    const card = (l) => `
      <div class="leadCard">
        ...
        <div class="leadBtns"><button class="leadBtn leadBtnP" data-action="leadMove" data-leadid="${l.ID}" data-tostatus="${escHtml(String(leadStageId('EM ATENDIMENTO')||''))}" data-userid="${userId}">VOLTAR PARA EM ATENDIMENTO</button><button class="leadBtn leadBtnD" data-action="leadDeleteForever" data-leadid="${l.ID}" data-userid="${userId}">EXCLUIR DEFINITIVAMENTE</button></div>
      </div>`;
...
.map((d) => parseDeal(d, maps)).filter(Boolean)
  ...
  async function openAllUsersNoFutureFollowupModal() {
    await loadGlobalLeadsBulk(true).catch(()=>{});
    const sAtendido = String(leadStageId("ATENDIDO") || "");
    const sQual = String(leadStageId("QUALIFICADO") || "");
    const leads = (STATE.globalLeadsAll || []).filter((l) => [sAtendido, sQual].includes(String(l.STATUS_ID || "")) && !hasFutureFollowupForLeadOrig(leadOrigemId(l) || l.ID));
    openModal("Leads sem FOLLOW-UP — todas as users", `<div style="font-size:12px;font-weight:950;opacity:.82;margin-bottom:8px">Total: <strong>${leads.length}</strong></div><div style="max-height:70vh;overflow:auto;display:flex;flex-direction:column;gap:8px">${leads.length ? leads.map((l) => { const owner = USERS.find((u) => String(u.userId) === String(l._ownerUserId || l.ASSIGNED_BY_ID || "")); return `<div class="leadCard"><div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><div class="leadTitle">${escHtml(leadTitle(l))}</div><div style="display:flex;gap:6px;flex-wrap:wrap"><span class="eqd-tag">User: ${escHtml(owner ? owner.name : String(l._ownerUserId || l.ASSIGNED_BY_ID || ''))}</span><button class="leadBtn" data-action="leadEditModal" data-userid="${escHtml(String(l._ownerUserId || l.ASSIGNED_BY_ID || ''))}" data-leadid="${escHtml(String(l.ID))}">Editar</button><button class="leadBtn" data-action="leadFollowupModal" data-userid="${escHtml(String(l._ownerUserId || l.ASSIGNED_BY_ID || ''))}" data-leadid="${escHtml(String(l.ID))}">Reagendar FUP</button></div></div><div class="leadMeta">${leadOperadora(l) ? `<span>Operadora: <strong>${escHtml(leadOperadora(l))}</strong></span>` : ''}${leadTelefone(l) ? `<span>Telefone: <strong>${escHtml(leadTelefone(l))}</strong></span>` : ''}${leadDataHora(l) ? `<span>Data/Hora: <strong>${escHtml(fmt(leadDataHora(l)))}</strong></span>` : ''}</div></div>`; }).join('') : `<div class="eqd-empty">Nenhum lead sem FOLLOW-UP futuro.</div>`}</div>`, { full:true, wide:true });
  }

  async function openLeadAnalysisModal() {
    await loadGlobalLeadsBulk(true).catch(()=>{});
    const hiddenKey = 'EQD_LEAD_ANALYSIS_HIDDEN_USERS';
    let hiddenUsers = [];
    try { hiddenUsers = JSON.parse(localStorage.getItem(hiddenKey) || '[]'); } catch(_) {}
    ...
    const render = () => {
      const rows = USERS.filter((u) => !hiddenUsers.includes(String(u.userId))).map((u) => {
        ...
        return `<tr><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08);font-weight:900">${escHtml(u.name)}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${novoCt}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${emAt}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${atd}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${q}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${semFu}</td><td style="padding:8px;border-bottom:1px solid rgba(0,0,0,.08)">${fuLate}</td></tr>`;
      }).join("");
      const checks = USERS.map((u) => `<label style="display:inline-flex;gap:6px;align-items:center;padding:4px 8px;border:1px solid rgba(0,0,0,.10);border-radius:999px;background:#fff"><input class="leadAnaHideChk" type="checkbox" value="${u.userId}" ${hiddenUsers.includes(String(u.userId)) ? 'checked' : ''}/> Ocultar ${escHtml(u.name)}</label>`).join('');
      openModal("Análise de Leads", `<div style="display:flex;gap:10px;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;margin-bottom:10px"><div style="display:flex;gap:8px;flex-wrap:wrap">${checks}</div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="eqd-btn" id="leadAnaApply">Aplicar ocultação</button><button class="eqd-btn" id="leadAnaNoFuAll">Leads sem FOLLOW-UP</button></div></div><div style="overflow:auto"><table style="width:100%;border-collapse:collapse;font-size:12px;font-weight:900"><thead><tr><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">User</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">Novo Lead</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">Em atendimento</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">Atendido</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">Qualificado</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">Sem FUP</th><th style="text-align:left;padding:8px;border-bottom:1px solid rgba(0,0,0,.12)">FUP atrasados</th></tr></thead><tbody>${rows}</tbody></table></div>`, { wide:true, full:true });
      const apply = document.getElementById('leadAnaApply');
      if (apply) apply.onclick = () => { hiddenUsers = [...document.querySelectorAll('.leadAnaHideChk:checked')].map((x) => String(x.value || '')); try { localStorage.setItem(hiddenKey, JSON.stringify(hiddenUsers)); } catch(_) {} render(); };
      const noFuBtn = document.getElementById('leadAnaNoFuAll');
      if (noFuBtn) noFuBtn.onclick = () => openAllUsersNoFutureFollowupModal();
    };
    render();
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    try{
      ...
      if (act === "leadEditModal") { const uid = String(a.getAttribute("data-userid")||""); const leadId = String(a.getAttribute("data-leadid")||""); if (!uid || !leadId) return; return openLeadEditModal(uid, leadId); }
